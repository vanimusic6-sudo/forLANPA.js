(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dossier{position:relative;background:#2b2e31;border-radius:16px;padding:26px 28px;margin-bottom:18px;animation:swRise .55s cubic-bezier(.22,1,.36,1) both}' +
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // Возраст вычисляем ДО проверок — нужен для guard'ов точности
            var age = dom.age;
            ...
            // GUARD ТОЧНОСТИ: на детском рейтинге (<=12) overview-сигналы секса/наготы игнорируем
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v39.0 (precise regex, kids guard)');
    }
