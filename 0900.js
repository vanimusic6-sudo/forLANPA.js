(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v58';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            /* ОДИН кубик, без лишних слоёв */
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;background:linear-gradient(145deg,#ffffff 0%,#e8eaec 100%);border:2px solid #c5c7c9;border-radius:14px;box-shadow:0 3px 8px rgba(0,0,0,.2);display:flex;align-items:center;justify-content:center}' +
            '.sw-dice svg{width:78%;height:78%;display:block}' +
            '.sw-dice.sw-run{animation:swToss 1.6s cubic-bezier(.3,.6,.3,1) forwards}' +
            '@keyframes swToss{0%{transform:translateX(0) rotate(0)}20%{transform:translateX(calc(var(--sw-dist)*.5)) rotate(200deg)}40%{transform:translateX(var(--sw-dist)) rotate(400deg)}55%{transform:translateX(calc(var(--sw-dist)*.8)) rotate(560deg)}70%{transform:translateX(calc(var(--sw-dist)*.55)) rotate(680deg)}85%{transform:translateX(calc(var(--sw-dist)*.25)) rotate(720deg)}100%{transform:translateX(0) rotate(720deg)}}' +
            '@media(prefers-reduced-motion:reduce){.sw-dice.sw-run{animation:none !important}.sw-dicebtn{transition:none !important}}' +
    ...
    function buildReadyInner(a) {
        ...
        return '<div class="sw-dossier">' + badges +
            ...
            '<span class="sw-dice" id="sw-dice">' + pipsSVG(1) + '</span></button>' +
    ...
    function bindDice(html) {
        ...
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                dice.innerHTML = pipsSVG(rndFace());
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v58.0 (single-layer dice, no ghosting)');
    }
