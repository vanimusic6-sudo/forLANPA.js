(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v54';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-label{font-family:' + COND + ';font-size:2.35em;font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;white-space:nowrap;opacity:0;transition:opacity .3s ease}' +
            '.sw-label.show{opacity:1}' +
            '.sw-label.res-yes{color:#1b5e20}.sw-label.res-no{color:#b71c1c}' +
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;perspective:220px;--sw-half:32px}' +
            ...
            '.sw-face{position:absolute;inset:0;background:linear-gradient(180deg,#fbfbfc,#e8eaec);border:2px solid #cfd2d5;border-radius:12px;display:block}' +
            ...
            '.sw-dice.sw-run{animation:swX 1.2s cubic-bezier(.45,0,.55,1) forwards}' +
            '.sw-dice.sw-run::after{animation:swShadow 1.2s linear forwards}' +
            '.sw-dice.sw-run .sw-cube{animation:swCube 1.2s cubic-bezier(.3,.7,.3,1) forwards}' +
            '@keyframes swX{0%{transform:translateX(0)}22%{transform:translateX(var(--sw-dist))}78%{transform:translateX(var(--sw-dist))}100%{transform:translateX(0)}}' +
            '@keyframes swCube{0%{transform:rotateZ(0)}22%{transform:rotateZ(360deg)}34%{transform:rotateZ(360deg) rotateX(180deg)}46%{transform:rotateZ(360deg) rotateX(180deg) rotateY(180deg)}58%{transform:rotateZ(360deg) rotateX(360deg) rotateY(180deg)}70%{transform:rotateZ(360deg) rotateX(360deg) rotateY(360deg)}78%,100%{transform:rotateZ(720deg)}}' +
            '@keyframes swShadow{0%,22%,78%,100%{transform:scale(1);opacity:.6}34%{transform:scale(.55);opacity:.25}46%{transform:scale(1.05);opacity:.6}58%{transform:scale(.75);opacity:.35}70%{transform:scale(1.02);opacity:.55}}' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v54.0 (true 3D dice cube)');
    }
