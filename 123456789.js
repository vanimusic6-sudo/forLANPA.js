(function () {
    ...
    window._sw_diceCleanup = null;
    window._sw_started = false;
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-modal-content{padding:28px 32px 48px;color:#e9ebeb;font-family:' + DISPLAY + ';font-size:20px;box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y;overscroll-behavior:contain;scroll-behavior:smooth;animation:swModalIn .42s cubic-bezier(.22,1,.36,1) both;transform-origin:50% 8%}' +
            ...
            '.sw-dossier::after{content:"";position:absolute;inset:-40% -20%;background:linear-gradient(105deg,transparent 35%,rgba(255,255,255,.055) 47%,rgba(255,255,255,.11) 50%,transparent 62%);transform:translateX(-80%) rotate(3deg);pointer-events:none;animation:swDossierSweep 1.1s cubic-bezier(.22,1,.36,1) .25s 1 both}' +
            ...
            '.sw-dicebtn::after{content:"";position:absolute;inset:0;background:linear-gradient(110deg,transparent 25%,rgba(255,255,255,.18) 42%,rgba(255,255,255,.05) 52%,transparent 68%);transform:translateX(-120%);pointer-events:none}' +
            ...
            '@keyframes swButtonSheen{from{transform:translateX(-120%)}to{transform:translateX(120%)}}' +
            ...
            '.sw-dice-spin{animation:swDiceSpinPremium 1.42s cubic-bezier(.2,.78,.2,1) 1}' +
            '@keyframes swDiceSpinPremium{0%{transform:rotate(0deg) scale(1)}12%{transform:rotate(110deg) scale(1.02)}30%{transform:rotate(280deg) scale(1.075)}52%{transform:rotate(455deg) scale(1.115)}73%{transform:rotate(610deg) scale(1.045)}88%{transform:rotate(680deg) scale(1.012)}100%{transform:rotate(720deg) scale(1)}}' +
            ...
            '.sw-label{font-family:' + COND + ';font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;white-space:nowrap;opacity:0;transition:opacity .3s ease,transform .3s ease;letter-spacing:-.02em}' +
            ...
            '#sw-dice-face{width:100%;height:100%;display:block;transition:opacity .12s ease-in,transform .2s ease}' +
            ...
            '@media(prefers-reduced-motion:reduce){.sw-dice-spin{animation:none !important}.sw-dicebtn{transition:none !important}.sw-dicebtn::after{animation:none !important}.sw-modal-content{animation:none !important}.sw-dossier::after{animation:none !important}.sw-list li{transition:opacity .2s ease !important;transform:none !important}.sw-meter-fill::after{animation:none !important}.sw-verdict-word{transition:opacity .2s ease !important;transform:none !important}}' +
    ...
    function cleanupModal() {
        ...
        window._sw_activeInteractive = null;
        clearLoader();
        if (window._sw_diceCleanup) {
            try { window._sw_diceCleanup(); } catch(e) {}
            window._sw_diceCleanup = null;
        }
        if (window._sw_restoreTimer) {
            clearTimeout(window._sw_restoreTimer);
    ...
    function bindDice(html, cfg) {
        ...
        window._sw_diceCleanup = cleanupDice;

        function scheduleSwap() {
            faceTimer = setTimeout(function(){
                if (stopCycle) return;
                if (Date.now() - startTime > 1100) return;
                var f; do { f = rndFace(); } while (f === currentFace);
                currentFace = f;
                dice.style.opacity = '0.18';
                dice.style.transform = 'scale(.93) rotate(' + ((Math.random() > .5 ? 1 : -1) * 4) + 'deg)';
                setTimeout(function(){
                    if (stopCycle) return;
        ...
        btnJ.off('click hover:enter keydown');
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                setTimeout(function(){
                    ...
                    label.style.fontSize = cfg.dice_font + 'px';
                    label.style.opacity = '0';
                    label.style.transform = 'scale(.98)';
                    setTimeout(function(){
                        if (!window._sw_currentModalHtml) return;
    ...
    function showModal(movie) {
        try {
            ...
            analyze(movie).then(function(a){
                if (window._sw_currentModalHtml !== html) return;
                clearLoader();
                ...
                setTimeout(function(){
                    ...
                    html.find('.sw-list li').each(function(i){ var li = $(this); setTimeout(function(){ li.addClass('appear'); }, 80 + i * 45); });
                    if (window._sw_blocknav) highlightVisible();
                }, 100);
                try { Lampa.Controller.toggle('should_watch_modal_enhanced'); } catch(e) {}
            }).catch(function(err){
                clearLoader();
    ...
    function startPlugin() {
        if (window._sw_started) return;
        window._sw_started = true;
        try {
        ...
        try { if (!window._sw_controllerRegistered) { registerController(); } } catch(e) {}
