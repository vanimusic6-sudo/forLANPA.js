(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            /* --- 3D-КУБИК: скруглённые грани + наклон в покое --- */
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;perspective:220px;--sw-half:32px;transform:rotate(-6deg)}' +
            '.sw-dice::after{content:"";position:absolute;bottom:-6px;left:10%;width:80%;height:8px;background:rgba(0,0,0,.25);border-radius:50%;filter:blur(4px);transform:scale(1);opacity:.6;pointer-events:none}' +
            '.sw-cube{position:absolute;inset:0;transform-style:preserve-3d;will-change:transform}' +
            '.sw-face{position:absolute;inset:0;background:linear-gradient(180deg,#fbfbfc,#e8eaec);border:2px solid #cfd2d5;border-radius:18px;display:block}' +
            '.sw-face svg{width:100%;height:100%;display:block}' +
            '.sw-face.f1{transform:translateZ(var(--sw-half))}' +
            '.sw-face.f2{transform:rotateY(180deg) translateZ(var(--sw-half))}' +
            '.sw-face.f3{transform:rotateY(90deg) translateZ(var(--sw-half))}' +
            '.sw-face.f4{transform:rotateY(-90deg) translateZ(var(--sw-half))}' +
            '.sw-face.f5{transform:rotateX(90deg) translateZ(var(--sw-half))}' +
            '.sw-face.f6{transform:rotateX(-90deg) translateZ(var(--sw-half))}' +
            '.sw-dice.sw-run{animation:swX 1.6s cubic-bezier(.45,0,.55,1) forwards}' +
            '.sw-dice.sw-run::after{animation:swShadow 1.6s linear forwards}' +
            '.sw-dice.sw-run .sw-cube{animation:swCube 1.6s linear forwards}' +
            /* подкат с наклоном -> выпрямляется в полёте -> наклон при приземлении */
            '@keyframes swX{0%{transform:translateX(0) rotate(-6deg)}22%{transform:translateX(var(--sw-dist)) rotate(0deg)}78%{transform:translateX(var(--sw-dist)) rotate(0deg)}100%{transform:translateX(0) rotate(-6deg)}}' +
            /* БРОСОК: подскок по дуге + кувырок через rotateX (не Рубик) */
            '@keyframes swCube{0%{transform:rotateZ(0) rotateX(0) translateY(0)}22%{transform:rotateZ(360deg) rotateX(0) translateY(0)}32%{transform:rotateZ(360deg) rotateX(180deg) translateY(-34px)}46%{transform:rotateZ(360deg) rotateX(360deg) translateY(0) scale(1.06,.94)}58%{transform:rotateZ(360deg) rotateX(450deg) translateY(-16px)}70%{transform:rotateZ(360deg) rotateX(630deg) translateY(-4px)}78%,100%{transform:rotateZ(360deg) rotateX(720deg) translateY(0) scale(1,1)}}' +
            '@keyframes swShadow{0%,22%,78%,100%{transform:scale(1);opacity:.6}32%{transform:scale(.5);opacity:.2}46%{transform:scale(1.08);opacity:.65}58%{transform:scale(.7);opacity:.35}70%{transform:scale(1.02);opacity:.55}}' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v55.0 (rounded dice, bigger text, softer throw, resting tilt)');
    }
