 ...
 * Version: 50.2 (Ultimate Polish & TV D-Pad Hybrid Navigation)
...
(function () {
    ...
    // Strict Binary Verdicts for Dice Roll (Technical Requirement: exclude 'СПОРНО')
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dossier{position:relative;background:linear-gradient(160deg,#363c3f 0%,#272c2f 100%);border:1px solid rgba(255,255,255,.07);border-radius:20px;padding:32px 36px;margin-bottom:20px;box-shadow:0 8px 24px rgba(0,0,0,.35);animation:swRise .5s cubic-bezier(.22,1,.36,1) both}' +
            ...
            '.sw-verdict-word.yes{color:#4caf50;text-shadow:0 2px 14px rgba(76,175,80,.35)}' +
            '.sw-verdict-word.no{color:#e53935;text-shadow:0 2px 14px rgba(229,57,53,.35)}' +
            '.sw-verdict-word.maybe{color:#fb8c00;text-shadow:0 2px 14px rgba(251,140,0,.35)}' +
            '.sw-meter{height:5px;border-radius:3px;background:rgba(255,255,255,.1);overflow:hidden;margin-top:4px}' +
            '.sw-meter-fill{height:100%;width:0;transition:width 1s cubic-bezier(.22,1,.36,1)}' +
            '.sw-meter-fill.yes{background:linear-gradient(90deg,#2e7d32,#4caf50);box-shadow:0 0 10px rgba(76,175,80,.5)}' +
            '.sw-meter-fill.no{background:linear-gradient(90deg,#b71c1c,#e53935);box-shadow:0 0 10px rgba(229,57,53,.5)}' +
            '.sw-meter-fill.maybe{background:linear-gradient(90deg,#e65100,#fb8c00);box-shadow:0 0 10px rgba(251,140,0,.5)}' +
            ...
            '.sw-tab-btn.focus{box-shadow:0 0 0 3px #ffffff,0 8px 20px rgba(0,0,0,.5);transform:scale(1.02);color:#fff}' +
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // Calculate Atmosphere & Pace on 10-point scale
            ...
            var mComplex = (hasGenre(genres, /фантастика|детектив|mystery/i) || inText(overview, /twist|mind|сон|подсознан/i)) ? 8.4 : 5.8;
            ...
            var targetAudience = '';
            var notSuitableFor = '';
            if (hasGenre(genres, /ужасы|хоррор/i) || inText(overview, /одержим|сталкер|маньяк|пугающ/i)) {
    ...
    function buildReadyInner(a) {
        ...
        return (movieTitle ? '<div style="font-family:' + COND + ';font-size:1.3em;font-weight:700;color:#9aa1a6;text-transform:lowercase;letter-spacing:0.04em;margin-bottom:12px">смотреть ли: ' + movieTitle + '</div>' : '') +
    ...
    function showModal(rawMovie) {
        try {
            ...
            var phases = ['Анализирую данные…','Проверяю TMDB…','Анализ настроений…','Оценка отзывов…','Формирую вердикт…'];
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v50.2 Plugin initialized successfully with memory-safe lifecycle and strict binary dice logic.');
    }
