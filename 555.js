(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v53';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;will-change:transform;perspective:800px}' +
            ...
            '.sw-dice-core{display:block;width:100%;height:100%;will-change:transform;transform-style:preserve-3d}' +
            '.sw-dice-core svg{width:100%;height:100%;display:block;transform:rotate(-8deg)}' +
            ...
            '.sw-dice.sw-run .sw-dice-core{animation:swRot3D 1.2s linear forwards}' +
            '.sw-dice.sw-run .sw-dice-core svg{animation:swSquash 1.2s cubic-bezier(.4,0,.6,1) forwards}' +
            ...
            '@keyframes swRot3D{from{transform:rotateX(0) rotateY(0)}to{transform:rotateX(var(--sw-rot-x)) rotateY(var(--sw-rot-y))}}' +
            '@keyframes swSquash{0%,22%,78%,100%{transform:rotate(-8deg) translateY(0) scale(1,1)}30%{transform:rotate(-8deg) translateY(calc(var(--sw-bounce) * -1)) scale(.94,1.06)}42%{transform:rotate(-8deg) translateY(0) scale(1.07,.93)}54%{transform:rotate(-8deg) translateY(calc(var(--sw-bounce) * -0.5)) scale(.97,1.04)}66%{transform:rotate(-8deg) translateY(0) scale(1.04,.96)}}' +
            '@keyframes swShadow{0%,22%,78%,100%{transform:scale(1);opacity:.6}30%{transform:scale(.55);opacity:.25}42%{transform:scale(1.05);opacity:.6}54%{transform:scale(.75);opacity:.35}66%{transform:scale(1.02);opacity:.55}}' +
            '@media(prefers-reduced-motion:reduce){.sw-dice.sw-run,.sw-dice.sw-run::after,.sw-dice.sw-run .sw-dice-core,.sw-dice.sw-run .sw-dice-core svg{animation:none !important}.sw-dicebtn{transition:none !important}}' +
    ...
    // Выносим на window, чтобы bindDice мог их вызвать через window.setFace
    window._sw_setFace = setFace;
    window._sw_rndFace = rndFace;
    ...
    function bindDice(html) {
        ...
        // click + hover:enter (для ТВ-пульта) + keydown
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                // Физика: случайные параметры
                var dist = Math.max(0, Math.round(btn.clientWidth / 2 - 40));
                var randomRotX = (Math.floor(Math.random() * 4) + 4) * 360;
                var randomRotY = (Math.floor(Math.random() * 4) + 4) * 360;
                var bounceHeight = Math.random() * 15 + 15;
                
                dice.style.setProperty('--sw-dist', dist + 'px');
                dice.style.setProperty('--sw-rot-x', randomRotX + 'deg');
                dice.style.setProperty('--sw-rot-y', randomRotY + 'deg');
                dice.style.setProperty('--sw-bounce', bounceHeight + 'px');

                // Сброс анимации (force reflow)
                dice.classList.remove('sw-run');
                void dice.offsetWidth;

                // Старт
                btnJ.addClass('rolling');
                dice.classList.add('sw-run');

                // Смена грани в середине полёта (600ms = половина от 1.2s)
                setTimeout(function() { 
                    if (window._sw_setFace && window._sw_rndFace) {
                        window._sw_setFace(dice, window._sw_rndFace()); 
                    }
                }, 600);

                // Финал (1200ms = длительность CSS-анимации)
                setTimeout(function() {
                    dice.classList.remove('sw-run');
                    btnJ.removeClass('rolling');
                ...
                console.error('[ShouldWatch] Dice error:', err);
                window._sw_rolling = false;
                btnJ.removeClass('rolling');
                dice.classList.remove('sw-run');
            }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v53.0 (3D dice, bugfix, sync timings)');
    }
