(function () {
    ...
    // ======================================================================
    // CSS РЯДА КАРТОЧЕК
    // Фон каждой карточки рисуется внутри её SVG (как в эскизах), поэтому
    // контейнер прозрачный. Обложка поднимается вровень с заголовком.
    // ======================================================================
    function injectCSS() {
        ...
        s.innerHTML =
            '.sw-cards-row{display:flex;gap:16px;margin:16px 0 26px;overflow-x:auto;padding:6px 2px 10px}' +
            ...
            '.sw-card--free{overflow:visible}' +
            '@media(max-width:640px){.sw-card{width:210px}}' +
            /* поднимаем обложку к верхнему краю заголовка; если в вашей теме
               съезжает — поставьте 0 или подберите значение */
            '.full-start__poster,.full-start-new__poster,.full-card__poster{margin-top:-56px}';
    ...
    // ======================================================================
    // DOM-СИГНАЛЫ СО СТРАНИЦЫ ЛАМПЫ И ЛОКАЛЬНЫЕ ДАННЫЕ ЛАМПЫ
    // ======================================================================
    ...
    // ======================================================================
    // РИСУНКИ КАРТОЧЕК — векторные, перерисованы с эскизов.
    // Единая сетка 400x300, фон каждой карточки — внутри её SVG.
    // ======================================================================
    ...
    /* «ВОЙНА» — камуфляж: оливковые пятна-подложки + серо-зелёные сверху */
    function artWar() {
        var olive = '#a4a94a', green = '#5f7d6b', g1 = '', g2 = '', i;
        var P = [[96,74,44,26],[236,62,52,28],[330,92,42,26],[86,150,40,24],[300,168,56,30],[176,208,52,28],[322,232,46,26],[110,246,42,24],[232,252,40,22]];
        for (i = 0; i < P.length; i++) {
            g1 += '<ellipse cx="' + (P[i][0] + 10) + '" cy="' + (P[i][1] + 8) + '" rx="' + P[i][2] + '" ry="' + P[i][3] + '"/>';
            g2 += '<ellipse cx="' + P[i][0] + '" cy="' + P[i][1] + '" rx="' + (P[i][2] - 4) + '" ry="' + (P[i][3] - 3) + '"/>';
        }
        return swBg('#20242b') + '<g fill="' + olive + '">' + g1 + '</g><g fill="' + green + '">' + g2 + '</g>' + swText(200, 178, 74, '#0c0c0c', 'война');
    }

    /* «ЮМОР» — смеющийся смайл с дорожками слёз + жёлтое слово */
    function artHumor() {
        var t = swBg('#20242b') + '<g transform="translate(104,96)">', i;
        t += '<circle r="62" fill="#f6c516"/>';
        t += '<circle cx="-22" cy="-16" r="14" fill="#000"/><circle cx="16" cy="-18" r="14" fill="#000"/>';
        t += '<circle cx="-17" cy="-21" r="5" fill="#fff"/><circle cx="21" cy="-23" r="5" fill="#fff"/>';
        t += '<path d="M -24 12 Q 0 40 30 8 Q 4 24 -24 12 Z" fill="#fff" stroke="#000" stroke-width="4" stroke-linejoin="round"/>';
        var L = [[-56,0],[-60,14],[-56,28],[-48,40],[-38,50]], R = [[50,-10],[58,2],[60,16]];
        for (i = 0; i < L.length; i++) t += '<circle cx="' + L[i][0] + '" cy="' + L[i][1] + '" r="6" fill="#8ec7e8"/>';
        for (i = 0; i < R.length; i++) t += '<circle cx="' + R[i][0] + '" cy="' + R[i][1] + '" r="6" fill="#8ec7e8"/>';
        t += '</g>';
        return t + swText(216, 208, 84, '#f6c516', 'юмор');
    }

    /* «ДОСТОЯНИЕ ЖАНРА» — две рамки: с портретом и чёрная */
    ...
    /* «СТРАХ» — топор с ремешком в верхней кромке, кровь под лезвием */
    function artFear() {
        var t = swBg('#20242b') + '<g transform="translate(238,64) rotate(6)">';
        t += '<path d="M -78 -16 L 18 -26 L 24 24 L -72 28 Z" fill="#63666d"/>';
        t += '<line x1="-72" y1="-8" x2="16" y2="-16" stroke="#9aa0a8" stroke-width="3"/>';
        t += '<rect x="18" y="-16" width="92" height="17" fill="#7a4a20"/>';
        t += '<rect x="102" y="-19" width="13" height="23" fill="#5a3a18"/>';
        t += '<path d="M 115 -8 q 26 10 15 34 q -9 17 -19 6" fill="none" stroke="#5a3a18" stroke-width="5"/>';
        t += '</g>';
        t += '<g fill="#6b0010"><ellipse cx="176" cy="92" rx="34" ry="16"/><ellipse cx="216" cy="98" rx="42" ry="20"/><ellipse cx="252" cy="110" rx="26" ry="22"/><ellipse cx="196" cy="108" rx="26" ry="14"/></g>';
        return t + swText(200, 200, 84, '#8b0012', 'страх');
    }

    /* «НАРК.» — шприц в правом верхнем углу, игла вниз-влево */
    function artDrugs() {
        var t = swBg('#202124') + '<g transform="translate(300,86) rotate(45)">';
        t += '<rect x="-22" y="-80" width="44" height="10" rx="4" fill="#8a8a8a"/>';   // упор
        t += '<rect x="-6" y="-72" width="12" height="26" fill="#8a8a8a"/>';            // шток
        t += '<rect x="-16" y="-48" width="32" height="70" rx="10" fill="#8a8a8a"/>';   // цилиндр
        t += '<path d="M -14 8 q 14 10 30 0" fill="none" stroke="#555" stroke-width="3"/>';
        t += '<line x1="0" y1="22" x2="0" y2="56" stroke="#bbb" stroke-width="3"/>';    // игла
        t += '<line x1="0" y1="52" x2="0" y2="62" stroke="#1a1a1a" stroke-width="5"/>'; // кончик
        t += '</g>';
        t += swSmall(185, 142, 32, '#f5f5f5', 'употребление');
        return t + swText(195, 228, 100, '#f5f5f5', 'нарк.', 'textLength="300" lengthAdjust="spacingAndGlyphs"');
    }

    /* «РЕАЛЬНЫЕ СОБЫТИЯ» — камера в правом верхнем углу, серый фон */
    function artReal() {
        var t = swBg('#4a4a52') + '<g transform="translate(302,72) rotate(-14)">';
        t += '<rect x="-46" y="-36" width="92" height="72" rx="16" fill="#232323"/>';
        t += '<circle cx="0" cy="4" r="22" fill="#000"/>';
        t += '<rect x="16" y="-44" width="14" height="9" fill="#000"/>';
        t += '</g>';
        t += swSmall(232, 142, 32, '#0c0c0c', 'реальные');
        return t + swText(200, 212, 84, '#0c0c0c', 'события', 'textLength="340" lengthAdjust="spacingAndGlyphs"');
    }

    /* «SEX» — бордовый фон, знак запрета, чёрное слово */
    function artSex() {
        var t = swBg('#8a0018');
        t += '<circle cx="200" cy="150" r="95" fill="none" stroke="#a3001e" stroke-width="16"/>';
        t += '<line x1="140" y1="82" x2="260" y2="218" stroke="#a3001e" stroke-width="26"/>';
        return t + swText(200, 185, 96, '#000', 'sex', 'font-style="italic"');
    }

    /* «РАЗБОЙ И КРИМИНАЛ» — наручники с цепочкой в правом верхнем углу */
    function artCrime() {
        var t = swBg('#2b2e33');
    ...
    /* «ПОДХОДИТ ДЛЯ ДЕТЕЙ» — карточка-мишка: уши, шапочка с помпоном */
    function artKids() {
        var tan = '#b5772f', t = '';
        t += '<circle cx="80" cy="80" r="45" fill="' + tan + '"/><circle cx="320" cy="80" r="45" fill="' + tan + '"/>';
        t += '<circle cx="80" cy="80" r="25" fill="#7a1635"/><circle cx="320" cy="80" r="25" fill="#7a1635"/>';
        t += '<rect x="40" y="70" width="320" height="200" rx="40" fill="' + tan + '"/>';
        t += '<polygon points="318,22 255,88 328,96" fill="#8a2148"/>';
        t += '<circle cx="318" cy="20" r="9" fill="#8a6a1a"/>';
        t += '<text x="200" y="152" text-anchor="middle" font-size="28" font-weight="800" fill="#0c0c0c" font-family="' + SW_FONT + '">ПОДХОДИТ ДЛЯ</text>';
        return t + swText(200, 240, 100, '#0c0c0c', 'детей', 'textLength="300" lengthAdjust="spacingAndGlyphs"');
    }

    // ======================================================================
    // КАКАЯ КАРТОЧКА КОГДА ВЫВОДИТСЯ.
    // Выводятся ТОЛЬКО отрисованные карточки: нет рисунка — параметр не
    // показывается. Дорисуешь новые — добавь функцию и строку в список.
    // ======================================================================
    var CARD_SLOTS = 6;
    var SW_CUSTOM_CARDS = [
        ...
        { id: 'kids',     re: /детский|семейный/i,                        art: artKids, free: true }
    ];
        ...
        F.forEach(function(f){
            ...
            for (var i = 0; i < SW_CUSTOM_CARDS.length; i++) {
                ...
                if (c.re.test(f.text) && !used[c.id]) {
                    used[c.id] = 1;
                    cards.push({ id: c.id, svg: c.art(), title: f.text, free: !!c.free });
                    break;
                }
            }
        });
    ...
    function renderCards(el, movie) {
        try {
            ...
            analyze(movie).then(function(result) {
                ...
                if (!cards.length) { row.remove(); return; } // нет рисунков — нет ряда
                cards.forEach(function(c){
                    row.append($('<div class="sw-card' + (c.free ? ' sw-card--free' : '') + '"></div>').attr('title', c.title).html(svgWrap(c.svg)));
                });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v68.0-cards (только отрисованные карточки)');
    }
