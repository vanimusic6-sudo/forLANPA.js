(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v70';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            /* РЯД КАРТОЧЕК (базовый серый, заменишь на свои стили) */
            '.sw-row-wrap{padding:10px 0 30px}' +
            '.sw-row-title{font-size:1.2em;font-weight:700;color:#e9ebeb;margin:0 0 12px;padding:0 4px}' +
            '.sw-row{display:flex;gap:20px;overflow-x:auto;overflow-y:hidden;padding:4px 4px 18px;-webkit-overflow-scrolling:touch}' +
            '.sw-row::-webkit-scrollbar{height:5px}.sw-row::-webkit-scrollbar-thumb{background:rgba(255,255,255,.18);border-radius:10px}' +
            '.sw-card{flex:0 0 auto;width:250px;min-height:270px;background:#26282b;border-radius:24px;padding:20px;box-sizing:border-box;display:flex;flex-direction:column;animation:swCardIn .4s ease both}' +
            '@keyframes swCardIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}' +
            '.sw-card-label{font-size:.8em;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:12px;color:#9aa1a6}' +
            '.sw-card-label.pros{color:#8bab74}.sw-card-label.cons{color:#c47b83}' +
            '.sw-card-verdict{font-size:2em;font-weight:800;text-transform:uppercase;line-height:1.1;margin-bottom:14px}' +
            '.sw-card-verdict.yes{color:#8bab74}.sw-card-verdict.no{color:#c47b83}.sw-card-verdict.maybe{color:#c99968}' +
            '.sw-card-meter{height:5px;border-radius:3px;background:rgba(255,255,255,.1);overflow:hidden;margin-top:auto}' +
            '.sw-card-meter-fill{height:100%;border-radius:3px}' +
            '.sw-card-meter-fill.yes{background:linear-gradient(90deg,#4d5f3a,#7d9a66)}' +
            '.sw-card-meter-fill.no{background:linear-gradient(90deg,#5d3138,#a25e67)}' +
            '.sw-card-meter-fill.maybe{background:linear-gradient(90deg,#7a5231,#c99968)}' +
            '.sw-card-list{margin:0;padding-left:18px;font-size:.95em;line-height:1.55;color:#cdd2d5;overflow:hidden}' +
            '.sw-card-list li{margin-bottom:8px}';
    ...
    /* ===== НОВЫЙ ИНТЕРФЕЙС: РЯД КАРТОЧЕК НА СТРАНИЦЕ ===== */
    function buildRow(a) {
        var prosHtml = a.pros.slice(0, 6).map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('');
        var consHtml = a.cons.slice(0, 6).map(function(c){ return '<li>' + esc(c) + '</li>'; }).join('');
        return '<div class="sw-row-wrap">' +
            '<div class="sw-row-title">Стоит ли смотреть?</div>' +
            '<div class="sw-row">' +
              '<div class="sw-card sw-card--verdict">' +
                '<div class="sw-card-label">Вердикт</div>' +
                '<div class="sw-card-verdict ' + a.vClass + '">' + esc(a.vWord) + '</div>' +
                '<div class="sw-card-meter"><div class="sw-card-meter-fill ' + a.vClass + '" style="width:' + a.norm + '%"></div></div>' +
              '</div>' +
              '<div class="sw-card sw-card--pros">' +
                '<div class="sw-card-label pros">✓ Аргументы за</div>' +
                '<ul class="sw-card-list">' + prosHtml + '</ul>' +
              '</div>' +
              '<div class="sw-card sw-card--cons">' +
                '<div class="sw-card-label cons">✗ Аргументы против</div>' +
                '<ul class="sw-card-list">' + consHtml + '</ul>' +
              '</div>' +
            '</div>' +
          '</div>';
    }
    ...
    function renderCards(el, movie) {
        try {
            ...
            if (el.find('.sw-row-wrap').length) return; // уже есть
            var row = $('<div class="sw-row-wrap"><div class="sw-row-title">Стоит ли смотреть?</div><div class="sw-row">' +
                '<div class="sw-card"></div><div class="sw-card"></div><div class="sw-card"></div>' +
                '</div></div>');
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v70.0 (card-row UI, no modal)');
    }
