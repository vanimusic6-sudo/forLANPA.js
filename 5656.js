(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v63';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#8fc49a,transparent);animation:swSlide 1.4s ease-in-out infinite}' +
            ...
            '.sw-mode-dot.active{background:#8fc49a}.sw-mode-dot.inactive{background:#64748b}' +
            ...
            '.sw-verdict-word.yes{color:#8fc49a}.sw-verdict-word.no{color:#d99a94}.sw-verdict-word.maybe{color:#d9bd8f}' +
            ...
            '.sw-meter-fill.yes{background:#8fc49a}.sw-meter-fill.no{background:#d99a94}.sw-meter-fill.maybe{background:#d9bd8f}' +
            '.sw-dicebtn{display:block;width:100%;height:130px;border-radius:14px;background:linear-gradient(180deg,#a8aaad 0%,#939599 100%);position:relative;overflow:hidden;border:none;margin:0 0 22px;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;transition:transform .2s ease}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 4px rgba(255,255,255,.5)}' +
            '.sw-dicebtn.shaking{animation:swShake 1.5s cubic-bezier(.36,.07,.19,.97) both}' +
            '@keyframes swShake{10%,90%{transform:translate3d(-1px,0,0)}20%,80%{transform:translate3d(2px,0,0)}30%,50%,70%{transform:translate3d(-4px,0,0)}40%,60%{transform:translate3d(4px,0,0)}}' +
            '.sw-label-wrap{position:absolute;top:0;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;padding:0 30px 0 120px;overflow:hidden}' +
            /* ШРИФТ ЕЩЁ КРУПНЕЕ */
            '.sw-label{font-family:' + COND + ';font-size:4.4em;font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;white-space:nowrap;opacity:0;transition:opacity .3s ease;letter-spacing:-.02em}' +
            '.sw-label.show{opacity:1}' +
            '.sw-label.res-yes{color:#477052}.sw-label.res-no{color:#9a5a54}' +
            ...
            '.premium-dice-svg{width:100%;height:100%;border-radius:22%;box-shadow:0 4px 12px rgba(0,0,0,.2);transition:opacity .18s ease}' +
            '@media(prefers-reduced-motion:reduce){.sw-dicebtn.shaking{animation:none !important}.sw-dicebtn{transition:none !important}}' +
            ...
            '.sw-col.pros::before{background:linear-gradient(180deg,#8fc49a,#5f8f6c)}' +
            '.sw-col.cons::before{background:linear-gradient(180deg,#d99a94,#a06a64)}' +
            '.sw-title{font-size:.85em;font-weight:700;margin-bottom:14px;text-transform:lowercase;letter-spacing:.03em}' +
            '.sw-title.pros{color:#b9dcc0}.sw-title.cons{color:#ecc9c5}' +
    ...
    function bindDice(html) {
        ...
        // Плавная последовательная смена: fade-out → замена (всегда другая грань) → fade-in
        function scheduleSwap(){
            faceTimer = setTimeout(function(){
                if (stopCycle) return;
                var svg = dice.querySelector('svg');
                ...
                faceTimer = setTimeout(function(){
                    ...
                    var s2 = dice.querySelector('svg');
                    if (s2){ s2.style.opacity = '0'; void s2.offsetWidth; s2.style.opacity = '1'; }
                    scheduleSwap();
                }, 150);
            }, 240);
        }
        ...
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                btnJ.removeClass('shaking'); void btn.offsetWidth; btnJ.addClass('shaking');
                scheduleSwap();
                setTimeout(function(){
                    clearCycle();
    ...
    function showModal(movie) {
        try {
            ...
            analyze(movie).then(function(a){
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#d99a94">Ошибка анализа</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v63.0 (unified palette, bigger text, smooth face swap)');
    }
