(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v71';
    ...
    /* ===== ИКОНКИ-ПЛАШКИ (заменяй на свои нарисованные) ===== */
    ...
    /* Реестр плашек: word + цвет + иконка. Добавляй свои сюда. */
    var PLAQUES = {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            '.sw-row-wrap{padding:6px 0 30px}' +
            '.sw-row-title{font-size:1.2em;font-weight:700;color:#e9ebeb;margin:0 0 14px;padding:0 4px}' +
            '.sw-row{display:flex;gap:22px;overflow-x:auto;overflow-y:hidden;padding:44px 4px 20px;-webkit-overflow-scrolling:touch}' +
            '.sw-row::-webkit-scrollbar{height:5px}.sw-row::-webkit-scrollbar-thumb{background:rgba(255,255,255,.18);border-radius:10px}' +
            /* ПЛАШКА */
            '.sw-plaque{position:relative;flex:0 0 auto;width:250px;height:190px;background:#26282b;border:1px solid #3a3d42;border-radius:26px;display:flex;align-items:center;justify-content:center;overflow:visible;animation:swCardIn .4s ease both}' +
            '@keyframes swCardIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}' +
            '.sw-plaque-word{font-family:' + COND + ';font-size:2.6em;font-weight:800;text-transform:lowercase;letter-spacing:.01em;line-height:1}' +
            '.sw-plaque-icon{position:absolute;top:-36px;right:10px;width:120px;height:86px;pointer-events:none}' +
            '.sw-plaque-icon svg{width:100%;height:100%}' +
            '.sw-plaque--verdict .sw-plaque-word{font-size:2.4em}' +
            '.sw-plaque-meter{position:absolute;left:22px;right:22px;bottom:18px;height:5px;border-radius:3px;background:rgba(255,255,255,.1);overflow:hidden}' +
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (rt.total >= 4 && rt.tone === 'pos') add('reviews','pro','💬 Зрители в основном хвалят', 14, 'rev');
            else if (rt.total >= 4 && rt.tone === 'neg') add('reviews','con','💬 Зрители в основном критикуют', 16, 'rev');
    ...
    function buildRow(a) {
        ...
        var cards = '<div class="sw-plaque sw-plaque--verdict">' +
        ...
        Object.keys(PLAQUES).forEach(function(id){
            if (a.marks[id]) cards += plaqueHtml(PLAQUES[id]);
        });
        return '<div class="sw-row-wrap"><div class="sw-row-title">Стоит ли смотреть?</div><div class="sw-row">' + cards + '</div></div>';
    }
    ...
    function renderCards(el, movie) {
        try {
            ...
            var row = $('<div class="sw-row-wrap"><div class="sw-row-title">Стоит ли смотреть?</div><div class="sw-row"><div class="sw-plaque"></div><div class="sw-plaque"></div><div class="sw-plaque"></div></div></div>');
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v71.0 (plaque cards)');
    }
