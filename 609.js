(function () {
    ...
    var DISPLAY = '"Plus Jakarta Sans","Trebuchet MS","Segoe UI",system-ui,sans-serif';
    var GENRE_ID_ANIM = 16, GENRE_ID_FAMILY = 10751, GENRE_ID_KIDS = 10762;
    var TMDB_TIMEOUT_MS = 6500;
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                '.sw-modal-content{padding:24px 28px 48px;color:#f3f4f6;font-family:' + DISPLAY + ';box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y}' +
                '.sw-modal-content::-webkit-scrollbar{width:6px}.sw-modal-content::-webkit-scrollbar-thumb{background:rgba(255,255,255,.25);border-radius:3px}' +
                '.sw-body{animation:swFadeIn .38s cubic-bezier(.16,1,.3,1)}' +
                '@keyframes swFadeIn{from{opacity:0;transform:translateY(12px) scale(.99)}to{opacity:1;transform:translateY(0) scale(1)}}' +
                '.sw-loader{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;padding:76px 20px;min-height:50vh;color:#9ca3af}' +
                '.sw-loader-emoji{font-size:3.6em;line-height:1;animation:swFloat 2.4s ease-in-out infinite}' +
                '@keyframes swFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-10px) scale(1.06)}}' +
                '.sw-loader-text{font-size:1.1em;font-weight:700;min-height:1.5em;transition:opacity .25s ease;text-align:center;color:#e5e7eb}' +
                '.sw-loader-progress{width:240px;height:5px;border-radius:3px;background:rgba(255,255,255,.1);overflow:hidden;position:relative;margin-top:6px}' +
                '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#10b981,#34d399,transparent);animation:swSlide 1.2s cubic-bezier(.4,0,.2,1) infinite}' +
                '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
                '.sw-dossier{position:relative;background:linear-gradient(145deg,#1f2328,#17191c);border:1px solid rgba(255,255,255,.12);border-radius:20px;padding:26px 30px;margin-bottom:20px;box-shadow:0 12px 36px rgba(0,0,0,.45);animation:swRise .45s cubic-bezier(.22,1,.36,1) both}' +
                '@keyframes swRise{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}' +
                '.sw-badges{position:absolute;top:20px;right:20px;display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;max-width:55%}' +
                '.sw-mode-badge{display:inline-flex;align-items:center;gap:6px;font-size:.72em;padding:5px 14px;border-radius:20px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;background:rgba(255,255,255,.08);color:#9ca3af;border:1px solid rgba(255,255,255,.06)}' +
                '.sw-mode-dot{width:6px;height:6px;border-radius:50%;display:inline-block}' +
                '.sw-mode-dot.active{background:#10b981;box-shadow:0 0 10px rgba(16,185,129,.9)}' +
                '.sw-mode-dot.inactive{background:#6b7280}' +
                '.sw-verdict-word{font-size:3em;font-weight:900;letter-spacing:.02em;line-height:1;margin:0 0 18px;text-transform:uppercase;opacity:0;transform:translateY(10px);transition:opacity .45s cubic-bezier(.16,1,.3,1),transform .45s cubic-bezier(.16,1,.3,1)}' +
                '.sw-verdict-word.appear{opacity:1;transform:translateY(0)}' +
                '.sw-verdict-word.yes{color:#34d399;text-shadow:0 0 35px rgba(52,211,153,.35)}.sw-verdict-word.no{color:#f87171;text-shadow:0 0 35px rgba(248,113,113,.35)}.sw-verdict-word.maybe{color:#fbbf24;text-shadow:0 0 35px rgba(251,191,36,.35)}' +
                '.sw-meter{height:7px;border-radius:4px;background:rgba(255,255,255,.12);overflow:hidden;border:1px solid rgba(255,255,255,.05)}' +
                '.sw-meter-fill{height:100%;width:0;border-radius:4px;transition:width .9s cubic-bezier(.22,1,.36,1)}' +
                '.sw-meter-fill.yes{background:linear-gradient(90deg,#059669,#10b981,#34d399)}.sw-meter-fill.no{background:linear-gradient(90deg,#dc2626,#ef4444,#f87171)}.sw-meter-fill.maybe{background:linear-gradient(90deg,#d97706,#f59e0b,#fbbf24)}' +
                '.sw-dicebtn{position:relative;display:block;width:100%;height:108px;background:linear-gradient(135deg,#23272d,#1a1d21);border:none;border-radius:20px;margin:0 0 22px;overflow:hidden;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;transition:box-shadow .25s ease,transform .18s ease;box-shadow:0 8px 24px rgba(0,0,0,.35)}' +
                '.sw-dicebtn:hover{transform:translateY(-1px);box-shadow:0 12px 28px rgba(0,0,0,.45)}' +
                '.sw-dicebtn:active{transform:scale(.99)}' +
                '.sw-dicebtn.focus{box-shadow:0 0 0 4px rgba(52,211,153,.8),0 0 25px rgba(52,211,153,.3)}' +
                '.sw-custom-button-enhanced,.sw-custom-button-enhanced.focus{outline:none}' +
                '.sw-custom-button-enhanced.focus{background:rgba(255,255,255,.18);color:#fff;box-shadow:none;border-color:transparent}' +
                '.sw-label-wrap{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:0 110px;overflow:hidden}' +
                '.sw-label{font-family:' + DISPLAY + ';font-size:2.1em;font-weight:900;color:#9ca3af;letter-spacing:.02em;text-align:center;white-space:nowrap;opacity:1;transition:opacity .25s ease,transform .25s ease}' +
                '.sw-label.hidden{opacity:0;transform:scale(.92)}' +
                '.sw-label.res-yes{color:#34d399;text-shadow:0 0 25px rgba(52,211,153,.5)}.sw-label.res-no{color:#f87171;text-shadow:0 0 25px rgba(248,113,113,.5)}' +
                '.sw-dice{position:absolute;left:20px;top:50%;width:74px;height:74px;margin-top:-37px;will-change:transform;transform:translateZ(0);filter:drop-shadow(0 6px 14px rgba(0,0,0,.5))}' +
                '.sw-dice svg{width:100%;height:100%;display:block;transform:rotate(-6deg)}' +
                '.sw-dice.sw-rolling{animation:swRoll .92s cubic-bezier(.25,.46,.45,.94) forwards}' +
                '@keyframes swRoll{0%{transform:translateX(0) rotate(0) translateY(0) scale(1)}20%{transform:translateX(calc(var(--sw-dist) * .2)) rotate(140deg) translateY(-16px) scale(1.1)}40%{transform:translateX(calc(var(--sw-dist) * .45)) rotate(320deg) translateY(0) scale(.94)}65%{transform:translateX(calc(var(--sw-dist) * .75)) rotate(540deg) translateY(-10px) scale(1.05)}85%{transform:translateX(calc(var(--sw-dist) * .94)) rotate(680deg) translateY(0) scale(.98)}100%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0) scale(1)}}' +
                '.sw-tabs-bar{display:flex;gap:10px;margin-bottom:20px;padding:6px;background:rgba(255,255,255,.05);border-radius:16px}' +
                '.sw-tab-toggle{flex:1;padding:12px 18px;border:none;border-radius:12px;background:transparent;color:#9ca3af;font-size:1.05em;font-weight:800;cursor:pointer;transition:all .2s ease;display:flex;align-items:center;justify-content:center;gap:8px}' +
                '.sw-tab-toggle:hover{color:#fff;background:rgba(255,255,255,.08)}' +
                '.sw-tab-toggle.active{background:rgba(255,255,255,.15);color:#fff;box-shadow:0 4px 12px rgba(0,0,0,.3)}' +
                '.sw-tab-toggle.active.moods{background:rgba(245,158,11,.2);color:#fbbf24;border:1px solid rgba(245,158,11,.3)}' +
                '.sw-tab-toggle.focus{box-shadow:0 0 0 3px #34d399}' +
                '.sw-columns{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:22px}' +
                '.sw-col{position:relative;background:linear-gradient(145deg,#1f2328,#17191c);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:22px 24px;box-shadow:0 8px 24px rgba(0,0,0,.3)}' +
                '.sw-col::before{content:"";position:absolute;top:0;left:24px;right:24px;height:3px;border-radius:3px}' +
                '.sw-col.pros::before{background:linear-gradient(90deg,#059669,#10b981,#34d399)}.sw-col.cons::before{background:linear-gradient(90deg,#dc2626,#ef4444,#f87171)}' +
                '.sw-title{font-size:.88em;font-weight:900;margin-bottom:16px;text-transform:uppercase;letter-spacing:.06em}' +
                '.sw-title.pros{color:#34d399}.sw-title.cons{color:#f87171}' +
                '.sw-list{margin:0;padding-left:18px;font-size:.96em;line-height:1.65;color:#d1d5db}' +
                '.sw-list li{margin-bottom:10px;opacity:0;transform:translateX(-10px);transition:opacity .35s ease,transform .35s ease}' +
                '.sw-list li.appear{opacity:1;transform:translateX(0)}' +
                '.sw-mood-panel{background:linear-gradient(145deg,#1f2328,#17191c);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:22px 24px;margin-bottom:22px;box-shadow:0 8px 24px rgba(0,0,0,.3)}' +
                '.sw-mood-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-top:14px}' +
                '.sw-mood-card{background:rgba(255,255,255,.04);border-radius:12px;padding:12px 14px;display:flex;flex-direction:column;gap:6px}' +
                '.sw-mood-head{display:flex;justify-content:space-between;font-size:.92em;font-weight:700;color:#e5e7eb}' +
                '.sw-mood-bar{width:100%;height:6px;background:rgba(255,255,255,.1);border-radius:3px;overflow:hidden}' +
                '.sw-mood-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,#f59e0b,#fbbf24)}' +
                '.sw-tags-wrap{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px;padding-top:14px;border-top:1px solid rgba(255,255,255,.08)}' +
                '.sw-tag{padding:6px 12px;border-radius:20px;background:rgba(255,255,255,.06);font-size:.85em;color:#d1d5db;border:1px solid rgba(255,255,255,.08)}' +
                '.sw-focusable{outline:none;cursor:pointer}' +
                '@media(max-width:640px){.sw-modal-content{padding:18px 18px 36px}.sw-columns{grid-template-columns:1fr}.sw-verdict-word{font-size:2.3em}.sw-label{font-size:1.6em}.sw-label-wrap{padding:0 84px}.sw-dice{width:62px;height:62px;margin-top:-31px;left:14px}}';
    ...
    function scrollContainerTo(el, center) {
        ...
        try {
            ...
            try { $(cn).stop(true, false).animate({ scrollTop: target }, 160, 'swing'); }
    ...
    function scrollStep(dir) {
        ...
        try { $(cn).stop(true, false).animate({ scrollTop: target }, 160, 'swing', highlightVisible); }
    ...
    function tmdbGet(path, lang) {
        return new Promise(function(res){
            var done = false;
            var to = setTimeout(function(){ if (!done) { done = true; res(null); } }, TMDB_TIMEOUT_MS);
            ...
            try {
                ...
                if (Lampa.Request && typeof Lampa.Request.get === 'function') {
                    Lampa.Request.get(url, function(d){ finish(d && d.status_code ? null : d); }, function(){ finish(null); }, { dataType: 'json' });
                } else if (typeof fetch !== 'undefined') {
                    fetch(url).then(function(r){ return r.json(); }).then(function(d){ finish(d && d.status_code ? null : d); }).catch(function(){ finish(null); });
                } else finish(null);
            } catch(e) { finish(null); }
        });
    }
    ...
    function computeMoods(kw, genres) {
        ...
        var moods = [];
        if (mm.fear >= 5) { moods.push({ name: 'Тревожный', pct: Math.min(80, mm.fear * 9) }); moods.push({ name: 'Напряжённый', pct: Math.min(70, mm.fear * 8) }); }
    ...
    function findSparkle(meta, credits) {
        ...
        var interestingTags = [
            ...
            { re: /based on true story|основан на реальных событиях|true story/i, text: '✨ Основано на реальных событиях' },
            ...
            { re: /cannes|venice|berlin|film festival winner|каннск|венецианск/i, text: '🎬 Призёр кинофестиваля' },
            { re: /cult film|культов/i, text: '🎭 Культовый фильм' },
            { re: /time travel|путешестви во времен/i, text: '⏰ Путешествия во времени' },
            { re: /heist|ограблен/i, text: '💼 История ограбления' },
            { re: /post.apocalyptic|постапокалипсис/i, text: '🌍 Постапокалиптический мир' },
            { re: /dystopia|антиутопи/i, text: '🏙 Антиутопия' },
            { re: /coming of age|взрослен/i, text: '🌱 История взросления' },
            { re: /road movie|road trip|дорожн приключен/i, text: '🛣 Дорожное приключение' },
            { re: /survival|выживан/i, text: '🏕 История выживания' },
            { re: /revenge|месть|возмезд/i, text: '⚔️ История мести' },
            { re: /con artist|мошенник|аферист/i, text: '🎩 История афериста' },
            { re: /space|космос|moon|lunar|mars|марс/i, text: '🚀 Космическая одиссея' },
            { re: /artificial intelligence|ai|искусственн интеллект/i, text: '🤖 Искусственный интеллект' },
            { re: /alternate reality|параллельн мир|multiverse|мультивселен/i, text: '🌌 Параллельные миры' },
            { re: /biographical|biopic|биограф/i, text: '📖 Биографическая история' }
        ];
        var texts = [meta.enOv].concat(meta.kw);
        interestingTags.forEach(function(t){
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            else if (/4K|UHD|2160p/i.test(q)) add('card','pro','🎥 4K Ultra HD', 10, 'quality');
            ...
            var CAPS = { card: 55, user: 50, tmdb: 45, moods: 40, reviews: 28, lampa: 18, sparkle: 50 };
            var per = {};
            ...
            if (lowConf) score = Math.round(score * 0.65);
            if (score > 100) score = 100; if (score < -100) score = -100;
            var norm = Math.round((score + 100) / 2);
            var vClass = score >= 20 ? 'yes' : (score <= -20 ? 'no' : 'maybe');
            var vWord = score >= 20 ? 'СТОИТ' : (score <= -20 ? 'НЕ СТОИТ' : 'СПОРНО');
            ...
            if (!pros.length) pros.push('ℹ️ Нет явных преимуществ');
            if (!cons.length) cons.push((blG.length || blA.length || blD.length) ? '✅ Фильтры чисты' : '✅ Минусов нет');
            return { pros: pros, cons: cons, review: rt, score: score, norm: norm, vClass: vClass, vWord: vWord, mode: metaRich ? 'TMDB' : 'TAGS', metaRich: metaRich, mm: mm, moods: moods };
        });
    }
    ...
    function diceSVG(n) {
        ...
        for (var i = 0; i < p.length; i++) c += '<circle cx="' + p[i][0] + '" cy="' + p[i][1] + '" r="9.5" fill="#181a1d"/>';
        return '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect x="5" y="5" width="90" height="90" rx="24" fill="#e5e7eb" stroke="#9ca3af" stroke-width="5"/>' + c + '</svg>';
    }
    function rndFace() { return 1 + Math.floor(Math.random() * 6); }

    function buildReadyInner(a) {
        var badges = '<div class="sw-badges">' +
            '<span class="sw-mode-badge"><span class="sw-mode-dot ' + (a.metaRich ? 'active' : 'inactive') + '"></span>TMDB ENHANCED</span>' +
            '</div>';
        
        var mm = a.mm || {};
        var moods = a.moods || [];
        var moodBars = [
            { label: '⚡ Темп и ритм', val: mm.pace || 0 },
            { label: '💥 Экшен и боевые сцены', val: mm.action || 0 },
            { label: '😱 Страх и хоррор', val: mm.fear || 0 },
            { label: '🔪 Насилие и жестокость', val: mm.violence || 0 },
            { label: '😢 Драматизм / грусть', val: mm.sadness || 0 },
            { label: '🤬 Нецензурная лексика', val: mm.language || 0 }
        ];

        var moodCardsHtml = moodBars.map(function(mb){
            var pct = Math.min(100, Math.round((mb.val / 10) * 100));
            return '<div class="sw-mood-card">' +
                '<div class="sw-mood-head"><span>' + esc(mb.label) + '</span><span>' + mb.val + ' / 10</span></div>' +
                '<div class="sw-mood-bar"><div class="sw-mood-fill" style="width:' + pct + '%"></div></div>' +
            '</div>';
        }).join('');

        var tagsHtml = moods.length ? '<div class="sw-tags-wrap">' + moods.map(function(m){
            return '<span class="sw-tag">' + esc(m.name) + ' <b style="color:#fbbf24">' + m.pct + '%</b></span>';
        }).join('') + '</div>' : '';
        ...
        return '' +
            ...
            '<button class="sw-dicebtn sw-focusable" id="sw-dice-btn" tabindex="0">' +
                '<span class="sw-label-wrap"><span class="sw-label" id="sw-dice-label">Бросить кости</span></span>' +
                '<span class="sw-dice" id="sw-dice">' + diceSVG(2) + '</span>' +
            '</button>' +
            '<div class="sw-tabs-bar">' +
                '<button class="sw-tab-toggle active sw-focusable" id="sw-tab-btn-verdict" tabindex="0">📋 Плюсы и минусы</button>' +
                '<button class="sw-tab-toggle moods sw-focusable" id="sw-tab-btn-moods" tabindex="0">🔥 Атмосфера & Темп</button>' +
            '</div>' +
            '<div class="sw-columns" id="sw-tab-content-verdict">' +
            ...
            '</div>' +
            '<div class="sw-mood-panel" id="sw-tab-content-moods" style="display:none">' +
                '<div class="sw-title" style="color:#fbbf24">🎭 Восприятие и эмоциональный фон картины</div>' +
                '<div class="sw-mood-grid">' + moodCardsHtml + '</div>' +
                tagsHtml +
            '</div>';
    }

    function bindTabs(html) {
        var btnV = html.find('#sw-tab-btn-verdict');
        var btnM = html.find('#sw-tab-btn-moods');
        var panV = html.find('#sw-tab-content-verdict');
        var panM = html.find('#sw-tab-content-moods');

        btnV.on('click hover:enter', function(){
            btnV.addClass('active'); btnM.removeClass('active');
            panV.show(); panM.hide();
            if (window._sw_blocknav) highlightVisible();
        });
        btnM.on('click hover:enter', function(){
            btnM.addClass('active'); btnV.removeClass('active');
            panV.hide(); panM.show();
            if (window._sw_blocknav) highlightVisible();
        });
    }
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){
            try {
                ...
                dice.style.transition = 'transform .45s cubic-bezier(.34,1.56,.64,1)';
                dice.style.transform = 'translateX(0) rotate(0deg)';
                setTimeout(function(){ dice.style.transition = ''; dice.style.transform = ''; }, 480);
            } catch(e) {}
        });
        ...
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var maxRoll = Math.max(40, btn.clientWidth - dice.offsetWidth - 36);
                var dist = Math.min(btn.clientWidth * 0.42, maxRoll);
                ...
                var t0 = Date.now(), DUR = 900, frame = 0;
    ...
    function showModal(movie) {
        try {
            ...
            var phases = [
                { emoji: '🔍', text: 'Анализирую картину…' },
                { emoji: '📊', text: 'Загружаю метаданные TMDB…' },
                { emoji: '🎭', text: 'Рассчитываю атмосферу и жанры…' },
                { emoji: '💬', text: 'Сканирую мнения зрителей…' },
                { emoji: '⚖️', text: 'Взвешиваю плюсы и минусы…' }
            ];
            ...
            window._sw_loaderTimer = setInterval(function(){
                ...
                if (t.length) { t.css('opacity', 0); setTimeout(function(){ t.text(phases[pi].text).css('opacity', 1); }, 160); }
                if (e.length) setTimeout(function(){ e.text(phases[pi].emoji); }, 100);
            }, 600);
            ...
            analyze(movie).then(function(a){
                ...
                setTimeout(function(){
                    ...
                    html.find('.sw-list li').each(function(i){ var li = $(this); setTimeout(function(){ li.addClass('appear'); }, i * 30); });
                    if (window._sw_blocknav) highlightVisible();
                }, 50);
                Lampa.Controller.toggle('should_watch_modal_enhanced');
            }).catch(function(err){
                clearLoader(); console.error('[SW] analyze:', err);
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#ef4444">Не удалось выполнить анализ. Попробуйте ещё раз.</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v37.0 Enhanced (smoother roll physics, improved contrast & typography)');
    }
