(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v56';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-body{animation:swFadeIn .4s ease-out}' +
            ...
            '.sw-dossier{position:relative;background:linear-gradient(160deg,#363c3f,#2b3134);border:1px solid rgba(255,255,255,.05);border-radius:24px;padding:34px 40px;margin-bottom:22px;box-shadow:0 6px 18px rgba(0,0,0,.25);animation:swRise .45s ease-out both}' +
            ...
            '.sw-verdict-word{font-family:' + COND + ';font-size:3em;font-weight:700;letter-spacing:.02em;line-height:1;margin:0 0 24px;text-transform:uppercase;opacity:0;transition:opacity .45s ease}' +
            '.sw-verdict-word.appear{opacity:1}' +
            '.sw-verdict-word.yes{color:#43a047}.sw-verdict-word.no{color:#c05050}.sw-verdict-word.maybe{color:#dd8047}' +
            '.sw-meter{height:4px;border-radius:2px;background:rgba(255,255,255,.1);overflow:hidden}' +
            '.sw-meter-fill{height:100%;width:0;transition:width .8s ease-out}' +
            '.sw-meter-fill.yes{background:linear-gradient(90deg,#2f7d33,#4caf50)}' +
            '.sw-meter-fill.no{background:linear-gradient(90deg,#8e2f2f,#c04040)}' +
            '.sw-meter-fill.maybe{background:linear-gradient(90deg,#b45f2e,#dd8047)}' +
            /* --- КНОПКА --- */
            '.sw-dicebtn{display:block;width:100%;height:110px;border-radius:14px;background:linear-gradient(180deg,#a8aaad 0%,#939599 100%);position:relative;overflow:hidden;border:none;margin:0 0 22px;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;transition:transform .2s ease}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 4px rgba(255,255,255,.5)}' +
            '.sw-dicebtn.rolling{transform:scale(.985)}' +
            '.sw-label-wrap{position:absolute;top:0;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;padding:0 90px 0 110px;overflow:hidden}' +
            '.sw-label{font-family:' + COND + ';font-size:2.8em;font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;white-space:nowrap;opacity:0;transition:opacity .3s ease}' +
            '.sw-label.show{opacity:1}' +
            '.sw-label.res-yes{color:#1b5e20}.sw-label.res-no{color:#b71c1c}' +
            /* --- 3D КУБИК (ОПТИМИЗИРОВАННЫЙ) --- */
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;perspective:600px}' +
            '.sw-dice::after{content:"";position:absolute;bottom:-8px;left:10%;width:80%;height:10px;background:radial-gradient(ellipse,rgba(0,0,0,.3) 0%,transparent 70%);border-radius:50%;filter:blur(3px);transform:scale(1);opacity:.7;pointer-events:none;transition:transform .1s,opacity .1s}' +
            '.sw-cube{position:absolute;inset:0;transform-style:preserve-3d;will-change:transform;transform:rotateX(-20deg) rotateY(20deg)}' +
            '.sw-face{position:absolute;width:64px;height:64px;background:linear-gradient(145deg,#ffffff 0%,#e8eaec 100%);border:2px solid #c5c7c9;border-radius:14px;box-shadow:inset 0 2px 4px rgba(255,255,255,.8),inset 0 -2px 4px rgba(0,0,0,.1);backface-visibility:hidden}' +
            '.sw-face svg{width:100%;height:100%;display:block}' +
            '.sw-face.f1{transform:translateZ(32px)}' +
            '.sw-face.f2{transform:rotateY(180deg) translateZ(32px)}' +
            '.sw-face.f3{transform:rotateY(90deg) translateZ(32px)}' +
            '.sw-face.f4{transform:rotateY(-90deg) translateZ(32px)}' +
            '.sw-face.f5{transform:rotateX(90deg) translateZ(32px)}' +
            '.sw-face.f6{transform:rotateX(-90deg) translateZ(32px)}' +
            /* Анимация броска */
            '.sw-dice.sw-run{animation:swThrow 1.8s cubic-bezier(.25,.1,.25,1) forwards}' +
            '.sw-dice.sw-run::after{animation:swShadow 1.8s ease-out forwards}' +
            '.sw-dice.sw-run .sw-cube{animation:swSpin3D 1.8s cubic-bezier(.15,.85,.3,1) forwards}' +
            /* Траектория броска: дуга + возврат */
            '@keyframes swThrow{0%{transform:translateX(0) translateY(0)}15%{transform:translateX(calc(var(--sw-dist)*.3)) translateY(-40px)}30%{transform:translateX(calc(var(--sw-dist)*.6)) translateY(-20px)}45%{transform:translateX(var(--sw-dist)) translateY(0)}60%{transform:translateX(calc(var(--sw-dist)*.8)) translateY(-10px)}75%{transform:translateX(calc(var(--sw-dist)*.6)) translateY(-5px)}90%{transform:translateX(calc(var(--sw-dist)*.4)) translateY(0)}100%{transform:translateX(0) translateY(0)}}' +
            /* 3D вращение: хаотичное по всем осям */
            '@keyframes swSpin3D{0%{transform:rotateX(-20deg) rotateY(20deg) rotateZ(0)}15%{transform:rotateX(160deg) rotateY(200deg) rotateZ(180deg)}30%{transform:rotateX(340deg) rotateY(380deg) rotateZ(360deg)}45%{transform:rotateX(520deg) rotateY(560deg) rotateZ(540deg)}60%{transform:rotateX(700deg) rotateY(740deg) rotateZ(720deg)}75%{transform:rotateX(880deg) rotateY(920deg) rotateZ(900deg)}90%{transform:rotateX(1060deg) rotateY(1100deg) rotateZ(1080deg)}100%{transform:rotateX(1240deg) rotateY(1280deg) rotateZ(1260deg)}}' +
            /* Тень под кубиком */
            '@keyframes swShadow{0%,100%{transform:scale(1);opacity:.7}15%,30%{transform:scale(.6);opacity:.3}45%{transform:scale(1.1);opacity:.8}60%{transform:scale(.7);opacity:.4}75%{transform:scale(.85);opacity:.5}90%{transform:scale(1);opacity:.7}}' +
            '@media(prefers-reduced-motion:reduce){.sw-dice.sw-run,.sw-dice.sw-run::after,.sw-dice.sw-run .sw-cube{animation:none !important}.sw-dicebtn{transition:none !important}}' +
            '.sw-columns{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;align-items:stretch}' +
            '.sw-col{position:relative;background:linear-gradient(160deg,#343a3d,#2c3235);border:1px solid rgba(255,255,255,.04);border-radius:8px;padding:24px 28px 24px calc(13% + 28px);box-shadow:0 4px 12px rgba(0,0,0,.2)}' +
            '.sw-col::before{content:"";position:absolute;left:0;top:0;bottom:0;width:13%;border-radius:8px 0 0 8px}' +
            '.sw-col.pros::before{background:linear-gradient(180deg,#5c7149,#46563a)}' +
            '.sw-col.cons::before{background:linear-gradient(180deg,#7c4147,#5d3138)}' +
            '.sw-title{font-size:.85em;font-weight:700;margin-bottom:14px;text-transform:lowercase;letter-spacing:.03em}' +
            '.sw-title.pros{color:#9fc787}.sw-title.cons{color:#e09a9a}' +
            '.sw-list{margin:0;padding-left:18px;font-size:1.02em;line-height:1.65;color:#cdd2d5}' +
            '.sw-list li{margin-bottom:10px;opacity:0;transition:opacity .35s ease}' +
            '.sw-list li.appear{opacity:1}' +
            '.sw-focusable{outline:none;cursor:pointer}' +
            '@media(max-width:640px){.sw-modal-content{padding:20px 18px 36px}.sw-columns{grid-template-columns:1fr}.sw-col{padding:20px 22px 20px calc(16% + 22px)}.sw-col::before{width:16%}.sw-verdict-word{font-size:2.2em}.sw-label{font-size:1.8em}.sw-label-wrap{padding:0 70px 0 84px}.sw-dice{width:52px;height:52px;margin-top:-26px;left:14px}}';
    ...
    function pipsSVG(n) {
        ...
        return '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">' + c + '</svg>';
    }
    ...
    function buildReadyInner(a) {
        ...
        return '<div class="sw-dossier">' + badges +
            ...
            '<span class="sw-dice" id="sw-dice"><span class="sw-cube">' +
            '<span class="sw-face f1" id="sw-face-front">' + pipsSVG(1) + '</span>' +
            '<span class="sw-face f2">' + pipsSVG(2) + '</span>' +
            '<span class="sw-face f3">' + pipsSVG(3) + '</span>' +
            '<span class="sw-face f4">' + pipsSVG(4) + '</span>' +
            '<span class="sw-face f5">' + pipsSVG(5) + '</span>' +
            '<span class="sw-face f6">' + pipsSVG(6) + '</span>' +
            '</span></span></button>' +
    ...
    function bindDice(html) {
        var btnJ = html.find('#sw-dice-btn'); var btn = btnJ[0];
        var dice = html.find('#sw-dice')[0];
        var label = html.find('#sw-dice-label')[0];
        var front = html.find('#sw-face-front')[0];
        ...
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                if (front) front.innerHTML = pipsSVG(rndFace());
                var dist = Math.max(0, Math.round(btn.clientWidth / 2 - 40));
                dice.style.setProperty('--sw-dist', dist + 'px');
                dice.classList.remove('sw-run'); void dice.offsetWidth;
                btnJ.addClass('rolling'); dice.classList.add('sw-run');
                setTimeout(function(){
                    dice.classList.remove('sw-run'); btnJ.removeClass('rolling');
                ...
                window._sw_rolling = false; btnJ.removeClass('rolling'); dice.classList.remove('sw-run');
            }
        });
    }
    ...
    function showModal(movie) {
        try {
            ...
            analyze(movie).then(function(a){
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#c05050">Ошибка анализа</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v56.0 (beautiful 3D dice with real throw)');
    }
