(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            /* РЯД КРУПНЫХ КАРТОЧЕК (без заголовка) */
            '.sw-row-wrap{padding:0 0 30px}' +
            '.sw-row{display:flex;gap:28px;overflow-x:auto;overflow-y:hidden;padding:56px 8px 30px;-webkit-overflow-scrolling:touch}' +
            '.sw-row::-webkit-scrollbar{height:5px}.sw-row::-webkit-scrollbar-thumb{background:rgba(255,255,255,.18);border-radius:10px}' +
            /* КРУПНАЯ ПЛАШКА */
            '.sw-plaque{position:relative;flex:0 0 auto;width:300px;height:250px;background:linear-gradient(160deg,#26282b,#202225);border:1px solid #34373c;border-radius:32px;display:flex;align-items:center;justify-content:center;overflow:visible;box-shadow:0 10px 24px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.04);animation:swCardIn .55s cubic-bezier(.22,1,.36,1) both}' +
            ...
            '.sw-plaque-icon{position:absolute;top:-40px;right:14px;width:140px;height:100px;pointer-events:none}' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v72.0 (big plaques, no title, staggered reveal)');
    }
