(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '#sw-dice{position:absolute;left:18px;top:50%;width:70px;height:70px;margin-top:-35px;display:flex;align-items:center;justify-content:center;perspective:240px}' +
            '#sw-dice-face{width:100%;height:100%;transition:transform .12s ease-in,opacity .12s ease-in;transform-style:preserve-3d}' +
    ...
    var NEGATORS = ['не','ни','вовсе','нисколько'];
    ...
    function bindDice(html, cfg) {
        ...
        function scheduleSwap(){
            faceTimer = setTimeout(function(){
                ...
                currentFace = f;
                dice.style.transform = 'rotateY(90deg) scale(.8)';
                dice.style.opacity = '0.15';
                setTimeout(function(){
                    if (stopCycle) return;
                    dice.innerHTML = pipsSVG(f);
                    dice.style.transform = 'rotateY(0deg) scale(1)';
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v65.3 (token-aware review sentiment, phase-emoji loader, smoother dice flip)');
    }
