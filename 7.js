(function () {
    ...
    function addBtn(el, movie) {
        try {
            ...
            // Кнопка "Стоит ли?"
            if (!el.find('.sw-custom-button-enhanced').length) {
            ...
            // Inline-бейджи
            if (!el.find('.sw-inline-tags').length) {
                ...
                Promise.all([loadCredits(movie), loadMeta(movie)]).then(function(arr){
                    ...
                    var sparks = findSparkle(meta, credits);
                    ...
                    if (votes >= 3000 && rating >= 8.0) tags.push({ type: 'pro', label: '✨ ' + rating.toFixed(1) });
                    else if (votes >= 100 && rating < 5) tags.push({ type: 'con', label: '📉 ' + rating.toFixed(1) });
                    if (/CAM|TS|HDCAM|HDRIP/i.test(q)) tags.push({ type: 'con', label: '📺 Экранка' });
                    else if (/4K|UHD|2160p/i.test(q)) tags.push({ type: 'pro', label: '🎥 4K' });
                    if (age !== null && age >= 18) tags.push({ type: 'con', label: '🔞 18+' });
                    else if (age !== null && age <= 6) tags.push({ type: 'pro', label: '🧸 Детский' });
                    
                    sparks.forEach(function(s){ tags.push({ type: 'neutral', label: s }); });
                    ...
                    if (tags.length) {
                        ...
                        if (renderArea.length) renderArea.append(html);
                        else el.append(html);
                    }
                });
            }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v37.0 (final hybrid: button + badges + modal)');
    }
