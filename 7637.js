(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v62';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            /* КНОПКА С ТРЯСКОЙ */
            '.sw-dicebtn{display:block;width:100%;height:120px;border-radius:14px;background:linear-gradient(180deg,#a8aaad 0%,#939599 100%);position:relative;overflow:hidden;border:none;margin:0 0 22px;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;transition:transform .2s ease}' +
            ...
            '.sw-label-wrap{position:absolute;top:0;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;padding:0 30px 0 110px;overflow:hidden}' +
            /* ТЕКСТ КРУПНЕЕ */
            '.sw-label{font-family:' + COND + ';font-size:3.6em;font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;white-space:nowrap;opacity:0;transition:opacity .3s ease;letter-spacing:-.02em}' +
            ...
            '.premium-dice-svg{width:100%;height:100%;border-radius:22%;box-shadow:0 4px 12px rgba(0,0,0,.2);transition:opacity .2s ease}' +
    ...
    function pipsSVG(n) {
        ...
        // Красный только для единицы, остальные — серо-чёрные
        var isOne = (n === 1);
    ...
    function bindDice(html) {
        ...
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                // Плавная смена граней (медленнее)
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v62.0 (gray pips, slow face swap, bigger text)');
    }
