(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            '.sw-custom-button-enhanced{cursor:pointer;transition:background .25s ease,color .25s ease}' +
            '.sw-custom-button-enhanced.focus{background:rgba(255,255,255,.18);color:#fff}' +
            ...
            '.sw-dossier{position:relative;background:linear-gradient(160deg,rgba(44,47,50,.94),rgba(30,32,35,.9));border:1px solid rgba(255,255,255,.07);border-radius:24px;padding:34px 40px;margin-bottom:22px;box-shadow:0 10px 28px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.05);animation:swRise .5s cubic-bezier(.22,1,.36,1) both;transform-origin:50% 50%}' +
            '.sw-dossier.sw-shake{animation:swShake .6s cubic-bezier(.36,.07,.19,.97) both}' +
            '@keyframes swShake{0%,100%{transform:translateX(0) rotate(0)}10%{transform:translateX(-6px) rotate(-.8deg)}20%{transform:translateX(6px) rotate(.8deg)}30%{transform:translateX(-5px) rotate(-.6deg)}40%{transform:translateX(5px) rotate(.6deg)}50%{transform:translateX(-3px) rotate(-.4deg)}60%{transform:translateX(3px) rotate(.4deg)}70%{transform:translateX(-2px) rotate(-.2deg)}80%{transform:translateX(2px) rotate(.2deg)}90%{transform:translateX(-1px) rotate(0)}}' +
            ...
            '.sw-dicebtn{display:flex;align-items:center;justify-content:center;width:100%;height:110px;border-radius:16px;background:linear-gradient(180deg,#adaeb1 0%,#8f9195 100%);position:relative;overflow:hidden;border:none;margin:0 0 22px;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;transition:transform .25s cubic-bezier(.22,1,.36,1),box-shadow .25s ease,background .35s cubic-bezier(.22,1,.36,1);box-shadow:0 6px 16px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.25)}' +
            '.sw-dicebtn:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.28)}' +
            '.sw-dicebtn:active{transform:translateY(0)}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 3px rgba(255,255,255,.35),0 8px 22px rgba(0,0,0,.28)}' +
            '.sw-dicebtn.sw-dicebtn-result-yes{background:linear-gradient(180deg,#8bab74 0%,#6d8659 100%)}' +
            '.sw-dicebtn.sw-dicebtn-result-yes .sw-dicebtn-text{color:#fff}' +
            '.sw-dicebtn.sw-dicebtn-result-no{background:linear-gradient(180deg,#c47b83 0%,#82505a 100%)}' +
            '.sw-dicebtn.sw-dicebtn-result-no .sw-dicebtn-text{color:#fff}' +
            '.sw-dicebtn-text{font-family:' + COND + ';font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;letter-spacing:-.02em;transition:opacity .28s cubic-bezier(.22,1,.36,1),transform .28s cubic-bezier(.22,1,.36,1),color .3s ease;opacity:1;transform:scale(1)}' +
            ...
            '@media(prefers-reduced-motion:reduce){.sw-modal-content,.sw-dossier,.sw-meter-fill::after,.sw-dossier.sw-shake{animation:none !important}.sw-dicebtn,.sw-col,.sw-dicebtn-text,.sw-verdict-word,.sw-list li{transition:none !important}}' +
            '@media(max-width:640px){.sw-modal-content{padding:20px 18px 36px}.sw-columns{grid-template-columns:1fr}.sw-col{padding:20px 22px 20px calc(16% + 22px)}.sw-col::before{width:16%}.sw-verdict-word{font-size:2.2em}.sw-dicebtn{height:90px}}';
    ...
    function buildReadyInner(a) {
        ...
        return '<div class="sw-dossier" id="sw-dossier">' + badges +
            ...
            '<span class="sw-dicebtn-text" id="sw-dice-text">бросить</span>' +
            '</button>' +
    ...
    function bindDice(html, cfg) {
        ...
        var btn = btnJ[0];
        var textEl = html.find('#sw-dice-text')[0];
        var dossier = html.find('#sw-dossier')[0];
        if (!btn || !textEl || !dossier) return;

        textEl.style.fontSize = cfg.dice_font + 'px';
        var rolled = false;

        btnJ.on('click hover:enter keydown', function(e) {
            if (e.type === 'keydown' && e.keyCode !== 13 && e.keyCode !== 32) return;
            if (window._sw_rolling || rolled) return;
            window._sw_rolling = true;
            try {
                textEl.style.opacity = '0';
                textEl.style.transform = 'scale(.92)';
                dossier.classList.remove('sw-shake');
                void dossier.offsetWidth;
                dossier.classList.add('sw-shake');
                setTimeout(function(){
                    if (!window._sw_currentModalHtml) return;
                    var finalResult = Math.random() > 0.5 ? 'смотреть' : 'не смотреть';
                    textEl.textContent = finalResult;
                    if (finalResult === 'смотреть') {
                        btnJ.addClass('sw-dicebtn-result-yes');
                    } else {
                        btnJ.addClass('sw-dicebtn-result-no');
                    }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v66.1 (shake verdict, no dice, original close)');
    }
