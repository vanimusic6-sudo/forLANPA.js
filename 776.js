(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v57';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px}' +
            '.sw-dice::after{content:"";position:absolute;bottom:-8px;left:10%;width:80%;height:10px;background:radial-gradient(ellipse,rgba(0,0,0,.3) 0%,transparent 70%);border-radius:50%;filter:blur(3px);transform:scale(1);opacity:.7;pointer-events:none;transition:all .15s}' +
            '.sw-cube{position:absolute;inset:0;background:linear-gradient(145deg,#ffffff 0%,#e8eaec 100%);border:2px solid #c5c7c9;border-radius:14px;box-shadow:0 4px 12px rgba(0,0,0,.15),inset 0 2px 4px rgba(255,255,255,.8);will-change:transform}' +
            '.sw-face{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;border-radius:12px}' +
            '.sw-face svg{width:80%;height:80%}' +
            '.sw-dice.sw-run{animation:swBounce 1.6s cubic-bezier(.2,.8,.3,1) forwards}' +
            '.sw-dice.sw-run::after{animation:swShadow 1.6s ease-out forwards}' +
            '.sw-dice.sw-run .sw-cube{animation:swRoll 1.6s cubic-bezier(.4,.0,.2,1) forwards}' +
            '@keyframes swBounce{0%{transform:translateX(0) translateY(0)}15%{transform:translateX(calc(var(--sw-dist)*.4)) translateY(-35px)}30%{transform:translateX(calc(var(--sw-dist)*.7)) translateY(-15px)}45%{transform:translateX(var(--sw-dist)) translateY(0)}60%{transform:translateX(calc(var(--sw-dist)*.75)) translateY(-8px)}75%{transform:translateX(calc(var(--sw-dist)*.5)) translateY(-3px)}90%{transform:translateX(calc(var(--sw-dist)*.3)) translateY(0)}100%{transform:translateX(0) translateY(0)}}' +
            '@keyframes swRoll{0%{transform:rotate(0deg)}100%{transform:rotate(720deg)}}' +
            '@keyframes swShadow{0%,100%{transform:scale(1);opacity:.7}15%,30%{transform:scale(.5);opacity:.3}45%{transform:scale(1.1);opacity:.8}60%{transform:scale(.65);opacity:.4}75%{transform:scale(.8);opacity:.5}90%{transform:scale(1);opacity:.7}}' +
    ...
    function buildReadyInner(a) {
        ...
        return '<div class="sw-dossier">' + badges +
            ...
            '<span class="sw-dice" id="sw-dice"><span class="sw-cube"><span class="sw-face" id="sw-face-front">' + pipsSVG(1) + '</span></span></span></button>' +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v57.0 (simple beautiful 2D dice)');
    }
