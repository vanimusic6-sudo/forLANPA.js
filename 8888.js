(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v64';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#6d8a54,transparent);animation:swSlide 1.4s ease-in-out infinite}' +
            '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
            /* ГРАФИТОВЫЙ ПОЛУПРОЗРАЧНЫЙ ФОН */
            '.sw-dossier{position:relative;background:linear-gradient(160deg,rgba(42,45,48,.92),rgba(32,34,37,.88));border:1px solid rgba(255,255,255,.05);border-radius:24px;padding:34px 40px;margin-bottom:22px;box-shadow:0 6px 18px rgba(0,0,0,.3);animation:swRise .45s ease-out both}' +
            ...
            '.sw-mode-dot.active{background:#6d8a54}.sw-mode-dot.inactive{background:#64748b}' +
            ...
            /* ТВОИ ЦВЕТА: зелёный / бордовый / коричневый */
            '.sw-verdict-word.yes{color:#7d9a66}.sw-verdict-word.no{color:#b06a72}.sw-verdict-word.maybe{color:#b98a5e}' +
            ...
            '.sw-meter-fill.yes{background:linear-gradient(90deg,#4d5f3a,#6d8a54)}' +
            '.sw-meter-fill.no{background:linear-gradient(90deg,#5d3138,#8a4a52)}' +
            '.sw-meter-fill.maybe{background:linear-gradient(90deg,#7a5231,#a97948)}' +
            ...
            '.sw-dicebtn.shaking{animation:swShake 1.6s cubic-bezier(.36,.07,.19,.97) both}' +
            ...
            '.sw-col{position:relative;background:linear-gradient(160deg,rgba(40,43,46,.9),rgba(30,32,35,.86));border:1px solid rgba(255,255,255,.04);border-radius:8px;padding:24px 28px 24px calc(13% + 28px);box-shadow:0 4px 12px rgba(0,0,0,.25)}' +
            '.sw-col::before{content:"";position:absolute;left:0;top:0;bottom:0;width:13%;border-radius:8px 0 0 8px}' +
            '.sw-col.pros::before{background:linear-gradient(180deg,#5a6f45,#40512f)}' +
            '.sw-col.cons::before{background:linear-gradient(180deg,#6d3a42,#4e2830)}' +
    ...
    function bindDice(html) {
        ...
        // Смена грани БЕЗ растворения, в такт тряске.
        // Останавливаемся заранее (~1000мс), чтобы финальная грань висела дольше.
        function scheduleSwap(){
            faceTimer = setTimeout(function(){
                if (stopCycle) return;
                if (Date.now() - startTime > 1000) return; // стоп, финальная грань зависает
        ...
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                setTimeout(function(){
                    ...
                    // НЕ меняем грань в последний момент — остаётся текущая
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v64.0 (your palette, graphite bg, dice rhythm)');
    }
