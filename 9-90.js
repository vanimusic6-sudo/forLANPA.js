(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v60';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            /* ТВОЙ ДИЗАЙН КУБИКА (исправленный) */
            '#sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;display:flex;align-items:center;justify-content:center}' +
            '.premium-dice-svg{width:100%;height:100%;border-radius:22%;box-shadow:0 4px 12px rgba(198,40,40,.15)}' +
            /* АНИМАЦИЯ С ВОЗВРАТОМ */
            '#sw-dice.sw-run{animation:rollAction 1.6s cubic-bezier(0.2,0.8,0.2,1) forwards}' +
            '@keyframes rollAction{0%{transform:translateX(0) rotate(0deg) scale(1)}25%{transform:translateX(calc(var(--sw-dist)*0.3)) translateY(-20px) rotate(90deg) scale(1.1)}50%{transform:translateX(calc(var(--sw-dist)*0.6)) translateY(0) rotate(180deg) scale(.95)}75%{transform:translateX(calc(var(--sw-dist)*0.9)) translateY(-10px) rotate(270deg) scale(1.05)}90%{transform:translateX(var(--sw-dist)) translateY(0) rotate(360deg) scale(1)}100%{transform:translateX(0) translateY(0) rotate(720deg) scale(1)}}' +
    ...
    function pipsSVG(n) {
        ...
        for (var i = 0; i < p.length; i++) {
            c += '<circle cx="' + p[i][0] + '" cy="' + p[i][1] + '" r="9.5" fill="url(#diceGrad)"/>';
        }
        return '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" class="premium-dice-svg">' +
            '<defs><linearGradient id="diceGrad" x1="0%" y1="0%" x2="100%" y2="100%">' +
    ...
    function buildReadyInner(a) {
        ...
        return '<div class="sw-dossier">' + badges +
            ...
            '<span id="sw-dice">' + pipsSVG(1) + '</span></button>' +
    ...
    function bindDice(html) {
        ...
        btnJ.on('click hover:enter keydown', function(e) {
            ...
            try {
                ...
                var dist = Math.min(150, Math.round(btn.clientWidth / 2 - 40));
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v60.0 (your design, bugfree)');
    }
