(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v48';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-verdict-word.yes{color:#3f9142}.sw-verdict-word.no{color:#b23a3a}.sw-verdict-word.maybe{color:#d9773f}' +
            ...
            '.sw-meter-fill.maybe{background:linear-gradient(90deg,#b45f2e,#d9773f)}' +
            ...
            '.sw-col{position:relative;background:linear-gradient(160deg,#343a3d,#2c3235);border:1px solid rgba(255,255,255,.04);border-radius:8px;padding:24px 28px 24px calc(13% + 28px);min-height:180px;box-shadow:0 4px 12px rgba(0,0,0,.2)}' +
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // Полоска: баланс весов "за" и "против" — движется от фактов и вердикта
    ...
    function bindDice(html) {
        ...
        $(btn).on('hover:enter click keydown', function(e){
            ...
            setTimeout(function(){
                // кубик крутится с ТЕКУЩЕЙ гранью (не меняется при вращении)
                dice.classList.add('sw-spin');
                setTimeout(function(){
                    dice.classList.remove('sw-spin');
                    // новая грань выбирается в момент возврата и держится до след. броска
    ...
    function showModal(movie) {
        try {
            ...
            analyze(movie).then(function(a){
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#b23a3a">Ошибка анализа</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v48.0 (steady dice face, dynamic meter, harmonized UI)');
    }
