(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#7ec260,transparent);animation:swSlide 1.3s linear infinite}' +
                '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
                '.sw-dossier{position:relative;background:#2b2e31;border-radius:16px;padding:26px 28px;margin-bottom:18px;animation:swRise .45s cubic-bezier(.22,1,.36,1) both}' +
                ...
                '.sw-verdict-word{font-size:2.7em;font-weight:800;letter-spacing:.01em;line-height:1;margin:0 0 16px;text-transform:uppercase;opacity:0;transform:translateY(8px);transition:opacity .4s ease,transform .4s ease}' +
                ...
                '.sw-meter-fill{height:100%;width:0;border-radius:3px;transition:width .8s cubic-bezier(.25,.8,.3,1)}' +
                ...
                '.sw-label{font-family:' + DISPLAY + ';font-size:2.2em;font-weight:800;color:#878c91;letter-spacing:.01em;text-align:center;white-space:nowrap;opacity:1;transition:opacity .25s ease}' +
                ...
                '.sw-dice.sw-rolling{animation:swRoll .85s cubic-bezier(.25,.46,.45,.94) forwards}' +
                ...
                '.sw-list li{margin-bottom:9px;opacity:0;transform:translateX(-8px);transition:opacity .35s ease,transform .35s ease}' +
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){
            try {
                ...
                setTimeout(function(){ dice.style.transition = ''; dice.style.transform = ''; }, 560);
            } catch(e) {}
        });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 850, frame = 0;
    ...
    function showModal(movie) {
        try {
            ...
            var phases = [
                ...
                { emoji: '📊', text: 'Смотрю TMDB…' },
                { emoji: '🎭', text: 'Считаю настроения…' },
                { emoji: '💬', text: 'Читаю отзывы…' },
            ...
            window._sw_loaderTimer = setInterval(function(){
                ...
                if (t.length) { t.css('opacity', 0); setTimeout(function(){ t.text(phases[pi].text).css('opacity', 1); }, 180); }
                if (e.length) setTimeout(function(){ e.text(phases[pi].emoji); }, 120);
            }, 650);
            ...
            analyze(movie).then(function(a){
                ...
                setTimeout(function(){
                    ...
                    html.find('.sw-list li').each(function(i){ var li = $(this); setTimeout(function(){ li.addClass('appear'); }, i * 35); });
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#e05b56">Не удалось выполнить анализ. Попробуйте ещё раз.</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v37.0 (perf + wording pass)');
    }
