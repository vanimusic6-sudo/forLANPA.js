(function () {
    ...
    function getSettings() {
        return {
            ...
            font_scale: parseInt(getSetting('font_scale', '16')) || 16
        };
    }
    ...
    function initSettings() {
        try {
            ...
            [
                ...
                { name: 'font_scale', type: 'select', title: 'Размер шрифта', values: {'16':'Обычный','18':'Крупный','20':'Очень крупный'}, default: '16' },
                { name: 'reset_cache', type: 'toggle', title: 'Сбросить кэш данных', description: 'Включите и откройте плагин — кэш очистится', default: false }
            ].forEach(function(p) {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-modal-content{padding:26px 30px 48px;color:#d6dade;font-family:' + DISPLAY + ';font-size:16px;box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y}' +
            ...
            '.sw-body{animation:swFadeIn .4s ease}' +
            ...
            '.sw-loader-text{font-size:1em;font-weight:600;min-height:1.5em;transition:opacity .3s ease;text-align:center}' +
            ...
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#86b57f,transparent);animation:swSlide 1.6s linear infinite}' +
            '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
            '.sw-dossier{position:relative;background:#24272a;border-radius:14px;padding:28px 30px;margin-bottom:20px;animation:swRise .5s cubic-bezier(.22,1,.36,1) both}' +
            ...
            '.sw-mode-badge{display:inline-flex;align-items:center;gap:6px;font-size:.64em;padding:5px 12px;border-radius:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:rgba(255,255,255,.06);color:#9aa1a6}' +
            '.sw-mode-dot{width:5px;height:5px;border-radius:50%;display:inline-block}' +
            '.sw-mode-dot.active{background:#86b57f}.sw-mode-dot.inactive{background:#7d8388}' +
            '.sw-verdict-word{font-size:2.4em;font-weight:800;letter-spacing:.02em;line-height:1;margin:0 0 18px;text-transform:uppercase;opacity:0;transform:translateY(8px);transition:opacity .5s ease,transform .5s ease}' +
            '.sw-verdict-word.appear{opacity:1;transform:translateY(0)}' +
            '.sw-verdict-word.yes{color:#86b57f}.sw-verdict-word.no{color:#d97b74}.sw-verdict-word.maybe{color:#d9ae5f}' +
            ...
            '.sw-meter-fill.yes{background:#86b57f}.sw-meter-fill.no{background:#d97b74}.sw-meter-fill.maybe{background:#d9ae5f}' +
            '.sw-dicebtn{position:relative;display:block;width:100%;height:100px;background:#eef0f2;border:none;border-radius:14px;margin:0 0 22px;overflow:hidden;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent}' +
            ...
            '.sw-label{font-family:' + DISPLAY + ';font-size:2em;font-weight:800;color:#878c91;letter-spacing:.02em;text-align:center;white-space:nowrap;opacity:1;transition:opacity .25s ease}' +
            '.sw-label.hidden{opacity:0}' +
            '.sw-label.res-yes{color:#4a7d46}.sw-label.res-no{color:#b3564f}' +
            '.sw-dice{position:absolute;left:16px;top:50%;width:70px;height:70px;margin-top:-35px;will-change:transform;transform:translateZ(0)}' +
            '.sw-dice svg{width:100%;height:100%;display:block;transform:rotate(-8deg)}' +
            '.sw-dice.sw-rolling{animation:swRollFull 1.6s cubic-bezier(.3,.5,.3,1) forwards}' +
            ...
            '.sw-col{position:relative;background:#24272a;border-radius:12px;padding:24px 26px}' +
            '.sw-col::before{content:"";position:absolute;top:0;left:26px;right:26px;height:2px;border-radius:2px}' +
            '.sw-col.pros::before{background:#86b57f}.sw-col.cons::before{background:#d97b74}' +
            '.sw-title{font-size:.84em;font-weight:700;margin-bottom:16px;text-transform:uppercase;letter-spacing:.05em}' +
            '.sw-title.pros{color:#86b57f}.sw-title.cons{color:#d97b74}' +
            '.sw-list{margin:0;padding-left:20px;font-size:1em;line-height:1.65;color:#bfc4c8}' +
            '.sw-list li{margin-bottom:10px;opacity:0;transform:translateX(-8px);transition:opacity .4s ease,transform .4s ease}' +
            '.sw-list li.appear{opacity:1;transform:translateX(0)}' +
            '.sw-focusable{outline:none;cursor:pointer}' +
            '@media(max-width:640px){.sw-modal-content{padding:18px 18px 34px}.sw-columns{grid-template-columns:1fr}.sw-verdict-word{font-size:1.9em}.sw-label{font-size:1.4em}.sw-label-wrap{padding:0 84px}.sw-dice{width:56px;height:56px;margin-top:-28px;left:12px}}';
    ...
    function showModal(movie) {
        try {
            // Сброс кэша по настройке
            try {
                if (getSetting('reset_cache', false) === true || getSetting('reset_cache', 'false') === 'true') {
                    _metaCache = {}; _domCache = null;
                    Lampa.Storage.set(PLUGIN_ID + '_reset_cache', false);
                }
            } catch(e) {}
            ...
            analyze(movie).then(function(a){
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#d97b74">Ошибка анализа</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v41.0 (calm UI, dice fixed, cache reset, font scale)');
    }
