(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-label{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:' + DISPLAY + ';font-size:2.2em;font-weight:800;color:#878c91;letter-spacing:.01em;padding:0 108px;text-align:center;white-space:nowrap;overflow:hidden;opacity:0;transition:opacity .5s ease}' +
                '.sw-label.appear{opacity:1}' +
                ...
                '.sw-dice.sw-rolling{animation:swRoll 1.8s cubic-bezier(.33,.65,.3,1) forwards}' +
                '@keyframes swRoll{0%{transform:translateX(0) rotate(0) translateY(0)}15%{transform:translateX(calc(var(--sw-dist) * .15)) rotate(108deg) translateY(-14px)}30%{transform:translateX(calc(var(--sw-dist) * .32)) rotate(216deg) translateY(0)}45%{transform:translateX(calc(var(--sw-dist) * .49)) rotate(324deg) translateY(-11px)}60%{transform:translateX(calc(var(--sw-dist) * .66)) rotate(432deg) translateY(0)}75%{transform:translateX(calc(var(--sw-dist) * .83)) rotate(540deg) translateY(-8px)}100%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0)}}' +
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){
            try {
                ...
                dice.style.transition = 'transform .8s cubic-bezier(.3,1.3,.4,1)';
                dice.style.transform = 'translateX(0) rotate(0deg)';
                setTimeout(function(){ dice.style.transition = ''; dice.style.transform = ''; }, 850);
            } catch(e) {}
        });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 1800, frame = 0;
                var iv = setInterval(function(){
                    frame++;
                    if (frame % 2 === 0) dice.innerHTML = diceSVG(rndFace());
                    var p = Math.min(1, (Date.now() - t0) / DUR);
                    if (p < 1) {
                        // просто обновляем грани
                    } else {
                        clearInterval(iv);
                        dice.innerHTML = diceSVG(rndFace());
                        setTimeout(function(){
                            label.textContent = final;
                            label.className = 'sw-label appear ' + (final === 'СМОТРЕТЬ' ? 'res-yes' : 'res-no');
                            window._sw_rolling = false;
                        }, 100);
                    }
                }, 60);
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v27.0 (fixed exit, sparkle, smooth dice)');
    }
