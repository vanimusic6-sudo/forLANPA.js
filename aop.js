...
ЗАВИСАНИЯ И «ВТОРОЕ НАЖАТИЕ НИЧЕГО НЕ ДЕЛАЕТ» (баг с видео)
...
// ═══════════════════════════════════════════ СЕТЬ: очередь + склейка + кэш (с поддержкой Smart TV)
const Net = {
...
checkTV: () => {
    ...
    try {
        ...
        Net._isTV = /smart-?tv|tizen|web0s|webos|netcast|hbbtv|aftb|bravia|philipstv|maple|vidaa|hisense|vidaa/i.test(ua);
...
// ── Метод через fetch (работает лучше на Smart TV)
_fetch: (url, done) => {
    const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
    const timeout = setTimeout(() => {
        if (controller) controller.abort();
        done('timeout');
    }, Net.checkTV() ? 25000 : 15000);
    ...
    fetch(url, opts).then(r => {
        ...
        if (!r.ok) return done('http ' + r.status);
        ...
        clearTimeout(timeout);
        const msg = (e && e.name === 'AbortError') ? 'timeout' : ('fetch:' + (e && e.message || 'err'));
        done(msg);
    });
},

// ── Метод через XMLHttpRequest (fallback)
_xhr: (url, done) => {
    ...
    const timeout = setTimeout(() => finish('timeout'), Net.checkTV() ? 25000 : 15000);
    ...
    try {
        ...
        xhr.onerror = () => { clearTimeout(timeout); finish('net'); };
        xhr.ontimeout = () => { clearTimeout(timeout); finish('timeout'); };
        
        try { xhr.timeout = Net.checkTV() ? 25000 : 15000; } catch (e) {}
...
// ── ГЛАВНАЯ ФУНКЦИЯ: пробуем fetch, если не вышло — XHR, если и это не вышло — прокси
_start: (url) => {
    Net.running++;
    ...
    if (typeof fetch === 'function') {
        Net._fetch(url, (err1, data1) => {
            if (!err1) return finish(null, data1);
            console.warn('[Капсула] fetch не прошёл:', err1, '— пробуем XHR');
            Net._xhr(url, (err2, data2) => {
                if (!err2) return finish(null, data2);
                console.warn('[Капсула] XHR не прошёл:', err2);
                
                if (Net.checkTV() && (err2 === 'net' || err2 === 'timeout' || (err2 + '').indexOf('http 0') > -1)) {
                    const proxies = [
                    ...
                    let pi = 0;
                    const tryProxy = () => {
                        if (pi >= proxies.length) return finish(err2);
                        Net._fetch(proxies[pi++], (err3, data3) => {
                            if (!err3) {
                                console.log('[Капсула] прокси сработал');
                                return finish(null, data3);
                            }
                            tryProxy();
                        });
                    };
...
console.log('[Капсула] v' + VERSION + ' запущена | Smart TV:', Net.checkTV() ? 'ДА' : 'НЕТ' + ' | User-Agent:', navigator.userAgent);
