(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v46';
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            '.sw-custom-button-enhanced{cursor:pointer;transition:background .25s ease,color .25s ease,transform .25s ease,box-shadow .25s ease}' +
            '.sw-custom-button-enhanced.focus{background:rgba(255,255,255,.18);color:#fff;transform:scale(1.02);box-shadow:0 0 15px rgba(255,255,255,.1)}' +
            '.sw-modal-content{padding:28px 32px 48px;color:#f1f5f9;font-family:' + DISPLAY + ';font-size:20px;box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y}' +
            ...
            '.sw-body{animation:swFadeIn .4s cubic-bezier(.16,1,.3,1)}' +
            '@keyframes swFadeIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}' +
            '.sw-loader{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;padding:60px 20px;min-height:45vh;color:#94a3b8}' +
            '.sw-loader-emoji{font-size:3.2em;line-height:1;animation:swFloat 2s cubic-bezier(.45,0,.55,1) infinite}' +
            '@keyframes swFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-10px) scale(1.05)}}' +
            '.sw-loader-text{font-size:1em;font-weight:600;min-height:1.5em;text-align:center;color:#cbd5e1;letter-spacing:.02em}' +
            '.sw-loader-progress{width:220px;height:4px;border-radius:4px;background:rgba(255,255,255,.08);overflow:hidden;position:relative;margin-top:8px}' +
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#10b981,transparent);animation:swSlide 1.2s cubic-bezier(.4,0,.2,1) infinite}' +
            '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
            '.sw-dossier{position:relative;background:linear-gradient(145deg,#1e293b,#0f172a);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:30px;margin-bottom:24px;box-shadow:0 10px 30px rgba(0,0,0,.4);animation:swRise .5s cubic-bezier(.16,1,.3,1) both}' +
            '@keyframes swRise{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}' +
            '.sw-badges{position:absolute;top:22px;right:22px;display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;max-width:55%}' +
            '.sw-mode-badge{display:inline-flex;align-items:center;gap:6px;font-size:.65em;padding:6px 14px;border-radius:20px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;background:rgba(255,255,255,.06);color:#94a3b8;border:1px solid rgba(255,255,255,.05)}' +
            '.sw-mode-dot{width:6px;height:6px;border-radius:50%;display:inline-block}' +
            '.sw-mode-dot.active{background:#10b981;box-shadow:0 0 8px #10b981}.sw-mode-dot.inactive{background:#64748b}' +
            '.sw-verdict-word{font-size:2.8em;font-weight:900;letter-spacing:.02em;line-height:1;margin:0 0 18px;text-transform:uppercase;opacity:0;transform:translateY(10px) scale(.96);transition:opacity .4s ease,transform .45s cubic-bezier(.16,1,.3,1)}' +
            '.sw-verdict-word.appear{opacity:1;transform:translateY(0) scale(1)}' +
            '.sw-verdict-word.yes{color:#10b981;text-shadow:0 0 25px rgba(16,185,129,.35)}' +
            '.sw-verdict-word.no{color:#ef4444;text-shadow:0 0 25px rgba(239,68,68,.35)}' +
            '.sw-verdict-word.maybe{color:#f59e0b;text-shadow:0 0 25px rgba(245,158,11,.35)}' +
            '.sw-meter{height:6px;border-radius:3px;background:rgba(255,255,255,.08);overflow:hidden}' +
            '.sw-meter-fill{height:100%;width:0;border-radius:3px;transition:width 1s cubic-bezier(.16,1,.3,1)}' +
            '.sw-meter-fill.yes{background:linear-gradient(90deg,#059669,#10b981)}' +
            '.sw-meter-fill.no{background:linear-gradient(90deg,#dc2626,#ef4444)}' +
            '.sw-meter-fill.maybe{background:linear-gradient(90deg,#d97706,#f59e0b)}' +
            '.sw-dicebtn{position:relative;display:block;width:100%;height:90px;background:linear-gradient(135deg,#f8fafc,#e2e8f0);border:none;border-radius:16px;margin:0 0 24px;overflow:hidden;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;box-shadow:0 8px 20px rgba(0,0,0,.25);transition:background .4s ease,box-shadow .4s ease,transform .4s ease}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 4px #10b981,0 10px 25px rgba(0,0,0,.3);transform:translateY(-2px)}' +
            '.sw-dicebtn.morph{background:transparent;box-shadow:none}' +
            '.sw-label-wrap{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:0 96px;overflow:hidden}' +
            '.sw-label{font-family:' + DISPLAY + ';font-size:1.8em;font-weight:800;color:#334155;letter-spacing:.01em;text-align:center;white-space:nowrap;opacity:1;transition:opacity .25s ease}' +
            '.sw-label.hidden{opacity:0}' +
            '.sw-label.res-yes{color:#059669}.sw-label.res-no{color:#dc2626}' +
            '.sw-dice{position:absolute;left:18px;top:50%;width:60px;height:60px;margin-top:-30px;will-change:transform;transition:left .4s cubic-bezier(.16,1,.3,1),margin-left .4s cubic-bezier(.16,1,.3,1)}' +
            '.sw-dice svg{width:100%;height:100%;display:block;transform:rotate(-6deg)}' +
            '.sw-dicebtn.morph .sw-dice{left:50%;margin-left:-30px}' +
            '.sw-dice.sw-spin{animation:swSpin .85s cubic-bezier(.34,1.56,.64,1) forwards}' +
            '@keyframes swSpin{0%{transform:rotate(0) scale(1)}50%{transform:rotate(360deg) scale(1.2)}100%{transform:rotate(720deg) scale(1)}}' +
            '.sw-columns{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px}' +
            '.sw-col{position:relative;background:#0f172a;border:1px solid rgba(255,255,255,.05);border-radius:16px;padding:24px 26px;box-shadow:0 4px 15px rgba(0,0,0,.2)}' +
            '.sw-col::before{content:"";position:absolute;top:0;left:24px;right:24px;height:3px;border-radius:3px}' +
            '.sw-col.pros::before{background:linear-gradient(90deg,transparent,#10b981,transparent)}' +
            '.sw-col.cons::before{background:linear-gradient(90deg,transparent,#ef4444,transparent)}' +
            '.sw-title{font-size:.85em;font-weight:800;margin-bottom:16px;text-transform:uppercase;letter-spacing:.08em}' +
            '.sw-title.pros{color:#10b981}.sw-title.cons{color:#ef4444}' +
            '.sw-list{margin:0;padding-left:20px;font-size:1.05em;line-height:1.65;color:#cbd5e1}' +
            '.sw-list li{margin-bottom:10px;opacity:0;transform:translateX(-10px);transition:opacity .35s ease,transform .35s cubic-bezier(.16,1,.3,1)}' +
            '.sw-list li.appear{opacity:1;transform:translateX(0)}' +
            '.sw-focusable{outline:none;cursor:pointer}' +
            '@media(max-width:640px){.sw-modal-content{padding:20px 18px 36px}.sw-columns{grid-template-columns:1fr}.sw-verdict-word{font-size:2.2em}.sw-label{font-size:1.3em}.sw-label-wrap{padding:0 70px}.sw-dice{width:50px;height:50px;margin-top:-25px;left:14px}.sw-dicebtn.morph .sw-dice{margin-left:-25px}}';
    ...
    function buildReadyInner(a) {
        ...
        return '<div class="sw-dossier">' + badges +
            ...
            '<span class="sw-dice" id="sw-dice">' + diceSVG(2) + '</span></button>' +
    ...
    function showModal(movie) {
        try {
            ...
            analyze(movie).then(function(a){
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#ef4444">Ошибка анализа</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v46.0 (premium design)');
    }
