(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            /* --- кнопка-кубик (дизайн-ИИ, адаптировано) --- */
            '.sw-dicebtn{height:110px;border-radius:14px;background:linear-gradient(180deg,#a8aaad 0%,#939599 100%);position:relative;overflow:hidden;will-change:transform;transition:transform .2s cubic-bezier(.16,1,.3,1);border:none;margin:0 0 22px;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent}' +
            ...
            '.sw-label{font-family:' + COND + ';font-size:2.35em;font-weight:700;color:#43464a;text-transform:lowercase;text-align:center;white-space:nowrap;opacity:0;transform:translateX(-8px);letter-spacing:-.02em;will-change:transform,opacity;transition:opacity .25s ease-out,transform .25s ease-out,letter-spacing .25s ease-out}' +
            '.sw-label.show{opacity:1;transform:translateX(0);letter-spacing:.03em}' +
            ...
            '.sw-dice::after{content:"";position:absolute;bottom:-6px;left:10%;width:80%;height:8px;background:rgba(0,0,0,.25);border-radius:50%;filter:blur(4px);transform:scale(1);opacity:.6;transition:transform .1s,opacity .1s;pointer-events:none}' +
            '.sw-dicebtn.spinning .sw-dice{transform:translateX(var(--sw-dist))}' +
            ...
            '@media(prefers-reduced-motion:reduce){.sw-dice,.sw-dicebtn,.sw-label{transition:opacity .2s ease-in-out !important}.sw-dice.sw-spin .sw-dice-core{animation:none !important}.sw-dice.sw-spin::after{animation:none !important}}' +
            ...
            setTimeout(function(){
                ...
                setTimeout(function(){
                    ...
                    setTimeout(function(){
                        ...
                        label.className = 'sw-label show ' + (final === 'смотреть' ? 'res-yes' : 'res-no');
                        window._sw_rolling = false;
                    }, 300);
                }, 900);
            }, 400);
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v50.0 (design-ai dice, two-layer, shadow)');
    }
