...
(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dicebtn.focus{background:#2f353a!important;transform:scale(1.01);box-shadow:0 0 0 2px rgba(255,255,255,.8),0 4px 12px rgba(0,0,0,.3)}' +
            ...
            '.sw-dice{position:absolute;left:16px;top:50%;width:52px;height:52px;margin-top:-26px;perspective:300px}' +
            '.sw-dice-core{display:block;width:100%;height:100%;transform-style:preserve-3d;filter:drop-shadow(0 4px 6px rgba(0,0,0,.4))}' +
            '.sw-dice-core svg{width:100%;height:100%;display:block;backface-visibility:hidden}' +
            '.sw-dice.sw-spin .sw-dice-core{animation:swSpin3d 0.8s cubic-bezier(.4,.1,.4,1) infinite}' +
            '@keyframes swSpin3d{0%{transform:rotateX(0) rotateY(0) scale(1)}25%{transform:rotateX(180deg) rotateY(90deg) scale(.92)}50%{transform:rotateX(360deg) rotateY(180deg) scale(1)}75%{transform:rotateX(540deg) rotateY(270deg) scale(.92)}100%{transform:rotateX(720deg) rotateY(360deg) scale(1)}}' +
            ...
            '.sw-metric-card{position:relative;background:#1e2226;border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:12px 14px;box-shadow:0 4px 12px rgba(0,0,0,.2);transition:transform .2s ease;opacity:0;transform:translateY(6px)}' +
            '.sw-metric-card.appear{opacity:1;transform:translateY(0);transition:opacity .4s ease,transform .4s ease}' +
            ...
            '.sw-metric-bar-fill{height:100%;width:0;border-radius:4px;transition:width .8s ease}' +
            // === Audience box ===
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // --- Simplified atmosphere: 4 key metrics ---
    ...
    function buildReadyInner(a) {
        ...
        if (a.prosEmpty && a.consEmpty) {
            prosConsHtml = '<div class="' + colsClass + '">' +
                '<div class="sw-col sw-empty-col"><ul class="sw-list">' + a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
                '</div>';
        } else if (a.prosEmpty) {
            prosConsHtml = '<div class="' + colsClass + '">' +
        ...
        function activateTab(target, el) {
            ...
            if (target === 'atmos') {
                html.find('#sw-pane-atmos .sw-metric-card').each(function(i){
                    ...
                    setTimeout(function(){ card.addClass('appear'); }, i * 60);
                });
            }
    ...
    function bindDice(html) {
        ...
        function rollDice() {
            ...
            // Cycle faces during the roll so the 3D spin shows changing pips
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v52.7 initialized: balanced UI, refined dice animation, adaptive pros/cons layout.');
    }
