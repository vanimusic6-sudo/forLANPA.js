/**
Capsule Mod v18.1 — «Капсула» (Smart TV Edition)
Рекомендательная капсула для Lampa (TMDb).

ГЛАВНОЕ В v18.1 (Smart TV Edition)
─────────────────────────────────────────────────────────────
ПОЛНАЯ СОВМЕСТИМОСТЬ С ANDROID TV / SMART TV
— каскад из 5 способов получения данных:
    1) Lampa.TMDB.api() — использует прокси из настроек Lampa
    2) прямой fetch
    3) прокси tmdb.cinema-numbers.space
    4) прокси corsproxy.io
    5) XMLHttpRequest
— safeImgUrl() для постеров: использует Lampa.Utils.img()
— расширенное определение Smart TV (Android TV, Fire TV, Mi Box, Shield...)
— диагностика в консоли: какие механизмы доступны
— белые цвета текста при фокусе (не чёрные)

ОБНОВЛЕНИЕ РЕАЛЬНО МЕНЯЕТ ФИЛЬМЫ
— курсор страниц: каждый запрос к TMDb уходит на ДРУГИЕ страницы
— ротация sort_by и порогов рейтинга
— журнал показов Seen с «поколениями»
— проверка новизны: если пересечение > 34 %, набор пересобирается

ЗАВИСАНИЯ И «ВТОРОЕ НАЖАТИЕ НИЧЕГО НЕ ДЕЛАЕТ»
— все асинхронные ветки закрыты once()
— у флага busy есть сторож (watchdog)
— токен сборки: ответ старой сборки не перетирает новую
— модалка теперь одна, без стека

СКОРОСТЬ
— очередь запросов с лимитом 6 параллельных + склейка
— кэш ответов TMDb с ограничением объёма
— профиль вкуса кэшируется на 5 минут

АНИМАЦИИ И ЗАДНИКИ
— единый rAF, авто-определение мощности устройства
— «лёгкий режим» отключает backdrop-filter
— свет от постера подгружается заранее
*/
(function () {
'use strict';
...
// ═══════════════════════════════════════════ БЕЗОПАСНЫЙ URL КАРТИНКИ (Smart TV)
// Использует встроенный в Lampa механизм — он гарантированно работает там,
// где работают каталоги фильмов. Если недоступен — fallback на прямой URL.
const safeImgUrl = (path, size) => {
    if (!path) return '';
    try {
        if (window.Lampa && window.Lampa.Utils && typeof window.Lampa.Utils.img === 'function') {
            return window.Lampa.Utils.img(path);
        }
        if (window.Lampa && window.Lampa.TMDB && typeof window.Lampa.TMDB.image === 'function') {
            return window.Lampa.TMDB.image(path);
        }
    } catch (e) {}
...
const parallel = (tasks, done) => {
    ...
    let guard = setTimeout(() => finish(out), 20000);
...
// ═══════════════════════════════════════════ СЕТЬ: КАСКАД ИЗ 5 СПОСОБОВ (Smart TV)
// Порядок попыток:
//   1) Lampa.TMDB.api() — использует прокси из настроек Lampa (главный!)
//   2) Прямой fetch к api.themoviedb.org
//   3) Прокси tmdb.cinema-numbers.space
//   4) Прокси corsproxy.io
//   5) XMLHttpRequest как последний шанс
const Net = {
    mem: {}, memKeys: [], inflight: {}, queue: [], running: 0,
    _isTV: null, _lampaApi: null, _lampaChecked: false,

    checkTV: () => {
        if (Net._isTV !== null) return Net._isTV;
        try {
            const ua = (navigator.userAgent || '').toLowerCase();
            Net._isTV = /smart-?tv|tizen|web0s|webos|netcast|hbbtv|aftb|aftm|aftt|aftss|aftssst|bravia|philipstv|maple|vidaa|hisense|android.*tv|miui.*tv|shield|mibox|mi.tv|google.tv|appletv|firetv|rokutv|opera.tv|viera|aquos/i.test(ua);
        } catch (e) { Net._isTV = false; }
        return Net._isTV;
    },

    checkLampaApi: () => {
        if (Net._lampaChecked) return Net._lampaApi;
        Net._lampaChecked = true;
        try {
            if (window.Lampa && window.Lampa.TMDB && typeof window.Lampa.TMDB.api === 'function') {
                Net._lampaApi = true;
            } else {
                Net._lampaApi = false;
            }
        } catch (e) { Net._lampaApi = false; }
        return Net._lampaApi;
    },
    ...
    // ── СПОСОБ 1: через Lampa.TMDB.api (использует прокси пользователя!)
    _tryLampaApi: (path, params, done) => {
        if (!Net.checkLampaApi()) return setTimeout(() => done('no-lampa-api'), 0);
        let finished = false;
        const finish = (err, data) => { if (finished) return; finished = true; done(err, data); };
        const timeout = setTimeout(() => finish('lampa-timeout'), 12000);
        try {
            const cleanPath = path.replace(/^\//, '');
            window.Lampa.TMDB.api(cleanPath, params || {}, (response) => {
                clearTimeout(timeout);
                if (response && typeof response === 'object' && !response.error && (response.results || response.id || response.page !== undefined || response.genres || response.name || response.title)) {
            ...
            clearTimeout(timeout);
            finish('lampa-err:' + (e && e.message));
        }
    },

    // ── СПОСОБ 2-4: универсальный fetch с разными URL
    _tryFetch: (url, done, timeout) => {
        timeout = timeout || (Net.checkTV() ? 25000 : 15000);
        let finished = false;
        const finish = (err, data) => { if (finished) return; finished = true; done(err, data); };
        if (typeof fetch !== 'function') return setTimeout(() => finish('no-fetch'), 0);
        let abortCtrl = null;
        let abortTimeout = null;
        try {
            if (typeof AbortController !== 'undefined') abortCtrl = new AbortController();
            abortTimeout = setTimeout(() => { try { if (abortCtrl) abortCtrl.abort(); } catch (e) {} finish('timeout'); }, timeout);
        } catch (e) {
            abortTimeout = setTimeout(() => finish('timeout'), timeout);
        }
        const opts = { method: 'GET', cache: 'default', mode: 'cors' };
        if (abortCtrl) opts.signal = abortCtrl.signal;
        try {
            fetch(url, opts).then(r => {
                if (abortTimeout) clearTimeout(abortTimeout);
                if (!r.ok) return finish('http ' + r.status);
                return r.text().then(txt => {
                    try { finish(null, JSON.parse(txt)); } catch (e) { finish('parse'); }
                }).catch(() => finish('parse'));
            }).catch(e => {
                if (abortTimeout) clearTimeout(abortTimeout);
                finish((e && e.name === 'AbortError') ? 'timeout' : ('fetch:' + (e && e.message || 'err')));
            });
        } catch (e) {
            if (abortTimeout) clearTimeout(abortTimeout);
            finish('fetch-init:' + (e && e.message));
        }
    },

    // ── СПОСОБ 5: XMLHttpRequest
    _tryXhr: (url, done, timeout) => {
        timeout = timeout || (Net.checkTV() ? 25000 : 15000);
        let finished = false;
        const finish = (err, data) => { if (finished) return; finished = true; done(err, data); };
        let xhr;
        const abortTimeout = setTimeout(() => { try { if (xhr) xhr.abort(); } catch (e) {} finish('timeout'); }, timeout);
        try {
            ...
            xhr.onreadystatechange = () => {
                ...
                if (xhr.status >= 200 && xhr.status < 400) {
                    try {
                        let d = xhr.response;
                        if (!d && xhr.responseText) d = JSON.parse(xhr.responseText);
                        if (!d) return finish('empty');
                        finish(null, d);
                    } catch (e) { finish('parse'); }
                } else finish('http ' + xhr.status);
            };
            xhr.onerror = () => { clearTimeout(abortTimeout); finish('net'); };
            xhr.send();
        } catch (e) { clearTimeout(abortTimeout); finish('xhr-init:' + (e && e.message)); }
    },

    // ── ГЛАВНАЯ ЛОГИКА: каскад из 5 попыток
    _start: (url, path, params) => {
        Net.running++;
        ...
        const urlWithParams = Net.url(path, params);
        let attempt = 0;
        const next = () => {
            attempt++;
            switch (attempt) {
                case 1:
                    Net._tryLampaApi(path, params, (err, data) => {
                        if (!err) { Net._remember(url, data); return finish(null, data); }
                        next();
                    });
                    break;
                case 2:
                    Net._tryFetch(urlWithParams, (err, data) => {
                        if (!err) { Net._remember(url, data); return finish(null, data); }
                        next();
                    });
                    break;
                case 3:
                    const cnUrl = 'https://tmdb.cinema-numbers.space/3' + path + '?api_key=' + Net.key() + '&language=' + LANG + (params ? '&' + Object.keys(params).map(k => k + '=' + encodeURIComponent(params[k])).join('&') : '');
                    Net._tryFetch(cnUrl, (err, data) => {
                        if (!err) { Net._remember(url, data); return finish(null, data); }
                        next();
                    });
                    break;
                case 4:
                    Net._tryFetch('https://corsproxy.io/?' + encodeURIComponent(urlWithParams), (err, data) => {
                        if (!err) { Net._remember(url, data); return finish(null, data); }
                        next();
                    });
                    break;
                case 5:
                    Net._tryXhr(urlWithParams, (err, data) => {
                        if (!err) { Net._remember(url, data); return finish(null, data); }
                        console.error('[Капсула] ❌ Все 5 способов не сработали для:', path, '| Ошибка:', err);
                        finish(err || 'all-failed');
                    });
                    break;
            }
        };
    ...
    _pump: () => {
        while (Net.running < MAX_PARALLEL && Net.queue.length) {
            const task = Net.queue.shift();
            Net._start(task.url, task.path, task.params);
        }
    },
    ...
    get: (path, params, ok, fail, opts) => {
        ...
        Net.queue.push({ path, params, url });
        Net._pump();
    },
    abortPending: () => { const q = Net.queue; Net.queue = []; q.forEach(t => Net._deliver(t.url, 'aborted')); },
    ...
    renderStep: () => {
        ...
        if (s === 0) {
            ...
            const cards = Onboard.moviesList.map(m => {
                ...
                if (m.poster_path) { const im = el('img'); im.loading = 'lazy'; im.decoding = 'async'; im.alt = ''; im.src = safeImgUrl(m.poster_path, 'w342'); c.appendChild(im); }
...
const View = {
    ...
    render: () => {
        ...
        if (m.poster_path) {
            ...
            img.src = safeImgUrl(m.poster_path, 'w500');
            pos.appendChild(img);
        }
    ...
    preload: () => {
        ...
        [1, -1].forEach(d => {
            ...
            if (it && it.poster_path) { const im = new Image(); im.decoding = 'async'; im.src = safeImgUrl(it.poster_path, 'w342'); }
        });
    },
    setGlow: (m) => {
        if (!View.glow) return;
        if (!pGet('glow', true)) { removeClass(View.glow, 'on'); return; }
        const url = m.backdrop_path ? safeImgUrl(m.backdrop_path, 'w780') : (m.poster_path ? safeImgUrl(m.poster_path, 'w342') : '');
...
const CapsuleComponent = function () {
    ...
    this.start = () => {
        App.active = true;
        Fx.last = 0;
        // Диагностика Smart TV
        console.log('[Капсула v' + VERSION + '] Запущена | Smart TV:', Net.checkTV() ? 'ДА' : 'НЕТ', '| Lampa.TMDB.api:', Net.checkLampaApi() ? 'ДА' : 'НЕТ', '| UA:', navigator.userAgent);
