(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-label{font-family:' + DISPLAY + ';font-size:2.2em;font-weight:800;color:#878c91;letter-spacing:.01em;text-align:center;white-space:nowrap;opacity:1;transform:translateY(0);transition:opacity .3s ease,transform .3s ease}' +
                '.sw-label.fade-out{opacity:0;transform:translateY(-8px)}' +
                '.sw-label.fade-in{opacity:0;transform:translateY(8px)}' +
                ...
                '.sw-dice.sw-rolling{animation:swRoll 1.05s cubic-bezier(.33,.66,.3,1) forwards}' +
                '@keyframes swRoll{0%{transform:translateX(0) rotate(0) translateY(0) scale(1)}12%{transform:translateX(calc(var(--sw-dist) * .12)) rotate(86deg) translateY(-14px) scale(1.06)}28%{transform:translateX(calc(var(--sw-dist) * .28)) rotate(202deg) translateY(0) scale(1)}44%{transform:translateX(calc(var(--sw-dist) * .44)) rotate(317deg) translateY(-10px) scale(1.05)}60%{transform:translateX(calc(var(--sw-dist) * .6)) rotate(432deg) translateY(0) scale(1)}76%{transform:translateX(calc(var(--sw-dist) * .76)) rotate(547deg) translateY(-6px) scale(1.03)}100%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0) scale(1)}}' +
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            var dIll = dim(/terminal illness|cancer|chemotherapy|leukemia|leukaemia|tumor|tumour|alzheimer|parkinson|aids|muscular dystrophy|brain tumor|lung cancer|incurable/, /онколог|злокачествен|химиотерап|лейкеми|неизлечим|терминальн|альцгеймер|паркинсон|рак лёгких|рак мозга|опухол/);
    ...
    function buildReadyInner(a) {
        ...
        return '' +
            ...
            '<button class="sw-dicebtn sw-focusable" id="sw-dice-btn" tabindex="0">' +
                '<span class="sw-label-wrap"><span class="sw-label reveal" id="sw-dice-label">Бросить кости</span></span>' +
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){
            try {
                ...
                dice.style.transition = 'transform .55s cubic-bezier(.34,1.56,.64,1)';
                dice.style.transform = 'translateX(0) rotate(0deg)';
                setTimeout(function(){ dice.style.transition = ''; dice.style.transform = ''; }, 600);
            } catch(e) {}
        });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 1050, frame = 0;
                var iv = setInterval(function(){
                    ...
                    if (p >= 1) {
                        ...
                        label.className = 'sw-label fade-out';
                        setTimeout(function(){
                            label.textContent = final;
                            label.className = 'sw-label fade-in';
    ...
    function showModal(movie) {
        try {
            ...
            var html = $('<div class="sw-modal-content"><div id="sw-body"><div class="sw-loader"><div class="sw-loader-emoji" id="sw-loader-emoji">🔍</div><div class="sw-loader-text" id="sw-loader-text">Загрузка…</div><div class="sw-loader-progress"></div></div></div></div>');
            ...
            window._sw_loaderTimer = setInterval(function(){
                ...
                if (t.length) { t.css('opacity', 0); setTimeout(function(){ t.text(phases[pi].text).css('opacity', 1); }, 150); }
                if (e.length) { e.css('transform', 'scale(0.7)'); setTimeout(function(){ e.text(phases[pi].emoji).css('transform', 'scale(1)'); }, 120); }
            }, 400);
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v33.0 (fast load, dice text fix, kids fix)');
    }
