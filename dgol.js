...
//
// На части телевизоров (Tizen/WebOS/старый Android TV WebView) прямой
// XMLHttpRequest на api.themoviedb.org рвётся там, где на ПК всё работает:
// устаревший TLS-стек в прошивке, блокировка TMDb провайдером у части
// пользователей, отсутствие DoH/VPN, которые на компьютере часто есть
// в браузере. Поэтому транспорт двухуровневый:
//   1) если сама Lampa предоставляет свой сетевой класс (Lampa.Reguest) —
//      используем его: он уже умеет то, что нужно именно на ТВ-прошивках;
//   2) если его нет или он падает — обычный XHR как и раньше.
// Плюс: можно задать свой домен/прокси TMDb в настройках, если провайдер
// блокирует themoviedb.org на уровне DNS/IP именно на телевизоре.
...
const Net = {
    ...
    base: () => { const custom = String(pGet('tmdb_proxy', '') || '').trim().replace(/\/+$/, ''); return custom || TMDB_DEFAULT; },
    url: (path, params) => {
        let u = Net.base() + path + '?api_key=' + Net.key() + '&language=' + LANG;
    ...
    // XHR — универсальный запасной транспорт
    _xhr: (url, cb) => {
    ...
    // Lampa.Reguest — сетевой класс самой Lampa (если платформа его предоставляет)
    _lampaNet: null,
    _lampaTried: false,
    _lampa: (url, cb) => {
        const finish = once(cb);
    ...
    _start: (url) => {
        ...
        const finish = once((err, data) => {
            Net.running--;
            if (err) { Net.failStreak++; Net.lastErr = err; } else { Net.failStreak = 0; Net.lastErr = ''; Net._remember(url, data); }
            Net._deliver(url, err, data);
            Net._pump();
        });
        // сперва пробуем сетевой класс Lampa (лучше работает на ТВ),
        // при неудаче — обычный XHR как запасной путь
        ...
        History.stats((stats) => {
            ...
            Taste.enrich(items, 20, (cache) => {
                ...
                store({
                    ...
                    avgVote: votes.length ? votes.reduce((a, b) => a + b, 0) / votes.length : 0,                    seeds, watched, stats,
...
// ═══════════════════════════════════════════ ТЕМЫ
const THEMES = {
    astro: { name: 'Космос', cls: 'cm-t-astro', fx: 'astro', sys: 'ORBITAL UPLINK: ESTABLISHED', quotes: ['«Хьюстон, у нас проблема»', '«Космос ждёт»'], load: ['ПРОКЛАДЫВАЮ КУРС', 'СОБИРАЮ КАПСУЛУ'], vars: { '--cm-bg': '#05070D', '--cm-accent': '#FF7A2F', '--cm-accent2': '#7FD8FF', '--cm-text': '#E8ECF5', '--cm-sub': '#8695AC', '--cm-radius': '1.2em' } },    breakingbad: { name: 'Лаборатория', cls: 'cm-t-bb', fx: 'lab', sys: 'LAB NET: ONLINE', quotes: ['«Скажи моё имя»', '«Химия — это сила»'], load: ['СОБИРАЮ КАПСУЛУ'], vars: { '--cm-bg': '#0B0E08', '--cm-accent': '#D6E24A', '--cm-accent2': '#1FAE96', '--cm-text': '#EDF2E0', '--cm-sub': '#9AAE8C', '--cm-radius': '.6em' } },
...
.cm-bar{width:100%;max-width:64em;display:flex;align-items:center;justify-content:space-between;gap:.5em;
 ...
 -webkit-backdrop-filter:blur(16px);backdrop-filter:blur(16px); animation:cm-in-up .48s cubic-bezier(.22,.7,.25,1) both}
...
.cm-opt.cm-focus{background:rgba(var(--cm-accent-rgb),.18);color:var(--cm-text);border-color:var(--cm-accent);transform:translateX(3px);box-shadow:0 .4em 1em rgba(var(--cm-accent-rgb),.2)}
...
const View = {
    ...
    renderEmpty: () => {
        ...
        hero.appendChild(el('div', 'cm-why', netBad
            ? 'Не удалось получить данные с TMDb (' + esc(Net.lastErr || 'ошибка сети') + '). Если это происходит только на ТВ — провайдер, скорее всего, блокирует themoviedb.org на этом устройстве. Задайте свой прокси/зеркало TMDb в настройках или проверьте сеть.'
            : 'Нет истории Lampa или связи с TMDb. Пройдите короткий тест — капсула соберётся под ваш вкус за минуту.'));
        const acts = el('div', 'cm-acts');
        const bTest = el('div', 'cm-act primary', netBad ? 'Повторить' : 'Пройти тест');
        bTest._cmAction = netBad ? (() => { Net.drop(); Net.failStreak = 0; View.refreshCapsule(true); }) : (() => Onboard.start());
        const bRetry = el('div', 'cm-act secondary', netBad ? 'Прокси TMDb' : 'Повторить');
        bRetry._cmAction = netBad ? (() => Settings.askProxy()) : (() => { Net.drop(); Taste.invalidate(); View.refreshCapsule(true); });
...
const UI = {
    settings: () => {
        Modal.open({
            ...
            items: [
                ...
                { label: 'Прокси / зеркало TMDb', hint: pGet('tmdb_proxy', '') ? 'задан — на ТВ, где TMDb недоступен напрямую' : 'не задан, напрямую', onSelect: () => Settings.askProxy() },
    ...
    perf: () => {
        const cur = pGet('perf', 'auto');        const opts = [{ v: 'auto', l: 'Авто', h: 'определяется по устройству' }, { v: 'light', l: 'Лёгкий', h: 'без размытий, меньше частиц — для ТВ' }, { v: 'high', l: 'Максимум', h: 'все эффекты' }];
        Modal.open({
...
const Settings = {
    askKey: () => askText('Ключ TMDb', pGet('tmdb_key', ''), (v) => { pSet('tmdb_key', String(v).trim()); Net.drop(); Taste.invalidate(); notify('Ключ сохранён'); }),
    askProxy: () => askText('Прокси TMDb (например https://mirror.example.com/3, слово «сброс» — вернуть напрямую)', pGet('tmdb_proxy', ''), (v) => {
        let val = String(v || '').trim();
        if (val && val.toLowerCase() === 'сброс') val = '';
        pSet('tmdb_proxy', val);
        Net.drop(); Net.failStreak = 0; Net._lampaTried = false; Taste.invalidate();
        notify(val ? 'Прокси сохранён' : 'Прокси сброшен, используется TMDb напрямую');
        View.refreshCapsule(true);
    })
};
