/**
 * Capsule Mod v9.1 — «Капсула»
 *
 * Изолированное пространство внутри Lampa: минимум визуального шума,
 * пять-шесть фильмов вместо бесконечных лент, космонавт-компаньон,
 * который объясняет, почему именно это.
 *
...
(function () {
    ...
    /* ---- storage: сначала Lampa, потом чистый localStorage, потом наш префикс ---- */
    ...
    /* =========================================================================
       4. ЧТЕНИЕ ИСТОРИИ LAMPA  (главный фикс)
       ========================================================================= */
    /*
       В актуальной Lampa storage-ключ `favorite` выглядит так:
         { card:[ {полные карточки} ], history:[id,id,...], like:[id...],
           wath:[id...], book:[id...], look:[id...], viewed:[id...],
           continued:[id...], thrown:[id...], scheduled:[id...] }
       То есть списки — это ID, а не объекты. Старый код искал c.id у числа
       и получал пустую историю. Плюс `timeline` ключуется по хешу файла,
       из него id карточки вытащить нельзя — оттуда берём только «смотрел вообще».
       Дополнительно считываем отдельные ключи, если они существуют вне `favorite`.
    */
    var WEIGHTS = {
    ...
    var History = {
        read: function () {
            ...
            /* --- 2. отдельные ключи, встречающиеся в разных сборках --- */
            ...
            /* --- 3. timeline: пробуем достать id из ключей --- */
            ...
            if (timeline && typeof timeline === 'object') {
                for (var tk in timeline) {
                    ...
                    if (m) {
                        var id = m[2] || m[1];
                        var type = m[1] === 'tv' ? 'tv' : 'movie';
                        bump(id, 1.5, type, null);
                    }
                }
            }

            /* --- 4. дописываем тип из найденных карточек --- */
            var out = [];
        ...
        /** Сколько вообще есть данных — для диагностики и приветствия. */
        stats: function () {
            var items = this.read();
            var withCards = 0;
    ...
    var Taste = {
        ...
        /** Тянем жанры и теги для карточек, о которых знаем только id. */
    ...
    var Capsule = {
        ...
        /** Главный сбор: кандидаты из вкуса, отбор шести с объяснением. */
        build: function (taste, opts, cb) {
            ...
            // A. «похожее на то, что смотрел» — по двум сильнейшим семенам
            ...
            // C. по любимым тегам — самое точное попадание
            if (topK.length) {
                var kids = [];
            ...
            // D. страховка — если истории нет вообще
            ...
            parallel(tasks, function (packs) {
                ...
                if (picked.length < 3) {
                    // ослабляем: берём просто хорошее кино в любимом жанре
                    Net.get('/discover/movie', {
        ...
        pick: function (all, taste, force) {
            ...
            for (i = 0; i < all.length; i++) {
                ...
                if (seen[key]) {
                    // встретился из нескольких источников — это сильный сигнал
                    seen[key]._score += 3.5;
            ...
            // не даём капсуле стать однообразной: максимум 2 фильма одной «ветки»
            var bySrc = {}, final = [];
        ...
        run: function (query, taste, cb) {
            ...
            this.resolveTags(ctx.tags, function (kwIds) {
                ...
                if (ctx.type !== 'tv') tasks.push(function (done) {
                    Net.get('/discover/movie', discover('movie'), function (d) {
                        done(markList(d && d.results, 'movie', 'search', { query: query }));
                    }, function () { done([]); });
                });
                if (ctx.type !== 'movie') tasks.push(function (done) {
                    Net.get('/discover/tv', discover('tv'), function (d) {
                        done(markList(d && d.results, 'tv', 'search', { query: query }));
                    }, function () { done([]); });
                });
                if (ctx.tokens.length) tasks.push(function (done) {
                    Net.get('/search/multi', { query: query, page: 1, include_adult: false }, function (d) {
                        done(markList(d && d.results, null, 'search', { query: query }));
                    }, function () { done([]); });
                });
    ...
    var CSS = [
        ...
        /* звёздное поле + свет от постера */
        '.cm-stars{position:absolute;top:-10%;left:-10%;width:120%;height:120%;opacity:.4;',
        ...
        'background:radial-gradient(90% 70% at 65% 40%,rgba(5,7,13,.25),rgba(5,7,13,.88) 62%,#05070D 100%);}',

        /* приборная панель сверху: только иконки */
        '.cm-bar{position:absolute;top:0;left:0;right:0;height:4em;display:flex;align-items:center;padding:0 1.2em;z-index:40;}',
        '.cm-bar-r{margin-left:auto;display:flex;align-items:center;}',
        '.cm-ico{width:2.6em;height:2.6em;margin-left:.45em;border-radius:50%;display:flex;align-items:center;justify-content:center;',
        'background:rgba(232,236,245,.05);cursor:pointer;transition:background .18s,transform .18s,box-shadow .18s;}',
        '.cm-ico svg{width:1.2em;height:1.2em;fill:#9FB0C8;}',
        '.cm-ico.cm-focus{background:#FF7A2F;transform:scale(1.08);box-shadow:0 0 0 .14em rgba(255,122,47,.3);}',
        '.cm-ico.cm-focus svg{fill:#05070D;}',
        '.cm-ico.spin svg{animation:cm-spin .9s linear infinite;}',
        '@keyframes cm-spin{to{transform:rotate(360deg);}}',

        /* иллюминатор — главный элемент */
        '.cm-stage{position:absolute;top:0;left:0;right:0;bottom:0;display:flex;align-items:center;padding:4em 3em 1.5em 3em;}',
        '.cm-port{position:relative;display:flex;align-items:center;width:100%;max-width:72em;margin:0 auto;',
        'border-radius:1.5em;padding:1.8em;background:rgba(16,21,32,.65);',
        'box-shadow:inset 0 0 0 1px rgba(159,176,200,.1);}',
        '.cm-port:before{content:"";position:absolute;top:0;left:0;right:0;bottom:0;border-radius:1.5em;pointer-events:none;',
        'background:linear-gradient(115deg,rgba(255,255,255,.06) 0%,rgba(255,255,255,0) 34%);}',

        '.cm-hero-poster{position:relative;flex:none;width:16em;height:24em;border-radius:1em;overflow:hidden;background:#0B0F18;',
        ...
        '.cm-count{font-size:.7em;letter-spacing:.24em;color:#6C7C94;margin-bottom:.6em;}',
        ...
        '.cm-why{position:relative;padding-left:1em;font-size:1em;line-height:1.4;color:#C6D2E4;margin-bottom:.8em;max-width:30em;}',
        '.cm-why:before{content:"";position:absolute;left:0;top:.3em;bottom:.3em;width:.16em;border-radius:.1em;background:#FF7A2F;}',
        '.cm-acts{display:flex;flex-wrap:wrap;margin-top:.8em;}',
        '.cm-act{display:flex;align-items:center;padding:.75em 1.3em;border-radius:.7em;margin:0 .6em .6em 0;cursor:pointer;',
        'background:rgba(232,236,245,.07);font-size:1em;font-weight:500;color:#DCE4F0;transition:background .16s,transform .16s,color .16s;}',
        '.cm-act svg{width:1em;height:1em;fill:currentColor;margin-right:.5em;}',
        '.cm-act.primary{background:#FF7A2F;color:#05070D;}',
        '.cm-act.cm-focus{background:#FFFFFF;color:#05070D;transform:scale(1.03);}',
        '.cm-act.primary.cm-focus{background:#FF9354;color:#05070D;}',

        /* нижний ряд капсулы — маленькие постеры без подписей */
        '.cm-tray{position:absolute;left:0;right:0;bottom:1em;display:flex;justify-content:center;z-index:30;}',
        '.cm-tray-in{display:flex;align-items:center;padding:.5em;border-radius:1em;background:rgba(9,12,20,.55);',
        'box-shadow:inset 0 0 0 1px rgba(159,176,200,.1);}',
        '.cm-mini{position:relative;flex:none;width:3.8em;height:5.6em;border-radius:.6em;overflow:hidden;margin:0 .35em;',
        ...
        '.cm-mini.cm-focus{opacity:1;transform:translateY(-.4em) scale(1.08);box-shadow:0 .4em .8em rgba(0,0,0,.5),0 0 0 .12em #FF7A2F;}',

        /* космонавт */
        '.cm-astro-wrap{position:absolute;left:1.2em;bottom:.8em;z-index:35;display:flex;align-items:flex-end;}',
        '.cm-astro{width:7.5em;height:8.5em;flex:none;cursor:pointer;border-radius:1em;transition:transform .2s,box-shadow .2s;}',
        ...
        '.cm-astro.cm-focus{transform:scale(1.06);box-shadow:0 0 0 .14em #FF7A2F;background:rgba(255,122,47,.08);}',
        ...
        '.cm-say{margin:0 0 1.2em .8em;max-width:22em;padding:.7em 1em;border-radius:.9em .9em .9em .25em;',
        'background:rgba(12,17,27,.88);box-shadow:inset 0 0 0 1px rgba(159,176,200,.12);font-size:.9em;line-height:1.35;color:#C6D2E4;}',
        ...
        '.cm-load-ring{width:4em;height:4em;border-radius:50%;box-shadow:inset 0 0 0 .16em rgba(159,176,200,.15);position:relative;}',
        '.cm-load-ring:after{content:"";position:absolute;top:-.16em;left:-.16em;right:-.16em;bottom:-.16em;border-radius:50%;',
        'border:.16em solid transparent;border-top-color:#FF7A2F;animation:cm-spin 1.1s linear infinite;}',
        '.cm-load-txt{margin-top:1em;font-size:.7em;letter-spacing:.22em;color:#6C7C94;}',

        /* модальные */
        '.cm-ov{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(3,5,9,.82);z-index:999999;display:flex;',
        'align-items:center;justify-content:center;padding:1.2em;}',
        '.cm-modal{width:34em;max-width:100%;max-height:86%;overflow-y:auto;padding:1.6em;border-radius:1.2em;',
        'background:#0B0F18;box-shadow:inset 0 0 0 1px rgba(159,176,200,.12);}',
        '.cm-modal::-webkit-scrollbar{width:0;}',
        '.cm-modal h3{margin:0 0 .3em;font-size:1.25em;font-weight:700;letter-spacing:-.01em;}',
        '.cm-modal p{margin:0 0 1em;color:#8695AC;font-size:.95em;line-height:1.5;}',
        '.cm-modal p b{color:#C6D2E4;}',
        '.cm-opt{display:block;width:100%;text-align:left;padding:.8em 1em;margin-bottom:.45em;border-radius:.7em;',
        'background:rgba(232,236,245,.05);color:#DCE4F0;font-size:.95em;cursor:pointer;transition:background .15s,transform .15s,color .15s;}',
        '.cm-opt.cm-focus{background:#FF7A2F;color:#05070D;transform:scale(1.01);}',
        ...
        'background:rgba(232,236,245,.05);transition:background .15s,transform .15s,color .15s;}',
        '.cm-chip.cm-focus{background:#7FD8FF;color:#05070D;transform:scale(1.05);}',
        '.cm-input{width:100%;padding:.8em 1em;margin-bottom:.8em;border-radius:.7em;font-size:1em;color:#fff;outline:none;',
        'background:rgba(232,236,245,.05);border:1px solid rgba(159,176,200,.15);}',

        '.cm-toast{position:fixed;left:50%;bottom:1.8em;transform:translateX(-50%) translateY(1em);z-index:1000001;opacity:0;',
        'padding:.7em 1.2em;border-radius:.7em;background:rgba(11,15,24,.95);color:#DCE4F0;font-size:.9em;',
        'box-shadow:inset 0 0 0 1px rgba(159,176,200,.12);transition:opacity .25s,transform .25s;}',
        '.cm-toast.on{opacity:1;transform:translateX(-50%) translateY(0);}',

        '@media (hover:hover){.cm-act:hover,.cm-opt:hover,.cm-chip:hover{background:rgba(232,236,245,.12);}.cm-mini:hover{opacity:1;}}',

        /* планшет / телефон */
        '@media (max-width:1000px){',
        '.cm-root{font-size:13px;}',
        '.cm-stage{padding:4em 1em 8em;align-items:flex-start;}',
        '.cm-port{flex-direction:column;align-items:flex-start;padding:1em;border-radius:1.2em;}',
        '.cm-hero-poster{width:7em;height:10.5em;margin:0 0 .8em;}',
        '.cm-name{font-size:1.6em;}',
        '.cm-act{flex:1 1 42%;justify-content:center;}',
        '.cm-astro{width:4.5em;height:5.2em;}',
        '.cm-say{display:none;}',
        '.cm-tray{bottom:.6em;}',
        '}',
    ...
    /* Космонавт в невесомости, менее мультяшный */
    var SVG_ASTRO = [
        '<svg viewBox="0 0 200 220" xmlns="http://www.w3.org/2000/svg">',
        '<defs>',
        '<linearGradient id="cmBody" x1="0" y1="0" x2="0" y2="1">',
        '<stop offset="0" stop-color="#D9E0EB"/><stop offset="1" stop-color="#8895A9"/>',
        '</linearGradient>',
        '<radialGradient id="cmVisor" cx="0.35" cy="0.3" r="0.9">',
        '<stop offset="0" stop-color="#2C5C86"/><stop offset="0.5" stop-color="#0C1727"/><stop offset="1" stop-color="#05070D"/>',
        '</radialGradient>',
        '<linearGradient id="cmPack" x1="0" y1="0" x2="1" y2="1">',
        '<stop offset="0" stop-color="#B0BAC9"/><stop offset="1" stop-color="#6A788F"/>',
        '</linearGradient>',
        '</defs>',
        '<path d="M10 215 C 18 190, 26 170, 32 150" fill="none" stroke="#4A5568" stroke-width="4" stroke-linecap="round" opacity="0.8"/>',
        ...
        '<path d="M30 38 L 18 28 L 42 32 Z" fill="#FF7A2F"/>',
        ...
        '<path d="M162 145 L 152 160 L 170 155 Z" fill="#7FD8FF"/>',
        ...
        '<circle cx="90" cy="65" r="2" fill="#FFFFFF" opacity="0.9"/>',
        ...
        '<circle cx="85" cy="112" r="3" fill="#FF7A2F"/>',
        '<circle cx="95" cy="112" r="3" fill="#7FD8FF"/>',
        '<circle cx="105" cy="112" r="3" fill="#FFFFFF" opacity="0.6"/>',
        '</g>',
        '</svg>'
    ].join('');

    /* =========================================================================
       10. НАВИГАЦИЯ (пульт + мышь + сенсор)
       ========================================================================= */
    var Nav = {
    ...
    /* горизонтальный свайп по пустому месту переключает фильм капсулы */
    ...
    var View = {
        ...
        greet: function () {
            ...
            if (!t || t.empty || !t.count) {
                this.speak('Истории пока нет. Скажи настроение — соберу капсулу под него.');
                return;
            }
            var parts = [];
        ...
        render: function () {
            ...
            // убрали метаданные, оставили только причину
        ...
        reload: function (icon) {
            ...
            Taste.build(function (taste) {
                ...
                Capsule.build(taste, { force: true }, function (list) {
                    ...
                    self.render();
                    self.speak('Собрал новую капсулу.');
                });
            });
        },
    ...
    var Companion = {
        open: function () {
            ...
            Modal.open({
                ...
                text: 'Выбери состояние — соберу капсулу под него. Или скажи словами, что хочешь.',
                chips: chips,
                items: [
    ...
    var Settings = {
        ...
        diagnose: function () {
            ...
            if (!st.total) {
                html += 'Пусто. Обычно это значит, что фильмы ещё не открывались в этом профиле, либо включён другой профиль Lampa. ' +
                    'Открой пару карточек или добавь в избранное — и вернись сюда.';
            } else {
                var gs = [];
    ...
    (function () {
        try {
            ...
            addMenu();
            console.log('[Капсула] v9.1 загружена');
        } catch (e) {
