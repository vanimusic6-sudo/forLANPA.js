Обнови UI кино-подборки. Только правки:
1. Управление:
   - Смартфон: свайп вправо — следующий фильм, свайп влево — предыдущий.
   - ТВ: кнопки вправо/влево — те же действия.
2. Убери робота из интерфейса.
3. Кнопка «Изменить набор» внизу должна открывать меню:
   - «Обновить этот набор»
   - «Выбрать по настроению»
4. Исправь мобильную вёрстку: сейчас интерфейс выглядит некорректно. Сделай адаптив, чтобы всё помещалось и не ломалось.
5. Усиль визуальные отсылки к фильмам. «Матрица» уже отличная — остальные доведи до такого же уровня явности.                                                                                                                                                                            
Ничего лишнего не меняй, сохрани текущий смысл и контент.                                                                      /**
 * Capsule Mod v14.6 — «Капсула»
 *
 ...
 * 6. Топ-пул без истории вкуса расширен с одной страницы до 100 кандидатов
 *    (5 страниц TMDb × 20), после чего из него по-прежнему выбирается капсула.
...
(function () {
    ...
    const Capsule = {
        ...
        build: (taste, opts, cb) => {
            ...
            if (!taste.seeds?.length && !topG.length) {
                // Без истории вкуса используем не одну страницу, а полноценный TOP-100:
                // 5 страниц TMDb по 20 фильмов. Capsule.pick() по-прежнему отдаёт
                // пользователю только CAPSULE_SIZE карточек, но теперь выбирает их
                // из более широкого и устойчивого пула.
                tasks.push(done => {
                    const topPages = [1, 2, 3, 4, 5];
                    const pageTasks = topPages.map(topPage => finish => {
                        Net.get('/discover/movie', {
                            ...
                            page: topPage,
                            include_adult: false
                        }, (d) => finish(markList(d?.results, 'movie', 'top')),
                           () => finish([]),
                           { force });
                    });
                    parallel(pageTasks, packs => {
                        const pool = packs.flat().filter(Boolean).slice(0, TOP_POOL_SIZE);
                        done(pool);
                    });
                });
                tasks.push(done => {
    ...
    // === 11. CSS ===
    const CSS = `
        .cm-root{position:fixed;inset:0;min-height:100dvh;z-index:999998;overflow:hidden;color:var(--cm-text);background:var(--cm-bg);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;-webkit-tap-highlight-color:transparent;user-select:none;transition:background .4s}
        ...
        .cm-t-sw:after{content:"";position:absolute;right:-6%;top:-8%;width:34%;padding-top:34%;border-radius:50%;pointer-events:none;background:radial-gradient(circle at 35% 35%,rgba(255,255,255,.18),rgba(255,255,255,.035) 55%,transparent 60%);box-shadow:inset -1.2em -1.2em 2em rgba(0,0,0,.55),0 0 0 .08em rgba(255,232,31,.08);}
        ...
        .cm-t-bb:after{content:"⚗";position:absolute;right:1%;bottom:-7%;font-size:18em;line-height:1;opacity:.1;color:var(--cm-accent2);pointer-events:none;text-shadow:0 0 1.2em rgba(214,226,74,.18)}
        .cm-t-bb .cm-port{background-image:linear-gradient(rgba(214,226,74,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(31,174,150,.035) 1px,transparent 1px);background-size:2.2em 2.2em}
        ...
        .cm-t-cp:after{content:"";position:absolute;inset:0;pointer-events:none;opacity:.58;mix-blend-mode:overlay;background:repeating-linear-gradient(0deg,rgba(255,255,255,.07) 0 1px,transparent 1px 3px),linear-gradient(90deg,transparent 0 72%,rgba(255,42,109,.12) 72% 72.2%,transparent 72.2% 100%)}
        ...
        .cm-t-noir:before{content:"";position:absolute;inset:-10% -10%;pointer-events:none;opacity:.13;background-image:repeating-linear-gradient(112deg,rgba(255,255,255,.55) 0 1px,transparent 1px 26px),repeating-linear-gradient(70deg,rgba(255,255,255,.4) 0 1px,transparent 1px 34px);animation:cm-rainstreak 5s linear infinite}
        @keyframes cm-rainstreak{0%{transform:translateY(0)}100%{transform:translateY(4%)}}
        .cm-t-panda:before{content:"";position:absolute;left:0;top:0;bottom:0;width:14%;pointer-events:none;opacity:.22;background:repeating-linear-gradient(90deg,var(--cm-accent2) 0 6px,transparent 6px 26px);box-shadow:inset -1px 0 rgba(231,182,92,.3)}
        .cm-t-panda:after{content:"🐼";position:absolute;left:-2%;bottom:-8%;font-size:19em;line-height:1;opacity:.085;filter:saturate(.85);pointer-events:none}
        .cm-t-rm:before{content:"🌀";position:absolute;right:-3%;top:-8%;font-size:18em;line-height:1;opacity:.1;color:var(--cm-accent2);pointer-events:none;filter:drop-shadow(0 0 1em rgba(58,209,255,.2));animation:cm-rm-float 8s ease-in-out infinite}
        @keyframes cm-rm-float{0%,100%{transform:translate3d(0,0,0) rotate(0)}50%{transform:translate3d(-.4em,.35em,0) rotate(8deg)}}
        ...
        @media (max-width:1000px){
            ...
            .cm-stage{
                padding:calc(.8em + env(safe-area-inset-top)) calc(.8em + env(safe-area-inset-right))
                         calc(.8em + env(safe-area-inset-bottom)) calc(.8em + env(safe-area-inset-left));
            ...
            .cm-acts{gap:.5em;margin-right:0}
            .cm-act{flex:1 1 100%;justify-content:center;margin:0;min-width:0}
            .cm-tray{width:100%;max-width:100%}
        }
        @media (max-width:760px){
            .cm-root{font-size:clamp(11px,3.25vw,12px)}
            .cm-stage{
                min-height:100dvh;
                padding:calc(.55em + env(safe-area-inset-top)) calc(.55em + env(safe-area-inset-right))
                         calc(.55em + env(safe-area-inset-bottom)) calc(.55em + env(safe-area-inset-left));
                align-items:flex-start;
            ...
            .cm-port{
                ...
                padding:.75em;
                margin:0 auto;
                border-radius:max(.45em,var(--cm-radius));
            }
            .cm-hero-poster{
                width:min(9em,36vw);
                min-width:5.8em;
            ...
            .cm-hero{width:100%;min-width:0;max-width:100%;flex:1}
            .cm-count{font-size:.68em;letter-spacing:.16em;text-align:center}
            .cm-name{font-size:clamp(1.35em,6vw,1.7em);text-align:center;line-height:1.08}
            .cm-why{font-size:.9em;line-height:1.35;max-width:none;margin:0 0 .7em;padding-left:.85em}
            .cm-acts{
                display:grid;
            ...
            .cm-act.primary{grid-column:1 / -1}
            .cm-act svg{flex:none;width:1em;height:1em;margin-right:.4em}
            .cm-tray{padding-top:.65em;width:100%;max-width:100%}
            .cm-tray-in{width:100%;max-width:100%;overflow-x:auto}
            .cm-mini{width:3em;height:4.45em;margin:0 .22em}
            .cm-ov{padding:calc(.5em + env(safe-area-inset-top)) calc(.5em + env(safe-area-inset-right))
                         calc(.5em + env(safe-area-inset-bottom)) calc(.5em + env(safe-area-inset-left))}
            .cm-modal{width:min(100%,34em);max-height:calc(100dvh - 1em);padding:1em;box-sizing:border-box}
            .cm-chips{grid-template-columns:1fr 1fr;gap:.35em}
            .cm-chip{font-size:.82em;padding:.55em .65em;min-width:0;white-space:normal}
        }
        @media (max-width:400px){
            .cm-chips{grid-template-columns:1fr}
            .cm-hero-poster{width:min(8em,42vw);min-width:5.5em}
            .cm-act{font-size:.9em}
        }
    ...
    let swipe = { x: 0, y: 0, on: false, cancelled: false };
    document.addEventListener('touchstart', (e) => {
        if (!App.active || Modal.active() || !e.touches?.length) return;
    ...
    document.addEventListener('touchmove', (e) => {
        if (!swipe.on || !e.touches?.length) return;
        const dx = e.touches[0].clientX - swipe.x;
        const dy = e.touches[0].clientY - swipe.y;
        // Вертикальный жест оставляем обычному скроллу страницы/карточки.
        if (Math.abs(dy) > 18 && Math.abs(dy) > Math.abs(dx) * 1.15) swipe.cancelled = true;
    ...
    document.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(dx) > 70 && Math.abs(dx) > Math.abs(dy) * 1.6) {
            // Вправо = следующий фильм, влево = предыдущий.
            View.step(dx > 0 ? 1 : -1);
        }
    }, true);
    ...
    (() => {
        try {
            ...
            addMenu();
            console.log('[Капсула] v14.6 загружена');
        } catch (e) {
