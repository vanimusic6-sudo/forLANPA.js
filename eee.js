(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v43';
    ...
    function initSettings() {
        try {
            ...
            [
                ...
                { name: 'font_scale', type: 'select', title: 'Размер шрифта', values: {'16':'16','18':'18','20':'20','24':'24','28':'28','32':'32'}, default: '20' },
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            '.sw-custom-button-enhanced{cursor:pointer}' +
            '.sw-custom-button-enhanced.focus{background:rgba(255,255,255,.14);color:#fff}' +
            '.sw-modal-content{padding:26px 30px 48px;color:#e3e7ea;font-family:' + DISPLAY + ';font-size:20px;box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y}' +
            '.sw-modal-content::-webkit-scrollbar{width:5px}.sw-modal-content::-webkit-scrollbar-thumb{background:rgba(255,255,255,.15);border-radius:3px}' +
            ...
            '.sw-loader{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:70px 20px;min-height:50vh;color:#9aa3a9}' +
            '.sw-loader-emoji{font-size:3em;line-height:1;animation:swFloat 2.2s ease-in-out infinite}' +
            '@keyframes swFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}' +
            '.sw-loader-text{font-size:1em;font-weight:600;min-height:1.5em;text-align:center;color:#9aa3a9}' +
            '.sw-loader-progress{width:200px;height:3px;border-radius:2px;background:rgba(255,255,255,.08);overflow:hidden;position:relative;margin-top:8px}' +
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#79c07d,transparent);animation:swSlide 1.4s linear infinite}' +
            '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
            '.sw-dossier{position:relative;background:#212529;border-radius:14px;padding:28px 30px;margin-bottom:20px;animation:swRise .55s cubic-bezier(.22,1,.36,1) both}' +
            '@keyframes swRise{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}' +
            '.sw-badges{position:absolute;top:20px;right:20px;display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end;max-width:55%}' +
            '.sw-mode-badge{display:inline-flex;align-items:center;gap:6px;font-size:.64em;padding:5px 12px;border-radius:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:rgba(255,255,255,.06);color:#9aa3a9}' +
            '.sw-mode-dot{width:5px;height:5px;border-radius:50%;display:inline-block}' +
            '.sw-mode-dot.active{background:#79c07d}.sw-mode-dot.inactive{background:#7d8388}' +
            '.sw-verdict-word{font-size:2.6em;font-weight:800;letter-spacing:.02em;line-height:1;margin:0 0 18px;text-transform:uppercase;opacity:0;transform:translateY(8px) scale(.98);transition:opacity .5s ease,transform .55s cubic-bezier(.22,1,.36,1)}' +
            '.sw-verdict-word.appear{opacity:1;transform:translateY(0) scale(1)}' +
            '.sw-verdict-word.yes{color:#79c07d}.sw-verdict-word.no{color:#e0655c}.sw-verdict-word.maybe{color:#d9a441}' +
            '.sw-meter{height:4px;border-radius:2px;background:rgba(255,255,255,.12);overflow:hidden}' +
            '.sw-meter-fill{height:100%;width:0;border-radius:2px;transition:width .9s cubic-bezier(.25,.8,.3,1)}' +
            '.sw-meter-fill.yes{background:#79c07d}.sw-meter-fill.no{background:#e0655c}.sw-meter-fill.maybe{background:#d9a441}' +
            '.sw-dicebtn{position:relative;display:block;width:100%;height:100px;background:#eef1f3;border:none;border-radius:14px;margin:0 0 22px;overflow:hidden;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 3px rgba(255,255,255,.7)}' +
            ...
            '.sw-label{font-family:' + DISPLAY + ';font-size:2em;font-weight:800;color:#8a9096;letter-spacing:.02em;text-align:center;white-space:nowrap;opacity:1;transition:opacity .3s ease}' +
            '.sw-label.hidden{opacity:0}' +
            '.sw-label.res-yes{color:#3e7d41}.sw-label.res-no{color:#b3564f}' +
            '.sw-dice{position:absolute;left:16px;top:50%;width:70px;height:70px;margin-top:-35px;will-change:transform}' +
            '.sw-dice svg{width:100%;height:100%;display:block;transform:rotate(-8deg)}' +
            '.sw-dice.sw-rolling{animation:swRollFull 1.6s cubic-bezier(.33,.66,.3,1) forwards}' +
            '@keyframes swRollFull{0%{transform:translateX(0) rotate(0) translateY(0)}10%{transform:translateX(calc(var(--sw-dist) * .1)) rotate(72deg) translateY(-10px)}30%{transform:translateX(calc(var(--sw-dist) * .3)) rotate(216deg) translateY(0)}50%{transform:translateX(calc(var(--sw-dist) * .5)) rotate(360deg) translateY(-8px)}70%{transform:translateX(var(--sw-dist)) rotate(540deg) translateY(0)}80%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0)}90%{transform:translateX(calc(var(--sw-dist) * .4)) rotate(660deg) translateY(-6px)}100%{transform:translateX(0) rotate(720deg) translateY(0)}}' +
            '.sw-columns{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:22px}' +
            '.sw-col{position:relative;background:#212529;border-radius:12px;padding:24px 26px}' +
            '.sw-col::before{content:"";position:absolute;top:0;left:26px;right:26px;height:2px;border-radius:2px}' +
            '.sw-col.pros::before{background:#79c07d}.sw-col.cons::before{background:#e0655c}' +
            '.sw-title{font-size:.84em;font-weight:700;margin-bottom:16px;text-transform:uppercase;letter-spacing:.05em}' +
            '.sw-title.pros{color:#79c07d}.sw-title.cons{color:#e0655c}' +
            '.sw-list{margin:0;padding-left:20px;font-size:1.05em;line-height:1.7;color:#c9cfd4}' +
            '.sw-list li{margin-bottom:10px;opacity:0;transform:translateX(-8px);transition:opacity .4s ease,transform .4s cubic-bezier(.25,.8,.25,1)}' +
            '.sw-list li.appear{opacity:1;transform:translateX(0)}' +
            '.sw-focusable{outline:none;cursor:pointer}' +
            '@media(max-width:640px){.sw-modal-content{padding:18px 18px 34px}.sw-columns{grid-template-columns:1fr}.sw-verdict-word{font-size:2em}.sw-label{font-size:1.4em}.sw-label-wrap{padding:0 84px}.sw-dice{width:56px;height:56px;margin-top:-28px;left:12px}}';
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){ dice.classList.remove('sw-rolling'); });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var dist = Math.max(40, btn.clientWidth - dice.offsetWidth - 24);
                ...
                var t0 = Date.now(), DUR = 1600, frame = 0;
                var iv = setInterval(function(){
                    frame++;
                    if (frame % 3 === 0) dice.innerHTML = diceSVG(rndFace());
    ...
    function showModal(movie) {
        try {
            ...
            analyze(movie).then(function(a){
                ...
                html.find('#sw-body').html('<div class="sw-body" style="text-align:center;padding:48px 20px;color:#e0655c">Ошибка анализа</div>');
            });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v43.0 (smooth old anims + fixed dice)');
    }
