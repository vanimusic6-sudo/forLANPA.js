(function () {
    ...
    function injectCSS() {
        if (document.getElementById('sw-plugin-styles-v57')) return;
        ['sw-plugin-styles-v51', 'sw-plugin-styles-v52', 'sw-plugin-styles-v53', 'sw-plugin-styles-v54', 'sw-plugin-styles-v55', 'sw-plugin-styles-v56'].forEach(function(id){
            var old = document.getElementById(id);
            if (old && old.parentNode) old.parentNode.removeChild(old);
        });
        var s = document.createElement('style');
        s.id = 'sw-plugin-styles-v57';
        s.innerHTML =
            ...
            '.sw-custom-button-enhanced:hover,.sw-custom-button-enhanced.focus{background:#343a40!important;color:#fff!important;transform:scale(1.02);box-shadow:0 0 0 2px rgba(255,255,255,.85),0 4px 12px rgba(0,0,0,.4)!important}' +
            '.sw-custom-button-enhanced:hover svg path,.sw-custom-button-enhanced.focus svg path{fill:#fff}' +
            ...
            '.sw-dicebtn:hover,.sw-dicebtn.focus{background:#2f353a!important;box-shadow:0 0 0 2px rgba(255,255,255,.85),0 4px 12px rgba(0,0,0,.3)!important}' +
            ...
            '.sw-tab-btn:hover,.sw-tab-btn.focus{opacity:1;background:#343a40!important;color:#fff!important;box-shadow:0 0 0 2px rgba(255,255,255,.85),0 2px 8px rgba(0,0,0,.3)!important}' +
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            moods.forEach(function(md) {
                ...
                if (/напряжен|триллер|саспенс/.test(n)) { mFear += (pct / 12); mPace += (pct / 25); }
                if (/тревож|страх|ужас/.test(n)) { mFear += (pct / 10); }
                if (/задумчив|драматич|груст|трогательн/.test(n)) { mDrama += (pct / 12); }
                if (/романтич|любов/.test(n)) { mDrama += (pct / 15); }
                if (/загадоч|тайн|детектив/.test(n)) { mImmersion += (pct / 15); mFear += (pct / 20); }
            ...
            if (kwSlow) mPace -= 1.5;
            if (gHorror) mFear += 3.0;
            if (gThr) mFear += 1.5;
            if (kwScary) mFear += 1.5;
            ...
            if (gHistoricN || dWar.k || dWar.o) {
                aud(8, 'любителям исторических драм и военных сюжетов', 'тем, кто не любит неторопливое повествование с историческим контекстом');
                aud(6, 'интересующимся конкретной эпохой или событием', 'зрителям, которым тяжело даются военные и трагические темы');
                aud(5, 'ценителям аутентичных костюмов и декораций', 'фанатам современной динамики и футуризма');
            }
            if (gCrime && gThr) {
