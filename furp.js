 ...
 *
 * Изменения в v17.1:
 *  — Доработаны все фоновые анимации (плавнее, мягче, с учётом DPR и
 *    производительности).
 *  — Мобильный интерфейс поднят ещё выше, карточка компактнее.
 *  — Тема «Сон» затемнена до глубокого графита, тёплые акценты сохранены.
 *  — Мелкие исправления в навигации и обработке событий.
 *  — Найт‑Сити удалена (как и было).
 */
(function () {
    'use strict';
    ...
    const SCENES = {
        ...
        // Лаборатория: тёплые стеклянные пузырьки, более крупные и мягкие.
        lab: () => {
            const mk = (bottom) => ({
                x: .05 + Math.random() * .9,
        ...
        // Матрица: цифровой дождь, оптимизировано.
        matrix: () => {
        ...
        // Свиток: иероглифы мягко всплывают, сакура летит чуть заметнее.
        scroll: () => {
        ...
        // Портал: порталы крупнее, мебель больше, свечение при появлении.
        portal: () => {
            const N = 4;
        ...
        // Галактика: солнце/луна по реальному времени, добавлены лёгкие звёзды.
        galaxy: () => {
            let stars = [];
            const makeStars = (W, H) => {
        ...
        // Нуар: лёгкий косой дождь и медленный дым, чуть плотнее.
        noir: () => {
    ...
    // === CSS (оптимизирован, мобильная версия поднята выше) ===
    const CSS = `
.cm-root{
...
/* Мобильная раскладка: интерфейс поднят выше */
...
@media (max-width:480px){
    .cm-stage{padding:1.8em .5em .8em}
...
`;
    ...
    (() => {
        try {
            ...
            addMenu();
            console.log('[Капсула] v17.1 загружена');
