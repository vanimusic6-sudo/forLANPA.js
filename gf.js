(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-verdict-word{font-size:2.7em;font-weight:800;letter-spacing:.01em;line-height:1;margin:0 0 16px;text-transform:uppercase;opacity:0;transform:translateY(8px);transition:opacity .6s ease,transform .6s ease}' +
                ...
                '.sw-meter-fill{height:100%;width:0;border-radius:3px;transition:width 1.2s cubic-bezier(.25,.8,.3,1)}' +
                ...
                '.sw-label{font-family:' + DISPLAY + ';font-size:2.2em;font-weight:800;color:#878c91;letter-spacing:.01em;text-align:center;white-space:nowrap;opacity:0;transform:translateX(-20px);transition:opacity .8s ease,transform .8s cubic-bezier(.25,.46,.45,.94)}' +
                '.sw-label.reveal{opacity:1;transform:translateX(0)}' +
                ...
                '.sw-dice.sw-rolling{animation:swRoll 1.3s cubic-bezier(.25,.46,.45,.94) forwards}' +
                ...
                '.sw-list li{margin-bottom:9px;opacity:0;transform:translateX(-8px);transition:opacity .5s ease,transform .5s ease}' +
        ...
        try {
            Lampa.Controller.add('should_watch_modal_enhanced', {
                ...
                back: function() {
                    cleanupModal();
                    restorePrev();
                    setTimeout(function(){ try { Lampa.Modal.close(); } catch(e) {} }, 50);
                }
            });
    ...
    function bindDice(html) {
        ...
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 1300, frame = 0;
        ...
        try {
            ...
            analyze(movie).then(function(a){
                ...
                setTimeout(function(){
                    ...
                    html.find('.sw-list li').each(function(i){ var li = $(this); setTimeout(function(){ li.addClass('appear'); }, i * 60); });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v34.0 (optimized + slower animations)');
    }
