 ...
 *
 * Замысел: изолированное тихое место внутри Lampa. Одна карточка за раз,
 * минимум шума, ноль бесконечной ленты. Фильм должен находить тебя сам.
 *
 * Что нового в v17:
 *  1. Фоновые сцены переписаны на requestAnimationFrame (плавно, с учётом DPR,
 *     ставятся на паузу в фоне вкладки, уважают prefers-reduced-motion).
 *  2. Починен роутинг эффектов: раньше половина тем молча падала в общий
 *     запасной вариант (золотые кольца). Теперь у каждой темы своя сцена:
 *     Лаборатория — поднимающиеся пузырьки;
 *     Свиток — всплывающие иероглифы и приглушённая сакура;
 *     Портал — порталы в случайных местах перебрасывают мебель;
 *     Галактика — солнце/луна по реальному времени суток;
 *     Матрица — цифровой дождь; Космос — звёздный дрейф и орбиты;
 *     Нуар — лёгкий дождь и дым.
 *  3. Тема «Сон» переведена на графит с тёплой янтарной палитрой.
 *  4. Тема «Найт-Сити» убрана.
 *  5. Мобильная раскладка поднята к верху экрана, карточка компактнее.
 *  6. CSS собран заново: одна таблица без дублей, rgba-токены вместо
 *     color-mix, запасные варианты для aspect-ratio и clamp.
 */
(function () {
    'use strict';
    ...
    const Capsule = {
        ...
        pick: (all, taste, force) => {
            ...
            // Одна «дикая карта» вне основного жанра — чтобы капсула не замыкалась на себе.
    ...
    const THEMES = {
        ...
        inception: {
            ...
            // Графит + тёплый янтарь: тёмно, но не холодно.
            vars: { '--cm-bg': '#0C0C0E', '--cm-accent': '#E9B487', '--cm-accent2': '#C98A5E', '--cm-text': '#F1E8DF', '--cm-sub': '#8B8279', '--cm-radius': '1em' }
        },
        dune: {
    ...
    // === ФОНОВЫЕ СЦЕНЫ ===
    // Каждая сцена — фабрика, возвращающая { step, draw(ctx,W,H,dt,t), resize?(W,H) }.
    // step — минимальный интервал кадра в мс: тяжёлым сценам хватает 30 к/с.
    const SCENES = {

        // Космос: медленный звёздный дрейф, орбитальные эллипсы, редкий метеор.
        astro: () => {
            const stars = Array.from({ length: 80 }, () => ({ x: Math.random(), y: Math.random(), r: .3 + Math.random() * 1.1, p: Math.random() * 6.28, v: .004 + Math.random() * .014 }));
            ...
            return {
                ...
                draw(ctx, W, H, dt) {
                    ...
                    stars.forEach(s => {
                        ...
                        ctx.globalAlpha = .14 + .13 * Math.sin(t * 1.2 + s.p);
                    ...
                    if (shoot) {
                        ...
                        if (shoot.p >= 1) shoot = null;
                            ...
                            ctx.globalAlpha = .30 * Math.sin(shoot.p * Math.PI);
        ...
        // Лаборатория: тёплые стеклянные пузырьки медленно поднимаются вверх.
        lab: () => {
            const mk = (bottom) => ({
                x: .05 + Math.random() * .9,
                y: bottom ? 1.05 + Math.random() * .35 : Math.random(),
                r: 2 + Math.random() * 13,
                v: .025 + Math.random() * .075,
                w: Math.random() * 6.28,
                ws: .5 + Math.random()
            });
            const bubbles = Array.from({ length: 26 }, () => mk(false));
            let t = 0;
            return {
                step: 1000 / 30,
                draw(ctx, W, H, dt) {
                    ...
                    bubbles.forEach(b => {
                        ...
                        const fade = clamp(b.y * 6, 0, 1) * clamp((1.05 - b.y) * 4, 0, 1);
                        ctx.globalAlpha = .10 * fade;
                        ctx.strokeStyle = '#D6E24A'; ctx.lineWidth = 1;
                        ctx.beginPath(); ctx.arc(x, y, b.r, 0, 6.283); ctx.stroke();
                        ctx.globalAlpha = .05 * fade;
                        ctx.fillStyle = '#1FAE96';
                        ctx.beginPath(); ctx.arc(x, y, b.r * .92, 0, 6.283); ctx.fill();
                        ctx.globalAlpha = .13 * fade;
                        ctx.fillStyle = '#F4FBDA';
                        ctx.beginPath(); ctx.arc(x - b.r * .33, y - b.r * .33, Math.max(.7, b.r * .2), 0, 6.283); ctx.fill();
                    });
                    ctx.globalAlpha = 1;
                }
            };
        ...
        // Матрица: цифровой дождь. Символы меняются точечно, а не всем столбцом.
        matrix: () => {
                ...
                draw(ctx, W, H, dt) {
                    ...
                    for (let i = 0; i < cols.length; i++) {
                        ...
                        for (let k = 0; k < c.len; k++) {
                            ...
                            const f = 1 - k / c.len;
                            if (k === 0) { ctx.globalAlpha = .42; ctx.fillStyle = '#C8FFD4'; }
                            else { ctx.globalAlpha = .20 * f * f; ctx.fillStyle = '#00FF41'; }
        ...
        // Свиток: иероглифы мягко всплывают вверх, приглушённая сакура летит вниз.
        scroll: () => {
            ...
            const mkP = (top) => ({
                ...
                vy: .018 + Math.random() * .034,
                rot: Math.random() * 6.28,
                vr: (Math.random() - .5) * 1.3,
                sw: Math.random() * 6.28
            });
            const glyphs = Array.from({ length: 14 }, () => mkG(false));
            const petals = Array.from({ length: 22 }, () => mkP(false));
            let t = 0;
            return {
                step: 1000 / 30,
                draw(ctx, W, H, dt) {
                    ...
                    glyphs.forEach(g => {
                        ...
                        const fade = clamp(g.y * 3.5, 0, 1) * clamp((1.06 - g.y) * 3.5, 0, 1);
                        ctx.globalAlpha = (.035 + .022 * Math.sin(t * .8 + g.a)) * fade;
                    ...
                    petals.forEach(p => {
                        ...
                        ctx.globalAlpha = .06 + .02 * Math.sin(t + p.sw);
        ...
        // Портал: порталы вспыхивают в случайных местах и перебрасывают мебель.
        portal: () => {
            const N = 4;
            const mkPortal = () => ({ x: .12 + Math.random() * .76, y: .14 + Math.random() * .72, r: 24 + Math.random() * 34, age: 0, ttl: 8 + Math.random() * 8, ph: Math.random() * 6.28 });
            ...
            const items = Array.from({ length: 6 }, mkItem);
            ...
            return {
                ...
                draw(ctx, W, H, dt) {
                    ...
                    portals.forEach(p => {
                        ...
                        const inn = clamp(p.age / 1.2, 0, 1);
                        const out = clamp((p.ttl - p.age) / 1.2, 0, 1);
                        const life = inn * out;
                        const pulse = 1 + .07 * Math.sin(t * 1.5 + p.ph);
                        const x = p.x * W, y = p.y * H, r = p.r * pulse * (.5 + .5 * life);
                        ctx.lineWidth = 2.2;
                        ctx.globalAlpha = .09 * life; ctx.strokeStyle = '#7CFF6B';
                        ctx.beginPath(); ctx.ellipse(x, y, r, r * .82, 0, 0, 6.283); ctx.stroke();
                        ctx.globalAlpha = .06 * life; ctx.strokeStyle = '#3AD1FF'; ctx.lineWidth = 1.2;
                        ctx.beginPath(); ctx.ellipse(x, y, r * .66, r * .54, 0, 0, 6.283); ctx.stroke();
                        ctx.globalAlpha = .035 * life; ctx.fillStyle = '#7CFF6B';
                        ctx.beginPath(); ctx.ellipse(x, y, r * .9, r * .74, 0, 0, 6.283); ctx.fill();
                    });
                    const base = Math.min(W, H) * .045;
                    items.forEach((o, i) => {
                        o.p += o.v * dt;
                        ...
                        const y = (a.y + (b.y - a.y) * q) * H - Math.sin(q * Math.PI) * H * .07;
                        const s = base * (.25 + .75 * Math.sin(q * Math.PI));
                        ctx.globalAlpha = .10 * Math.sin(q * Math.PI);
        ...
        // Галактика: солнце или луна идёт по орбите строго по местному времени.
        galaxy: () => {
            let t = 0;
            return {
                step: 1000 / 30,
                draw(ctx, W, H, dt) {
                    ...
                    ctx.lineWidth = 1;
                    ctx.globalAlpha = .07; ctx.strokeStyle = night ? '#9FB0FF' : '#FFE81F';
                    ...
                    g.addColorStop(0, night ? 'rgba(190,203,255,.22)' : 'rgba(255,226,80,.24)');
        ...
        // Нуар: лёгкий косой дождь и медленный дым низко над кадром.
        noir: () => {
            const mkDrop = () => ({ x: Math.random() * 1.15 - .1, y: -.05 - Math.random() * .4, v: .7 + Math.random() * .8, len: .03 + Math.random() * .04 });
            const drops = Array.from({ length: 34 }, () => { const d = mkDrop(); d.y = Math.random(); return d; });
            const smoke = Array.from({ length: 7 }, () => ({ x: Math.random() * 1.2 - .1, y: .5 + Math.random() * .45, r: 40 + Math.random() * 70, v: .012 + Math.random() * .02, ph: Math.random() * 6.28 }));
            let t = 0;
            return {
                step: 1000 / 30,
                draw(ctx, W, H, dt) {
                    ...
                    smoke.forEach(s => {
                        s.x += s.v * dt;
                        if (s.x > 1.25) { s.x = -.25; s.y = .5 + Math.random() * .45; }
                        ctx.globalAlpha = .016 + .006 * Math.sin(t * .5 + s.ph);
                        ctx.fillStyle = '#BFBFBF';
                        ctx.beginPath(); ctx.arc(s.x * W, (s.y + Math.sin(t * .3 + s.ph) * .01) * H, s.r, 0, 6.283); ctx.fill();
                    });
        ...
        // Сон: тёплые концентрические слои, медленный «дрейф глубины».
        inception: () => {
            let t = 0;
            return {
                step: 1000 / 30,
                draw(ctx, W, H, dt) {
                    ...
                    ctx.lineWidth = 1;
                    for (let i = 0; i < 9; i++) {
                        const r = (i + 1) * base * .055 * (1 + .035 * Math.sin(t * .4 + i * .6));
                        ctx.globalAlpha = .014 + i * .0022;
        ...
        // Арракис: длинные песчаные волны.
        dune: () => {
            let t = 0;
            return {
                step: 1000 / 30,
        ...
        // Неон: плотный косой дождь с розово-голубыми отблесками.
        blade: () => {
            const mk = () => ({ x: Math.random() * 1.15 - .1, y: -.05 - Math.random() * .4, v: .9 + Math.random() * 1.1 });
            const drops = Array.from({ length: 40 }, () => { const d = mk(); d.y = Math.random(); return d; });
            return {
                step: 1000 / 30,
                draw(ctx, W, H, dt) {
                    ctx.lineWidth = 1;
        ...
        // Тихий вечер: почти невидимые тёплые облака.
        dream: () => {
            const clouds = Array.from({ length: 8 }, () => ({ x: Math.random(), y: Math.random(), r: 45 + Math.random() * 95, v: .012 + Math.random() * .018 }));
            return {
                step: 1000 / 24,
                draw(ctx, W, H, dt) {
                    clouds.forEach(c => {
    ...
    const Themes = {
        ...
        apply: (key, root) => {
            ...
            // Токены в rgb-виде: rgba(var(--cm-accent-rgb),.08) работает везде, color-mix — нет.
    ...
    const CSS = `
/* ============================================================
   КАПСУЛА — единая таблица стилей.
   Правило одно: тишина. Крупный постер, короткий текст,
   две кнопки. Всё остальное — шёпотом.
   ============================================================ */
...
/* --- Системная строка: почти невидима, но задаёт характер темы --- */
.cm-sysline{
...
/* ============================================================
   АДАПТИВ. На телефоне карточка живёт у верхней кромки:
   большой палец дотягивается до кнопок, глаза не бегают вниз.
   ============================================================ */
@media (max-width:900px){
    .cm-stage{padding:2.9em 1em 1em}
...
@media (max-width:700px){
    .cm-sysline{top:.7em;left:1em;font-size:.5em}
    .cm-stage{
        justify-content:flex-start;
        padding:2.35em .6em .9em;
        gap:.5em;
    }
    .cm-port{
        margin:0;
...
@media (max-width:480px){
    .cm-stage{padding:2.1em .5em .8em}
...
/* Широкие экраны и ТВ: чуть крупнее, чтобы читалось с дивана */
...
`;
    ...
    // Свайп: по дистанции или скорости, с коротким виброоткликом.
