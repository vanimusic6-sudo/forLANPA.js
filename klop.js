 ...
 *
 * Интегрирован премиальный интерфейс:
 * — динамический размер шрифта (Dynamic Font Size)
 * — нижняя панель-трей (Bottom Bar)
 * — тактильные свайпы (Tactile Swipes)
 * — улучшены системы анализа, управления и скорость за счёт:
 *   · дедупликации сетевых запросов (pending queue)
 *   · ограничения одновременных запросов (parallel concurrency)
 *   · интеллектуального кэширования и защиты от двойного взвешивания истории
 *   · адаптивной навигации по сетке и трею
 */
(function () {
    'use strict';
    ...
    // === СЕТЬ (с дедупликацией и контролем параллелизма) ===
    const Net = {
        mem: {},
        pending: {},
        ...
        get: (path, params, ok, fail, opts = {}) => {
            ...
            // Дедупликация: если такой запрос уже выполняется, цепляемся к нему
            if (Net.pending[url]) {
                Net.pending[url].push({ ok, fail });
                return;
            }
            Net.pending[url] = [{ ok, fail }];
            const xhr = new XMLHttpRequest();
            xhr.open('GET', url, true);
            xhr.timeout = 12000;
            const finish = (err, data) => {
                const queue = Net.pending[url];
                delete Net.pending[url];
                if (!queue) return;
                queue.forEach(cb => {
                    if (err) { if (cb.fail) cb.fail(err); }
                    else { cb.ok(data); }
                });
            };
    ...
    // parallel с ограничением одновременных задач
    const parallel = (tasks, done, limit = 3) => {
        ...
        let running = 0;
        const worker = () => {
            while (idx < tasks.length && running < limit) {
                const i = idx++;
                running++;
                tasks[i]((r) => {
                    out[i] = r;
...
<circle cx="94" cy="90" r="4.5" fill="#FF7A2F"/>
    ...
    const updateViewportFont = () => {
        ...
        else if (w <= 1000) fs = 13;
        else fs = Math.min(16, Math.max(12, w / 80));
        if (h < 420) fs = Math.min(fs, 10);
        View.root.style.fontSize = fs + 'px';
    };
    window.addEventListener('resize', updateViewportFont);
    window.addEventListener('orientationchange', updateViewportFont);
    ...
    // === КОМПОНЕНТ LAMPA ===
    const App = { active: false, fallback: false, _enteredBefore: false };
