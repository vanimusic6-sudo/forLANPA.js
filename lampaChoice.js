(function () {
    ...
    window._sw_blocknav = false;     // ← по умолчанию OFF, включается только для ТВ/ПК
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                '.sw-modal-content{padding:22px 26px 44px;color:#fff;font-family:' + DISPLAY + ';box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;touch-action:pan-y}' +
                ...
                /* Тач-режим: гасим любые фокус-эффекты и scroll-margin, чтобы нативный свайп был плавным */
    ...
    /* scrollContainerTo: программно подкручивает к элементу.
       ВАЖНО: на тач-устройствах (когда _sw_blocknav === false) НЕ вызывается,
       чтобы не прерывать инерционный свайп. */
    function scrollContainerTo(el, center) {
        if (!window._sw_blocknav) return;          // ← ключевой фикс для смартфонов
        try {
    ...
    /* ===== ФОКУС-КОЛЬЦО (только для ТВ/ПК с клавиатурой/пультом) ===== */
    ...
    function highlightVisible() {
        if (!window._sw_blocknav) return;          // ← на таче не подсвечиваем «ближайший»
    ...
    function scrollStep(dir) {
        if (!window._sw_blocknav) return;          // ← на таче не дёргаем скролл из кода
        var cn = getScrollContainer();
        if (!cn) return;
    ...
    function swKeyCapture(e) {
        if (!window._sw_blocknav) return;           // ← на таче ничего не перехватываем
    ...
    /* ===== ОТКРЫТИЕ МОДАЛКИ С МЕНЯЮЩИМСЯ ЭМОДЗИ ===== */
    function showModal(movie) {
        try {
            ...
            // capture-обработчик Enter — только для TV/ПК
    ...
    function startPlugin() {
        try {
            // Возвращаем корректный детектор устройства
            ...
            // blocknav = true только на TV или на ПК без тача
        ...
        console.log('[ShouldWatch] v13.0 (smooth touch scroll)');
    }
