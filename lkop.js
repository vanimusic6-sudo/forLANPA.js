...
function addMenuItem() {
    var mk = function () {
        ...
        if (window.jQuery || window.$) {
            btn = $('<li class="menu__item selector"><div class="menu__ico">' + SVG_MENU + '</div><div class="menu__text">Capsule Mod</div></li>');
            btn.on('hover:enter', function () { Lampa.Activity.push({ url: '', title: 'Capsule Mod', component: COMPONENT, page: 1 }); });
            $('.menu .menu__list').eq(0).append(btn);
        } else {
            btn = el('li', 'menu__item', '<div class="menu__ico">' + SVG_MENU + '</div><div class="menu__text">Capsule Mod</div>');
            btn.onclick = function () { try { Lampa.Activity.push({ url: '', title: 'Capsule Mod', component: COMPONENT, page: 1 }); } catch (e) {} };
            var list = document.querySelector('.menu .menu__list'); if (list) list.appendChild(btn);
        }
    };
    try {
        if (window.appready) mk();
        else if (window.Lampa && Lampa.Listener && Lampa.Listener.follow) { Lampa.Listener.follow('app', function (e) { if (e.type === 'ready') mk(); }); setTimeout(function () { if (window.appready) mk(); }, 1500); }
        else mk();
    } catch (e) {}
}
function addLampaSettings() {
    try {
        ...
        Lampa.SettingsApi.addParam({ component: 'capsule_mod', param: { name: 'capsule_mod_mascot', type: 'toggle', default: true }, field: { name: 'Маскот-робот', description: 'Показывать робота-помощника в режиме капсулы' }, onChange: function () { if (state.root && state.view === 'main') renderMain(); } });
        Lampa.SettingsApi.addParam({ component: 'capsule_mod', param: { name: 'capsule_mod_depth', type: 'select', values: ['За месяц', 'За все время'], default: 'За месяц' }, field: { name: 'Глубина анализа', description: 'Период истории просмотров для матрицы интересов' } });
        Lampa.SettingsApi.addParam({ component: 'capsule_mod', param: { name: 'capsule_mod_sens', type: 'select', values: ['Точные совпадения', 'Больше экспериментов'], default: 'Точные совпадения' }, field: { name: 'Алгоритм', description: 'Строгость рекомендательного движка' } });
        Lampa.SettingsApi.addParam({ component: 'capsule_mod', param: { name: 'capsule_mod_tmdb_key', type: 'input', default: '' }, field: { name: 'TMDB API ключ', description: 'Пусто = встроенный ключ по умолчанию' } });
        Lampa.SettingsApi.addParam({ component: 'capsule_mod', param: { name: 'capsule_mod_reset', type: 'trigger', default: false }, field: { name: 'Сбросить профиль', description: 'Очистить и пересчитать матрицу предпочтений' }, onChange: function () { Engine.resetProfile(function (p) { state.profile = p; notify('Capsule Mod: профиль пересчитан'); }); } });
    } catch (e) {}
}
function startPlugin() {
    window.plugin_capsule_mod = true; injectCSS();
    try { if (window.Lampa && Lampa.Component && Lampa.Component.add) Lampa.Component.add(COMPONENT, CapsuleComponent); } catch (e) {}
    addLampaSettings(); addMenuItem();
    console.log('[CapsuleMod] v1.0.0 зарегистрирован');
}
if (typeof window.Lampa === 'undefined') { setTimeout(function () { if (typeof window.Lampa !== 'undefined') startPlugin(); }, 300); }
else if (window.appready) startPlugin();
else if (Lampa.Listener && Lampa.Listener.follow) { Lampa.Listener.follow('app', function (e) { if (e.type === 'ready') startPlugin(); }); setTimeout(function () { if (window.appready && !window.plugin_capsule_mod) startPlugin(); }, 1500); }
else startPlugin();
})();