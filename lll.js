(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-mode-badge{position:absolute;top:20px;right:20px;display:inline-flex;align-items:center;gap:6px;font-size:.68em;padding:4px 12px;border-radius:14px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:rgba(255,255,255,.07);color:#9aa1a6}' +
                '.sw-mode-dot{width:5px;height:5px;border-radius:50%;display:inline-block}' +
                '.sw-mode-dot.active{background:#7ec260;box-shadow:0 0 8px rgba(126,194,96,.8)}.sw-mode-dot.inactive{background:#7d8388}' +
                '.sw-verdict-word{font-size:2.7em;font-weight:800;letter-spacing:.01em;line-height:1;margin:0 0 12px;text-transform:uppercase;opacity:0;transform:translateY(8px);transition:opacity .45s ease,transform .45s ease}' +
                ...
                '.sw-verdict-reason{font-size:1.02em;color:#b9c0c5;line-height:1.7;margin:0 0 20px;max-width:72ch;opacity:0;transition:opacity .45s ease .1s}' +
                '.sw-verdict-reason.appear{opacity:1}' +
                ...
                '.sw-dicebtn.focus{box-shadow:0 0 0 4px rgba(126,194,96,.65)}' +
                ...
                '@keyframes swRoll{' +
                    '0%{transform:translateX(0) rotate(0) translateY(0)}' +
                    '20%{transform:translateX(calc(var(--sw-dist) * .2)) rotate(140deg) translateY(-12px)}' +
                    '40%{transform:translateX(calc(var(--sw-dist) * .42)) rotate(280deg) translateY(0)}' +
                    '60%{transform:translateX(calc(var(--sw-dist) * .63)) rotate(430deg) translateY(-9px)}' +
                    '80%{transform:translateX(calc(var(--sw-dist) * .84)) rotate(580deg) translateY(0)}' +
                    '100%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0)}}' +
    ...
    function reviewStats(reviews) {
        var posRe = /шедевр|великолепн|потрясающ|восхитит|блестящ|лучш|мощн|гениальн|masterpiece|brilliant|amazing|great|best|loved|perfect|outstanding|must-watch|замечательн|превосходн|отличн|весел|смешн|funny|fun/i;
    ...
    /* ===== НАСТРОЕНИЯ (как в Lampa: из блока карточки или из ключевых слов TMDB) ===== */
    function readMoodsFromDOM() {
        try {
            ...
            if (found >= 3 || moods.length >= 3) return { metrics: mm, moods: moods, fromLampa: true };
        } catch(e) {}
        return null;
    }
    function computeMoods(kw, genres) {
        ...
        var violence = cnt([/violenc/,/gore/,/blood/,/murder/,/brutal/,/torture/,/weapon/,/massacre/,/stab/,/war/,/crime/]);
        var sadness = cnt([/sad/,/grief/,/loss/,/funeral/,/terminal illness/,/crying/,/melanchol/,/depress/,/traged/,/orphan/,/death/]);
        ...
        if (hasGenre(genres, /comedy|комедия/i) || cnt([/comedy/,/humor/,/parody/,/satire/]) >= 1) moods.push({ name: 'Весёлый', pct: 35 });
        ...
        return { metrics: mm, moods: moods, fromLampa: false };
    }

    /* ===== Lampa-локальные сигналы ===== */
    function lampaLocal(movie) {
        ...
        try {
            ...
            if (window.Lampa && Lampa.Favorite) {
                try { if (typeof Lampa.Favorite.check === 'function') out.inFavorite = !!Lampa.Favorite.check(card); } catch(e) {}
                if (!out.inFavorite && typeof Lampa.Favorite.list === 'function') {
                    ['scheduled','later','viewed','favorite'].forEach(function(n){
                        try {
            ...
            if (Lampa.Timeline) {
                try {
                    var t = typeof Lampa.Timeline.get === 'function' ? Lampa.Timeline.get(id) : null;
                    if (t) {
                        if (t.duration > 0 && t.time >= 0) out.viewedPercent = Math.round(100 * t.time / t.duration);
                        else if (t.percent >= 0) out.viewedPercent = Math.round(t.percent);
                    }
                } catch(e) {}
            }
        } catch(e) {}
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            var ctx = { kw: meta.kw };
            var moodsData = readMoodsFromDOM() || computeMoods(meta.kw, genres);
            var mm = moodsData.metrics || {}, moods = moodsData.moods || [];

            /* байесовская поправка рейтинга — точный вердикт при малом числе голосов */
            var M = 150, C = 6.1;
            ...
            /* контент-флаги: сначала теги TMDB, описание — только как неподтверждённая подсказка */
            function dim(kwRe, ovRe) { var k = hasKw(ctx, kwRe); var o = !!ovRe && inAnyText(ovBoth, ovRe); return { k: k, o: o }; }
            var dViol = dim(/violenc|gore|murder|blood|tortur|brutal|weapon|massacre|execution|stab|slaughter/, /насил|жесток|кров|убийств|резн|стрельб/);
            var dDrugs = dim(/drug|narcotic|addiction|meth|cocaine|heroin|marijuan|overdose|dealer|cartel/, /нарко|кокаин|героин|марихуан/);
            var dNud = dim(/nudity|topless|strip club|stripper|full frontal/, /обнаж|нагот|голы/);
            var dSex = dim(/sex scene|sexual content|orgy|prostitut|erotic|one night stand/, /эротик|откровен|проститутк|интимн/);
            var dSmoke = dim(/smoking|cigarette|cigar/, /курени|сигарет/);
            var dAlc = dim(/alcohol|drunkenness|drunk|booze/, /алкогол|водк|выпив|пьян/);
            var dProf = dim(/profanity|strong language|swearing|f word/, /нецензур|мат|ругательств/);
            var dSuic = dim(/suicide|self harm/, /суицид|самоубийств/);
            var dHor = hasGenre(genres, /horror|ужас|slasher/i) || hasKw(ctx, /horror|scary|slasher|haunted|possession|demon/);

            var hardAdult = dDrugs.k || dNud.k || dSex.k || dViol.k || !!movie.adult;
            var familyOK;
            if (isAnim) familyOK = !hardAdult && !dSuic.k && (age === null || age <= 12) && (hasFamilyGenre || kidsKw || (age !== null && age <= 12));
            else familyOK = !hardAdult && !dHor && !dSuic.k && rating >= 5 && ((age !== null && age <= 12) || hasFamilyGenre || kidsKw);
            ...
            /* факты по источникам, у каждого — потолок вклада (нет доминации) */
            var F = [];
            ...
            if (yr === now) add('card','pro','🆕 Свежая новинка ' + yr + ' года', 6);
            else if (yr === now - 1) add('card','pro','🔥 Актуальная новинка — все обсуждают', 6);
            if (votes > 0 && votes < 100) add('card','con','❓ Всего ' + votes + ' оценок — рейтинг ненадёжен', 14);
            if (votes === 0) add('card','con','⚠️ Нет оценок — данных недостаточно', 16);
            if (votes >= 100 && adj < cfg.min_rating) add('card','con','📉 Может разочаровать: рейтинг ' + rating.toFixed(1) + ' ниже вашего порога', 22);
            if (votes >= 50 && rating > 0 && rating < 5) add('card','con','📉 Низкие оценки зрителей (' + rating.toFixed(1) + ')', 26);
            if (runtime > 0 && runtime <= 95) add('card','pro','🕐 Удобный хронометраж — ' + runtime + ' мин', 7);
            if (runtime > 180) add('card','con','⌛ Очень длинный (' + runtime + ' мин)', 12);
            else if (runtime > 150) add('card','con','⌛ Довольно длинный (' + runtime + ' мин)', 8);
            if (/CAM|TS|HDCAM|HDRIP|TELECINE|SCR|WORKPRINT|TELESYNC/i.test(q)) add('card','con','📺 Плохое качество изображения и звука (' + q + ')', 26);
            else if (/4K|UHD|2160p/i.test(q)) add('card','pro','🎥 Отличное 4K-качество', 10);
            else if (q) add('card','pro','🎥 Хорошее качество (' + q + ')', 7);
            ...
            /* — TMDB: ценз, теги, трейлер — */
            if (age !== null && age >= 18) add('tmdb','con','🔞 Только для взрослых (' + age + '+)', 14);
            else if (age !== null && age >= 16) add('tmdb','con','🔞 Не для детей (' + age + '+)', 12);
            if (familyOK) add('tmdb','pro','👨👩‍ Подходит для семейного просмотра' + (age !== null ? ' (ценз ' + age + '+)' : ''), 16);
            if (isAnim && familyOK) add('tmdb','pro','🧸 Безопасный детский контент', 10);
            if (isAnim && !familyOK) add('tmdb','con','🎭 Анимация для взрослой аудитории' + (age !== null ? ' (' + age + '+)' : ''), 14);
            if (dViol.k) add('tmdb','con','🔪 Жестокие и кровавые сцены', 16); else if (dViol.o) add('tmdb','con','🔪 В описании упоминается жестокость', 8);
            if (dDrugs.k) add('tmdb','con','💉 Затрагивается тема наркотиков', 14); else if (dDrugs.o) add('tmdb','con','💉 В описании упоминаются наркотики', 7);
            if (dNud.k) add('tmdb','con','🫣 Есть сцены с наготой', 12);
            if (dSex.k) add('tmdb','con','💋 Откровенные сцены', 12);
            if (dSuic.k) add('tmdb','con','⚠️ Затрагивается тема суицида', 16);
            if (dProf.k && (mm.language || 0) < 4) add('tmdb','con','🤬 Присутствует нецензурная лексика', 8);
            if (dAlc.k) add('tmdb','con','🍺 Присутствует алкоголь', 6);
            if (dSmoke.k) add('tmdb','con','🚬 Показано курение', 5);
            ...
            /* — настроения и метрики (как в Lampa) — */
            if (mm.fear >= 6) add('moods','con','😱 Может напугать: много пугающих моментов', 14);
            else if (mm.fear >= 3.5) add('moods','pro','😬 Держит в напряжении до финала', 9);
            ...
            if (mm.violence >= 6) add('moods','con','🩸 Много насилия и жестокости', 12);
            if (mm.sadness >= 6) add('moods','pro','😢 Сильная эмоциональная история — может растрогать', 10);
            if (mm.language >= 5) add('moods','con','🤬 Много нецензурной лексики', 10);
            moods.forEach(function(md){
                var n = (md.name || '').toLowerCase();
                if (/вес[её]л|комедий|юмор/.test(n) && md.pct >= 20) add('moods','pro','😂 Поднимет настроение', 14);
                else if (/напряжен/.test(n) && md.pct >= 30) add('moods','pro','🔥 Напряжённый сюжет — не оторваться', 8);
                else if (/тревож/.test(n) && md.pct >= 40) add('moods','pro','😰 Тревожная атмосфера — держит в тонусе', 6);
                ...
                else if (/груст/.test(n) && md.pct >= 20) add('moods','con','😔 Может нагрустить', 6);
            });
            if (hasGenre(genres, /comedy|комедия/i) && (mm.sadness || 0) <= 3) add('moods','pro','😂 Лёгкий комедийный тон — поднимет настроение', 10);

            /* — отзывы — */
            if (rt.total >= 2 && rt.tone === 'pos') add('reviews','pro','💬 Зрители в восторге: ' + rt.pos + ' из ' + rt.total + ' отзывов положительные', 20);
            if (rt.total >= 2 && rt.tone === 'neg') add('reviews','con','💬 Может разочаровать: ' + rt.neg + ' из ' + rt.total + ' отзывов отрицательные', 22);
            if (rt.total >= 2 && rt.tone === 'mix') { add('reviews','pro','💬 Отзывы полярные: ' + rt.pos + ' из ' + rt.total + ' положительных', 6); add('reviews','con','💬 Часть зрителей осталась недовольна', 8); }

            /* — локальные данные Lampa — */
            if (local.inFavorite && local.favList === 'viewed') add('lampa','con','👁 Вы уже смотрели — стоит ли пересматривать?', 8);
            else if (local.inFavorite) add('lampa','pro','🔖 Уже в ваших закладках — вы планировали посмотреть', 8);
            if (local.viewedPercent >= 90) add('lampa','con','👁 Вы уже досмотрели (' + local.viewedPercent + '% просмотра)', 8);
            else if (local.viewedPercent >= 10) add('lampa','con','⏸ Вы бросили просмотр на ' + local.viewedPercent + '% — возможно, не зашло', 12);

            /* ===== подсчёт с потолками по источникам ===== */
            var CAPS = { card: 55, user: 50, tmdb: 40, moods: 40, reviews: 28, lampa: 18 };
            var per = {};
            ...
            var activeSrc = 1 + (meta.kw.length || age !== null ? 1 : 0) + (rt.total >= 2 ? 1 : 0) + (moodsData.fromLampa || moods.length ? 1 : 0) + (local.inFavorite || local.viewedPercent > 0 ? 1 : 0);
            ...
            return { pros: pros, cons: cons, review: rt, score: score, norm: norm, vClass: vClass, vWord: vWord, reason: reason, mode: (meta.kw.length || age !== null || rt.total) ? 'TMDB' : 'TAGS' };
        });
    }
    ...
    /* ===== РЕНДЕР ===== */
    var DICE_SVG = '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="6" width="88" height="88" rx="22" fill="#b9bdc2" stroke="#71767b" stroke-width="6"/><circle cx="36" cy="36" r="9" fill="#17181a"/><circle cx="64" cy="64" r="9" fill="#17181a"/></svg>';

    function buildReadyInner(a) {
        var badge = a.mode === 'TMDB'
            ? '<span class="sw-mode-badge"><span class="sw-mode-dot active"></span>TMDB</span>'
            : '<span class="sw-mode-badge"><span class="sw-mode-dot inactive"></span>TAGS</span>';
        var quote = '';
        if (a.review.sample && a.review.total >= 2) {
            ...
            quote = '<div class="sw-quote' + toneCls + '"><div class="sw-quote-text">«' + esc(txt) + '»</div><div class="sw-quote-meta">— ' + esc(a.review.sample.author) + ', отзыв зрителя</div></div>';
        }
        return '' +
            ...
            '<button class="sw-dicebtn sw-focusable" id="sw-dice-btn" tabindex="0">' +
                ...
                '<span class="sw-dice" id="sw-dice">' + DICE_SVG + '</span>' +
            '</button>' +
    ...
    function bindDice(html) {
        ...
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 1050, pool = 'СМОТРЕТЬНЕ';
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v15.0 (moods + smooth dice)');
    }
