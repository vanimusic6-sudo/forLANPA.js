(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                '.sw-modal-content{padding:20px 24px 40px;color:#fff;font-family:' + DISPLAY + ';box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y;overscroll-behavior:contain}' +
    ...
    function lampaLocal(movie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (dDrugs.k) add('tmdb','con','💉 Затрагивается тема наркотиков', 14, 'drugs'); else if (dDrugs.o) add('tmdb','con','💉 В описании упоминаются наркотики', 7, 'drugs');
            ...
            if (mm.violence >= 6) add('moods','con','🩸 Много насилия и жестокости', 12, 'violence');
            else if (dViol.k) add('tmdb','con','🔪 Жестокие и кровавые сцены', 16, 'violence');
            else if (dViol.o) add('tmdb','con','🔪 В описании упоминается жестокость', 8, 'violence');
            if (mm.language >= 5) add('moods','con','🤬 Много нецензурной лексики', 10, 'lang');
            else if (dProf.k) add('tmdb','con','🤬 Присутствует нецензурная лексика', 8, 'lang');
            if (mm.fear >= 6) add('moods','con','😱 Может напугать: много пугающих моментов', 14, 'scare');
            if (mm.fear >= 3.5 && mm.fear < 6) add('moods','pro','😬 Держит в напряжении до финала', 9, 'tension');
            ...
            if (rt.total >= 2 && rt.tone === 'pos') add('reviews','pro','💬 Зрители в восторге: ' + rt.pos + ' из ' + rt.total + ' отзывов положительные', 20, 'rev');
            else if (rt.total >= 2 && rt.tone === 'neg') add('reviews','con','💬 Может разочаровать: ' + rt.neg + ' из ' + rt.total + ' отзывов отрицательные', 22, 'rev');
            else if (rt.total >= 2 && rt.tone === 'mix') { add('reviews','pro','💬 Отзывы полярные: ' + rt.pos + ' из ' + rt.total + ' положительных', 6, 'rev'); add('reviews','con','💬 Часть зрителей осталась недовольна', 8); }
            ...
            else if (local.viewedPercent >= 10) add('lampa','con','⏸ Вы бросили просмотр на ' + local.viewedPercent + '% — возможно, не зашло', 12, 'lampa');var CAPS = { card: 55, user: 50, tmdb: 40, moods: 40, reviews: 28, lampa: 18 };
            var per = {};
        ...
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var iv = setInterval(function(){
                    ...
                    if (frame % 2 === 0) dice.innerHTML = diceSVG(rndFace()); /* грани меняются во время прокатки */
                    if (p < 1) {
                        ...
                        clearInterval(iv);
                        dice.innerHTML = diceSVG(rndFace()); /* выпавшая грань */
    ...
    function showModal(movie) {
        try {
            ...
            /* тач-скролл: не даём глобальным свайп-обработчикам Lampa тормозить прокрутку */
            try {
                var mcEl = html[0];
                ['touchstart','touchmove','touchend'].forEach(function(t){
                    mcEl.addEventListener(t, function(e){ e.stopPropagation(); }, { passive: true });
                });
            } catch(e) {}
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v16.0 (dedupe, strict favorites, live dice faces, touch scroll fix)');
    }
