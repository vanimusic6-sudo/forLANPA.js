(function () {
    ...
    /* ===== КИНОПОИСК: отзывы + возраст (только с токеном) ===== */
    function kpGet(url) {
        return new Promise(function(res){
            try {
    ...
    function loadKp(movie) {
        ...
        return kpGet('https://api.kinopoisk.dev/v1.4/movie/search?query=' + encodeURIComponent(title) + (yr ? '&year=' + yr : '') + '&limit=1').then(function(s){
            ...
            if (!id) return { reviews: null, age: ageKp };
            return kpGet('https://api.kinopoisk.dev/v1.3/review?filmId=' + id + '&page=1&limit=8').then(function(r){
                ...
                return { reviews: list, age: ageKp };
            });
        }).catch(function(){ return null; });
    }

    /* ===== НАСТРОЕНИЯ + возраст из карточки Lampa ===== */
    function readMoodsFromDOM() {
        try {
            ...
            if (am) ageDom = parseInt(am[1], 10);
            if (found >= 3 || moods.length >= 3) return { metrics: mm, moods: moods, age: ageDom, fromLampa: true };
        } catch(e) {}
        return null;
    }
    function computeMoods(kw, genres) {
        ...
        return { metrics: mm, moods: moods, age: null, fromLampa: false };
    }
    ...
    function lampaLocal(movie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie)), loadKp(movie)]).then(function(arr){
            ...
            /* мульти-сигнальная уверенность: теги + жанры + описание + ценз + метрики Lampa */
            ...
            var gAct = hasGenre(genres, /action|боевик/i);
            var cViol = (dViol.k?50:0) + ((mm.violence||0)>=6?40:((mm.violence||0)>=4?20:0)) + (gHorror||gWar?20:(gCrime&&gThr?15:(gCrime&&gAct?10:0))) + (dViol.o?20:0) + (meta.age!==null&&meta.age>=18?15:(meta.age!==null&&meta.age>=16?10:0));
            var cDrugs = (dDrugs.k?50:0) + (dDrugs.o?25:0) + (gCrime&&hasGenre(genres,/drama|драма|bio|биограф/i)?15:0) + (meta.age!==null&&meta.age>=16?10:0);
            var cSex = (dSex.k?50:0) + (dNud.k?30:0) + (dSex.o||dNud.o?25:0) + (meta.age!==null&&meta.age>=18?20:0);
            var cProf = (dProf.k?50:0) + ((mm.language||0)>=5?40:((mm.language||0)>=3?20:0)) + (dProf.o?20:0) + (meta.age!==null&&meta.age>=18?10:0);
            ...
            var cSuic = (dSuic.k?50:0) + (dSuic.o?30:0) + (hasGenre(genres,/drama|драма/i)?5:0);
            var cAlc = (dAlc.k?40:0) + (dAlc.o?25:0) + (meta.age!==null&&meta.age>=16?10:0);
            var cSmoke = (dSmoke.k?40:0) + (dSmoke.o?25:0) + (meta.age!==null&&meta.age>=12?5:0);

            /* возраст: TMDB -> Кинопоиск -> карточка Lampa -> вывод по содержанию */
            var age = meta.age;
            var ageInferred = false;
            if (age === null && kp && kp.age !== null && kp.age !== undefined) age = kp.age;
            if (age === null && moodsData.age) age = moodsData.age;
            if (age === null && (cSex >= 50 || cDrugs >= 50 || cViol >= 60)) { age = 16; ageInferred = true; }

            var hardAdult = cViol >= 50 || cDrugs >= 50 || cSex >= 50 || !!movie.adult;
            var familyOK;
            if (isAnim) familyOK = !hardAdult && cSuic < 50 && (age === null || age <= 12) && (hasFamilyGenre || kidsKw || (age !== null && age <= 12));
            else familyOK = !hardAdult && cFear < 50 && cSuic < 50 && rating >= 5 && ((age !== null && age <= 12) || hasFamilyGenre || kidsKw);
            ...
            if (cDrugs >= 50) add('tmdb','con','💉 Затрагивается тема наркотиков', 14, 'drugs');
            if (cSex >= 50) add('tmdb','con','🫣 Нагота или откровенные сцены', 12, 'sex');
            if (cProf >= 50) add('tmdb','con','🤬 Много нецензурной лексики', 10, 'lang');
            if (cFear >= 50) add('moods','con','😱 Может напугать: много пугающих моментов', 14, 'scare');
            else if ((mm.fear||0) >= 3.5) add('moods','pro','😬 Держит в напряжении до финала', 9, 'tension');
            if (cSuic >= 50) add('tmdb','con','⚠️ Затрагивается тема суицида', 16, 'suic');
            ...
            moods.forEach(function(md){
                ...
                else if (/тревож/.test(n) && md.pct >= 40) add('moods','pro','😰 Тревожная атмосфера', 6, 'tension');
                ...
                else if (/груст/.test(n) && md.pct >= 20) add('moods','con','😔 Может заставить грустить', 6, 'sad');
            });
            ...
            var activeSrc = 1 + (meta.kw.length || meta.age !== null ? 1 : 0) + (rt.total >= 2 ? 1 : 0) + (moodsData.fromLampa || moods.length ? 1 : 0) + (local.inFavorite || local.viewedPercent > 0 ? 1 : 0) + (kpUsed || (kp && kp.age) ? 1 : 0);
            ...
            return { pros: pros, cons: cons, review: rt, score: score, norm: norm, vClass: vClass, vWord: vWord, mode: (meta.kw.length || meta.age !== null || rt.total || kpUsed) ? 'TMDB' : 'TAGS' };
        });
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v18.0 (multi-signal facts, kp support, no reason block)');
    }
