/**
 * Capsule Mod v13.0 — «Капсула»
 * - 8 тем с отсылками к фильмам (цитаты, эффекты, декор)
 * - Робот перерисован, 8 стилей рисовки под темы, живёт в Компаньоне
 * - Поднята нижняя панель, добавлена верхняя панель
 * - Полная оптимизация под пульт (2D-навигация) и сенсор (тапы, свайпы, long-press)
 * - Исправлены баги: details, enrich-callback, пустые параметры, прелоад постеров
 */
...
(function () {
    ...
    const rnd = (arr) => arr.length ? arr[Math.floor(Math.random() * arr.length)] : '';
    ...
    const MOODS = [
        ...
        { label: '🌆 Киберпанк', q: 'киберпанк неон фантастика антиутопия' },
        { label: '🎩 Нуар', q: 'нуар детектив классика' }
    ];
    ...
    // === 7. МОДЕЛЬ ВКУСА (исправлен множественный cb в enrich) ===
        ...
        build: (cb) => {
            History.stats((stats) => {
                ...
                Taste.enrich(items, 14, (cache) => {
                    if (!cache) return cb({ empty: true, count: 0, genres: [], keywords: [], seeds: [], watched: {}, stats });
            ...
            parallel(tasks, (packs) => {
                ...
                if (picked.length < 3) {
                    const p = {
                        ...
                        'vote_average.gte': 7.0, page: 1, include_adult: false
                    };
                    if (topG.length) p.with_genres = topG[0].id;
                    Net.get('/discover/movie', p, (d) => {
                        cb(Capsule.pick(all.concat(markList(d?.results, 'movie', 'relax')), taste, force));
                    }, () => cb(picked));
                } else cb(picked);
            });
        },
    ...
    const parseQuery = (raw) => {
        ...
        const ctx = {
            raw, genresM: [], genresT: [], tags: [], tokens: [],
            type: 'any', yearFrom: 0, yearTo: 0, minVote: 5.8, minVotes: 40
        };
        ...
        words.forEach(w => { if (w.length >= 4 && !STOP_WORDS.includes(w)) ctx.tokens.push(w); });
    ...
    // === 10. ТЕМЫ (8 штук: усиленные отсылки, цитаты, эффекты) ===
    const THEMES = {
        astro: {
            name: 'Космос', cls: 'cm-t-astro',
            quotes: ['«Хьюстон, у нас проблема»', '«Любовь — единственное, что преодолевает время и пространство»', '«Не зови меня земным»', '«Космос ждёт»'],
            load: ['ПРОКЛАДЫВАЮ КУРС', 'СЧИТАЮ ТРАЕКТОРИЮ', 'ЗАПУСКАЮ ДВИГАТЕЛИ'],
            vars: {
        ...
        breakingbad: {
            ...
            quotes: ['«Скажи моё имя»', '«Я — тот, кто стучит»', '«Лучше звоните Солу»', '«Химия — это изучение материи»'],
            load: ['ВАРЮ СИНЬКУ', 'ПРОВЕРЯЮ ЧИСТОТУ 99.1%', 'ПРЯЧУСЬ ОТ ХЭНКА'],
            vars: {
        ...
        matrix: {
            ...
            load: ['СЛЕЖУ ЗА БЕЛЫМ КРОЛИКОМ', 'ВЗЛАМЫВАЮ МАТРИЦУ', 'УВОРАЧИВАЮСЬ ОТ ПУЛЬ'],
            vars: {
        ...
        panda: {
            ...
            quotes: ['«Случайностей не бывает»', '«Внутренний покой»', '«Прошлое — прошлое»', '«Твоё время пришло»'],
            load: ['ЧИТАЮ СВИТКИ', 'ИЩУ ВНУТРЕННИЙ ПОКОЙ', 'ТРЕНИРУЮ НЕИСТОВУЮ ПЯТЁРКУ'],
            vars: {
        ...
        rickmorty: {
            ...
            quotes: ['«Вубба-лубба-даб-даб»', '«Рик и Морти, навсегда»', '«Это не игра, это опасно»', '«Покажи мне, что умеешь»'],
            load: ['ПРЫГАЮ ЧЕРЕЗ ПОРТАЛ', 'ИЩУ ФЛАМИНГО', 'СМЕШИВАЮ ИЗМЕРЕНИЯ'],
            vars: {
        ...
        starwars: {
            ...
            quotes: ['«Да пребудет с тобой Сила»', '«У меня плохое предчувствие»', '«Я твой отец»', '«Давным-давно, в далёкой галактике…»'],
            load: ['ГИПЕРПРЫЖОК', 'СЧИТАЮ ПАРСЕКИ', 'ЗАРЯЖАЮ ЛАЗЕРЫ'],
            vars: {
                '--cm-bg': '#000308', '--cm-accent': '#FFE81F', '--cm-accent2': '#4BD5FF',
                '--cm-text': '#F2F4F8', '--cm-sub': '#9AA0A6',
                '--cm-panel': 'rgba(8,12,20,.72)', '--cm-panel2': 'rgba(4,8,14,.6)',
                '--cm-chip': 'rgba(255,232,31,.07)', '--cm-radius': '.8em'
            }
        },
        cyberpunk: {
            name: 'Найт-Сити', cls: 'cm-t-cp',
            quotes: ['«Проснись, самурай»', '«Высокие технологии, низкая жизнь»', '«Найт-Сити не прощает»', '«Сгореть ярко — не сгореть вовсе»'],
            load: ['ВЗЛАМЫВАЮ СЕТЬ', 'ВСТАВЛЯЮ ИМПЛАНТЫ', 'ИЩУ ЗАКАЗ В НАЙТ-СИТИ'],
            vars: {
                '--cm-bg': '#0A0A12', '--cm-accent': '#FF2A6D', '--cm-accent2': '#05D9E8',
                '--cm-text': '#EAF2FF', '--cm-sub': '#7A86A0',
                '--cm-panel': 'rgba(12,12,24,.74)', '--cm-panel2': 'rgba(8,8,18,.62)',
                '--cm-chip': 'rgba(5,217,232,.07)', '--cm-radius': '.4em'
            }
        },
        noir: {
            name: 'Нуар', cls: 'cm-t-noir',
            quotes: ['«Забудь её, Джейк»', '«Никто не идеален»', '«Вечный сон»', '«Дождь смывает все следы»'],
            load: ['ЛИСТАЮ ДЕЛО', 'СЛЕЖУ ПОД ДОЖДЁМ', 'ДОПРАШИВАЮ СВИДЕТЕЛЕЙ'],
            vars: {
                '--cm-bg': '#0B0B0B', '--cm-accent': '#E6E6E6', '--cm-accent2': '#B48A3C',
                '--cm-text': '#EDEDED', '--cm-sub': '#8A8A8A',
                '--cm-panel': 'rgba(18,18,18,.8)', '--cm-panel2': 'rgba(12,12,12,.68)',
                '--cm-chip': 'rgba(230,230,230,.06)', '--cm-radius': '.3em'
            }
        }
    };
    ...
    const Themes = {
        ...
        quote: (key) => rnd((THEMES[key || Themes.current()] || THEMES.astro).quotes || []),
        loadLine: () => rnd((THEMES[Themes.current()] || THEMES.astro).load || ['СОБИРАЮ КАПСУЛУ']),
        fx: (key, root) => {
    ...
    // === 11. CSS ===
    const CSS = `
        .cm-root { position: fixed; top:0; left:0; right:0; bottom:0; z-index:999998; overflow:hidden;
        ...
        .cm-root .cm-act, .cm-root .cm-opt, .cm-root .cm-chip, .cm-root .cm-mini, .cm-root .cm-theme-chip { touch-action: manipulation; }
        ...
        .cm-glow { position:absolute; top:-25%; left:-25%; width:150%; height:150%; background-size:cover;
            background-position:center; opacity:0; filter: blur(80px) saturate(150%); transition: opacity .8s ease; }
        ...
        /* верхняя панель */
        .cm-bar { position:absolute; top:0; left:0; right:0; z-index:40; }
        .cm-bar-in { display:flex; align-items:center; gap:.7em; padding:.7em 1.4em; }
        .cm-logo { font-size:.72em; letter-spacing:.32em; color:var(--cm-sub); font-weight:700; }
        .cm-theme-chip { padding:.4em .95em; border-radius:1.2em; background:var(--cm-chip); font-size:.78em;
            cursor:pointer; transition: background .15s, transform .15s, color .15s; box-shadow: inset 0 0 0 1px rgba(255,255,255,.08); }
        .cm-theme-chip.cm-focus { background:var(--cm-accent); color:#05070D; transform:scale(1.07); }
        .cm-hint { margin-left:auto; font-size:.66em; color:var(--cm-sub); opacity:.85; letter-spacing:.08em; }
        .cm-stage { position:absolute; top:0; left:0; right:0; bottom:0; display:flex; align-items:center;
            justify-content:center; padding: 3.2em 3em 8.2em 3em; }
        .cm-port { position:relative; display:flex; align-items:center; width:100%; max-width:72em;
        ...
        .cm-port:before { content:""; position:absolute; top:0; left:0; right:0; bottom:0; border-radius:var(--cm-radius);
            pointer-events:none; background: linear-gradient(115deg, rgba(255,255,255,.06) 0%, rgba(255,255,255,0) 34%); }
        .cm-hero-poster { position:relative; flex:none; width:16em; height:24em; border-radius: calc(var(--cm-radius) * 0.7);
            overflow:hidden; background:#0B0F18; box-shadow: 0 1em 2em rgba(0,0,0,.5); margin-right:1.8em; cursor:pointer; }
        ...
        .cm-why { position:relative; padding-left:1em; font-size:1em; line-height:1.4; color:var(--cm-text);
            opacity:.85; margin-bottom:1em; max-width:30em; }
        ...
        .cm-act { display:flex; align-items:center; justify-content:center; padding:.9em 1.6em;
            border-radius: calc(var(--cm-radius) * 0.5); margin:0 .6em .6em 0; cursor:pointer;
            background: var(--cm-chip); font-size:1.05em; font-weight:600; color:var(--cm-text);
        ...
        /* нижняя панель — поднята */
        .cm-tray { position:absolute; left:0; right:0; bottom:2.8em; display:flex; justify-content:center; z-index:30; }
        .cm-tray-in { display:flex; align-items:center; padding:.5em; border-radius:1em; background:var(--cm-panel2);
            box-shadow: inset 0 0 0 1px rgba(255,255,255,.08), 0 .6em 1.6em rgba(0,0,0,.45); max-width:94%; overflow-x:auto; }
        .cm-tray-in::-webkit-scrollbar { width:0; height:0; }
        .cm-mini { position:relative; flex:none; width:3.8em; height:5.6em; border-radius:.5em; overflow:hidden;
            margin:0 .35em; background:#0B0F18; cursor:pointer; opacity:.5;
        ...
        .cm-mini.cm-focus { opacity:1; transform: translateY(-.4em) scale(1.08);
            box-shadow: 0 .4em .8em rgba(0,0,0,.5), 0 0 0 .12em var(--cm-accent); }
        .cm-load { position:absolute; top:0; left:0; right:0; bottom:0; display:flex; flex-direction:column;
        ...
        .cm-load-ring:after { content:""; position:absolute; top:-.16em; left:-.16em; right:-.16em; bottom:-.16em;
            border-radius:50%; border:.16em solid transparent; border-top-color: var(--cm-accent); animation: cm-spin 1.1s linear infinite; }
        @keyframes cm-spin { to { transform: rotate(360deg); } }
        .cm-load-txt { margin-top:1em; font-size:.7em; letter-spacing:.22em; color:var(--cm-sub); }
        /* робот в компаньоне */
        .cm-robot-wrap { display:flex; flex-direction:column; align-items:center; margin:-.2em 0 .8em; }
        .cm-robot { width:7.2em; cursor:pointer; }
        .cm-robot svg { width:100%; height:auto; overflow:visible; display:block; }
        .cm-robot-quote { max-width:24em; text-align:center; font-size:.85em; color:var(--cm-sub);
            font-style:italic; min-height:1.3em; margin-top:.5em; transition: opacity .3s; }
        .cmr-float, .cmr-float2 { transform-origin: 60px 70px; }
        .cmr-float { animation: cmr-float 4.5s ease-in-out infinite; }
        .cmr-float2 { animation: cmr-float 5.8s ease-in-out infinite reverse; }
        ...
        @keyframes cmr-blink { 0%,91%,100% { transform: scaleY(1); } 95% { transform: scaleY(.12); } }
        ...
        @keyframes cmr-flame { from { transform: scaleY(.8); } to { transform: scaleY(1.2); } }
        .cmr-portal { stroke: rgba(124,255,107,.55); stroke-width:5; stroke-dasharray:12 16; fill:none;
            animation: cmr-spin 6s linear infinite; transform-origin:60px 64px; }
        @keyframes cmr-spin { to { transform: rotate(360deg); } }
        .cmr-visor { animation: cmr-visor 2.4s infinite; }
        @keyframes cmr-visor { 0%,100% { opacity:1; } 45% { opacity:.55; } 50% { opacity:.9; } 55% { opacity:.5; } }
        .cmr-smoke { animation: cmr-smoke 3s ease-out infinite; }
        @keyframes cmr-smoke { 0% { opacity:0; transform: translateY(4px); } 30% { opacity:.7; } 100% { opacity:0; transform: translateY(-6px); } }
        /* модалки */
        .cm-ov { position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,.78); z-index:999999;
            display:flex; align-items:center; justify-content:center; padding:1.2em; }
        .cm-modal { width:36em; max-width:100%; max-height:86%; overflow-y:auto; padding:1.6em;
            border-radius:var(--cm-radius); background:var(--cm-panel2); box-shadow: inset 0 0 0 1px rgba(255,255,255,.1); }
        ...
        .cm-opt { display:block; width:100%; text-align:left; padding:.85em 1em; margin-bottom:.45em;
            border-radius: calc(var(--cm-radius) * 0.5); background:var(--cm-chip); color:var(--cm-text);
            font-size:.95em; cursor:pointer; transition: background .15s, transform .15s, color .15s; }
        .cm-opt:active { transform: scale(.98); }
        .cm-opt.cm-focus { background:var(--cm-accent); color:#05070D; transform:scale(1.02); }
        .cm-opt small { display:block; font-size:.75em; opacity:.7; margin-top:.1em; }
        .cm-chips { display:flex; flex-wrap:wrap; margin-bottom:.8em; }
        .cm-chip { padding:.6em .95em; margin:0 .45em .45em 0; border-radius:1em; font-size:.9em; cursor:pointer;
            background:var(--cm-chip); color:var(--cm-text); transition: background .15s, transform .15s, color .15s; }
        .cm-chip.cm-focus { background:var(--cm-accent2); color:#05070D; transform:scale(1.06); }
        .cm-input { width:100%; padding:.8em 1em; margin-bottom:.8em; border-radius:calc(var(--cm-radius) * 0.5);
            font-size:1em; color:#fff; outline:none; background:var(--cm-chip); border:1px solid rgba(255,255,255,.15); }
        .cm-toast { position:fixed; left:50%; bottom:1.8em; transform:translateX(-50%) translateY(1em); z-index:1000001;
            opacity:0; padding:.7em 1.2em; border-radius:.7em; background:var(--cm-panel2); color:var(--cm-text);
            font-size:.9em; box-shadow: inset 0 0 0 1px rgba(255,255,255,.12); transition: opacity .25s, transform .25s; }
        ...
        @media (hover:hover) {
            ...
            .cm-act.primary:hover { background: var(--cm-accent); }
            .cm-mini:hover { opacity:1; }
            .cm-theme-chip:hover { background: rgba(255,255,255,.14); }
        }
        /* === тематические эффекты и декор === */
        .cm-t-bb .cm-port { border:1px solid rgba(214,226,74,.18); }
        .cm-t-bb:before { content:""; position:absolute; left:0; right:0; bottom:0; height:100%; pointer-events:none; opacity:.7;
            background-image:
                radial-gradient(6px 6px at 20% 110%, rgba(31,174,150,.3), transparent 60%),
                radial-gradient(4px 4px at 70% 115%, rgba(214,226,74,.25), transparent 60%),
                radial-gradient(5px 5px at 45% 112%, rgba(31,174,150,.2), transparent 60%);
            animation: cm-bubbles 9s linear infinite; }
        @keyframes cm-bubbles { from { background-position: 0 0, 0 0, 0 0; } to { background-position: 0 -130%, 0 -160%, 0 -110%; } }
        ...
        .cm-t-panda .cm-port { background-image: linear-gradient(var(--cm-panel), var(--cm-panel)),
            repeating-linear-gradient(115deg, rgba(255,255,255,.025) 0 2px, transparent 2px 6px);
            border:1px solid rgba(231,182,92,.25); }
        .cm-t-panda .cm-port:after { content:""; position:absolute; right:-.6em; top:-.6em; width:5em; height:5em; pointer-events:none;
            background: radial-gradient(circle at 60% 40%, rgba(216,67,60,.35), transparent 62%),
                        radial-gradient(circle at 30% 70%, rgba(20,16,11,.5), transparent 55%);
            filter: blur(1px); border-radius:50%; }
        ...
        .cm-t-rm .cm-port:after { content:""; position:absolute; right:-1.4em; top:-1.4em; width:5.4em; height:5.4em;
            border-radius:50%; pointer-events:none; filter: blur(1px);
            background: conic-gradient(from 0deg, transparent 0%, rgba(124,255,107,.55) 30%, rgba(58,209,255,.45) 60%, transparent 75%);
            animation: cmr-spin 5s linear infinite; }
        ...
        .cm-t-sw:before { content:""; position:absolute; inset:0; pointer-events:none; opacity:.16;
            background: repeating-linear-gradient(90deg, transparent 0 140px, rgba(255,255,255,.5) 140px 141px);
            animation: cm-hyper 1.4s linear infinite; }
        @keyframes cm-hyper { to { background-position: -282px 0; } }
        .cm-t-sw .cm-port { border:1px solid rgba(255,232,31,.16); }
        .cm-t-sw .cm-name { text-shadow: 0 0 18px rgba(255,232,31,.25); }
        .cm-t-cp:before { content:""; position:absolute; left:0; right:0; bottom:0; height:40%; pointer-events:none; opacity:.5;
            background:
                linear-gradient(transparent, rgba(255,42,109,.14)),
                repeating-linear-gradient(90deg, rgba(5,217,232,.16) 0 1px, transparent 1px 64px),
                repeating-linear-gradient(0deg, rgba(5,217,232,.16) 0 1px, transparent 1px 36px);
            transform: perspective(420px) rotateX(56deg); transform-origin: bottom; }
        .cm-t-cp .cm-port { border:1px solid rgba(255,42,109,.25); }
        .cm-t-cp .cm-name { text-shadow: 2px 0 0 rgba(255,42,109,.5), -2px 0 0 rgba(5,217,232,.5); }
        .cm-t-cp .cm-act.cm-focus { box-shadow: 0 0 0 .14em var(--cm-accent2), 0 0 18px rgba(255,42,109,.5); }
        .cm-t-noir:after { content:""; position:absolute; inset:0; pointer-events:none;
            background: radial-gradient(80% 62% at 50% 42%, transparent 42%, rgba(0,0,0,.8) 100%);
            animation: cm-flicker 3.6s steps(10) infinite; }
        @keyframes cm-flicker { 0%,100% { opacity:.95; } 50% { opacity:.78; } }
        .cm-t-noir .cm-hero-poster img, .cm-t-noir .cm-mini img { filter: grayscale(1) contrast(1.15) brightness(.92); }
        .cm-t-noir .cm-mini.active img { filter: grayscale(.85) contrast(1.1); }
        .cm-t-noir .cm-port { border:1px solid rgba(230,230,230,.14); }
        .cm-t-noir .cm-act.cm-focus { background:#EDEDED; }
        ...
        @media (max-width: 1000px) {
            ...
            .cm-stage { padding: 3em 1em 9em; align-items:flex-start; }
            .cm-port { flex-direction:column; align-items:flex-start; padding:1em; }
            .cm-hero-poster { width:100%; height:auto; aspect-ratio:2/3; max-height:16em; margin:0 0 .8em; }
            .cm-name { font-size:1.6em; }
            .cm-act { flex:1 1 100%; }
            .cm-tray { bottom:1.6em; }
        }
    ...
    // === 12. ИКОНКИ + РОБОТ (8 стилей рисовки под темы) ===
        ...
        svg: (key) => {
            switch (key) {
                ...
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <g class="cmr-float2">
                        <circle cx="60" cy="42" r="24" fill="#E9D33C"/>
                        <rect x="40" y="34" width="40" height="18" rx="9" fill="#20261A"/>
                        <circle cx="52" cy="43" r="6" fill="#D6E24A" class="cmr-eye"/>
                        <circle cx="68" cy="43" r="6" fill="#D6E24A" class="cmr-eye"/>
                        <circle cx="60" cy="60" r="7" fill="#3A4430"/>
                        <rect x="56" y="57" width="8" height="3" rx="1.5" fill="#1FAE96"/>
                        <rect x="36" y="66" width="48" height="44" rx="10" fill="#E9D33C"/>
                        <rect x="36" y="66" width="48" height="44" rx="10" fill="none" stroke="#B7B32A" stroke-width="2"/>
                        <rect x="52" y="74" width="16" height="20" rx="3" fill="#F4F0D0"/>
                        <path d="M56 80 h8 M56 86 h8" stroke="#8A8A5A" stroke-width="1.6"/>
                        <rect x="28" y="70" width="8" height="26" rx="4" fill="#C9C22E"/>
                        <rect x="84" y="70" width="8" height="26" rx="4" fill="#C9C22E"/>
                        <path d="M96 82 h9 l4 13 a4 4 0 0 1 -4 4 h-9 a4 4 0 0 1 -4 -4 z" fill="none" stroke="#1FAE96" stroke-width="2"/>
                        <circle cx="100" cy="93" r="2.4" fill="#1FAE96" class="cmr-pulse"/>
                        <line x1="60" y1="18" x2="60" y2="10" stroke="#1FAE96" stroke-width="2"/>
                        <circle cx="60" cy="8" r="3" fill="#1FAE96" class="cmr-pulse"/>
                    </g>
                </svg>`;
                case 'matrix': return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <g class="cmr-float" fill="none" stroke="#00FF41" stroke-width="2">
                        <rect x="40" y="18" width="40" height="30" rx="4"/>
                        <rect x="48" y="28" width="6" height="6" fill="#00FF41" stroke="none" class="cmr-eye"/>
                        <rect x="66" y="28" width="6" height="6" fill="#00FF41" stroke="none" class="cmr-eye"/>
                        <path d="M52 40 h16"/>
                        <rect x="36" y="54" width="48" height="44" rx="6"/>
                        <path d="M44 64 h20 M44 72 h28 M44 80 h16 M44 88 h24" stroke-width="1.4" opacity=".8"/>
                        <rect x="26" y="58" width="8" height="30" rx="3"/>
                        <rect x="86" y="58" width="8" height="30" rx="3"/>
                        <path d="M46 98 v14 M74 98 v14"/>
                        <path d="M60 18 v-8" class="cmr-pulse"/>
                    </g>
                </svg>`;
                case 'panda': return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <g class="cmr-float2">
                        <line x1="92" y1="58" x2="99" y2="106" stroke="#3E7A3E" stroke-width="4" stroke-linecap="round"/>
                        <path d="M30 34 q30 -20 60 0 q-30 10 -60 0 z" fill="#8A5A2B"/>
                        <path d="M30 34 q30 -20 60 0" fill="none" stroke="#5A3A1B" stroke-width="3" stroke-linecap="round"/>
                        <circle cx="60" cy="52" r="18" fill="#14100B"/>
                        <circle cx="53" cy="50" r="3" fill="#F4E9D2" class="cmr-eye"/>
                        <circle cx="67" cy="50" r="3" fill="#F4E9D2" class="cmr-eye"/>
                        <path d="M40 70 q20 -8 40 0 q6 24 -2 38 q-18 8 -36 0 q-8 -14 -2 -38 z" fill="#14100B"/>
                        <path d="M42 80 q18 10 36 0" stroke="#D8433C" stroke-width="5" fill="none" stroke-linecap="round"/>
                        <path d="M38 74 q-10 10 -6 22" stroke="#14100B" stroke-width="7" fill="none" stroke-linecap="round"/>
                        <path d="M82 74 q10 10 6 22" stroke="#14100B" stroke-width="7" fill="none" stroke-linecap="round"/>
                    </g>
                </svg>`;
                case 'rickmorty': return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <ellipse cx="60" cy="64" rx="46" ry="40" class="cmr-portal"/>
                    <g class="cmr-float">
                        <circle cx="60" cy="40" r="20" fill="#B8FF9E" stroke="#2E7D4F" stroke-width="3"/>
                        <circle cx="52" cy="38" r="6" fill="#fff" stroke="#2E7D4F" stroke-width="2"/>
                        <circle cx="68" cy="38" r="6" fill="#fff" stroke="#2E7D4F" stroke-width="2"/>
                        <circle cx="52" cy="38" r="2" fill="#111" class="cmr-eye"/>
                        <circle cx="68" cy="38" r="2" fill="#111" class="cmr-eye"/>
                        <path d="M52 50 q8 6 16 0" stroke="#2E7D4F" stroke-width="2.5" fill="none"/>
                        <rect x="42" y="60" width="36" height="40" rx="10" fill="#7CFF6B" stroke="#2E7D4F" stroke-width="3"/>
                        <rect x="50" y="70" width="20" height="12" rx="4" fill="#3AD1FF"/>
                        <path d="M42 66 q-12 8 -8 22" stroke="#2E7D4F" stroke-width="3" fill="none"/>
                        <path d="M78 66 q12 8 8 22" stroke="#2E7D4F" stroke-width="3" fill="none"/>
                        <path d="M60 20 v-8 l6 -4" stroke="#2E7D4F" stroke-width="2.5" fill="none"/>
                        <circle cx="66" cy="8" r="3" fill="#3AD1FF" class="cmr-pulse"/>
                    </g>
                </svg>`;
                case 'starwars': return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <g class="cmr-float2">
                        <ellipse cx="60" cy="114" rx="20" ry="5" fill="rgba(75,213,255,.3)" class="cmr-flame"/>
                        <path d="M38 44 a22 22 0 0 1 44 0 z" fill="#E8E8E8"/>
                        <rect x="38" y="44" width="44" height="6" fill="#9AA0A6"/>
                        <rect x="56" y="24" width="8" height="6" rx="2" fill="#4BD5FF"/>
                        <circle cx="66" cy="36" r="4" fill="#111"/>
                        <circle cx="66" cy="36" r="1.6" fill="#FFE81F" class="cmr-eye"/>
                        <path d="M40 52 h40 l6 46 h-52 z" fill="#D6D6D6"/>
                        <rect x="50" y="60" width="20" height="14" rx="3" fill="#20344A"/>
                        <circle cx="60" cy="67" r="3" fill="#4BD5FF" class="cmr-pulse"/>
                        <rect x="44" y="80" width="32" height="6" rx="3" fill="#FFE81F"/>
                        <rect x="30" y="56" width="8" height="28" rx="4" fill="#B9B9B9"/>
                        <rect x="82" y="56" width="8" height="28" rx="4" fill="#B9B9B9"/>
                    </g>
                </svg>`;
                case 'cyberpunk': return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <g class="cmr-float">
                        <path d="M40 20 h40 l6 14 -6 14 h-40 l-6 -14 z" fill="#101322" stroke="#05D9E8" stroke-width="2"/>
                        <rect x="44" y="30" width="32" height="8" rx="4" fill="#FF2A6D" class="cmr-visor"/>
                        <path d="M36 54 h48 l8 10 v34 l-8 8 h-48 l-8 -8 v-34 z" fill="#101322" stroke="#FF2A6D" stroke-width="2"/>
                        <path d="M48 66 v28 M60 62 v36 M72 66 v28" stroke="#05D9E8" stroke-width="1.6" opacity=".9"/>
                        <circle cx="60" cy="74" r="3" fill="#FCEE0A" class="cmr-eye"/>
                        <path d="M28 58 l-8 6 v26 l8 6" fill="none" stroke="#05D9E8" stroke-width="3"/>
                        <path d="M92 58 l8 6 v26 l-8 6" fill="none" stroke="#05D9E8" stroke-width="3"/>
                    </g>
                </svg>`;
                case 'noir': return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <g class="cmr-float2">
                        <path d="M84 66 q10 -2 12 -10" stroke="#8A8A8A" stroke-width="2" fill="none" class="cmr-smoke"/>
                        <circle cx="98" cy="52" r="2" fill="#8A8A8A" class="cmr-smoke"/>
                        <ellipse cx="60" cy="26" rx="26" ry="6" fill="#1A1A1A"/>
                        <path d="M46 24 q14 -14 28 0 z" fill="#1A1A1A"/>
                        ...
                        <path d="M38 54 h44 l8 50 h-60 z" fill="#1A1A1A"/>
                        <path d="M60 54 l-6 50 M60 54 l6 50" stroke="#0B0B0B" stroke-width="3"/>
                        <rect x="56" y="60" width="8" height="8" fill="#B48A3C"/>
                    </g>
                </svg>`;
                default: return `
                <svg viewBox="0 0 120 130" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                        <linearGradient id="cmgA1" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#E8ECF5"/><stop offset="1" stop-color="#9AA8C0"/></linearGradient>
                        <linearGradient id="cmgA2" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#F8FAFF"/><stop offset="1" stop-color="#B9C6DC"/></linearGradient>
                    </defs>
                    <g class="cmr-float">
                        <path d="M52 108 q8 12 16 0 l-3 12 q-5 5 -10 0 z" fill="#FF7A2F" class="cmr-flame"/>
                        <rect x="38" y="62" width="44" height="46" rx="14" fill="url(#cmgA1)"/>
                        <rect x="46" y="74" width="28" height="16" rx="6" fill="#0B0F18"/>
                        <circle cx="60" cy="82" r="4" fill="#7FD8FF" class="cmr-eye"/>
                        <rect x="30" y="66" width="8" height="26" rx="4" fill="#8FA6C0"/>
                        <rect x="82" y="66" width="8" height="26" rx="4" fill="#8FA6C0"/>
                        <circle cx="60" cy="40" r="22" fill="url(#cmgA2)"/>
                        <rect x="44" y="34" width="32" height="14" rx="7" fill="#0B1524"/>
                        <circle cx="53" cy="41" r="3.2" fill="#FFD75E" class="cmr-eye"/>
                        <circle cx="67" cy="41" r="3.2" fill="#FFD75E" class="cmr-eye"/>
                        <path d="M46 30 q7 -5 14 -3" stroke="rgba(255,255,255,.55)" stroke-width="2" fill="none" stroke-linecap="round"/>
                        <line x1="60" y1="18" x2="60" y2="10" stroke="#FF7A2F" stroke-width="2"/>
                        <circle cx="60" cy="8" r="3" fill="#FF7A2F" class="cmr-pulse"/>
                    </g>
                </svg>`;
            }
        }
    ...
    // === 13. НАВИГАЦИЯ (2D, типы рядов, оптимизация под пульт) ===
    const Nav = {
        rows: [], r: 0, c: 0,
        ...
        paint: (silent) => {
            ...
            document.querySelectorAll('.cm-ov .cm-focus').forEach(n => removeClass(n, 'cm-focus'));
    ...
    const Modal = {
        ...
        open: (opts) => {
            ...
            if (opts.mascot) {
                ...
                const quote = el('div', 'cm-robot-quote', esc(opts.quote || Themes.quote()));
                rb.addEventListener('click', () => { quote.textContent = Themes.quote(); });
                wrap.appendChild(rb);
    ...
    const View = {
        ...
        list: [], idx: 0, taste: null,
        source: 'taste', sourceLabel: '',
        busy: false,
        _lastBuiltAt: 0,
        ...
        loading: (text) => {
            ...
            box.appendChild(el('div', 'cm-load-txt cm-mono', text || Themes.loadLine()));
        ...
        boot: (force) => {
            ...
            Taste.build((taste) => {
                ...
                Capsule.build(taste, { force }, (list) => {
                    ...
                    Capsule.remember(list.map(item => item.id));
                    View.list = list;
                    View.idx = 0;
                    View.render();
                    View.greet();
        ...
        buildBar: (stage) => {
            ...
            const barIn = el('div', 'cm-bar-in');
            barIn.appendChild(el('div', 'cm-logo cm-mono', 'КАПСУЛА'));
            const themeChip = el('div', 'cm-theme-chip', '🎨 ' + esc(THEMES[Themes.current()].name));
            themeChip._cmAction = () => Companion.themes();
            barIn.appendChild(themeChip);
            barIn.appendChild(el('div', 'cm-hint cm-mono',
                touchMode ? 'свайп — листать · тап по постеру — смотреть · долгий тап — подробнее'
                    : '← → ↑ ↓ навигация · OK выбрать · BACK назад'));
        ...
        renderEmpty: () => {
            View.stage.innerHTML = '';
            Nav.reset();
            const themeChip = View.buildBar(View.stage);
            ...
            retry._cmAction = () => { Net.drop(); View.refreshAllCapsules(); };
        ...
        go: (i) => {
            ...
            View.idx = i;
            View.render();
            const row = Nav.rows[r];
            if (row) {
                if (row.type === 'tray') Nav.setFocus(r, View.idx, true);
            ...
            Net.get(`/${type}/${m.id}`, {}, (d) => {
                ...
                Modal.open({
                    ...
                    items: [
                        ...
                        { label: '❓ Почему это в капсуле?', onSelect: () => Companion.why(m) },
                        { label: 'Закрыть' }
                    ]
                });
            }, () => { Modal.close(); notify('Не удалось загрузить'); });
        }
    };
    ...
    const Companion = {
        open: () => {
            ...
            Modal.open({
                ...
                text: 'Выбери настроение или скажи словами, что хочешь посмотреть. Тапни по роботу — сменю цитату.',
                chips,
                items: [
                    { section: '🔄 ДЕЙСТВИЯ' },
                    ...
                    { label: '🏠 Вернуть мою капсулу', onSelect: () => { View.loading(); View.boot(false); } },
        ...
        find: (query, label, kind) => {
            ...
            Search.run(query, View.taste, (list) => {
                ...
                View.idx = 0;
                View.source = 'search';
                View.sourceLabel = (label || query).toUpperCase().slice(0, 18);
                if (!View.list.length) { notify('Ничего не нашлось'); View.render(); return; }
                View.render();
            });
        },
        ...
        similar: () => {
            ...
            Net.get(`/${seed.type}/${seed.id}/similar`, { page: 1 }, (d) => {
                ...
                Capsule.remember(list.map(it => it.id));
                View.list = list.slice(0, CAPSULE_SIZE);
                View.idx = 0;
                View.source = 'search';
    ...
    const Settings = {
        ...
        tryRegister: () => {
            ...
            } catch (e) { Settings.registeredInLampa = false; }
        },
        openLampaMenu: () => {
            try {
    ...
    // === 19. УПРАВЛЕНИЕ (пульт + сенсор, троттлинг) ===
    ...
    const route = (kind) => {
        ...
        if (kind === 'left' || kind === 'right') {
            const row = Nav.rows[Nav.r];
            if (row?.type === 'tray') { Nav.moveH(kind); return; }
            if (!Nav.moveH(kind)) {
                const now = Date.now();
    ...
    (() => {
        try {
            ...
            console.log('[Капсула] v13.0 загружена');
        } catch (e) {
