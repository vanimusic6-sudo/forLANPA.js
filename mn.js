...
(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dice-core{display:block;width:100%;height:100%;filter:drop-shadow(0 4px 6px rgba(0,0,0,.4));transition:transform .15s cubic-bezier(.34,1.56,.64,1)}' +
            '.sw-dice-core svg{width:100%;height:100%;display:block}' +
            '.sw-dice-core.sw-face-out{transform:translateY(-10px)}' +
            ...
            '.sw-col.sw-empty-col{display:flex;align-items:center;justify-content:center;min-height:64px;opacity:.9}' +
    ...
    // Bounces the die's core up, swaps the face at the peak of the hop, then lets it
    // settle back down with a slight bounce (via the easing curve) — no fade/spin/shake.
    function setFaceSmooth(dice, f, cb) {
        ...
        setTimeout(function(){
            ...
            setTimeout(function(){ if (cb) cb(); }, 150);
        }, 150);
    }
    ...
    function buildReadyInner(a) {
        ...
        var prosConsHtml = '<div class="sw-columns">' +
            '<div class="' + prosColClass + '">' + (a.prosEmpty ? '' : '<div class="sw-col-title">Плюсы</div>') + '<ul class="sw-list">' + a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
            '<div class="' + consColClass + '">' + (a.consEmpty ? '' : '<div class="sw-col-title">Минусы</div>') + '<ul class="sw-list">' + a.cons.map(function(c){ return '<li>' + esc(c) + '</li>'; }).join('') + '</ul></div>' +
            '</div>';
    ...
    function bindDice(html) {
        ...
        function rollDice() {
            ...
            // Roll out a handful of faces, each one hopping up then settling back
            // down with a bounce (via CSS easing), no fade/spin/shake.
            var stepsLeft = 6;
            function step() {
                stepsLeft--;
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v52.9 initialized: dice hop animation, no-scale focus ring, screenshot-matched pros/cons layout.');
    }
