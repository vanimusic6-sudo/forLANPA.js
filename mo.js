...
(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            // Smooth "knock" bounce: hop up, land, squash on impact, tiny rebound.
            // The face is swapped by JS exactly at the impact (squash) moment.
            ...
            // === Audience box ("кому стоит посмотреть" / "кому не стоит смотреть") ===
    ...
    // Finds the real scrollable ancestor of the modal body so that up/down can
    // scroll the content all the way to the very bottom on any TV browser.
    ...
    // Pure vertical scroll of the modal content (no focus change).
    function scrollModalBy(dir) {
        var el = findScrollParent();
        if (!el) {
            try {
                var sc = modalScroll();
                if (sc && sc.object && sc.object.length) el = sc.object[0];
                else if (sc && sc.obj && sc.obj.length) el = sc.obj[0];
            } catch(e) {}
        }
        if (!el) return;
    ...
    // Left/right: cycle through the functional buttons (dice <-> tabs).
    ...
    function registerController() {
        ...
        Lampa.Controller.add(CONTROLLER_ID, {
            ...
            // Up/down = PURE content scroll (reaches the very bottom of the modal).
            up: function () { scrollModalBy(-1); },
            down: function () { scrollModalBy(1); },
            // Left/right = switch between functional buttons.
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // Three-way ANALYSIS verdict: СТОИТ / СПОРНО / НЕ СТОИТ.
            // (The dice itself never rolls "СПОРНО" — see SW_DICE_VERDICTS.)
            var vClass, vWord;
    ...
    // Smooth knock animation: the die hops, lands and squashes on impact.
    // The face value is swapped EXACTLY at the knock (impact/squash) moment (~300ms
    // into the 560ms animation), so the change reads as caused by the knock.
    function knockDice(dice, f, cb) {
        ...
        void core[0].offsetWidth; // restart animation cleanly
        core.addClass('sw-knock');
        setTimeout(function(){ core.html(diceSVG(f)); }, 300); // swap on the knock
    ...
    function bindDice(html) {
        ...
        function rollDice() {
            ...
            // Dice only ever lands on СТОИТ or НЕ СТОИТ (never СПОРНО).
            ...
            // A series of smooth knocks; each knock swaps the face on impact.
            var stepsLeft = 5;
            function step() {
                stepsLeft--;
                var isLast = stepsLeft <= 0;
                knockDice(dice, rndFace(), function(){
