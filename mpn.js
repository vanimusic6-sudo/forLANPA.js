 ...
 * Version: 54.0 (Bugfix + analysis engine rework + safety/perf pass)
 * Compliant with Lampa API, Memory-safe, Pure jQuery/DOM, Zero external dependencies.
 *
 * Changelog vs 53.3:
 *  - Fixed: dice-roll flag (_sw_rolling) could get stuck "true" forever if the
 *    modal was closed mid-animation, permanently disabling the dice button
 *    on the next open.
 *  - Fixed: double-fire of tab switching / dice roll when a remote's D-Pad
 *    synthesizes both "hover:enter" and "click" for one physical press.
 *  - Fixed: DOM signal cache key collision — two different movies with no
 *    id/tmdb_id used to share cache key 'x' and could leak one movie's
 *    parsed page text (mood/age/review data) onto another.
 *  - Fixed: double restorePrevController() calls when both the controller's
 *    back() and Modal's onBack fire for the same close action.
 *  - Fixed: meta cache was an unbounded object cleared in one big sweep
 *    (spiky memory + cache stampede); replaced with a real LRU with a
 *    stable cap.
 *  - Fixed: in-flight requests for the same movie id were not de-duplicated
 *    — opening the modal twice quickly for the same title fired two full
 *    sets of TMDB requests.
 *  - Fixed: age<=12 safety clamp zeroed out overview-based sex/nudity
 *    signals but left keyword-based ones active, creating an inconsistent
 *    (and less safe) result for family content with mislabeled keywords.
 *  - Reworked: atmosphere metrics (pace/fear/drama/immersion) now trust
 *    real on-page signals when present instead of being overridden by
 *    genre heuristics via Math.max — heuristics are a fallback only, not
 *    a floor.
 *  - Reworked: hardAdult no longer treats a missing/false movie.adult flag
 *    as meaningful signal either way (it's frequently absent/false even
 *    for adult content upstream) — decision now rests only on the content
 *    signals we actually compute.
 *  - Hardened: all interpolated strings inserted into modal HTML go through
 *    esc(); added a defensive re-check before that.
 */
(function () {
    'use strict';
    ...
    // --- Bounded LRU cache for TMDB meta lookups -------------------------
    // Replaces the old "object that gets nuked wholesale past N keys" cache.
    // Real LRU semantics: touching an entry moves it to the front; eviction
    // drops the least-recently-used entry, not everything at once.
    var META_CACHE_LIMIT = 150;
    var _metaCache = new Map();      // id -> resolved meta object
    var _metaPending = new Map();    // id -> in-flight Promise (de-dupe)

    function metaCacheGet(id) {
        if (!_metaCache.has(id)) return undefined;
        var v = _metaCache.get(id);
        _metaCache.delete(id);
        _metaCache.set(id, v); // move to MRU position
        return v;
    }
    ...
    // --- DOM signal cache, keyed defensively ------------------------------
    // Guards against two different "id-less" movies sharing a cache slot.
    var _domCache = null;
    var _domCacheSeq = 0;
    ...
    window._sw_closing = false; // guards against double-close races
    ...
    function injectCSS() {
        if (document.getElementById('sw-plugin-styles-v56')) return;
        ['sw-plugin-styles-v51', 'sw-plugin-styles-v52', 'sw-plugin-styles-v53', 'sw-plugin-styles-v54', 'sw-plugin-styles-v55'].forEach(function(id){
            var old = document.getElementById(id);
            if (old && old.parentNode) old.parentNode.removeChild(old);
        });
        var s = document.createElement('style');
        s.id = 'sw-plugin-styles-v56';
        s.innerHTML =
    ...
    // Build a stable-ish cache key for a movie even when it has no id.
    // Falls back to a short fingerprint of title+year+overview length so two
    // different id-less movies don't collide on a shared 'x' bucket.
    ...
    function loadMeta(movie) {
        ...
        // De-dupe concurrent requests for the same id (e.g. modal opened
        // twice quickly, or re-entered before the first analysis finished).
    ...
    function findFeatures(meta, usedSparkles) {
        var feats = [];
        var texts = [meta.enOv || ''].concat(meta.kw || []);
        // Avoid re-listing the same underlying signal twice: if a feature's
        // text already ended up in "sparkles", don't repeat it as a
        // separate "feature" pro — it's the same fact shown twice.
        var usedSet = {};
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // Safety clamp for young-audience labeled content: zero out BOTH
            // keyword- and overview-derived sex/nudity signals consistently.
            // (Previously only the overview-based signal was cleared, which
            // left an inconsistent — and less safe — half-cleared state
            // when TMDB keywords were mislabeled for a kids' title.)
            if (age !== null && age <= 12) {
            ...
            // `movie.adult` is frequently absent/false upstream even for
            // clearly adult content, so it is treated as a weak positive
            // signal only — never as the deciding factor, and its absence
            ...
            // === Темп: доверяем реальным DOM-данным (dom.ok), эвристика жанра — только фолбэк ===
            var paceHeuristic = 6.0;
            ...
            if (!dom.ok && hasKw(ctx, /chase|погон|fast-paced|динамичн|экшн|action|gunfight|перестрелк|explosion|взрыв|heist|ограблен|car chase|fight|драка|поединок|speed|быстр|гонк|race/i)) {
                paceHeuristic = Math.max(paceHeuristic, 9.0);
            }
            // If we have a real on-page pace value, trust it. Genre heuristics
            // only fill in when the page didn't give us a number at all.
            var mPace = (mm.pace > 0) ? mm.pace : paceHeuristic;

            // === Напряжение: аналогично — реальные данные приоритетнее эвристики ===
            var fearBase = 3.0;
    ...
    // Central place that guarantees _sw_rolling never gets stuck "true".
    // Called whenever the modal is torn down, regardless of whether a dice
    ...
    // Debounced action wrapper: some Lampa remotes/browsers synthesize both
    // "hover:enter" and "click" for a single physical press. This collapses
    // both into a single logical action per short window so tabs/dice don't
    // double-fire.
    ...
    function bindTabs(html) {
        ...
        function activateTabRaw(target, el) {
            if (tabBtns.filter('[data-tab="' + target + '"]').hasClass('active')) {
                // Already active — just move focus ring, don't replay the
                // reveal animation.
    ...
    function bindDice(html) {
        ...
        function rollDiceRaw() {
            ...
            var stepsLeft = 5;
            function step() {
                // Bail out cleanly if the modal was torn down mid-roll —
                // and make sure the rolling flag is always cleared, so a
                // closed modal never leaves the next session's dice
    ...
    // Single point of truth for "close the modal". Both the controller's
    // back() and Lampa's Modal onBack funnel through here, guarded by
    // _sw_closing so a close triggered from one path can't also run the
    // other path's teardown + restorePrevController a second time.
    function doClose() {
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v54.0 initialized: bugfixes, LRU meta cache, request de-dup, safer clamps, dom-trusting atmosphere metrics.');
    }
