(function () {
    ...
    var FEATURES = [
        ...
        { re: /romance|романтик/i, text: '❤️ Романтическая история' }
    ];
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (votes > 0 && votes < 1500 && adj >= 7.6) add('card','pro','💎 Недооценённое кино', 12);
            ...
            else if (familyOK) add('tmdb','pro','👨‍👩‍👧 отлично подойдет для семейного вечера' + (age !== null ? ' (' + age + '+)' : ''), 16, 'family');
            ...
            if (dPsych.k || dPsych.o) add('tmdb','con','🧠 психологически гнетущая атмосфера', 8, 'psych');
            if (dIll.k || dIll.o) add('tmdb','con','🏥 Тема неизлечимой болезни', 6, 'ill');
            if (gWar) add('tmdb','con','🎖 Военная драма и сражения', 6, 'war');
            if (cChild >= 50) add('tmdb','con','🚸 жестокое обращение с детьми', 18, 'childabuse');
            ...
            else if (local.inFavorite) add('lampa','pro','🔖 вы планировали посмотреть', 8, 'lampa');
            ...
            if (consEmpty) cons.push((blG.length || blA.length || blD.length) ?  '✅ Явных минусов не обнаружено');
            ...
            else if (mFear <= 3.5 && !gComedyN) add('moods','con','😌 Спокойное развитие сюжета', 8);

            if (mDrama >= 8.0) add('moods','pro','😢 Очень сильная история', 14);
