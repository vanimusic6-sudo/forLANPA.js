 ...
 * Version: 53.0 (Mockup-matched UI: mono headings, lowercase tabs, audience arrows, full D-Pad)
...
(function () {
    ...
    function injectCSS() {
        if (document.getElementById('sw-plugin-styles-v53')) return;
        ['sw-plugin-styles-v51', 'sw-plugin-styles-v52'].forEach(function(id){
            var old = document.getElementById(id);
            if (old && old.parentNode) old.parentNode.removeChild(old);
        });
        var s = document.createElement('style');
        s.id = 'sw-plugin-styles-v53';
        s.innerHTML =
            ...
            // === Root wrap ===
            '.sw-wrap{--sw-green:#4CAF50;--sw-red:#EF5350;--sw-ink:#e9ecef;--sw-mut:#adb5bd;--sw-mono:"PT Mono","Ubuntu Mono","Droid Sans Mono","Courier New",monospace;padding:12px 6px 40px;color:var(--sw-ink);font-family:' + FONT + ';font-size:20px;box-sizing:border-box;-webkit-font-smoothing:antialiased}' +
            ...
            '.sw-verdict-word{position:relative;font-family:var(--sw-mono);font-size:2.2em;font-weight:700;letter-spacing:.04em;line-height:1;margin:0 0 16px;text-transform:uppercase;opacity:0;transform:translateY(10px);transition:opacity .55s cubic-bezier(.16,.8,.24,1),transform .55s cubic-bezier(.16,.8,.24,1)}' +
            ...
            // === Tabs (lowercase, white border on active) ===
            ...
            // === Pros/Cons columns (thick side bars, no titles) ===
            ...
            '.sw-list li{position:relative;margin-bottom:14px;padding-left:0;opacity:0;transform:translateY(6px);transition:opacity .45s cubic-bezier(.16,.8,.24,1),transform .45s cubic-bezier(.16,.8,.24,1)}' +
            ...
            // === Audience box ("кому стоит посмотреть") ===
            ...
            '.sw-audience-title{font-family:var(--sw-mono);font-size:.95em;font-weight:700;text-transform:lowercase;color:#fff;letter-spacing:.02em;margin-bottom:12px}' +
            '.sw-aud-line{display:flex;align-items:flex-start;gap:10px;font-family:var(--sw-mono);font-weight:700;font-size:.9em;line-height:1.45;margin-top:10px;text-transform:lowercase}' +
            '.sw-aud-line:first-of-type{margin-top:0}' +
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // --- Audience (lowercase, mockup wording) ---
            var targetAudience = '';
            var notSuitableFor = '';
    ...
    function buildReadyInner(a) {
        ...
        var audienceHtml = '<div class="sw-audience-box">' +
            ...
            (atmos.notSuitableFor ? '<div class="sw-aud-line no">' + ARROW_ICON + '<span>' + esc(atmos.notSuitableFor) + '</span></div>' : '') +
            '</div>';
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v53.0 initialized: mockup-matched UI (mono headings, lowercase tabs, audience arrows).');
    }
