(function () {
    ...
    var INTERESTING_TAGS = [
        ...
        { re: /based on comic|comic book|графическ роман/i, text: '✨ По мотивам комиксов' },
        { re: /based on video game|по мотивам игры|video game/i, text: '🎮 По мотивам видеоигры' },
        { re: /oscar winner|academy award|лауреат оскар/i, text: '🏆 Лауреат премии «Оскар»' },
        { re: /cannes|venice|berlin|film festival winner/i, text: '🎬 Призёр кинофестивалей' },
        { re: /cult film|культов/i, text: '🎭 Культовое кино' },
        { re: /remake|римейк/i, text: '🔁 Ремейк' },
        { re: /sequel|сиквел/i, text: '➕ Сиквел (продолжение)' },
        { re: /prequel|приквел/i, text: '⏪ Приквел (предыстория)' },
        { re: /time travel|путешестви[яе] во времени/i, text: '⏳ Путешествия во времени' },
        { re: /heist|ограблен/i, text: '💼 Сюжет об ограблении' },
        ...
        { re: /space|космос|moon|lunar|mars|марс/i, text: '🚀 Космическая тема' },
        { re: /biographical|biopic|биограф/i, text: '📖 Биографическая драма (байопик)' }
    ];

    var FEATURES = [
        { re: /plot twist|twist ending|неожиданн поворот|сюжетн поворот/i, text: '🌀 Неожиданные повороты сюжета' },
        { re: /superhero|супергеро/i, text: '🦸 Супергероика' },
        { re: /strong female lead|сильн героин/i, text: '💪 Сильная главная героиня' },
        { re: /musical|мюзикл/i, text: '🎶 Мюзикл' },
        { re: /magic|маги|волшеб/i, text: '🪄 Магия и волшебство' },
        { re: /dragon|дракон/i, text: '🐉 Драконы и мифика' },
        { re: /detective|детектив/i, text: '🕵️ Детективная интрига' },
        { re: /spy|шпион/i, text: '🕶 Шпионский триллер' },
        { re: /zombie|зомби/i, text: '🧟 Тема зомби' },
        { re: /vampire|вампир/i, text: '🧛 Вампирская тематика' },
        { re: /robot|android|робот/i, text: '🤖 Роботы и искусственный интеллект' },
        { re: /time loop|временн[ао]я петл/i, text: '🔁 Временная петля' },
        { re: /animal|животн/i, text: '🐾 О животных' },
        { re: /кошк|кошач/i, text: '🐱 С котиками' },
        { re: /собак|пёс|пес|dog/i, text: '🐶 С собаками' },
        { re: /friendship|дружб/i, text: '🤝 История крепкой дружбы' },
        { re: /romance|романтик/i, text: '❤️ Романтическая линия' }
    ];
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (meta.popularity > 100) add('card','pro','🔥 Громкий хит', 15, 'popular');
            else if (meta.popularity > 30) add('card','pro','📈 Популярный проект', 8, 'popular');

            if (votes >= 3000 && adj >= 8.0) add('card','pro','⭐ Отличный рейтинг: ' + rating.toFixed(1) + ' (' + fmtN(votes) + ' оценок)', 30, 'ratehi');
            ...
            if (votes > 0 && votes < 1500 && adj >= 7.6) add('card','pro','💎 Недооценённая жемчужина', 12);
            if (votes >= 2000 && adj >= 7.8 && yr > 0 && yr <= now - 5) add('card','pro','🏆 Признанная временем классика', 12);
            if (yr === now) add('card','pro','🆕 Свежий релиз ' + yr + ' года', 6, 'fresh');
            if (votes > 0 && votes < 100) add('card','con','❓ Мало оценок (всего ' + votes + ')', 14, 'rateunc');
            if (votes === 0) add('card','con','⚠️ Пока нет оценок', 16, 'rateunc');
            if (votes >= 100 && adj < cfg.min_rating) add('card','con','📉 Рейтинг ниже ожидаемого: ' + rating.toFixed(1), 22, 'ratelow');
            else if (votes >= 50 && rating > 0 && rating < 5) add('card','con','👎 Низкий средний балл: ' + rating.toFixed(1), 26, 'ratelow');
            if (runtime > 0 && runtime <= 95) add('card','pro','⏱ Компактный хронометраж: ' + runtime + ' мин', 7, 'runtime');
            else if (runtime > 150) add('card','con','⌛ Длительный хронометраж: ' + runtime + ' мин', 12, 'runtime');
            if (/CAM|TS|HDCAM|HDRIP|TELECINE|SCR|WORKPRINT|TELESYNC/i.test(q)) add('card','con','📺 Экранка / низкое качество видео', 26, 'quality');
            else if (/4K|UHD|2160p/i.test(q)) add('card','pro','🎥 Доступно в 4K UHD', 10, 'quality');
            else if (q) add('card','pro','🎥 Отличное качество (' + q + ')', 7, 'quality');

            if (mG.length) add('user','con','⛔ Нежелательные жанры: ' + mG.join(', '), 40);
            if (mA.length) add('user','con','⛔ Нежелательные актёры: ' + uniq(mA).slice(0,2).join(', '), 35);
            if (mD.length) add('user','con','⛔ Нежелательные режиссёры: ' + uniq(mD).slice(0,2).join(', '), 35);

            if (familyOK && isAnim) add('tmdb','pro','🧸 Подходит для детей' + (age !== null ? ' (' + age + '+)' : ''), 16, 'family');
            else if (familyOK) add('tmdb','pro','👨‍👩‍👧 Отлично для семейного вечера' + (age !== null ? ' (' + age + '+)' : ''), 16, 'family');
            if (age !== null && age >= 18) add('tmdb','con','🔞 Строго 18+ (взрослый контент)', 14, 'age');
            else if (age !== null && age >= 16) add('tmdb','con','⚠️ Возрастной рейтинг 16+', 12, 'age');
            ...
            if (cDrugs >= 30) add('tmdb','con','💉 Тема запрещённых веществ', 14, 'drugs');
            if (cSex >= 30) add('tmdb','con','🔞 Откровенные сцены', 12, 'sex');
            if (cProf >= 40) add('tmdb','con','🤬 Ненормативная лексика', 10, 'lang');
            if (cFear >= 40) add('moods','con','😱 Жуткая и пугающая атмосфера', 14, 'scare');
            if (cSuic >= 40) add('tmdb','con','⚠️ Депрессивный тон / тяжёлые темы', 16, 'suic');
            if (cAlc >= 40) add('tmdb','con','🍺 Сцены употребления алкоголя', 6, 'alc');
            if (cSmoke >= 40) add('tmdb','con','🚬 Сцены курения табака', 5, 'smoke');
            if (cGamb >= 40) add('tmdb','con','🎰 Азартные игры и ставки', 10, 'gamb');
            if (cCrime >= 50) add('tmdb','con','⚖️ Сюжет в криминальном мире', 8, 'crime');
            if (dPsych.k || dPsych.o) add('tmdb','con','🧠 Психологически тяжёлое повествование', 8, 'psych');
            if (dIll.k || dIll.o) add('tmdb','con','🏥 Тема неизлечимой болезни', 6, 'ill');
            if (gWar) add('tmdb','con','🎖 Военная драма и сражения', 6, 'war');
            if (cChild >= 50) add('tmdb','con','🚸 Тема жестокости к детям', 18, 'childabuse');
            if (dDarkCom) add('tmdb','pro','🖤 Остроумный чёрный юмор и сатира', 8, 'darkcom');
            if (dEpic.k || dEpic.o) add('tmdb','pro','🏛 Масштабная эпическая история', 12, 'epic');
            if (dRevenge.k || dRevenge.o) add('tmdb','pro','⚔️ Захватывающий сюжет о мести', 10, 'revenge');
            if (meta.hasTrailer) add('tmdb','pro','▶ Есть официальный трейлер', 5);
            if (hasGenre(genres, /documentary|документ/i)) add('tmdb','pro','🦉 Познавательное документальное кино', 8);

            if (rt.total >= 2 && rt.tone === 'pos') add('reviews','pro','💬 ' + who + ' тепло хвалят фильм', 20, 'rev');
            else if (rt.total >= 2 && rt.tone === 'neg') add('reviews','con','💬 ' + who + ' остались разочарованы', 22, 'rev');
            else if (rt.total >= 2 && rt.tone === 'mix') add('reviews','pro','💬 Мнения зрителей разделились', 10, 'rev');

            if (local.inFavorite && local.favList === 'viewed') add('lampa','con','👁 Вы уже смотрели этот фильм', 8, 'lampa');
            else if (local.inFavorite) add('lampa','pro','🔖 Фильм сохранён в ваших закладках', 8, 'lampa');
            if (local.viewedPercent >= 90) add('lampa','con','✅ Досмотрено до конца (' + local.viewedPercent + '%)', 8, 'lampa');
            else if (local.viewedPercent >= 10) add('lampa','con','⏸ Просмотр остановлен на ' + local.viewedPercent + '%', 12, 'lampa');
            ...
            if (prosEmpty) pros.push('ℹ️ Недостаточно данных для детального анализа');
            if (consEmpty) cons.push((blG.length || blA.length || blD.length) ? '✅ Без ваших стоп-жанров и триггеров' : '✅ Явных минусов не обнаружено');
            ...
            if (mPace >= 8.0) add('moods','pro','⚡ Высокая динамика и темп', 14);
            else if (mPace >= 7.0) add('moods','pro','⚡ Очень высокий темп', 12);
            else if (mPace >= 6.0) add('moods','pro','⚡ Бодрое и энергичное повествование', 10);
            else if (mPace > 0 && mPace <= 3.5 && runtime > 110) add('moods','con','🐢 Очень неторопливый и размеренный темп', 10);
            else if (mPace <= 4.5) add('moods','con','🐢 Неторопливое развитие событий', 8);

            if (mFear >= 8.0) add('moods','con','😱 Экстремально высокое напряжение', 16);
            else if (mFear >= 7.0) add('moods','pro','🔥 Напряжённый сюжет и саспенс', 14);
            else if (mFear >= 6.0) add('moods','pro','🔥 Держит в постоянном напряжении', 12);
            else if (mFear >= 5.0) add('moods','pro','🔍 Есть интрига и лёгкое напряжение', 8);
            else if (mFear <= 3.5 && !gComedyN) add('moods','con','😌 Спокойное развитие сюжета без саспенса', 8);

            if (mDrama >= 8.0) add('moods','pro','😢 Очень сильный эмоциональный отклик', 14);
            else if (mDrama >= 7.0) add('moods','pro','😢 Глубокая и трогательная драма', 12);
            else if (mDrama >= 6.0) add('moods','pro','🎭 Трогательная эмоциональная история', 10);
            else if (mDrama >= 5.0) add('moods','pro','🎭 Есть драматическая глубина', 8);
            else if (mDrama <= 3.5) add('moods','con','😐 Эмоционально сдержанное кино', 8);

            if (mImmersion >= 8.5) add('moods','pro','🌌 Полное погружение в атмосферу и визуал', 16);
            else if (mImmersion >= 7.5) add('moods','pro','🌌 Отличная атмосфера и работа с окружением', 14);
            else if (mImmersion >= 6.5) add('moods','pro','✨ Приятная атмосфера и детализация', 12);
            else if (mImmersion >= 5.5) add('moods','pro','✨ Выразительный визуальный стиль', 8);
            else if (mImmersion <= 4.0) add('moods','con','😑 Атмосфера выражена слабо', 8);
            ...
            if (gHorror || kwScary) {
                aud(9, 'любителям качественных хорроров и жутких историй', 'впечатлительным зрителям и тем, кто не любит скримеры');
                aud(7, 'поклонникам мистики и паранормальных явлений', 'тем, кто ищет лёгкое и позитивное кино на ночь');
                aud(6, 'фанатам мрачных психологических триллеров', 'зрителям, избегающим гнетущей атмосферы');
            }
            if (dEpic.k || dEpic.o || gHistoricN) {
                aud(9, 'любителям масштабных исторических саг и эпосов', 'тем, кто предпочитает современные и бытовые сюжеты');
                aud(8, 'ценителям зрелищных сражений и батальных сцен', 'зрителям, которых утомляют долгие вступления');
                aud(7, 'фанатам историй о чести, власти и предательстве', 'тем, кто не любит высокопарные диалоги');
            }
            if (isAnim && familyOK) {
                aud(9, 'для семейного просмотра с детьми любого возраста', 'зрителям, которые ищут взрослый и реалистичный сюжет');
                aud(7, 'детям и родителям для уютного совместного вечера', 'тем, кто принципиально не смотрит анимацию');
                aud(5, 'любителям добрых и поучительных историй', 'фанатам мрачного и бескомпромиссного кино');
            } else if (isAnim && !familyOK) {
                aud(8, 'поклонникам взрослой анимации со сложными темами', 'для семейного просмотра — фильм не подходит детям');
                aud(7, 'ценителям глубокого сюжета в анимационном стиле', 'тем, кто считает мультфильмы сугубо детским форматом');
            }
            if (hasFamilyGenre && familyOK && !isAnim) {
                aud(7, 'для уютного семейного просмотра в кругу близких', 'тем, кто настроен на мрачный взрослый сюжет');
                aud(6, 'любителям светлых историй о дружбе и взаимовыручке', 'зрителям, предпочитающим циничный юмор');
            }
            if (isSuperheroN) {
                aud(8, 'поклонникам супергероики и зрелищных спецэффектов', 'тем, кто не любит комиксовые штампы и суперспособности');
                aud(7, 'любителям динамичных блокбастеров и приключений', 'зрителям в поиске камерной драмы без фантастики');
                aud(5, 'фанатам масштабных киновселенных', 'тем, кто устал от однотипных историй спасения мира');
            }
            if (gWar) {
                aud(8, 'ценителям сильных военных драм и истории', 'тем, кому тяжело смотреть сцены боевых действий');
                aud(6, 'интересующимся конкретной исторической эпохой', 'зрителям, которые хотят отдохнуть и расслабиться');
            }
            if (gCrime && gThr) {
                aud(8, 'любителям криминальных триллеров с закрученным сюжетом', 'тем, кто избегает жестоких сцен и криминальной эстетики');
                aud(7, 'поклонникам нуара и историй о неоднозначных героях', 'зрителям, предпочитающим лёгкие жанры');
            } else if (gCrime) {
                aud(6, 'поклонникам криминальных историй и расследований', 'тем, кто выбирает спокойные и позитивные картины');
            } else if (gThr) {
                aud(6, 'любителям триллеров с непрерывным саспенсом', 'тем, кто ищет ненавязчивое фоновое кино');
            }
            if (gComedyN) {
                aud(8, 'любителям лёгких комедий и хорошего юмора', 'тем, кто настроен на серьёзную глубокую драму');
                aud(6, 'тем, кто хочет просто отдохнуть и посмеяться', 'зрителям, ожидающим строгий реализм');
                aud(5, 'ценителям остроумных диалогов и сатиры', 'фанатам прямолинейного боевика без шуток');
            }
            if (gRomanceN) {
                aud(7, 'поклонникам трогательных историй любви и отношений', 'тем, кто не любит мелодраматические сюжеты');
                aud(6, 'любителям красивых романтических драм', 'зрителям, которым важнее экшен и интрига');
            }
            if (gSciFiN && rating >= 7.3) {
                aud(8, 'ценителям продуманной научной фантастики со смыслом', 'любителям простых и прямолинейных сюжетов');
            } else if (gSciFiN) {
                aud(7, 'любителям фантастических миров и смелых идей', 'тем, кто ищет реалистичную бытовую историю');
                aud(6, 'фанатам футуризма и антиутопий', 'тем, кому не близки темы технологий будущего');
            }
            if (gFantasyN) {
                aud(7, 'поклонникам фэнтези и сказочных миров', 'тем, кто предпочитает реализм без магии');
                aud(6, 'любителям эпических странствий и легенд', 'зрителям, которые скептично относятся к сказкам');
            }
            if (gMysteryN) {
                aud(7, 'любителям запутанных детективов и загадок', 'тем, кому не нравится следить за сложными подсказками');
                aud(6, 'тем, кто любит разгадывать тайны вместе с героями', 'фанатам прямолинейного экшена без интриги');
            }
            if (gDocN) {
                aud(8, 'тем, кто интересуется темой и любит документалистику', 'зрителям, ищущим чисто игровое художественное кино');
                aud(6, 'любителям реальных историй, фактов и расследований', 'тем, кому скучен документальный темп');
            }
            if (dDarkCom) {
                aud(7, 'любителям чёрного юмора и смелой сатиры', 'тем, кого задевают шутки на мрачные и табуированные темы');
                aud(6, 'ценителям абсурда и нестандартного взгляда', 'зрителям с консервативным чувством юмора');
            }
            if (kwEmot || (mm.sadness || 0) >= 6 || mDrama >= 7) {
                aud(8, 'ценителям сильных эмоциональных драм, готовым к сопереживанию', 'зрителям, которым сейчас не хочется грустных эмоций');
                aud(6, 'любителям глубоких душевных историй с катарсисом', 'тем, кто ищет лёгкий способ расслабиться');
            }
            if (kwUplift) {
                aud(7, 'тем, кому хочется светлого и вдохновляющего кино', 'зрителям в поисках мрачного и безысходного сюжета');
                aud(6, 'тем, кому нужна вера в лучшее и заряд мотивации', 'скептикам, которые не любят хеппи-энды');
            }
            if (rating >= 8 && votes >= 1000) {
                aud(7, 'поклонникам высоко оценённого и признанного кино', 'зрителям, ищущим простое непринуждённое зрелище');
            } else if (votes > 0 && votes < 1500 && rating >= 7.6) {
                aud(6, 'тем, кто ищет редкие и недооценённые находки', 'тем, кто смотрит исключительно проверенные блокбастеры');
            }
            if (cViol >= 50) {
                aud(5, 'тем, кто спокойно относится к жёстким сценам на экране', 'чувствительным зрителям и тем, кто смотрит кино с детьми');
            }
            if (mPace <= 3.5) {
                aud(6, 'любителям неспешного, созерцательного повествования', 'тем, кому требуется непрерывный экшен и быстрая смена событий');
            }

            var fallbackTarget = 'зрителям, выбирающим фильм для приятного вечернего просмотра';
            var fallbackAvoid = 'зрителям с очень специфическими жанровыми предпочтениями';
    ...
    function showModal(rawMovie, triggerBtn) {
        try {
            ...
            var phases = [
                ...
                'Анализируем отзывы и оценки…',
                'Оцениваем атмосферу и динамику…',
