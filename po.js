 ...
 * Version: 52.6 (Balanced UI, improved analysis, simplified atmosphere, full D-Pad)
...
(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            // === Root wrap ===
            '.sw-wrap{--sw-green:#4CAF50;--sw-red:#EF5350;--sw-ink:#e9ecef;--sw-mut:#adb5bd;padding:12px 6px 40px;color:var(--sw-ink);font-family:' + FONT + ';font-size:20px;box-sizing:border-box;-webkit-font-smoothing:antialiased}' +
            ...
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,#dee2e6,transparent);animation:swSlide 1.2s ease-in-out infinite}' +
            '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
            // === Verdict dossier card ===
            '.sw-dossier{position:relative;background:#212529;border:1px solid #2a2e32;border-radius:16px;padding:24px 24px 18px;margin-bottom:14px;box-shadow:0 4px 16px rgba(0,0,0,.3);animation:swRise .4s ease both;overflow:hidden}' +
            '@keyframes swRise{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}' +
            '.sw-verdict-word{position:relative;font-size:2.5em;font-weight:700;letter-spacing:.03em;line-height:1;margin:0 0 16px;text-transform:uppercase;opacity:0;transform:translateY(10px);transition:opacity .55s cubic-bezier(.16,.8,.24,1),transform .55s cubic-bezier(.16,.8,.24,1)}' +
            ...
            '.sw-meter-fill{height:100%;width:0;border-radius:4px;transition:width 1.1s cubic-bezier(.16,.8,.24,1) .1s}' +
            '.sw-meter-fill.yes{background:var(--sw-green)}' +
            '.sw-meter-fill.no{background:var(--sw-red)}' +
            // === Dice button ===
            '.sw-dicebtn{position:relative;display:flex;align-items:center;justify-content:center;width:100%;height:84px;border-radius:16px;background:#262b30;border:1px solid rgba(255,255,255,.08);margin:0 0 14px;cursor:pointer;outline:none;transition:transform .25s cubic-bezier(.2,.8,.2,1),box-shadow .25s ease,background .25s ease;-webkit-tap-highlight-color:transparent;box-shadow:0 4px 12px rgba(0,0,0,.25)}' +
            '.sw-dicebtn.focus{background:#2f353a!important;transform:scale(1.015);box-shadow:0 0 0 2px rgba(255,255,255,.8),0 4px 12px rgba(0,0,0,.3)}' +
            '.sw-dicebtn:active{transform:scale(.98)}' +
            '.sw-dicebtn:not(.sw-rolling) .sw-dice{animation:swIdleBob 2.4s ease-in-out infinite}' +
            '@keyframes swIdleBob{0%,100%{transform:translateY(0)}50%{transform:translateY(-3px)}}' +
            '.sw-dice{position:absolute;left:16px;top:50%;width:52px;height:52px;margin-top:-26px}' +
            '.sw-dice-core{display:block;width:100%;height:100%;filter:drop-shadow(0 4px 6px rgba(0,0,0,.4));transition:opacity .12s ease,transform .12s ease}' +
            '.sw-dice-core svg{width:100%;height:100%;display:block}' +
            '.sw-dice-core.sw-face-out{opacity:0;transform:scale(.85)}' +
            ...
            '.sw-dice-label.res-yes{color:#7fd6a4}' +
            ...
            '.sw-tab-btn.focus{opacity:1;background:#343a40!important;transform:scale(1.01);box-shadow:0 0 0 2px rgba(255,255,255,.8),0 2px 8px rgba(0,0,0,.3)}' +
            '.sw-tab-btn.sw-disabled{opacity:.35;pointer-events:none}' +
            '.sw-tagplus,.sw-tagminus{padding:2px 10px;font-weight:700;text-transform:uppercase;border-radius:6px;font-size:.8em}' +
            '.sw-tagplus{background:#4CAF50;color:#fff}' +
            '.sw-tagminus{background:#EF5350;color:#fff}' +
            ...
            '.sw-columns.sw-single-col{grid-template-columns:1fr}' +
            ...
            '.sw-col.pros{border-left:4px solid #4CAF50}' +
            '.sw-col.cons{border-left:4px solid #EF5350}' +
            '.sw-col.sw-empty-col{display:flex;align-items:center;justify-content:center;min-height:64px;opacity:.9;border-left-color:#4CAF50!important}' +
            '.sw-col.sw-empty-col .sw-col-title{margin-bottom:0}' +
            ...
            '.sw-col.pros .sw-col-title{color:#7fd6a4}' +
            '.sw-col.cons .sw-col-title{color:#f19898}' +
            '.sw-list{margin:0;padding:0;list-style:none;font-size:0.95em;line-height:1.5;color:#eef1f2}' +
            '.sw-list li{position:relative;margin-bottom:10px;padding-left:0;opacity:0;transform:translateY(6px);transition:opacity .45s cubic-bezier(.16,.8,.24,1),transform .45s cubic-bezier(.16,.8,.24,1)}' +
            '.sw-list li:last-child{margin-bottom:0}' +
            '.sw-list li.appear{opacity:1;transform:translateY(0)}' +
            // === Atmosphere metrics (simplified) ===
            '.sw-metrics-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px}' +
            '.sw-metric-card{position:relative;background:#1e2226;border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:12px 14px;box-shadow:0 4px 12px rgba(0,0,0,.2);opacity:0;transform:translateY(8px);transition:opacity .45s cubic-bezier(.16,.8,.24,1),transform .45s cubic-bezier(.16,.8,.24,1)}' +
            ...
            '.sw-metric-score{font-size:1em;font-weight:700}' +
            '.sw-metric-score.hi{color:#EF5350}.sw-metric-score.mid{color:#FFB74D}.sw-metric-score.ok{color:#81C784}' +
            '.sw-metric-bar-bg{height:5px;border-radius:4px;background:rgba(255,255,255,.07);overflow:hidden;margin-bottom:0}' +
            '.sw-metric-bar-fill{height:100%;width:0;border-radius:4px;transition:width .9s cubic-bezier(.16,.8,.24,1) .1s}' +
            // === Audience box ===
            '.sw-audience-box{position:relative;background:#1e2a23;border:1px solid rgba(76,175,80,.3);border-radius:12px;padding:14px 16px;box-shadow:0 4px 12px rgba(0,0,0,.2);margin-top:10px;overflow:hidden}' +
            '.sw-audience-title{font-size:.75em;font-weight:700;text-transform:uppercase;color:#a5d6a7;letter-spacing:.04em;display:flex;align-items:center;gap:6px;margin-bottom:6px}' +
            '.sw-audience-text{font-size:.88em;line-height:1.5;color:#e6ece8;margin:0}' +
            '.sw-audience-warn{font-size:.8em;line-height:1.4;color:#ef9a9a;margin-top:8px;padding-top:8px;border-top:1px dashed rgba(239,83,80,.3)}' +
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            // --- Atmosphere metrics: core 4 always shown, content-advisory metrics shown only when signal exists ---
            ...
            // Content-advisory metrics: derived from the same c* confidence scores used for
            // pros/cons, converted to a 1-10 scale. Only included when there's an actual
            // signal (score > 0), so a clean family film doesn't show a wall of empty "0/10" cards.
            ...
            if (hasGenre(genres, /ужасы|хоррор/i) || inText(ovRu, /одержим|сталкер|маньяк|пугающ/i)) {
                ...
                notSuitableFor = 'Не рекомендуется тем, кто избегает тяжёлого психологического давления.';
                ...
                notSuitableFor = 'Не подходит тем, кто ищет жесткий взрослый экшен.';
            } else if (rating >= 7.8) {
    ...
    // Smoothly cross-fades the die's core from its current face to a new one,
    // instead of hard-swapping the SVG. Calls back once the new face is settled in.
    function setFaceSmooth(dice, f, cb) {
        var core = dice.find('.sw-dice-core');
        if (!core.length) { setFace(dice, f); if (cb) cb(); return; }
        core.addClass('sw-face-out');
        setTimeout(function(){
            core.html(diceSVG(f));
            // force reflow so the removal of sw-face-out actually transitions in
            void core[0].offsetWidth;
            core.removeClass('sw-face-out');
            setTimeout(function(){ if (cb) cb(); }, 120);
        }, 120);
    }
    ...
    function buildReadyInner(a) {
        ...
        var metricsHtml = (atmos.metrics || []).map(function(m){
            ...
            var fillBg = m.score >= 7.5 ? '#EF5350' : (m.score >= 5 ? '#FFB74D' : '#81C784');
        ...
        var colsClass = (a.prosEmpty || a.consEmpty) ? 'sw-columns sw-single-col' : 'sw-columns';
        var prosColClass = 'sw-col pros' + (a.prosEmpty ? ' sw-empty-col' : '');
        var consColClass = 'sw-col cons' + (a.consEmpty ? ' sw-empty-col' : '');

        var prosConsHtml;
        if (a.prosEmpty && a.consEmpty) {
            prosConsHtml = '<div class="' + colsClass + '">' +
                '<div class="sw-col sw-empty-col"><ul class="sw-list">' +
                a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') +
                a.cons.map(function(c){ return '<li>' + esc(c) + '</li>'; }).join('') +
                '</ul></div>' +
                '</div>';
        } else if (a.prosEmpty) {
            prosConsHtml = '<div class="' + colsClass + '">' +
                '<div class="' + consColClass + '"><div class="sw-col-title">Минусы</div><ul class="sw-list">' + a.cons.map(function(c){ return '<li>' + esc(c) + '</li>'; }).join('') + '</ul></div>' +
                '</div>';
        } else if (a.consEmpty) {
            prosConsHtml = '<div class="' + colsClass + '">' +
                '<div class="' + prosColClass + '"><div class="sw-col-title">Плюсы</div><ul class="sw-list">' + a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
                '</div>';
        } else {
            prosConsHtml = '<div class="' + colsClass + '">' +
                '<div class="' + prosColClass + '"><div class="sw-col-title">Плюсы</div><ul class="sw-list">' + a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
        ...
        return '<div class="sw-dossier">' +
            ...
            '<div class="sw-tabs">' +
            '<div class="sw-tab-btn selector sw-focusable active" data-tab="proscons" tabindex="0">' +
            '<span class="sw-tagplus">ПЛЮСЫ</span><span class="sw-tab-i">и</span><span class="sw-tagminus">МИНУСЫ</span>' +
            '</div>' +
            '<div class="sw-tab-btn selector sw-focusable" data-tab="atmos" tabindex="0">' +
    ...
    function bindDice(html) {
        ...
        function rollDice() {
            ...
            // Roll out a handful of faces, each one settling in with a smooth
            // cross-fade rather than an instant swap or a spin/shake effect.
            var stepsLeft = 6;
            function step() {
                stepsLeft--;
                var isLast = stepsLeft <= 0;
                var face = rndFace();
                setFaceSmooth(dice, face, function(){
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v52.8 initialized: smooth cross-fade dice, richer transitions, expanded atmosphere metrics, empty-column fix.');
    }
