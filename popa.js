...
(function () {
    ...
    */
    ...
    var History = {
        read: function () {
            ...
            for (var e = 0; e < extra.length; e++) {
                var list2 = sGet(extra[e], null);
                if (isArr(list2)) {
                    for (i = 0; i < list2.length; i++) {
                        var it = list2[i];
                        if (it && typeof it === 'object') { addCard(it); bump(it.id, 2.2, typeOf(it), it); }
                        else bump(it, 1.6, null, null);
                    }
                }
            }

            /* --- 3. дописываем тип из найденных карточек --- */
            var out = [];
    ...
    var Capsule = {
        ...
        reason: function (item, taste) {
            ...
            if (item._src === 'seed' && item._via && item._via.seed) {
                r = 'Похоже на «' + item._via.seed + '» из вашей истории';
            } else if (item._src === 'keyword' && item._via && item._via.kw) {
                r = 'Тема «' + item._via.kw + '» часто встречается в том, что вы смотрите';
                ...
                r = names.length ? 'Ваши ' + names.join(' и ') + ', оценка ' + (item.vote_average || 0).toFixed(1)
                    : 'Высокая оценка зрителей: ' + (item.vote_average || 0).toFixed(1);
        ...
        rank: function (list, ctx, taste) {
            ...
            for (i = 0; i < list.length; i++) {
                ...
                for (j = 0; j < gids.length; j++) {
                    ...
                    if (gWeight[mapped]) s += 2 * (gWeight[mapped] / maxG); // личный вкус тоже влияет
                }
    ...
    var CSS = [
        ...
        'background:linear-gradient(115deg,rgba(255,255,255,.10) 0%,rgba(255,255,255,0) 34%);}',
        ...
        '.cm-name{font-size:2.7em;font-weight:800;line-height:1.05;letter-spacing:-.02em;margin-bottom:.28em;}',
        ...
        'background:#0B0F18;box-shadow:inset 0 0 0 1px rgba(159,176,200,.16),0 2em 5em rgba(0,0,0,.7);}',
    ...
    /* Космонавт: шлем с бликом, оранжевые вставки скафандра, фал уходит за кадр */
    var SVG_ASTRO = [
        '<svg viewBox="0 0 170 190" xmlns="http://www.w3.org/2000/svg">',
        '<defs>',
        '<linearGradient id="cmSuit" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#FFFFFF"/><stop offset="1" stop-color="#B9C4D6"/></linearGradient>',
        '<radialGradient id="cmVisor" cx="0.34" cy="0.28" r="0.85"><stop offset="0" stop-color="#2C5C86"/><stop offset="0.5" stop-color="#0C1727"/><stop offset="1" stop-color="#05070D"/></radialGradient>',
        '<linearGradient id="cmPack" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#9AA7BC"/><stop offset="1" stop-color="#6E7C93"/></linearGradient>',
        '</defs>',
        ...
        '<circle cx="86" cy="102" r="3" fill="#7FD8FF"/>',
        '<circle cx="94" cy="102" r="3" fill="#E8ECF5" opacity=".7"/>',
        /* шлем */
        '<circle cx="85" cy="48" r="36" fill="url(#cmSuit)"/>',
        '<ellipse cx="85" cy="48" rx="27" ry="24" fill="url(#cmVisor)"/>',
        '<path d="M66 36 C 74 28, 88 27, 96 32 C 88 34, 76 39, 70 47 Z" fill="#FFFFFF" opacity=".35"/>',
    ...
    var View = {
        ...
        greet: function () {
            ...
            if (!t || t.empty || !t.count) {
                this.speak('Истории пока не вижу. Скажи настроение — и я соберу капсулу под него.');
                return;
            }
            var parts = [];
            ...
            var line = 'Прочитал ' + t.count + ' карточек из твоей истории';
            if (parts.length) line += ': чаще всего ' + parts.join(' и ');
            if (kw) line += ', тема — «' + kw + '»';
            this.speak(line + '. Держи шесть попаданий.');
        },
    ...
    (function () {
        try {
            ...
            addMenu();
            console.log('[Капсула] v9.0 загружена');
        } catch (e) {
