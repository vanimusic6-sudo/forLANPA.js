...
var CSS_TEXT = [
'.cm-root{position:fixed;left:0;top:0;right:0;bottom:0;background:#0f1115;z-index:99999;color:#fff;font-family:-apple-system,Segoe UI,Roboto,sans-serif;overflow:hidden}',
'.cm-header{position:absolute;top:0;left:0;right:0;height:4.2em;display:flex;align-items:center;padding:0 1.6em;background:#0f1115;z-index:20}',
...
'.cm-content{position:absolute;top:4.2em;bottom:0;left:0;right:0;overflow:hidden;padding:0 1.6em 10em}',
'.cm-row{margin-bottom:1.2em}',
'.cm-row-title{font-size:1.15em;color:#cfcfcf;margin:0.4em 0 0.6em 0.1em;font-weight:600}',
'.cm-strip{display:flex;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;padding:0.6em 0.2em;scrollbar-width:none}',
...
'.cm-robot-wrap{position:absolute;left:2.5em;bottom:1.5em;z-index:30;display:flex;align-items:flex-end}',
...
'.cm-bubble{position:relative;background:#222;border-radius:1em;padding:0.9em 1.2em;margin-left:1.4em;max-width:22em;font-size:1.05em;color:#eaeaea;line-height:1.4;box-shadow:0 0.2em 0.6em rgba(0,0,0,0.3)}',
...
'.cm-details{display:flex;height:100%;padding-bottom:8em}',
...
'.cm-modal-body{padding:0.4em;color:#ddd;font-size:1em;line-height:1.5;max-height:22em;overflow:auto}',
...
'@media (max-width:768px){.cm-root{font-size:12px}.cm-header{height:3.5em;padding:0 1em}.cm-hbtn{width:2em;height:2em}.cm-hbtn svg{width:1.2em;height:1.2em}.cm-title{font-size:1.2em}.cm-clock{font-size:1.2em}.cm-content{padding:0 1em 8em}.cm-robot-wrap{left:1em;bottom:0.8em}.cm-robot{width:7em;height:7em}.cm-bubble{max-width:16em;font-size:0.9em}.cm-details{flex-direction:column;padding-bottom:4em}.cm-d-left{width:100%;display:flex;gap:1em;align-items:center;margin-bottom:1em}.cm-d-left .cm-card{width:6em;height:9em}.cm-d-center{flex:none;margin:0.5em 0}.cm-d-right{width:100%;padding:0 0.5em}.cm-drawer{display:none}}',
...
function posterPlaceholder(title) { var t = esc(String(title || 'capsule').slice(0, 14)); return 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="200" height="300"><rect width="200" height="300" fill="#1d1d1d"/><rect x="8" y="8" width="184" height="284" rx="10" fill="none" stroke="#333" stroke-width="2"/><text x="100" y="155" fill="#8fd3ff" font-size="15" text-anchor="middle" font-family="sans-serif">' + t + '</text></svg>'); }
...
var Engine = {
    readHistory: function () {
        ...
        function addItem(item) {
            ...
            if (seen[key]) return;
            seen[key] = 1;
            out.push({ id: id, type: type, percent: percent, date: item.date || item.last_time || 0, favorite: false });
        }
        // 1. Lampa Timeline API
        try {
            if (window.Lampa && Lampa.Timeline && typeof Lampa.Timeline.view === 'function') {
                var tl = Lampa.Timeline.view(0, 100);
                if (tl && tl.forEach) tl.forEach(function (e) { addItem(e); });
            }
        } catch (e) {}
        // 2. Storage fallback
        try {
            var tl2 = sGet('timeline', {});
            if (tl2 && typeof tl2 === 'object') {
                for (var h in tl2) {
                    if (!Object.prototype.hasOwnProperty.call(tl2, h)) continue;
                    addItem(tl2[h]);
                }
            }
        } catch (e) {}
            ...
            if (window.Lampa && Lampa.Favorite && typeof Lampa.Favorite.get === 'function') {
                ...
                var walkFav = function (v) {
                    ...
                    if (typeof v === 'object' && v.id) {
                        ...
                        var k2 = t + ':' + v.id;
                        if (!seen[k2]) { seen[k2] = 1; out.push({ id: v.id, type: t, percent: 100, date: 0, favorite: true }); }
                    }
                };
                walkFav(fav);
            }
    ...
    discover: function (profile, extra, cb) {
        ...
        // Добавляем исключение документальных, если они не в with_genres
        if (!pos.includes('99')) {
            var withoutArr = params.without_genres ? params.without_genres.split(',') : [];
            if (!withoutArr.includes('99')) withoutArr.push('99');
            params.without_genres = withoutArr.join(',');
        }
        ...
        function one(t) {
            ...
            if (t === 'movie') p2['vote_count.gte'] = 150;
            else p2['vote_count.gte'] = 50;
            TMDB.get('/discover/' + t, p2, function (d) { var r = d.results || []; for (var j = 0; j < r.length; j++) { r[j].media_type = t; merged.push(r[j]); } fin(); }, fin);
        }
        one('movie'); one('tv');
    },
    ...
    buildBundle: function (profile, cb) {
        ...
        this.discover(profile, { sort_by: 'vote_average.desc' }, function (l) { out.stable = self.pick(l, profile, watched, seen, 'stable', 5); fin(); });
...
function parseUserQuery(text) {
    ...
    var rules = [ { re: /комед|смешн|весел|юмор/, g: 35 }, { re: /боевик|экшн|экшен/, g: 28 }, { re: /фантаст|космос/, g: 878 }, { re: /ужас|хоррор|страш/, g: 27 }, { re: /триллер|напряж/, g: 53 }, { re: /драм/, g: 18 }, { re: /романти|мелодрам|любов/, g: 10749 }, { re: /детектив|криминал/, g: 80 }, { re: /приключени/, g: 12 }, { re: /аниме|манга|мульт/, g: 16 }, { re: /семейн|детск/, g: 10751 }, { re: /фэнтези/, g: 14 }, { re: /документал/, g: 99 } ];
...
function renderMain() {
    ...
    if (existingRobotWrap) root.removeChild(existingRobotWrap);
    var content = root.querySelector('.cm-content'); content.innerHTML = '';
    var drawer = root.querySelector('.cm-drawer'); if (drawer) drawer.parentNode.removeChild(drawer);
    ...
    var rows = [[root.querySelector('[data-cm-action="back"]'), root.querySelector('[data-cm-action="settings"]')]];
    ...
    var robot = root.querySelector('[data-cm-action="robot"]'); if (robot) rows.push([robot]);
    FM.setRows(rows, 1, 0);
}
...
function renderDetails(movie) {
    ...
    var drawer = el('div', 'cm-drawer', '‹'); drawer.setAttribute('data-cm-action', 'drawer'); root.appendChild(drawer);
    FM.setRows([ [root.querySelector('[data-cm-action="back"]'), root.querySelector('[data-cm-action="settings"]')], [play, drawer], [].slice.call(right.querySelectorAll('.cm-ai-item'), 0) ], 1, 0);
...
function aiRandom() {
    ...
    TMDB.get('/discover/movie', { sort_by: 'popularity.desc', 'vote_average.gte': 6.5, 'vote_count.gte': 150 }, function (d) { var r = d.results || []; if (r.length) show(rnd(r)); }, function () { notify(robotPhrase('error')); });
}
    ...
    this.start = function () {
        try {
            Lampa.Controller.add(CTRL_NAME, {
                toggle: function () { FM.apply(); },
                up: function () { FM.move('up'); },
                down: function () { FM.move('down'); },
                left: function () { FM.move('left'); },
                right: function () { FM.move('right'); },
                enter: function () { var n = FM.current(); if (n) doAction(n); },
...
function startPlugin() {
    ...
    var boot = function () {
        ...
        setTimeout(function () {
            ...
            addMenuItem();
            console.log('[CapsuleMod] v3.0 готов');
        }, 300);
    };
