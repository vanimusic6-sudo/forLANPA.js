 ...
 * Version: 52.2 (Polished UI, realistic dice physics, improved analysis)
...
(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dice.sw-spin .sw-dice-core{animation:swThrow 1.15s cubic-bezier(.3,.7,.4,1) forwards}' +
            '@keyframes swThrow{' +
            '0%{transform:translateY(0) rotate(0) scale(1,1)}' +
            '20%{transform:translateY(-28px) rotate(360deg) scale(1.05,0.95)}' +
            '40%{transform:translateY(0) rotate(720deg) scale(0.95,1.05)}' +
            '55%{transform:translateY(-12px) rotate(900deg) scale(1.02,0.98)}' +
            '70%{transform:translateY(0) rotate(1080deg) scale(1,1)}' +
            '85%{transform:translateY(-4px) rotate(1260deg) scale(1.01,0.99)}' +
            '100%{transform:translateY(0) rotate(1440deg) scale(1,1)}' +
            '}' +
    ...
    function loadMeta(movie) {
        ...
        return Promise.all([
            ...
            tmdbGet('/' + type + '/' + id) // for popularity, release dates, etc.
        ]).then(function(arr){
            var kw = [];
    ...
    function bindDice(html) {
        ...
        function rollDice() {
            ...
            }, 1150); // matches new animation duration
            setTimeout(function(){
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v52.2 initialized: polished UI, realistic dice physics, improved analysis.');
    }
