(function () {
    ...
    const THEMES = {
        ...
        inception: { name: 'Сон', cls: 'cm-t-inception', sys: 'LIMBO LEVEL: 01 // STABLE', quotes: ['«Как долго мы здесь?»', '«Ты ждёшь поезд»', '«Не бойся глубины»', '«Время здесь течёт иначе»'], load: ['ПОГРУЖАЮСЬ В СОН', 'НАСТРАИВАЮ ГЛУБИНУ'], vars: { '--cm-bg': '#081016', '--cm-accent': '#B7E7FF', '--cm-accent2': '#D7B56D', '--cm-text': '#EAF5FA', '--cm-sub': '#76919E', '--cm-panel': 'rgba(10,22,30,.64)', '--cm-panel2': 'rgba(6,15,21,.55)', '--cm-chip': 'rgba(183,231,255,.06)', '--cm-radius': '1em' } },
        ...
        rain: (root, mode) => {
            ...
            if (mode === 'matrix') {
                ...
                timer=setInterval(()=>{
                    ...
                    for(let i=0;i<9;i++){const r=(i+1)*Math.min(canvas.width,canvas.height)*.055*(1+.04*Math.sin(t+i));ctx.globalAlpha=.018+i*.004;ctx.strokeStyle=i%2?'#B7E7FF':'#D7B56D';ctx.beginPath();ctx.ellipse(cx,cy,r,r*.48,Math.sin(t+i)*.08,0,Math.PI*2);ctx.stroke()}
                    ctx.globalAlpha=1;
                },80);
            } else if (mode === 'dune') {
                ...
                timer=setInterval(()=>{
                    ...
                    waves.forEach(i=>{ctx.globalAlpha=.018+i*.002;ctx.strokeStyle='#E7B46A';ctx.beginPath();for(let x=-20;x<canvas.width+20;x+=20){const y=canvas.height*(.35+i*.075)+Math.sin(x*.012+t+i)*12+i*2; x===-20?ctx.moveTo(x,y):ctx.lineTo(x,y)}ctx.stroke()});ctx.globalAlpha=1;
                },70);
            } else if (mode === 'blade') {
                const drops=Array.from({length:38},()=>({x:Math.random(),y:Math.random(),v:1+Math.random()*2}));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    drops.forEach(d=>{d.y+=d.v/120;if(d.y>1.1){d.y=-.1;d.x=Math.random()}ctx.globalAlpha=.025+.03*Math.random();ctx.strokeStyle=d.x>.5?'#74E5FF':'#F2B6FF';ctx.beginPath();ctx.moveTo(d.x*canvas.width,d.y*canvas.height);ctx.lineTo((d.x-.006)*canvas.width,(d.y+.035)*canvas.height);ctx.stroke()});ctx.globalAlpha=1;
                },55);
            } else if (mode === 'dream') {
                const clouds=Array.from({length:10},()=>({x:Math.random(),y:Math.random(),r:40+Math.random()*90,v:.02+Math.random()*.03}));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    clouds.forEach(c=>{c.x+=c.v/100;if(c.x>1.2)c.x=-.2;ctx.globalAlpha=.018;ctx.fillStyle=c.y<.5?'#B9C7FF':'#A9E7D5';ctx.beginPath();ctx.arc(c.x*canvas.width,c.y*canvas.height,c.r,0,Math.PI*2);ctx.fill()});ctx.globalAlpha=1;
                },90);
            } else {
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
    ...
    const CSS = `
        ...
        @media(max-width:700px){
            .cm-stage{justify-content:flex-start;padding:2.7em .55em .7em;gap:.5em}
        ...
        @media(max-width:420px){
            ...
            .cm-stage{padding-left:.4em;padding-right:.4em}
        }
