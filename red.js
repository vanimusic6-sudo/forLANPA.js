 ...
 * Version: 52.0 (Premium Visual Redesign — same structure/logic as v51, richer UI)
...
(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            /* ===== Trigger button on the full-card ===== */
            '.sw-custom-button-enhanced{cursor:pointer;position:relative;transition:background .3s cubic-bezier(.22,1,.36,1),color .3s,transform .25s cubic-bezier(.22,1,.36,1),box-shadow .3s;display:inline-flex;align-items:center;justify-content:center;border-radius:12px;user-select:none;overflow:hidden}' +
            '.sw-custom-button-enhanced::before{content:"";position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,209,102,.0),rgba(255,209,102,.0));transition:background .3s}' +
            '.sw-custom-button-enhanced.focus{background:linear-gradient(135deg,#ffd166,#ff7a3d)!important;color:#1a0f04!important;transform:scale(1.055);box-shadow:0 0 0 2px rgba(255,255,255,.55),0 10px 26px rgba(255,138,61,.45),0 0 34px rgba(255,180,90,.35)}' +
            '.sw-custom-button-enhanced.focus svg path{fill:#1a0f04}' +
            /* ===== Root wrap ===== */
            '.sw-wrap{--sw-gold:#f2c14e;--sw-gold-2:#ffb238;--sw-green:#33e08a;--sw-green-2:#17b869;--sw-red:#ff5b6e;--sw-red-2:#e8283f;--sw-ink:#eef2f4;--sw-mut:#9aa7b4;padding:8px 4px 36px;color:var(--sw-ink);font-family:' + DISPLAY + ';font-size:20px;box-sizing:border-box;-webkit-font-smoothing:antialiased}' +
            '.sw-body{animation:swFadeIn .5s cubic-bezier(.16,1,.3,1)}' +
            '@keyframes swFadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}' +
            /* ===== Loader ===== */
            '.sw-loader{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;padding:70px 20px;min-height:42vh;color:#9aa1a6}' +
            '.sw-loader-emoji{font-size:3.4em;line-height:1;filter:drop-shadow(0 6px 16px rgba(255,178,56,.35));animation:swFloat 2.4s ease-in-out infinite}' +
            '@keyframes swFloat{0%,100%{transform:translateY(0) rotate(-2deg)}50%{transform:translateY(-10px) rotate(2deg)}}' +
            '.sw-loader-text{font-size:1.05em;font-weight:600;min-height:1.5em;text-align:center;color:#dfe6ec;letter-spacing:.02em;background:linear-gradient(90deg,#dfe6ec,#ffd88a,#dfe6ec);background-size:200% auto;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;animation:swShine 2.6s linear infinite}' +
            '@keyframes swShine{0%{background-position:0% center}100%{background-position:200% center}}' +
            '.sw-loader-progress{width:240px;height:5px;border-radius:4px;background:rgba(255,255,255,.07);overflow:hidden;position:relative;margin-top:4px;box-shadow:inset 0 1px 2px rgba(0,0,0,.4)}' +
            '.sw-loader-progress::after{content:"";position:absolute;left:-100%;top:0;height:100%;width:100%;background:linear-gradient(90deg,transparent,var(--sw-gold-2),var(--sw-green),transparent);animation:swSlide 1.4s ease-in-out infinite;filter:drop-shadow(0 0 6px rgba(255,178,56,.5))}' +
            '@keyframes swSlide{0%{left:-100%}100%{left:100%}}' +
            /* ===== Verdict dossier card ===== */
            '.sw-dossier{position:relative;background:radial-gradient(120% 160% at 0% 0%,#20262b 0%,#14181b 55%,#101315 100%);border:1px solid rgba(255,255,255,.07);border-radius:20px;padding:30px 32px 26px;margin-bottom:18px;box-shadow:0 14px 40px rgba(0,0,0,.5),inset 0 1px 0 rgba(255,255,255,.04);animation:swRise .6s cubic-bezier(.16,1,.3,1) both;overflow:hidden}' +
            '.sw-dossier::after{content:"";position:absolute;top:-60%;right:-30%;width:70%;height:220%;background:radial-gradient(circle,rgba(242,193,78,.10),transparent 60%);pointer-events:none}' +
            '@keyframes swRise{from{opacity:0;transform:translateY(16px) scale(.985)}to{opacity:1;transform:translateY(0) scale(1)}}' +
            '.sw-verdict-word{position:relative;font-family:' + COND + ';font-size:3.3em;font-weight:800;letter-spacing:.035em;line-height:1;margin:0 0 20px;text-transform:uppercase;opacity:0;transform:translateY(14px) scale(.96);transition:opacity .6s ease,transform .6s cubic-bezier(.22,1.4,.36,1)}' +
            '.sw-verdict-word.appear{opacity:1;transform:translateY(0) scale(1)}' +
            '.sw-verdict-word.yes{background:linear-gradient(100deg,#7cffb2 0%,var(--sw-green) 45%,#0fae63 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 22px rgba(51,224,138,.5)) drop-shadow(0 3px 8px rgba(0,0,0,.4))}' +
            '.sw-verdict-word.no{background:linear-gradient(100deg,#ffb3ba 0%,var(--sw-red) 45%,#c21f34 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 22px rgba(255,91,110,.5)) drop-shadow(0 3px 8px rgba(0,0,0,.4))}' +
            '.sw-meter{height:7px;border-radius:5px;background:rgba(255,255,255,.06);overflow:hidden;box-shadow:inset 0 1px 3px rgba(0,0,0,.5)}' +
            '.sw-meter-fill{height:100%;width:0;border-radius:5px;transition:width 1.2s cubic-bezier(.16,1,.3,1);position:relative}' +
            '.sw-meter-fill::after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(255,255,255,0),rgba(255,255,255,.35),rgba(255,255,255,0));width:40%;animation:swMeterShine 2.2s ease-in-out infinite}' +
            '@keyframes swMeterShine{0%{transform:translateX(-120%)}100%{transform:translateX(340%)}}' +
            '.sw-meter-fill.yes{background:linear-gradient(90deg,#0fae63,var(--sw-green) 55%,#8dffc2)}' +
            '.sw-meter-fill.no{background:linear-gradient(90deg,#c21f34,var(--sw-red) 55%,#ffb3ba)}' +
            /* ===== Dice button ===== */
            '.sw-dicebtn{position:relative;display:flex;align-items:center;justify-content:center;width:100%;height:104px;border-radius:20px;background:linear-gradient(180deg,#262c31 0%,#181c1f 100%);border:1px solid rgba(255,255,255,.07);margin:0 0 16px;cursor:pointer;outline:none;overflow:hidden;transition:transform .25s cubic-bezier(.16,1,.3,1),box-shadow .25s;-webkit-tap-highlight-color:transparent;box-shadow:0 8px 22px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.05)}' +
            '.sw-dicebtn::before{content:"";position:absolute;inset:0;background:linear-gradient(120deg,rgba(242,193,78,.08),transparent 45%);pointer-events:none}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 3px var(--sw-gold),0 10px 30px rgba(0,0,0,.5),0 0 40px rgba(242,193,78,.35);transform:scale(1.02)}' +
            '.sw-dicebtn:active{transform:scale(.985)}' +
            '.sw-dice{position:absolute;left:18px;top:50%;width:70px;height:70px;margin-top:-35px;transition:transform .5s cubic-bezier(.3,1.4,.5,1);filter:drop-shadow(0 6px 10px rgba(0,0,0,.5))}' +
            ...
            '@keyframes swSpin{0%{transform:translateY(0) rotate(0) scale(1,1)}20%{transform:translateY(-20px) rotate(180deg) scale(.92,1.08)}40%{transform:translateY(0) rotate(360deg) scale(1.1,.9)}60%{transform:translateY(-9px) rotate(540deg) scale(.96,1.04)}80%{transform:translateY(0) rotate(680deg) scale(1.05,.95)}90%{transform:translateY(-2px) rotate(723deg) scale(1.01,.99)}100%{transform:translateY(0) rotate(720deg) scale(1,1)}}' +
            '.sw-dice-label{position:relative;font-family:' + COND + ';font-size:2.3em;font-weight:800;color:#fff;text-transform:lowercase;letter-spacing:.01em;text-align:center;transition:opacity .25s ease,transform .25s ease;pointer-events:none;padding:0 22px}' +
            '.sw-dice-label.hide{opacity:0;transform:translateY(10px) scale(.95)}' +
            '.sw-dice-label.res-yes{color:#8dffc2;text-shadow:0 0 20px rgba(51,224,138,.55)}' +
            '.sw-dice-label.res-no{color:#ffb3ba;text-shadow:0 0 20px rgba(255,91,110,.55)}' +
            /* ===== Tabs ===== */
            '.sw-tabs{display:grid;grid-template-columns:1fr 1fr;background:linear-gradient(180deg,#33393e,#282d31);border-radius:14px;overflow:hidden;margin-bottom:18px;padding:4px;gap:4px;box-shadow:inset 0 1px 3px rgba(0,0,0,.4)}' +
            '.sw-tab-btn{position:relative;min-height:54px;padding:10px 8px;display:flex;align-items:center;justify-content:center;gap:10px;background:transparent;border:none;outline:none;border-radius:11px;cursor:pointer;color:#c7cdd2;font-family:' + COND + ';font-size:1.15em;font-weight:800;text-transform:lowercase;opacity:.62;transition:opacity .25s,background .3s cubic-bezier(.22,1,.36,1),transform .2s;user-select:none}' +
            '.sw-tab-btn.active{opacity:1;color:#fff;background:linear-gradient(160deg,#565d63,#3d4247);box-shadow:0 4px 14px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.1)}' +
            '.sw-tab-btn.focus{opacity:1;box-shadow:inset 0 0 0 2px var(--sw-gold),0 0 18px rgba(242,193,78,.35);transform:scale(1.015)}' +
            '.sw-tab-btn.sw-disabled{opacity:.35;pointer-events:none}' +
            '.sw-tagplus{background:linear-gradient(135deg,#33e08a,#0fae63);border:none;color:#04150c;padding:2px 16px;font-weight:800;text-transform:uppercase;border-radius:6px;letter-spacing:.03em;box-shadow:0 2px 8px rgba(15,174,99,.35)}' +
            '.sw-tagminus{background:linear-gradient(135deg,#ff5b6e,#c21f34);border:none;color:#180307;padding:2px 16px;font-weight:800;text-transform:uppercase;border-radius:6px;letter-spacing:.03em;box-shadow:0 2px 8px rgba(194,31,52,.35)}' +
            '.sw-tab-i{color:#9aa7b4;font-weight:800;padding:0 6px}' +
            '.sw-tab-atmos{position:relative;width:100%;display:flex;align-items:center;justify-content:center;overflow:hidden}' +
            '.sw-tab-atmos svg{position:absolute;left:0;top:50%;width:100%;height:32px;transform:translateY(-50%);opacity:.9}' +
            '.sw-tab-atmos span{position:relative;color:#fff;letter-spacing:.02em;text-shadow:0 2px 6px rgba(0,0,0,.4)}' +
            '.sw-tab-pane{display:none;animation:swFadeIn .35s cubic-bezier(.16,1,.3,1)}' +
            '.sw-tab-pane.active{display:block}' +
            /* ===== Pros/Cons columns ===== */
            '.sw-columns{display:grid;grid-template-columns:1fr 1fr;gap:18px}' +
            '.sw-col{position:relative;background:linear-gradient(165deg,#22282c,#1a1f22);border:1px solid rgba(255,255,255,.05);border-radius:16px;padding:24px 26px;box-shadow:0 8px 24px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.03)}' +
            '.sw-col::before{content:"";position:absolute;top:0;left:14px;right:14px;height:3px;border-radius:0 0 4px 4px;filter:blur(.2px)}' +
            '.sw-col.pros::before{background:linear-gradient(90deg,var(--sw-green),#0fae63);box-shadow:0 0 12px rgba(51,224,138,.5)}' +
            '.sw-col.cons::before{background:linear-gradient(90deg,#ffb3ba,var(--sw-red));box-shadow:0 0 12px rgba(255,91,110,.5)}' +
            '.sw-list{margin:0;padding:0;list-style:none;font-size:1.02em;line-height:1.6;color:#eef1f2}' +
            '.sw-list li{position:relative;margin-bottom:15px;padding-left:2px;opacity:0;transform:translateX(-8px);transition:opacity .45s ease,transform .45s cubic-bezier(.22,1,.36,1)}' +
            '.sw-list li:last-child{margin-bottom:2px}' +
            '.sw-list li.appear{opacity:1;transform:translateX(0)}' +
            /* ===== Atmosphere metrics ===== */
            '.sw-metrics-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:18px}' +
            '.sw-metric-card{position:relative;background:linear-gradient(165deg,#22282c,#1a1f22);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:16px 18px;box-shadow:0 6px 18px rgba(0,0,0,.28);transition:transform .3s cubic-bezier(.22,1,.36,1)}' +
            '.sw-metric-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:9px}' +
            '.sw-metric-name{font-size:.85em;font-weight:700;color:#c7cdd2;text-transform:uppercase;letter-spacing:.04em}' +
            '.sw-metric-score{font-family:' + COND + ';font-size:1.2em;font-weight:800;letter-spacing:.02em}' +
            '.sw-metric-score.hi{color:#ff8a95;text-shadow:0 0 12px rgba(255,91,110,.4)}.sw-metric-score.mid{color:#ffc266;text-shadow:0 0 12px rgba(255,178,56,.35)}.sw-metric-score.ok{color:#5fe6a0;text-shadow:0 0 12px rgba(51,224,138,.35)}' +
            '.sw-metric-bar-bg{height:7px;border-radius:4px;background:rgba(255,255,255,.07);overflow:hidden;margin-bottom:7px;box-shadow:inset 0 1px 2px rgba(0,0,0,.4)}' +
            '.sw-metric-bar-fill{height:100%;border-radius:4px;transition:width 1s cubic-bezier(.16,1,.3,1)}' +
            '.sw-metric-desc{font-size:.74em;color:#8b98a5;line-height:1.35}' +
            '.sw-audience-box{position:relative;background:linear-gradient(160deg,#243228 0%,#182620 60%,#131f19 100%);border:1px solid rgba(51,224,138,.28);border-radius:18px;padding:18px 22px;box-shadow:0 10px 26px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.04);margin-top:14px;overflow:hidden}' +
            '.sw-audience-box::after{content:"";position:absolute;top:-40%;right:-20%;width:60%;height:180%;background:radial-gradient(circle,rgba(51,224,138,.12),transparent 65%);pointer-events:none}' +
            '.sw-audience-title{font-size:.86em;font-weight:800;text-transform:uppercase;color:#7fe6ac;letter-spacing:.05em;display:flex;align-items:center;gap:7px;margin-bottom:9px}' +
            '.sw-audience-text{font-size:.93em;line-height:1.6;color:#e6ece8;margin:0}' +
            '.sw-audience-warn{font-size:.85em;line-height:1.5;color:#ffb0b8;margin-top:10px;padding-top:10px;border-top:1px dashed rgba(255,91,110,.3)}' +
            '.sw-focusable{outline:none;cursor:pointer}' +
            '@media(max-width:680px){.sw-columns{grid-template-columns:1fr}.sw-metrics-grid{grid-template-columns:1fr}.sw-verdict-word{font-size:2.4em}.sw-dice-label{font-size:1.65em}.sw-dice{width:52px;height:52px;margin-top:-26px;left:14px}.sw-dossier{padding:22px 20px 20px}}';
    ...
    function diceSVG(n) {
        ...
        for (var i = 0; i < p.length; i++) {
            c += '<circle cx="' + p[i][0] + '" cy="' + p[i][1] + '" r="9.4" fill="url(#swPipGrad)"/>';
            c += '<circle cx="' + (p[i][0] - 2.2) + '" cy="' + (p[i][1] - 2.4) + '" r="2.4" fill="rgba(255,255,255,.55)"/>';
        }
        return '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">' +
            '<defs>' +
            '<linearGradient id="swFaceGrad" x1="0" y1="0" x2="1" y2="1">' +
            '<stop offset="0%" stop-color="#ffffff"/><stop offset="55%" stop-color="#f2f4f6"/><stop offset="100%" stop-color="#dfe3e7"/>' +
            '</linearGradient>' +
            '<linearGradient id="swPipGrad" x1="0" y1="0" x2="0" y2="1">' +
            '<stop offset="0%" stop-color="#ff6b52"/><stop offset="100%" stop-color="#c21f34"/>' +
            '</linearGradient>' +
            '<linearGradient id="swEdgeGrad" x1="0" y1="0" x2="0" y2="1">' +
            '<stop offset="0%" stop-color="rgba(255,255,255,.9)"/><stop offset="100%" stop-color="rgba(200,205,210,.4)"/>' +
            '</linearGradient>' +
            '</defs>' +
            '<rect x="5" y="5" width="90" height="90" rx="22" fill="rgba(0,0,0,.25)"/>' +
            '<rect x="6" y="6" width="88" height="88" rx="21" fill="url(#swFaceGrad)" stroke="url(#swEdgeGrad)" stroke-width="3"/>' +
            '<rect x="12" y="10" width="76" height="26" rx="14" fill="rgba(255,255,255,.55)"/>' +
            c + '</svg>';
    }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v52.0 initialized: premium UI redesign, native Lampa scroll/controller, strict binary verdicts.');
    }
