    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-dicebtn{position:relative;display:block;width:100%;height:108px;background:linear-gradient(135deg,#23272d,#1a1d21);border:1px solid rgba(255,255,255,.14);border-radius:20px;margin:0 0 22px;overflow:hidden;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;transition:border-color .25s ease,box-shadow .25s ease,transform .18s ease;box-shadow:0 8px 24px rgba(0,0,0,.35)}' +
                '.sw-dicebtn:hover{border-color:rgba(255,255,255,.28);transform:translateY(-1px)}' +
                '.sw-dicebtn:active{transform:scale(.99)}' +
                '.sw-dicebtn.focus{box-shadow:0 0 0 4px rgba(255,255,255,.9),0 0 25px rgba(255,255,255,.25);border-color:#fff}' +
    ...
    function bindDice(html) {
        ...
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var dist = Math.max(50, btn.clientWidth - dice.offsetWidth - 36);
