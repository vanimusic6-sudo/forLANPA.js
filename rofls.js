/**
 * Capsule Mod v14.4 — «Капсула»
 *
 * Правки этой ревизии (поверх v14.3):
 * 1. История вкуса больше не считается дважды. Lampa.Favorite.full() уже включает
 *    wath/look/like/book/scheduled/continued/thrown как под-массивы — плагин повторно
 *    читал их же как отдельные ключи хранилища и удваивал вес. Из-за этого модель вкуса
 *    (жанры/темы) могла быть перекошена и капсула — «мимо».
 * 2. Настоящая причина бага «обновить — не обновляет»: поиск по настроению/фразе
 *    (Search.rank) не знал о списке уже показанных id (Capsule.shown()) и мог отдать
 *    точно ту же шестёрку повторно. Теперь уже показанные фильмы сильно штрафуются
 *    по очкам и уступают место новым (а если новых категорически мало — не исчезают
 *    совсем, чтобы не остаться с пустым экраном).
 * 3. Плитки настроений — сетка 2×N, но навигация по ней (Modal.move) была одномерной:
 *    «вниз» на плитке фактически прыгало по диагонали. Теперь вверх/вниз ходят по
 *    столбцу, влево/вправо — по строке, переход в список ниже плиток — по границе сетки.
 *    Это то, ради чего вообще стоило делать сетку 2×N.
 * 4. Компаньон — космонавт, а не робот (так и задумывался персонаж): у стандартной
 *    маски теперь круглый гермошлем со светоотражением на визоре, скафандр с нагрудным
 *    пультом и кислородными шлангами вместо антенны и прямоугольной «робо-головы».
 *    Тематические маски (Лаборатория, Матрица и т.д.) не трогал — они уже герои своих тем.
 * 5. Интерфейс на телефоне: добавлен overflow-y на сцену и min-height ~2.7em для
 *    плиток/пунктов меню (было слишком узкое поле тапа), плюс отдельный брейкпоинт под
 *    низкие альбомные экраны, чтобы постер и трей не обрезались по высоте.
 * 6. Companion.similar теперь тоже выставляет View.busy — раньше свайп/повторное
 *    нажатие «Обновить» во время загрузки «Похожего» могли рассинхронизировать экран.
 */
...
(function () {
    ...
    const CSS = `
        ...
        @media (max-width:1000px){
            ...
            .cm-stage{padding:1em;align-items:flex-start}
...
<circle cx="94" cy="90" r="4.5" fill="#FF7A2F"/>
    ...
    const Companion = {
        ...
        themes: () => {
            ...
            Modal.open({ mascot: true, title: 'Оформление', text: 'Космонавт и стиль меняются вместе с темой.', items });
        },
