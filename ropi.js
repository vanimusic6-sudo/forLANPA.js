/**
 * Capsule Mod v14.0 — «Капсула»
 * - Трей перенесён внутрь карточки (под кнопки, как на скриншоте)
 * - Убраны все надписи сверху (не мешает шапке Lampa)
 * - Настройки капсулы убраны из Lampa Settings — всё в Компаньоне
 * - Компаньон скомпанован по разделам + подменю настроек
 * - «Обновить» обновляет ТЕКУЩУЮ капсулу (нуар → нуар и т.д.)
 * - Робот перерисован, 8 тем, стиль робота меняется с темой
 * - Полная оптимизация под пульт и сенсор
 */
...
(function () {
    ...
    const CSS = `
        ...
        .cm-why{position:relative;padding-left:1em;font-size:1em;line-height:1.4;opacity:.85;margin-bottom:1em;max-width:30em}
        ...
        .cm-act{display:flex;align-items:center;justify-content:center;padding:.9em 1.6em;border-radius:calc(var(--cm-radius)*.5);margin:0 .6em .6em 0;cursor:pointer;background:var(--cm-chip);font-size:1.05em;font-weight:600;color:var(--cm-text);transition:background .16s,transform .16s,color .16s,box-shadow .16s}
        ...
        /* ТРЕЙ ВНУТРИ КАРТОЧКИ */
        .cm-tray{margin-top:auto;padding-top:1em}
        .cm-tray-in{display:flex;align-items:center;overflow-x:auto;padding:.45em;border-radius:.9em;background:var(--cm-panel2);box-shadow:inset 0 0 0 1px rgba(255,255,255,.08);scrollbar-width:none}
        ...
        .cm-modal{width:38em;max-width:100%;max-height:86%;overflow-y:auto;padding:1.6em;border-radius:var(--cm-radius);background:var(--cm-panel2);box-shadow:inset 0 0 0 1px rgba(255,255,255,.1);scrollbar-width:none}
        ...
        .cm-chip{padding:.6em .95em;margin:0 .45em .45em 0;border-radius:1em;font-size:.9em;cursor:pointer;background:var(--cm-chip);color:var(--cm-text);transition:background .15s,transform .15s,color .15s}
        ...
        .cm-toast{position:fixed;left:50%;bottom:1.8em;transform:translateX(-50%) translateY(1em);z-index:1000001;opacity:0;padding:.7em 1.2em;border-radius:.7em;background:var(--cm-panel2);color:var(--cm-text);font-size:.9em;box-shadow:inset 0 0 0 1px rgba(255,255,255,.12);transition:opacity .25s,transform .25s}
    ...
    // === 12. ИКОНКИ + РОБОТ (ПЕРЕРИСОВАН v14) ===
...
<linearGradient id="gA2" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#F8FAFF"/><stop offset="1" stop-color="#B9C6DC"/></linearGradient>
    ...
    const View = {
        ...
        // Обновить ТЕКУЩУЮ капсулу (нуар → нуар, вкусовая → вкусовая)
        refreshCurrent: () => {
            if (View.busy) return;
        ...
        render: () => {
            ...
            // сенсор: тап — смотреть, удержание — подробнее
            ...
            // трей ВНУТРИ карточки, под кнопками
        ...
        showFound: (label, list, kind, query) => {
            View.busy = false;
            if (!list.length) { notify('😕 Ничего не нашлось'); View.render(); return; }
    ...
    // === 16. КОМПАНЬОН (скомпанован по разделам) ===
    const Companion = {
        open: () => {
            ...
            Modal.open({
                ...
                text: 'Настроение — кнопками, остальное — по разделам ниже.',
                chips,
                items: [
                    { section: '🔍 ПОИСК' },
                    { label: '🔍 Сказать словами', hint: '«триллер про космос 90-х», «нуар детектив классика»', onSelect: () => Companion.ask() },
                    { section: '🔄 КАПСУЛА' },
                    { label: `🔄 Обновить текущую: ${View.sourceLabel || 'КАПСУЛА'}`, hint: 'пересобрать именно открытый список', onSelect: () => View.refreshCurrent() },
    ...
    // === 17. НАСТРОЙКИ (только внутри робота, без Lampa Settings) ===
    const Settings = {
    ...
    (() => {
        try {
            ...
            addMenu();
            console.log('[Капсула] v14.0 загружена');
        } catch (e) {
