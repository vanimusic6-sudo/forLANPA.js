/* --- CAPSULE MOD v5.1 (Perfect Build) --- */
(function () {
'use strict';
...
/* ============================== СТИЛИ (Приглушенная палитра v2) ============================== */
var CSS_TEXT = [
':root{--cm-bg:#0e0f10;--cm-panel:#17181a;--cm-panel2:#202224;--cm-accent:#6b7a85;--cm-accent-soft:#4d5862;--cm-text:#c9cacc;--cm-sub:#6f7377;--cm-danger:#8b5353;--cm-good:#527d66;}',
...
'.cm-header{position:absolute;top:0;left:0;right:0;height:4.2em;display:flex;align-items:center;padding:0 1.8em;background:linear-gradient(var(--cm-bg) 60%,rgba(14,15,16,0));z-index:20}',
'.cm-hbtn{width:2.5em;height:2.5em;display:flex;align-items:center;justify-content:center;border-radius:0.8em;cursor:pointer;background:var(--cm-panel);transition:background .12s,transform .12s;border:1px solid #2a2d30}',
'.cm-hbtn svg{width:1.2em;height:1.2em;fill:var(--cm-text)}',
'.cm-hbtn.cm-focus{background:var(--cm-accent-soft);transform:scale(1.05);border-color:transparent}',
'.cm-title{margin-left:1.2em;font-size:1.2em;font-weight:600;color:var(--cm-text);letter-spacing:0.3px}',
'.cm-status{margin-left:0.9em;font-size:0.8em;color:var(--cm-sub)}',
'.cm-clock{margin-left:auto;font-size:1.2em;font-weight:500;color:var(--cm-sub);font-variant-numeric:tabular-nums}',
'.cm-content{position:absolute;top:4.2em;bottom:0;left:0;right:0;overflow-y:auto;overflow-x:hidden;padding:0.5em 2em 8em}',
'.cm-row{margin-bottom:1.5em}',
'.cm-row-head{display:flex;align-items:baseline;gap:0.6em;margin:0.4em 0 0.6em 0.1em}',
'.cm-row-title{font-size:1.05em;color:var(--cm-text);font-weight:600}',
'.cm-row-sub{font-size:0.8em;color:var(--cm-sub)}',
'.cm-strip{display:flex;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;padding:0.6em 0.2em 0.8em;scrollbar-width:none}',
'.cm-strip::-webkit-scrollbar{display:none}',
'.cm-card{position:relative;flex:none;width:10.5em;height:15.5em;border-radius:0.8em;overflow:hidden;margin-right:1.1em;background:var(--cm-panel);border:1px solid #26282a;transition:transform .15s,border-color .15s;cursor:pointer}',
'.cm-card img{width:100%;height:100%;object-fit:cover;display:block;pointer-events:none;filter:saturate(0.85) contrast(0.95)}',
'.cm-card.cm-focus{border-color:var(--cm-accent);transform:scale(1.05);z-index:2;box-shadow:0 0.4em 1.2em rgba(0,0,0,0.4)}',
'.cm-b-type{position:absolute;top:0.5em;left:0.5em;background:rgba(255,255,255,0.8);color:#111;font-size:0.65em;font-weight:700;padding:0.15em 0.5em;border-radius:0.4em;letter-spacing:.02em}',
'.cm-b-type.tv{background:rgba(107,122,133,0.9);color:#fff}',
'.cm-b-rate{position:absolute;bottom:0.4em;right:0.4em;background:rgba(0,0,0,0.6);font-size:0.75em;font-weight:600;padding:0.1em 0.5em;border-radius:0.5em}',
'.cm-b-match{position:absolute;top:0.5em;right:0.5em;background:rgba(82,125,102,0.85);color:#fff;font-size:0.6em;font-weight:700;padding:0.14em 0.45em;border-radius:0.4em}',
'.cm-t{position:absolute;left:0;right:0;bottom:0;padding:1.8em 0.6em 0.5em 0.6em;font-size:0.75em;color:#d0d1d3;background:linear-gradient(transparent,rgba(0,0,0,0.85));white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
'.cm-robot-wrap{position:absolute;left:2.2em;bottom:1.4em;z-index:30;display:flex;align-items:flex-end;max-width:70%}',
'.cm-robot{width:4em;height:5em;flex:none;cursor:pointer;will-change:transform;animation:cm-float 3.8s ease-in-out infinite}',
'.cm-robot.cm-focus{filter:drop-shadow(0 0 0.3em var(--cm-accent));transform:scale(1.06)}',
'.cm-robot svg{width:100%;height:100%}',
'.cm-bubble{position:relative;background:var(--cm-panel);border-radius:0.9em;padding:0.7em 1em;margin-left:1.1em;max-width:20em;font-size:0.92em;color:var(--cm-text);line-height:1.4;box-shadow:0 0.2em 0.5em rgba(0,0,0,0.2)}',
...
'.cm-d-left{flex:none;width:16em}',
'.cm-d-left .cm-card{width:15em;height:22.5em;margin:0;cursor:default}',
'.cm-d-meta{margin-top:0.9em;color:var(--cm-sub);font-size:0.9em;line-height:1.6}',
...
'.cm-d-genre{background:var(--cm-panel2);color:var(--cm-sub);font-size:0.75em;padding:0.2em 0.6em;border-radius:0.8em}',
'.cm-d-center{flex:none;width:11em;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:0.9em;padding-top:1em}',
'.cm-play{width:100%;height:5.8em;background:var(--cm-panel2);border-radius:1em;border:1px solid #2a2d30;display:flex;align-items:center;justify-content:center;gap:0.5em;cursor:pointer;transition:transform .15s,border-color .15s;flex-direction:column}',
'.cm-play svg{width:2em;height:2em;fill:var(--cm-text)}',
'.cm-play span{font-size:0.75em;color:var(--cm-sub)}',
'.cm-play.cm-focus{border-color:var(--cm-accent);transform:scale(1.04)}',
'.cm-feedback{display:flex;gap:0.6em;width:100%}',
'.cm-fbtn{flex:1;height:2.8em;border-radius:0.8em;background:var(--cm-panel2);border:1px solid #2a2d30;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:1.1em;opacity:.7}',
'.cm-fbtn.cm-focus{border-color:var(--cm-accent);transform:scale(1.05);opacity:1}',
'.cm-fbtn.cm-active-like{background:rgba(82,125,102,0.2);opacity:1}',
'.cm-fbtn.cm-active-dislike{background:rgba(139,83,83,0.2);opacity:1}',
'.cm-d-right{flex:1;padding-top:0.2em;min-width:0}',
'.cm-ai-item{background:var(--cm-panel);border:1px solid #26282a;border-radius:0.8em;padding:0.7em 1em;margin-bottom:0.65em;font-size:0.93em;color:var(--cm-text);cursor:pointer;transition:border-color .12s,background .12s}',
'.cm-ai-item.cm-focus{border-color:var(--cm-accent);background:var(--cm-panel2)}',
'.cm-overlay{position:fixed;inset:0;background:rgba(10,11,12,0.9);z-index:100000;display:flex;align-items:center;justify-content:center;padding:2em}',
'.cm-panel{background:var(--cm-panel);border-radius:1.1em;padding:1.6em 1.9em;min-width:26em;max-width:92%;max-height:86%;overflow-y:auto;box-shadow:0 1em 3em rgba(0,0,0,0.6)}',
'.cm-panel h2{margin:0 0 1em 0;font-size:1.2em;font-weight:600}',
'.cm-set-item{background:var(--cm-panel2);border:1px solid #26282a;border-radius:0.8em;padding:0.7em 1em;margin-bottom:0.6em;font-size:1em;color:var(--cm-text);cursor:pointer;display:flex;justify-content:space-between;gap:1em}',
...
'.cm-modal-body{padding:0.2em;color:var(--cm-text);font-size:0.94em;line-height:1.5;max-height:24em;overflow:auto}',
'.cm-modal-body h4{color:var(--cm-text);margin:0.6em 0 0.35em;font-weight:600}',
'.cm-chip{display:inline-block;background:var(--cm-panel2);border:1px solid #2a2d30;border-radius:1.6em;padding:0.5em 1.1em;margin:0 0.5em 0.6em 0;color:var(--cm-text);cursor:pointer}',
'.cm-chip.cm-focus{border-color:var(--cm-accent);background:#26292c}',
'.cm-input-fake{background:var(--cm-panel2);border:1px dashed #3a3d40;border-radius:0.8em;padding:0.7em 1em;margin-bottom:0.85em;color:var(--cm-sub);cursor:pointer}',
...
'.cm-modal-close{position:absolute;top:1em;right:1em;width:2.2em;height:2.2em;border-radius:0.6em;background:var(--cm-panel2);display:flex;align-items:center;justify-content:center;cursor:pointer;border:1px solid #2a2d30}',
'.cm-modal-close.cm-focus{border-color:var(--cm-accent)}',
'.cm-modal-close svg{width:1.1em;height:1.1em;fill:var(--cm-sub)}',
'@media (max-width:768px){.cm-root{font-size:11px}.cm-header{height:3.6em;padding:0 1em}.cm-hbtn{width:2.1em;height:2.1em}.cm-hbtn svg{width:1.15em;height:1.15em}.cm-title{font-size:1.05em}.cm-clock{font-size:1em}.cm-content{padding:0.3em 1em 8em}.cm-robot-wrap{left:1em;bottom:0.7em;max-width:88%}.cm-robot{width:3.2em;height:3.9em}.cm-bubble{max-width:14em;font-size:0.85em}.cm-details{flex-direction:column;padding-bottom:4em;gap:1em}.cm-d-left{width:100%;display:flex;gap:1em;align-items:flex-start}.cm-d-left .cm-card{width:6.4em;height:9.6em}.cm-d-meta{margin-top:0}.cm-d-center{width:100%;flex-direction:row}.cm-play{height:4em}.cm-d-right{width:100%}}',
...
/* ============================== ИКОНКИ (Робот как на скриншоте) ============================== */
var SVG_ROBOT = '<svg viewBox="0 0 100 130" xmlns="http://www.w3.org/2000/svg"><g fill="none" stroke="#c9cacc" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">' +
'<circle cx="50" cy="20" r="14" fill="#c9cacc" stroke="none"/>' + // Голова
'<circle cx="44" cy="17" r="4.5" fill="#0e0f10" stroke="none"/>' + // Глаз
'<path d="M42 30 C42 30 50 36 58 30" />' + // Улыбка
'<path d="M46 56 C34 56 26 68 24 86" />' + // Левая рука
'<path d="M54 56 C66 56 74 68 76 86" />' + // Правая рука
'<path d="M40 96 C38 106 36 114 34 122" />' + // Левая нога
'<path d="M60 96 C62 106 64 114 66 122" />' + // Правая нога
'<ellipse cx="50" cy="76" rx="26" ry="34" fill="#c9cacc" stroke="none"/>' + // Тело
'</g></svg>';
...
function posterPlaceholder(title) {
    ...
    return 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="200" height="300"><rect width="200" height="300" fill="#17181a"/><rect x="8" y="8" width="184" height="284" rx="10" fill="none" stroke="#2a2c2f" stroke-width="2"/><text x="100" y="155" fill="#6b7a85" font-size="14" text-anchor="middle" font-family="sans-serif">' + t + '</text></svg>');
}

/* ============================== НАСТРОЙКИ (Через настройки Lampa) ============================== */
...
function readHistoryRaw() {
    ...
    function addItem(item, favFlag) {
        ...
        if (seen[key]) { if (favFlag) for (var i = 0; i < out.length; i++) if (out[i].id === id && out[i].type === type) out[i].favorite = true; return; }
        seen[key] = 1;
        out.push({ id: id, type: type, percent: percent, date: date, favorite: !!favFlag });
    }
    try {
        if (window.Lampa && Lampa.Timeline) {
            if (typeof Lampa.Timeline.view === 'function') { var tl = Lampa.Timeline.view(0, 300); if (tl && tl.forEach) tl.forEach(function (e) { addItem(e, false); }); }
            if (typeof Lampa.Timeline.all === 'function') { var all = Lampa.Timeline.all(); if (all) { for (var k1 in all) if (Object.prototype.hasOwnProperty.call(all, k1)) addItem(all[k1], false); } }
        }
...
/* ============================== ДВИЖОК РЕКОМЕНДАЦИЙ v2 (Точный отсев) ============================== */
var Engine = {
    ...
    classify: function (hist) {
        ...
        for (var i = 0; i < hist.length; i++) {
            ...
            else if (it.percent > 0) w = WEIGHTS.completed * 0.3;
            if (w === 0) continue;
    ...
    discover: function (profile, extra, cb) {
        ...
        function one(t) {
            ...
            p2['vote_count.gte'] = t === 'movie' ? 50 : 20; /* Убрали мусор */
    ...
    pick: function (list, profile, watched, seen, mode, n) {
        ...
        for (i = 0; i < list.length && out.length < n * 4; i++) {
            ...
            /* Жесткая анти-попса: если много голосов, но нет совпадения жанров */
            var voteCount = mm.vote_count || 0;
            var popPenalty = (voteCount > 1000 && gn < 2) ? 15 : 0;
            var popularity = Math.min(10, voteCount / 200);
            var popWeight = gn > 1 ? 0.05 : 0.0;
            var score;
            if (mode === 'stable') score = gn * 0.6 + rt * 0.3 + popularity * popWeight - popPenalty;
            else score = gn * 0.4 + rt * 0.2 + recency * 0.3 + popularity * popWeight - popPenalty + Math.random() * 0.5;
            if (manualScore > 0) score += 3;
            if (popPenalty > 0) score -= popPenalty * 0.2;
...
var state = { view: 'main', cards: {}, bundle: null, profile: null, custom: null, current: null, root: null, clockTimer: null, modalStackDepth: 0, controllerStack: [] };
...
function renderDetails(movie) {
    ...
    /* Смотреть - открывает источник (Торренты/Онлайн/Трейлеры) */
...
function openFullCard(m) {
    /* Открытие Lampa "Full" - это прямой запуск плеера с торрентами/онлайн/трейлерами */
    try {
        Lampa.Activity.push({
            url: '',
            component: 'full',
            id: m.id,
            method: m.type,
            card: { id: m.id, title: m.title, name: m.type === 'tv' ? m.title : undefined, poster_path: m.poster_path, release_date: m.release_date, first_air_date: m.type === 'tv' ? m.release_date : undefined, vote_average: m.vote_average, overview: m.overview, genre_ids: m.genre_ids || [], media_type: m.type },
            source: 'tmdb'
        });
    } catch (e) { safeLog('openFullCard', e); notify('Не удалось открыть карточку Lampa'); }
}

/* ============================== МОДАЛЬНЫЕ ОКНА (Исправленные) ============================== */
function modalOpen(title, node, opts) {
    ...
    /* Сохраняем текущий контроллер для правильного возврата */
    try { state.controllerStack.push(Lampa.Controller.current); } catch (e) { state.controllerStack.push(null); }
...
function modalClose() {
    ...
    FocusManager.pop();
    try {
        var prev = state.controllerStack.pop();
        if (prev) Lampa.Controller.toggle(prev);
        else Lampa.Controller.toggle(CTRL_NAME);
    } catch (e) { safeLog('modalClose controller', e); }
}
function closeAllModals() {
    while (state.modalStackDepth > 0) {
        modalClose();
    }
}

/* Собственное текстовое поле (Исправленный ввод запроса) */
function askText(title, placeholder, onSubmit) {
    ...
    function submit() {
        var v = input.value || '';
        /* Исправлено: убран modalClose() здесь, чтобы не ломать интерфейс при вводе */
...
function aiApplyFilters(title, f) {
    /* Закрываем ВСЕ модалки (и окно запроса, и окно диалога) */
    closeAllModals();
    robotSay(f.phrase || robotPhrase('loading'));
    skeletonRows(); setStatus('Подбираю…');
...
function doAction(node) {
    ...
    switch (a) {
        ...
        case 'ai_random_play': if (state.cards['rand']) { closeAllModals(); openFullCard(state.cards['rand']); } return;
        case 'ai_mood': {
            var body = el('div', 'cm-modal-body');
...
function startPlugin() {
    ...
    var boot = function () {
        ...
        setTimeout(function () {
            ...
            registerSettings();
            addMenuItem();
            console.log('[CapsuleMod] v5.1 Perfect Build загружен');
        }, 300);
    };
