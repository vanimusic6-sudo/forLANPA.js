...
(function () {
    ...
    const Capsule = {
        ...
        build: (taste, opts, cb) => {
            ...
            // Несколько независимых потоков дают широкий пул кандидатов.
    ...
    const THEMES = {
        ...
        noir: { name: 'Нуар', cls: 'cm-t-noir', sys: 'CASE #1947: OPEN', quotes: ['«Забудь её, Джейк»', '«В этом городе все врут»', '«Туман всё скроет»', '«Свет фонаря — единственная правда»'], load: ['ЛИСТАЮ ДЕЛО', 'ПРОВЕРЯЮ УЛИКИ', 'СОБИРАЮ КАПСУЛУ'], vars: { '--cm-bg': '#0B0B0B', '--cm-accent': '#E6E6E6', '--cm-accent2': '#B48A3C', '--cm-text': '#EDEDED', '--cm-sub': '#8A8A8A', '--cm-panel': 'rgba(20,20,20,.8)', '--cm-panel2': 'rgba(14,14,14,.68)', '--cm-chip': 'rgba(255,255,255,.06)', '--cm-radius': '.3em' } },
        inception: { name: 'Сон', cls: 'cm-t-inception', sys: 'LIMBO LEVEL: 01 // STABLE', quotes: ['«Как долго мы здесь?»', '«Ты ждёшь поезд»', '«Не бойся глубины»', '«Время здесь течёт иначе»'], load: ['ПОГРУЖАЮСЬ В СОН', 'НАСТРАИВАЮ ГЛУБИНУ'], vars: { '--cm-bg': '#11100F', '--cm-accent': '#D8B58A', '--cm-accent2': '#B98F68', '--cm-text': '#EEE5D8', '--cm-sub': '#8E8173', '--cm-panel': 'rgba(25,22,20,.62)', '--cm-panel2': 'rgba(15,14,13,.58)', '--cm-chip': 'rgba(216,181,138,.055)', '--cm-radius': '1em' } },
        dune: { name: 'Арракис', cls: 'cm-t-dune', sys: 'ARRAKIS // DEEP DESERT', quotes: ['«Страх — убийца разума»', '«Пустыня помнит всё»', '«Слушай ветер»', '«Следуй за ритмом песка»'], load: ['СЛУШАЮ ПЕСКИ', 'НАСТРАИВАЮ КОМПАС'], vars: { '--cm-bg': '#17100A', '--cm-accent': '#E7B46A', '--cm-accent2': '#8EC7B1', '--cm-text': '#F4E7D1', '--cm-sub': '#AA9274', '--cm-panel': 'rgba(34,23,13,.62)', '--cm-panel2': 'rgba(22,15,9,.55)', '--cm-chip': 'rgba(231,180,106,.06)', '--cm-radius': '1.05em' } },
        bladerunner: { name: 'Неон', cls: 'cm-t-blade', sys: 'LOS ANGELES // 2049', quotes: ['«Я видел то, во что вы не поверите»', '«Память — это тоже история»', '«Город никогда не спит»', '«Ищи свет в дожде»'], load: ['ВКЛЮЧАЮ НЕОН', 'СИНХРОНИЗИРУЮ ПАМЯТЬ'], vars: { '--cm-bg': '#090D16', '--cm-accent': '#F2B6FF', '--cm-accent2': '#74E5FF', '--cm-text': '#EEF7FF', '--cm-sub': '#788DA1', '--cm-panel': 'rgba(12,17,28,.66)', '--cm-panel2': 'rgba(8,12,20,.55)', '--cm-chip': 'rgba(116,229,255,.06)', '--cm-radius': '.75em' } },
        dreamworks: { name: 'Тихий вечер', cls: 'cm-t-dream', sys: 'SLOW EVENING // PLAY', quotes: ['«Сегодня можно никуда не спешить»', '«Выбери историю и выдохни»', '«Оставь шум за дверью»', '«Пусть фильм найдёт тебя»'], load: ['НАСТРАИВАЮ ТИШИНУ', 'СОБИРАЮ ВЕЧЕР'], vars: { '--cm-bg': '#080C16', '--cm-accent': '#B9C7FF', '--cm-accent2': '#A9E7D5', '--cm-text': '#EAF0FF', '--cm-sub': '#77839C', '--cm-panel': 'rgba(15,20,34,.56)', '--cm-panel2': 'rgba(9,13,23,.5)', '--cm-chip': 'rgba(185,199,255,.05)', '--cm-radius': '1.25em' } }
    };
    const THEME_ORDER = ['astro', 'breakingbad', 'matrix', 'panda', 'rickmorty', 'starwars', 'cyberpunk', 'noir', 'inception', 'dune', 'bladerunner', 'dreamworks'];
    const Themes = {
        ...
        fx: (key, root) => {
            ...
            else if (key === 'inception') Themes.rain(root, 'inception');
            else if (key === 'dune') Themes.rain(root, 'dune');
            else if (key === 'bladerunner') Themes.rain(root, 'blade');
            else if (key === 'dreamworks') Themes.rain(root, 'dream');
        },
        rain: (root, mode) => {
            ...
            let timer;
            if (mode === 'lab') {
                // Laboratory: warm glass bubbles slowly rise upward.
                const bubbles = Array.from({length: 24}, () => ({
                    x: .12 + Math.random()*.76, y: 1.05 + Math.random()*.8,
                    r: 3 + Math.random()*12, v: .08 + Math.random()*.18,
                ...
                timer=setInterval(()=>{
                    ...
                    bubbles.forEach(b=>{
                        b.y -= b.v/60;
                        b.x += Math.sin(t+b.wob)*.0007;
                        if(b.y < -.12){b.y=1.08;b.x=.12+Math.random()*.76;}
                        ctx.globalAlpha=.055;
                        ctx.strokeStyle='#B7E6B0';
                        ctx.lineWidth=1;
                        ctx.beginPath();ctx.arc(b.x*canvas.width,b.y*canvas.height,b.r,0,Math.PI*2);ctx.stroke();
                        ctx.globalAlpha=.025;
                        ctx.fillStyle='#E5F7D8';
                        ctx.beginPath();ctx.arc(b.x*canvas.width-b.r*.25,b.y*canvas.height-b.r*.25,b.r*.55,0,Math.PI*2);ctx.fill();
                    });
                    ctx.globalAlpha=1;
                },45);
            } else if (mode === 'scroll') {
                // Scroll: soft glyphs float upward; muted sakura drifts sideways.
                const glyphs = Array.from({length: 18}, (_,i)=>({
                    x: Math.random(), y: Math.random()*1.2, s: 11+Math.random()*18,
                    v:.018+Math.random()*.035, rot:(Math.random()-.5)*.4, a:Math.random()*6
                }));
                const petals = Array.from({length: 18}, ()=>({
                    x:Math.random(), y:Math.random(), r:2+Math.random()*4,
                    vx:(Math.random()-.5)*.006, vy:.004+Math.random()*.009, rot:Math.random()*6
                }));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    ...
                    glyphs.forEach(g=>{
                        g.y-=g.v/60;
                        g.x+=Math.sin(t+g.a)*.0008;
                        if(g.y<-.12){g.y=1.08;g.x=Math.random();}
                        ctx.globalAlpha=.025+.02*Math.sin(t+g.a);
                        ctx.fillStyle='#E7D7B0';
                        ctx.font=`${g.s}px serif`;
                        ctx.save();ctx.translate(g.x*canvas.width,g.y*canvas.height);ctx.rotate(g.rot);ctx.fillText('永',0,0);ctx.restore();
                    });
                    petals.forEach(p=>{
                        p.x+=p.vx; p.y+=p.vy;
                        if(p.y>1.08){p.y=-.05;p.x=Math.random();}
                        if(p.x<-.05)p.x=1.05;if(p.x>1.05)p.x=-.05;
                        ctx.globalAlpha=.035;
                        ctx.fillStyle='#D9A7A7';
                        ctx.save();ctx.translate(p.x*canvas.width,p.y*canvas.height);ctx.rotate(p.rot);
                        ctx.beginPath();ctx.ellipse(0,0,p.r,p.r*.58,.55,0,Math.PI*2);ctx.fill();ctx.restore();
                    });
                    ctx.globalAlpha=1;
                },55);
            } else if (mode === 'portal') {
                // Portal: random soft portals appear and transfer tiny furniture silhouettes between them.
                const portals = Array.from({length: 4}, (_,i)=>({
                    x:.15+Math.random()*.7,y:.2+Math.random()*.6,r:32+Math.random()*30,phase:Math.random()*6
                }));
                const props = Array.from({length: 7}, (_,i)=>({
                    from:i%4,to:(i+1)%4,p:Math.random(),v:.0025+Math.random()*.004
                }));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    ...
                    portals.forEach((p,i)=>{
                        const pulse=1+.08*Math.sin(t*1.4+p.phase);
                        ctx.globalAlpha=.055;
                        ctx.strokeStyle=i%2?'#9EE8FF':'#C6A7FF';
                        ctx.lineWidth=2;
                        ctx.beginPath();ctx.arc(p.x*canvas.width,p.y*canvas.height,p.r*pulse,0,Math.PI*2);ctx.stroke();
                        ctx.globalAlpha=.018;
                        ctx.beginPath();ctx.arc(p.x*canvas.width,p.y*canvas.height,p.r*pulse*.62,0,Math.PI*2);ctx.stroke();
                    });
                    props.forEach((o,i)=>{
                        o.p+=o.v;
                        if(o.p>1){o.p=0;o.from=Math.floor(Math.random()*4);o.to=(o.from+1+Math.floor(Math.random()*3))%4;}
                        const a=portals[o.from],b=portals[o.to],q=o.p;
                        const x=a.x+(b.x-a.x)*q,y=a.y+(b.y-a.y)*q-Math.sin(q*Math.PI)*.09;
                        ctx.globalAlpha=.045*(1-Math.abs(q-.5)*1.3);
                        ctx.fillStyle='#D7D0C5';
                        ctx.fillRect(x*canvas.width-5,y*canvas.height-3,10,6);
                        ctx.fillRect(x*canvas.width-3,(y+.025)*canvas.height,6,7);
                    });
                    ctx.globalAlpha=1;
                },45);
            } else if (mode === 'galaxy') {
                // Galaxy: sun/moon rotates according to local time.
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    const now=new Date(), h=now.getHours()+now.getMinutes()/60;
                    const angle=(h/24)*Math.PI*2-Math.PI/2;
                    const cx=canvas.width*.5, cy=canvas.height*.43;
                    const orbit=Math.min(canvas.width,canvas.height)*.31;
                    const x=cx+Math.cos(angle)*orbit, y=cy+Math.sin(angle)*orbit*.55;
                    const night=h<6||h>=18;
                    ctx.globalAlpha=.12;
                    ctx.strokeStyle=night?'#B9C7FF':'#E9C978';
                    ctx.beginPath();ctx.arc(cx,cy,orbit,0,Math.PI*2);ctx.stroke();
                    ctx.globalAlpha=.08;
                    ctx.fillStyle=night?'#C5D0FF':'#F4D27C';
                    ctx.beginPath();ctx.arc(x,y,night?18:22,0,Math.PI*2);ctx.fill();
                    if(night){
                        ctx.globalAlpha=.11;ctx.fillStyle='#E8ECFF';
                        ctx.beginPath();ctx.arc(x-4,y-3,15,0,Math.PI*2);ctx.fill();
                    }
                    ctx.globalAlpha=1;
                },1000);
            } else if (mode === 'noir') {
                // Noir: restrained rain + very light moving smoke.
                const drops=Array.from({length:28},()=>({x:Math.random(),y:Math.random(),v:1+Math.random()*1.5}));
                const smoke=Array.from({length:6},()=>({x:Math.random(),y:.55+Math.random()*.35,r:35+Math.random()*55,v:.003+Math.random()*.004}));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    drops.forEach(d=>{
                        d.y+=d.v/120;if(d.y>1.08){d.y=-.08;d.x=Math.random();}
                        ctx.globalAlpha=.018;ctx.strokeStyle='#C9C9C9';ctx.lineWidth=.7;
                        ctx.beginPath();ctx.moveTo(d.x*canvas.width,d.y*canvas.height);ctx.lineTo((d.x-.005)*canvas.width,(d.y+.045)*canvas.height);ctx.stroke();
                    });
                    smoke.forEach(s=>{
                        s.x+=s.v/100;if(s.x>1.2)s.x=-.2;
                        ctx.globalAlpha=.012;ctx.fillStyle='#B5B5B5';
                ...
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    const cx=canvas.width*.5,cy=canvas.height*.52,t=Date.now()/2400;
                    for(let i=0;i<9;i++){const r=(i+1)*Math.min(canvas.width,canvas.height)*.055*(1+.04*Math.sin(t+i));ctx.globalAlpha=.012+i*.0025;ctx.strokeStyle=i%2?'#B7E7FF':'#D7B56D';ctx.beginPath();ctx.ellipse(cx,cy,r,r*.48,Math.sin(t+i)*.08,0,Math.PI*2);ctx.stroke()}
                    ctx.globalAlpha=1;
                },80);
            } else if (mode === 'dune') {
                timer=setInterval(()=>{
                    ...
                    for(let i=0;i<8;i++){ctx.globalAlpha=.012+i*.0018;ctx.strokeStyle='#E7B46A';ctx.beginPath();for(let x=-20;x<canvas.width+20;x+=20){const y=canvas.height*(.35+i*.075)+Math.sin(x*.012+t+i)*12+i*2;x===-20?ctx.moveTo(x,y):ctx.lineTo(x,y)}ctx.stroke()}ctx.globalAlpha=1;
                },70);
            } else if (mode === 'blade') {
                const drops=Array.from({length:32},()=>({x:Math.random(),y:Math.random(),v:1+Math.random()*1.7}));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    drops.forEach(d=>{d.y+=d.v/120;if(d.y>1.1){d.y=-.1;d.x=Math.random()}ctx.globalAlpha=.018+.015*Math.random();ctx.strokeStyle=d.x>.5?'#74E5FF':'#F2B6FF';ctx.beginPath();ctx.moveTo(d.x*canvas.width,d.y*canvas.height);ctx.lineTo((d.x-.006)*canvas.width,(d.y+.035)*canvas.height);ctx.stroke()});ctx.globalAlpha=1;
                },55);
            } else if (mode === 'dream') {
                const clouds=Array.from({length:8},()=>({x:Math.random(),y:Math.random(),r:40+Math.random()*90,v:.02+Math.random()*.03}));
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
                    ctx.clearRect(0,0,canvas.width,canvas.height);
                    clouds.forEach(c=>{c.x+=c.v/100;if(c.x>1.2)c.x=-.2;ctx.globalAlpha=.012;ctx.fillStyle=c.y<.5?'#B9C7FF':'#A9E7D5';ctx.beginPath();ctx.arc(c.x*canvas.width,c.y*canvas.height,c.r,0,Math.PI*2);ctx.fill()});ctx.globalAlpha=1;
                },90);
            } else {
                timer=setInterval(()=>{
                    if(!canvas.parentNode){clearInterval(timer);return;}
    ...
    const CSS = `
        ...
        .cm-t-rm:after,.cm-t-sw:after,.cm-t-cp:after,.cm-t-noir:after,.cm-t-inception:after,.cm-t-dune:after,.cm-t-blade:after,.cm-t-dream:after{
        ...
        /* Inception — медленные концентрические слои и «сонный» дрейф */
        .cm-t-inception:after{
            background:
                radial-gradient(ellipse at 50% 52%,transparent 0 18%,rgba(183,231,255,.08) 28%,transparent 48%),
                repeating-radial-gradient(ellipse at 50% 52%,transparent 0 26px,rgba(215,181,109,.025) 27px 28px);
            animation:cm-dream-depth 18s ease-in-out infinite alternate;
        }
        @keyframes cm-dream-depth{from{transform:scale(1) translateY(1%)}to{transform:scale(1.12) translateY(-1%)}}

        /* Dune — песчаная дымка и медленные ветровые волны */
        .cm-t-dune:after{
            background:
                radial-gradient(65% 35% at 20% 75%,rgba(231,180,106,.12),transparent 72%),
                radial-gradient(55% 30% at 78% 30%,rgba(142,199,177,.07),transparent 70%),
                repeating-linear-gradient(168deg,transparent 0 34px,rgba(231,180,106,.018) 35px 37px);
            animation:cm-sand 22s ease-in-out infinite alternate;
        }
        @keyframes cm-sand{from{transform:translateX(-2%) scale(1.02)}to{transform:translateX(2%) scale(1.08)}}

        /* Blade Runner — дождь, отражения и мягкий неон */
        .cm-t-blade:after{
            background:
                repeating-linear-gradient(105deg,transparent 0 28px,rgba(116,229,255,.022) 29px 30px),
                linear-gradient(115deg,transparent 28%,rgba(242,182,255,.055) 48%,transparent 65%);
            animation:cm-rain-city 8s linear infinite;
            mix-blend-mode:screen;
        }
        @keyframes cm-rain-city{from{background-position:0 0,0 0}to{background-position:90px 170px,-70px 0}}

        /* Тихий вечер — почти невидимые облака, без визуального шума */
        .cm-t-dream:after{
            background:
                radial-gradient(40% 35% at 20% 25%,rgba(185,199,255,.08),transparent 70%),
                radial-gradient(45% 40% at 80% 70%,rgba(169,231,213,.065),transparent 72%);
            filter:blur(8px);
            animation:cm-evening 20s ease-in-out infinite alternate;
        }
        @keyframes cm-evening{from{transform:translate3d(-1%,-1%,0) scale(1)}to{transform:translate3d(2%,2%,0) scale(1.08)}}
        ...
        .cm-port,.cm-bar,.cm-act,.cm-mchip,.cm-gchip{transition:transform .45s cubic-bezier(.22,.61,.36,1),opacity .45s ease,background-color .45s ease,border-color .45s ease}
        .cm-act:hover{transform:translateY(-1px)}
        .cm-port{will-change:transform}
        @media(max-width:700px){.cm-port{transform:translateY(0)}.cm-act:active{transform:scale(.985)}}

        /* === IMMERSION / CALM MODE === */
        .cm-sysline{opacity:.22;font-size:.56em;letter-spacing:.16em}
        .cm-root:before{opacity:.45;filter:blur(34px);animation-duration:24s}
        .cm-rain{opacity:.18}
        .cm-shade{
            background:
                linear-gradient(180deg,rgba(0,0,0,.08),transparent 32%,transparent 68%,rgba(0,0,0,.48)),
                radial-gradient(90% 80% at 50% 45%,rgba(0,0,0,.06),rgba(0,0,0,.62) 88%,var(--cm-bg) 100%);
        }
        .cm-stage{gap:.7em;padding:3.6em 1.1em 1.25em}
        .cm-port{
            max-width:64em;padding:1em;gap:1.35em;
            background:rgba(255,255,255,.045);border-color:rgba(255,255,255,.075);
            box-shadow:0 1.4em 3.5em rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.055);
            backdrop-filter:blur(24px) saturate(110%);
        }
        .cm-poster{width:13.5em;box-shadow:0 1em 2.4em rgba(0,0,0,.44),0 0 0 1px rgba(255,255,255,.09)}
        .cm-hero{padding:.1em}
        .cm-meta{gap:.35em;margin-bottom:.72em}
        .cm-mchip{padding:.34em .62em;font-size:.65em;opacity:.78;border-color:rgba(255,255,255,.075);box-shadow:none}
        .cm-mchip.src{opacity:.95}
        .cm-name{font-size:clamp(1.5rem,3vw,2.25rem);margin-bottom:.45em;letter-spacing:-.035em}
        .cm-ref{margin:-.05em 0 .55em;padding:.25em .55em;font-size:.6em;opacity:.68;border-left-width:1px;animation-duration:8s}
        .cm-genres{gap:.35em;margin-bottom:.55em}
        .cm-gchip{padding:.27em .62em;font-size:.64em;border-color:color-mix(in srgb,var(--cm-accent) 38%,transparent);background:rgba(255,255,255,.025);opacity:.82}
        .cm-why{padding:.48em .65em .48em .85em;margin-bottom:.55em;font-size:.76em;background:rgba(255,255,255,.025);border-color:rgba(255,255,255,.05);opacity:.8}
        .cm-over{font-size:.8em;line-height:1.55;margin-bottom:.7em;max-width:48em;color:color-mix(in srgb,var(--cm-sub) 92%,white)}
        .cm-acts{gap:.45em}
        .cm-act{min-height:2.7em;padding:.72em 1em;font-size:.82em;border-color:rgba(255,255,255,.075);box-shadow:none}
        .cm-act.primary{box-shadow:0 .4em 1.1em color-mix(in srgb,var(--cm-accent) 14%,transparent)}
        .cm-bar{
            max-width:64em;padding:.48em .62em;border-radius:1.15em;background:rgba(8,10,16,.4);
            border-color:rgba(255,255,255,.065);box-shadow:0 .7em 1.8em rgba(0,0,0,.18);
        }
        .cm-bar-btn{font-size:.76em;min-height:2.35em;opacity:.76}
        .cm-bar-btn.center{font-size:.8em;opacity:.92}
        .cm-modal{background:rgba(11,14,21,.86);border-color:rgba(255,255,255,.09);box-shadow:0 1.5em 4em rgba(0,0,0,.42)}
        .cm-opt{background:rgba(255,255,255,.035);border-color:rgba(255,255,255,.06)}
        .cm-onb{max-width:58em}
        .cm-onb-head{opacity:.62}
        .cm-onb-sub{opacity:.78}
        @media(max-width:700px){
            .cm-stage{justify-content:flex-start;padding:1.25em .55em .7em;gap:.45em;transform:translateY(-7vh)}
            .cm-port{margin-top:auto;margin-bottom:0;display:grid;grid-template-columns:7.4em 1fr;gap:.85em;padding:.65em;align-items:center}
            .cm-poster{width:7.4em;grid-row:1}
            .cm-hero{grid-column:2}
            .cm-name{font-size:clamp(1.25rem,6.2vw,1.7rem);line-height:1.08}
            .cm-over{-webkit-line-clamp:3;font-size:.75em}
            .cm-why{font-size:.7em}
            .cm-ref{max-width:95%}
            .cm-genres{display:none}
            .cm-acts{margin-top:.45em}
            .cm-act{min-height:2.55em;padding:.62em .75em;font-size:.78em}
            .cm-bar{margin-top:.35em}
            .cm-bar-btn{padding:.52em .48em}
            .cm-bar-btn .lbl{display:none}
            .cm-bar-btn.center .lbl{display:inline}
            .cm-rate{font-size:.7em;padding:.3em .5em}
        }
        @media(max-width:420px){
            .cm-port{grid-template-columns:6.4em 1fr}
            .cm-poster{width:6.4em}
            .cm-meta{margin-bottom:.4em}
            .cm-over{-webkit-line-clamp:2}
            .cm-acts .secondary{display:none}
            .cm-act.primary{flex:1}
            .cm-stage{padding-left:.4em;padding-right:.4em;transform:translateY(-5vh)}
        }
    ...
    const View = {
        ...
        render: () => {
            ...
            const chipSrc = el('div', 'cm-mchip src cm-mono', 'Капсула · ' + (View.idx + 1) + ' / ' + View.list.length);
    ...
    const CapsuleComponent = function () {
        ...
        this.destroy = () => {
            ...
            if (rain?._cmResize) window.removeEventListener('resize', rain._cmResize);
