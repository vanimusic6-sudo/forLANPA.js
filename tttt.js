(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-label{font-family:' + DISPLAY + ';font-size:2.2em;font-weight:800;color:#878c91;letter-spacing:.01em;text-align:center;white-space:nowrap;clip-path:inset(0 100% 0 0);transition:clip-path .95s cubic-bezier(.33,.65,.3,1)}' +
                '.sw-label.reveal{clip-path:inset(0 0 0 0)}' +
                ...
                '.sw-dice.sw-rolling{animation:swRoll .95s cubic-bezier(.33,.65,.3,1) forwards}' +
                '@keyframes swRoll{0%{transform:translateX(0) rotate(0) translateY(0)}20%{transform:translateX(calc(var(--sw-dist) * .2)) rotate(144deg) translateY(-10px)}45%{transform:translateX(calc(var(--sw-dist) * .45)) rotate(324deg) translateY(0)}70%{transform:translateX(calc(var(--sw-dist) * .7)) rotate(504deg) translateY(-7px)}100%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0)}}' +
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            var familyOK;
            if (isAnim) {
                var definitelyKids = (age !== null && age <= 6) && !hardAdult && cSuic < 40 && cFear < 40 && cViol < 30;
                familyOK = definitelyKids || (!hardAdult && cSuic < 40 && cProf < 40 && (age === null || age <= 12) && (hasFamilyGenre || kidsKw));
            } else {
            ...
            else if (rt.total >= 2 && rt.tone === 'mix') add('reviews','pro','💬 ' + who + ' разделились во мнениях', 10, 'rev');
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){
            try {
                ...
                dice.style.transition = 'transform .5s cubic-bezier(.3,1.3,.4,1)';
                dice.style.transform = 'translateX(0) rotate(0deg)';
                setTimeout(function(){ dice.style.transition = ''; dice.style.transform = ''; }, 550);
            } catch(e) {}
        });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 950, frame = 0;
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v29.0 (eye logo, reveal anim, fixed back)');
    }
