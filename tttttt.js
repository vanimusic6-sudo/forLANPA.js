(function () {
    ...
    function injectCSS() {
        try {
            ...
            s.innerHTML =
                ...
                '.sw-label{font-family:' + DISPLAY + ';font-size:2.2em;font-weight:800;color:#878c91;letter-spacing:.01em;text-align:center;white-space:nowrap;opacity:0;transform:translateY(10px);transition:opacity .5s ease,transform .5s cubic-bezier(.25,.46,.45,.94)}' +
                '.sw-label.reveal{opacity:1;transform:translateY(0)}' +
                ...
                '.sw-dice.sw-rolling{animation:swRoll 1.2s cubic-bezier(.34,1.56,.64,1) forwards}' +
                '@keyframes swRoll{0%{transform:translateX(0) rotate(0) translateY(0)}10%{transform:translateX(calc(var(--sw-dist) * .1)) rotate(72deg) translateY(-14px)}25%{transform:translateX(calc(var(--sw-dist) * .25)) rotate(180deg) translateY(0)}40%{transform:translateX(calc(var(--sw-dist) * .4)) rotate(288deg) translateY(-10px)}55%{transform:translateX(calc(var(--sw-dist) * .55)) rotate(396deg) translateY(0)}70%{transform:translateX(calc(var(--sw-dist) * .7)) rotate(504deg) translateY(-6px)}85%{transform:translateX(calc(var(--sw-dist) * .85)) rotate(612deg) translateY(0)}100%{transform:translateX(var(--sw-dist)) rotate(720deg) translateY(0)}}' +
                ...
                '@media(max-width:640px){.sw-modal-content{padding:16px 16px 32px}.sw-columns{grid-template-columns:1fr}.sw-verdict-word{font-size:2.1em}.sw-label{font-size:1.5em}.sw-label-wrap{padding:0 88px}.sw-dice{width:60px;height:60px;margin-top:-30px;left:12px}}';
    ...
    function findSparkle(meta, credits, genres) {
        var sparks = [];
        var interestingTags = [
            ...
            { re: /identity crisis|кризис идентичност/i, text: '🎭 Кризис идентичности' },
            ...
            { re: /political|политическ/i, text: '🏛 Политическая драма' },
            { re: /legal|courtroom|судебн/i, text: '⚖️ Судебная драма' },
            { re: /medical|hospital drama|медицинск/i, text: '🏥 Медицинская драма' },
        ...
        interestingTags.forEach(function(t){
            if (texts.some(function(txt){ return t.re.test(txt || ''); })) {
                sparks.push(t.text);
            }
        });
        if (credits && credits.crew) {
            ...
            if (dirs.length === 1) {
                var dirName = dirs[0].name;
                if (dirName) sparks.push('🎬 Режиссёр: ' + dirName);
            }
        }
    ...
    function analyze(movie) {
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (isAnim) {
                if (age !== null && age <= 12) {
                    familyOK = !hardAdult && cSuic < 40 && cFear < 40 && cViol < 30;
                } else if (age !== null && age >= 16) {
                    familyOK = false;
                } else {
                    familyOK = !hardAdult && cSuic < 40 && cProf < 40 && (hasFamilyGenre || kidsKw);
                }
            } else {
            ...
            var sparkles = findSparkle(meta, credits, genres);
            sparkles.forEach(function(s){ add('sparkle','pro',s,25); });

            if (votes >= 3000 && adj >= 8.0) add('card','pro','⭐ Высокий рейтинг ' + rating.toFixed(1) + ' (' + fmtN(votes) + ')', 30, 'ratehi');
            else if (votes >= 500 && adj >= cfg.min_rating) add('card','pro','⭐ Рейтинг ' + rating.toFixed(1) + ' — выше порога ' + cfg.min_rating.toFixed(1), 20, 'ratehi');
            else if (votes >= 100 && adj >= cfg.min_rating) add('card','pro','⭐ Рейтинг ' + rating.toFixed(1) + ' (' + fmtN(votes) + ')', 14, 'ratehi');
            if (votes > 0 && votes < 1500 && adj >= 7.6) add('card','pro','🔎 Скрытая жемчужина', 12);
            if (votes >= 2000 && adj >= 7.8 && yr > 0 && yr <= now - 5) add('card','pro','🏛 Классика', 12);
            if (yr === now) add('card','pro','🆕 Новинка ' + yr, 6, 'fresh');
            if (votes > 0 && votes < 100) add('card','con','❓ Мало оценок (' + votes + ')', 14, 'rateunc');
            if (votes === 0) add('card','con','⚠️ Нет оценок', 16, 'rateunc');
            if (votes >= 100 && adj < cfg.min_rating) add('card','con','📉 Рейтинг ' + rating.toFixed(1) + ' ниже порога', 22, 'ratelow');
            else if (votes >= 50 && rating > 0 && rating < 5) add('card','con','📉 Низкий рейтинг ' + rating.toFixed(1), 26, 'ratelow');
            if (runtime > 0 && runtime <= 95) add('card','pro','🕐 ' + runtime + ' мин', 7, 'runtime');
            else if (runtime > 180) add('card','con','⌛ ' + runtime + ' мин', 12, 'runtime');
            else if (runtime > 150) add('card','con','⌛ ' + runtime + ' мин', 8, 'runtime');
            ...
            else if (q) add('card','pro','🎥 ' + q, 7, 'quality');

            if (mG.length) add('user','con','⛔ ' + mG.join(', '), 40);
            if (mA.length) add('user','con','⛔ ' + uniq(mA).slice(0,2).join(', '), 35);
            if (mD.length) add('user','con','⛔ ' + uniq(mD).slice(0,2).join(', '), 35);
            ...
            if (age !== null && age >= 18) add('tmdb','con','🔞 18+', 14, 'age');
            else if (age !== null && age >= 16) add('tmdb','con','🔞 16+', 12, 'age');
            if (isAnim && !familyOK) add('tmdb','con','🎭 Взрослая анимация' + (age !== null ? ' (' + age + '+)' : ''), 14, 'adultanim');

            if (cViol >= 50) add('tmdb','con','🔪 Насилие', 16, 'violence');
            if (cDrugs >= 30) add('tmdb','con','💉 Наркотики', 14, 'drugs');
            if (cSex >= 30) add('tmdb','con','🫣 Нагота', 12, 'sex');
            if (cProf >= 40) add('tmdb','con','🤬 Мат', 10, 'lang');
            if (cFear >= 40) add('moods','con','😱 Страшно', 14, 'scare');
            else if ((mm.fear||0) >= 3.5) add('moods','pro','😬 Напряжённо', 9, 'tension');
            if (cSuic >= 40) add('tmdb','con','⚠️ Суицид', 16, 'suic');
            if (cAlc >= 40) add('tmdb','con','🍺 Алкоголь', 6, 'alc');
            ...
            if (dIll.k || dIll.o) add('tmdb','con','🏥 Болезнь', 6, 'ill');
            if (dWar.k || dWar.o) add('tmdb','con','🎖 Война', 8, 'war');
            if (dDarkCom) add('tmdb','pro','🖤 Чёрный юмор', 8, 'darkcom');
            if (meta.hasTrailer) add('tmdb','pro','▶ Трейлер', 5);
            if (hasGenre(genres, /documentary|документ/i)) add('tmdb','pro','🦉 Документальный', 8);

            if (mm.pace >= 6.5) add('moods','pro','⚡ Динамично', 12);
            else if (mm.pace > 0 && mm.pace <= 2.5 && runtime > 120) add('moods','con','🐢 Медленно', 8);
            if (mm.action >= 6) add('moods','pro','💥 Экшен', 12);
            if (mm.sadness >= 6) add('moods','pro','😢 Трогательно', 10, 'sad');
            moods.forEach(function(md){
                var n = (md.name || '').toLowerCase();
                if (/вес[её]л|комедий|юмор/.test(n) && md.pct >= 20) add('moods','pro','😂 Весело', 14, 'fun');
                else if (/напряжен/.test(n) && md.pct >= 30) add('moods','pro','🔥 Напряжённо', 8, 'tension');
                else if (/тревож/.test(n) && md.pct >= 40) add('moods','pro','😰 Тревожно', 6, 'tension');
                else if (/задумчив|драматич/.test(n) && md.pct >= 25) add('moods','pro','🎭 Глубоко', 8);
                else if (/романтич/.test(n) && md.pct >= 18) add('moods','pro','❤️ Романтично', 8);
                else if (/загадоч/.test(n) && md.pct >= 15) add('moods','pro','🕵️ Загадочно', 8);
                else if (/ностальг/.test(n) && md.pct >= 8) add('moods','pro','🕰 Ностальгия', 6);
                else if (/груст/.test(n) && md.pct >= 20) add('moods','con','😔 Грустно', 6, 'sad');
            });
            ...
            if (local.viewedPercent >= 90) add('lampa','con','👁 Досмотрено ' + local.viewedPercent + '%', 8, 'lampa');
    ...
    function buildReadyInner(a) {
        ...
        return '' +
            ...
            '<div class="sw-columns">' +
                '<div class="sw-col pros"><div class="sw-title pros">✓ За</div><ul class="sw-list">' + a.pros.map(function(p){ return '<li>' + esc(p) + '</li>'; }).join('') + '</ul></div>' +
                '<div class="sw-col cons"><div class="sw-title cons">✗ Против</div><ul class="sw-list">' + a.cons.map(function(c){ return '<li>' + esc(c) + '</li>'; }).join('') + '</ul></div>' +
            '</div>';
    }
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(){
            try {
                ...
                dice.style.transition = 'transform .7s cubic-bezier(.34,1.56,.64,1)';
                dice.style.transform = 'translateX(0) rotate(0deg)';
                setTimeout(function(){ dice.style.transition = ''; dice.style.transform = ''; }, 750);
            } catch(e) {}
        });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var t0 = Date.now(), DUR = 1200, frame = 0;
                var iv = setInterval(function(){
                    ...
                    if (p >= 1) {
                        ...
                        setTimeout(function(){
                            label.className = 'sw-label reveal ' + (final === 'СМОТРЕТЬ' ? 'res-yes' : 'res-no');
                        }, 200);
                        window._sw_rolling = false;
                    }
                }, 50);
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v31.0 (final)');
    }
