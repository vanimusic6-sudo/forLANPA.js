(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-dice{position:absolute;left:18px;top:50%;width:64px;height:64px;margin-top:-32px;will-change:transform}' +
            ...
            '.sw-dice.sw-run{animation:swRun 1.6s cubic-bezier(.3,.7,.3,1) forwards}' +
            '.sw-dice.sw-run::after{animation:swRunShadow 1.6s linear forwards}' +
            '@keyframes swRun{0%{transform:translateX(0) rotate(0) scale(1,1)}10%{transform:translateX(calc(var(--sw-dist)*.4)) rotate(120deg) scale(1.04,.96)}22%{transform:translateX(var(--sw-dist)) rotate(240deg) scale(1,1)}34%{transform:translateX(var(--sw-dist)) translateY(-16px) rotate(360deg) scale(.94,1.06)}46%{transform:translateX(var(--sw-dist)) translateY(0) rotate(480deg) scale(1.08,.92)}58%{transform:translateX(var(--sw-dist)) translateY(-8px) rotate(600deg) scale(.97,1.03)}70%{transform:translateX(var(--sw-dist)) translateY(0) rotate(720deg) scale(1.02,.98)}80%{transform:translateX(var(--sw-dist)) rotate(720deg) scale(1,1)}90%{transform:translateX(calc(var(--sw-dist)*.35)) rotate(720deg) scale(1,1)}100%{transform:translateX(0) rotate(720deg) scale(1,1)}}' +
            '@keyframes swRunShadow{0%,22%,80%,100%{transform:scale(1);opacity:.6}10%{transform:scale(.8);opacity:.4}34%{transform:scale(.5);opacity:.2}46%{transform:scale(1.05);opacity:.65}58%{transform:scale(.75);opacity:.35}70%{transform:scale(1.02);opacity:.6}}' +
            '@media(prefers-reduced-motion:reduce){.sw-dice.sw-run{animation:none !important}.sw-dice.sw-run::after{animation:none !important}.sw-dicebtn{transition:none !important}}' +
    ...
    function bindDice(html) {
        ...
        $(btn).on('hover:enter click keydown', function(e){
            ...
            _pendingFinal = { text: final, cls: (final === 'смотреть' ? 'res-yes' : 'res-no') };
            label.classList.remove('show');
            var dist = Math.max(0, Math.round(btn.clientWidth / 2 - 50));
            dice.style.setProperty('--sw-dist', dist + 'px');
            btnJ.addClass('rolling');
            dice.classList.add('sw-run');
            setTimeout(function(){ setFace(dice, rndFace()); }, 1150);
            setTimeout(function(){
                dice.classList.remove('sw-run');
                btnJ.removeClass('rolling');
                if (_pendingFinal) { label.textContent = _pendingFinal.text; label.className = 'sw-label show ' + _pendingFinal.cls; _pendingFinal = null; }
                window._sw_rolling = false;
            }, 1650);
        });
    }
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v51.0 (single-run dice, calm entrance)');
    }
