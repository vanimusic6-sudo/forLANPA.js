(function () {
    ...
    var SETTINGS_FLAG = 'sw_settings_ready_v73';
    ...
    var AXE_SVG = '<svg viewBox="0 0 140 100" xmlns="http://www.w3.org/2000/svg">' +
        ...
        '</svg>';
    var PORTRAIT_SVG = '<svg viewBox="0 0 150 100" xmlns="http://www.w3.org/2000/svg">' +
        '<g transform="rotate(-8 45 40)">' +
        '<rect x="20" y="6" width="46" height="60" fill="#7a1f2b" stroke="#8a5a1f" stroke-width="4"/>' +
        '<path d="M31 34 q0 -12 12 -12 q12 0 12 12 l0 6 q0 4 -3 6 l-18 0 q-3 -2 -3 -6 z" fill="#8a5a2f"/>' +
        '<circle cx="43" cy="32" r="9" fill="#e8c39a"/>' +
        '<circle cx="40" cy="31" r="1.6" fill="#0d0f0c"/><circle cx="46" cy="31" r="1.6" fill="#0d0f0c"/>' +
        '<path d="M30 66 q2 -14 13 -14 q11 0 13 14 z" fill="#2f7d46"/>' +
        '</g>' +
        '<rect x="78" y="26" width="42" height="46" fill="#0d0f0c" stroke="#3a3a10" stroke-width="3" transform="rotate(4 99 49)"/>' +
        '</svg>';
    var LAUGH_SVG = '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">' +
        '<circle cx="50" cy="52" r="40" fill="#f5c31c"/>' +
        '<circle cx="38" cy="42" r="9" fill="#0d0f0c"/><circle cx="41" cy="39" r="3" fill="#fff"/>' +
        '<circle cx="62" cy="42" r="9" fill="#0d0f0c"/><circle cx="65" cy="39" r="3" fill="#fff"/>' +
        '<path d="M32 64 q18 12 36 0 l-3 7 q-15 9 -30 0 z" fill="#fff" stroke="#0d0f0c" stroke-width="3"/>' +
        '<g fill="#7ec2e8"><circle cx="30" cy="52" r="3"/><circle cx="28" cy="60" r="3"/><circle cx="30" cy="68" r="3"/><circle cx="70" cy="50" r="3"/><circle cx="73" cy="57" r="3"/></g>' +
        '</svg>';
    var CAMERA_SVG = '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">' +
        '<g transform="rotate(-12 50 50)">' +
        '<rect x="18" y="26" width="62" height="50" rx="12" fill="#26282b"/>' +
        '<circle cx="49" cy="51" r="15" fill="#0d0f0c"/>' +
        '<rect x="66" y="18" width="9" height="9" fill="#0d0f0c"/>' +
        '</g></svg>';

    /* ===== РЕГИСТР ПЛАШЕК (кидай ещё — добавлю) ===== */
    var PLAQUES = {
        fear:      { word: 'страх',    color: '#8a0f1e', icon: AXE_SVG },
        war:       { word: 'война',    color: '#0d0f0c', cls: 'sw-plaque--war' },
        humor:     { word: 'юмор',     color: '#f5c31c', icon: LAUGH_SVG, left: true },
        genre:     { word: 'жанр',     sub: 'нелюбимый', color: '#8a8d92', icon: PORTRAIT_SVG },
        truestory: { word: 'события',  sub: 'реальные', color: '#0d0f0c', cls: 'sw-plaque--gray', icon: CAMERA_SVG },
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-plaque-icon.left{left:14px;right:auto;top:-30px}' +
            '.sw-plaque--war{background:radial-gradient(60px 40px at 15% 25%,#5a6b4a 55%,transparent 56%),radial-gradient(70px 45px at 70% 18%,#5a6b4a 55%,transparent 56%),radial-gradient(60px 40px at 38% 72%,#5a6b4a 55%,transparent 56%),radial-gradient(70px 45px at 85% 68%,#5a6b4a 55%,transparent 56%),radial-gradient(50px 35px at 55% 45%,#8a9a3a 55%,transparent 56%),#3a4226;border-color:#141808}' +
            '.sw-plaque--gray{background:#4a4750;border-color:#141414}' +
    ...
    function plaqueHtml(p, i) {
        return '<div class="sw-plaque ' + (p.cls || '') + '" style="animation-delay:' + (i * 0.07).toFixed(2) + 's">' +
            (p.icon ? '<div class="sw-plaque-icon ' + (p.left ? 'left' : '') + '">' + p.icon + '</div>' : '') +
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v73.0 (custom plaque art)');
    }
