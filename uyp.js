(function () {
    ...
    function injectCSS() {
        if (document.getElementById('sw-plugin-styles-v58')) return;
        ['sw-plugin-styles-v51', 'sw-plugin-styles-v52', 'sw-plugin-styles-v53', 'sw-plugin-styles-v54', 'sw-plugin-styles-v55', 'sw-plugin-styles-v56', 'sw-plugin-styles-v57'].forEach(function(id){
            var old = document.getElementById(id);
            if (old && old.parentNode) old.parentNode.removeChild(old);
        });
        var s = document.createElement('style');
        s.id = 'sw-plugin-styles-v58';
        s.innerHTML =
            '.sw-custom-button-enhanced{cursor:pointer;position:relative;transition:background .25s ease,color .25s,transform .2s ease;display:inline-flex;align-items:center;justify-content:center;border-radius:12px;user-select:none;overflow:hidden}' +
            '.sw-custom-button-enhanced::after{content:"";position:absolute;left:10%;right:10%;bottom:2px;height:0;background:#fff;border-radius:2px;transition:height .25s ease,opacity .25s ease;opacity:0}' +
            '.sw-custom-button-enhanced:hover{background:#2a2f34;color:#fff}' +
            '.sw-custom-button-enhanced:hover svg path{fill:#fff}' +
            '.sw-custom-button-enhanced.focus{background:#343a40!important;color:#fff!important;transform:scale(1.02)}' +
            '.sw-custom-button-enhanced.focus::after{height:3px;opacity:1}' +
            ...
            '.sw-dicebtn::after{content:"";position:absolute;left:10%;right:10%;bottom:3px;height:0;background:#fff;border-radius:2px;transition:height .25s ease,opacity .25s ease;opacity:0}' +
            '.sw-dicebtn:hover{background:#2b3136}' +
            '.sw-dicebtn.focus{background:#2f353a!important}' +
            '.sw-dicebtn.focus::after{height:3px;opacity:1}' +
            ...
            '.sw-tab-btn{position:relative;min-height:48px;padding:10px 8px;display:flex;align-items:center;justify-content:center;gap:8px;background:#1e2226;border:1px solid transparent;outline:none;border-radius:10px;cursor:pointer;color:#c7cdd2;font-family:var(--sw-mono);font-size:0.95em;font-weight:700;text-transform:lowercase;letter-spacing:.02em;opacity:.7;transition:opacity .18s,background .18s ease,border-color .18s;user-select:none;overflow:hidden}' +
            '.sw-tab-btn::after{content:"";position:absolute;left:10%;right:10%;bottom:2px;height:0;background:#fff;border-radius:2px;transition:height .25s ease,opacity .25s ease;opacity:0}' +
            '.sw-tab-btn:hover{opacity:.9;background:#22272b}' +
            '.sw-tab-btn.active{opacity:1;color:#fff;background:#262b30;border-color:#e9ecef}' +
            '.sw-tab-btn.focus{opacity:1;background:#343a40!important;color:#fff!important}' +
            '.sw-tab-btn.focus::after{height:3px;opacity:1}' +
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (gWar) add('tmdb','con','🎖 Военный фильм', 6, 'war');
            ...
            else if (score > 15) { vClass = 'yes'; vWord = 'СТОИТ'; }
            ...
            var kwSuspense = hasKw(ctx, /suspense|tension|thriller|напряжен|интриг|неожиданн поворот/i);
            ...
            if (mm.pace > 0) mPace = (mPace * 0.2) + (mm.pace * 0.8);
            if (mm.fear > 0) mFear = (mFear * 0.2) + (mm.fear * 0.8);
            if (mm.sadness > 0) mDrama = (mDrama * 0.2) + (mm.sadness * 0.8);
            if (mm.action > 0) mImmersion = (mImmersion * 0.3) + (mm.action * 0.7);

            moods.forEach(function(md) {
                var pct = md.pct || 0;
                var n = (md.name || '').toLowerCase();
                if (/вес[её]л|комедий|юмор|смешн/.test(n)) { mPace += (pct / 15); mImmersion += (pct / 20); }
                if (/напряжен|саспенс|тревож|страх|ужас/.test(n)) { mFear += (pct / 6); mPace += (pct / 25); }
                if (/задумчив|драматич|груст|трогательн/.test(n)) { mDrama += (pct / 10); }
                if (/романтич|любов/.test(n)) { mDrama += (pct / 15); }
                if (/загадоч|тайн|детектив/.test(n)) { mImmersion += (pct / 15); mFear += (pct / 12); }
                if (/ностальг|атмосфер/.test(n)) { mImmersion += (pct / 10); }
                if (/динамич|экшн|быстр/.test(n)) { mPace += (pct / 10); mImmersion += (pct / 15); }
            });

            if (gHorror) mFear += 3.5;
            if (gThr) mFear += 2.0;
            if (gMysteryN) mFear += 1.2;
            if (gCrime) mFear += 1.0;
            if (kwScary) mFear += 2.2;
            if (kwSuspense) mFear += 2.0;
            if (dPsych.k || dPsych.o) mFear += 1.8;
            if (cFear >= 30) mFear += 1.5;

            if (gActionN) mPace += 1.2;
            if (gAdvN) mPace += 0.6;
            if (kwChase) mPace += 1.0;
            if (kwSlow) mPace -= 1.5;
            if (gDramaN) mDrama += 1.5;
            if (kwEmot) mDrama += 1.5;
            if (gSciFiN || gFantasyN) mImmersion += 1.5;
            if (kwVisual || kwWorld) mImmersion += 1.5;

            if (gComedyN && !gHorror && !gThr) mFear -= 1.5;
            ...
            if (mPace >= 7.5) add('moods','pro','⚡ Очень высокий темп и динамика', 14);
            else if (mPace >= 6.0) add('moods','pro','⚡ Высокий темп и динамика', 10);
            else if (mPace > 0 && mPace <= 3.5 && runtime > 110) add('moods','con','🐢 Очень размеренный и медленный темп', 10);

            if (mFear >= 7.5) add('moods','con','😱 Очень страшно / высокое напряжение', 16);
            else if (mFear >= 5.5) add('moods','pro','🔥 Напряжённый сюжет и саспенс', 12);

            if (mDrama >= 7.5) add('moods','pro','😢 Сильные эмоции и глубокая драма', 14);
            else if (mDrama >= 6.0) add('moods','pro','🎭 Трогательный и эмоциональный', 10);

            if (mImmersion >= 8.0) add('moods','pro','🌌 Полное погружение и атмосфера', 16);
            else if (mImmersion >= 6.5) add('moods','pro','✨ Отличная атмосфера и визуал', 12);
            ...
            if (gHorror || kwScary) {
                aud(9, 'любителям хорроров и историй, где страшно по-настоящему', 'тем, кто плохо переносит скримеры и гнетущую атмосферу');
                aud(7, 'поклонникам мистики и историй про потустороннее', 'впечатлительным зрителям и тем, кто смотрит фильмы перед сном');
                aud(6, 'фанатам survival horror и мрачных триллеров', 'зрителям, которые ищут лёгкое и позитивное кино');
            }
            if (isAnim && familyOK) {
                aud(9, 'для семейного просмотра с детьми любого возраста', 'зрителям, которые ищут взрослый и жёсткий сюжет');
                aud(7, 'детям и родителям для уютного совместного вечера', 'тем, кто не любит анимацию во взрослом кинопросмотре');
                aud(5, 'любителям добрых и поучительных сказок', 'фанатам мрачного фэнтези и реализма');
            } else if (isAnim && !familyOK) {
                aud(8, 'поклонникам взрослой анимации со сложными темами', 'семейным зрителям — фильм не для детей');
                aud(7, 'ценителям глубокого сюжета в анимационном формате', 'тем, кто считает, что мультики только для детей');
            }
            if (hasFamilyGenre && familyOK && !isAnim) {
                aud(7, 'для уютного семейного просмотра за чашкой чая', 'тем, кто хочет напряжённый взрослый сюжет');
                aud(6, 'любителям добрых историй о дружбе и приключениях', 'зрителям, предпочитающим циничный и чёрный юмор');
            }
            if (isSuperheroN) {
                aud(8, 'поклонникам супергероики и масштабных зрелищных сцен', 'тем, кто не любит комиксовые условности и суперсилы');
                aud(7, 'любителям ярких экшен-приключений и спецэффектов', 'зрителям, которые ищут реалистичную драму без фантастики');
                aud(5, 'фанатам расширенных киновселенных и кроссоверов', 'тем, кто устал от типичных сюжетов про спасение мира');
            }
            if (gHistoricN && !gWar) {
                aud(8, 'любителям исторических драм и атмосферных эпохальных фильмов', 'тем, кто не любит неторопливое повествование с историческим контекстом');
                aud(6, 'ценителям аутентичных костюмов и декораций', 'фанатам современной динамики и футуризма');
            }
            if (gWar) {
                aud(8, 'любителям военных драм и фильмов о войне', 'тем, кто тяжело переносит сцены боевых действий и трагические темы');
                aud(6, 'интересующимся конкретной эпохой или военными событиями', 'зрителям, которые предпочитают мирные и позитивные сюжеты');
            }
            if (gCrime && gThr) {
                aud(8, 'любителям криминальных триллеров с запутанным сюжетом', 'тем, кто не переносит сцены насилия и мрачную атмосферу');
                aud(7, 'поклонникам нуара и историй про антигероев', 'зрителям, которые верят в абсолютное торжество добра');
            } else if (gCrime) {
                aud(6, 'поклонникам криминальных историй и детективной интриги', 'тем, кто предпочитает лёгкие развлекательные жанры');
            } else if (gThr) {
                aud(6, 'любителям триллеров с постоянным напряжением', 'зрителям, которым нужно расслабляющее кино для фона');
            }
            if (gComedyN) {
                aud(8, 'любителям лёгких комедий и позитивных историй', 'тем, кто предпочитает серьёзные драмы без юмора');
                aud(6, 'тем, кто хочет просто отдохнуть и посмеяться после работы', 'зрителям, ищущим глубокий драматический сюжет');
                aud(5, 'ценителям сатиры и искромётных диалогов', 'фанатам прямолинейного экшена без лишних слов');
            }
            if (gRomanceN) {
                aud(7, 'поклонникам романтических историй и любви', 'тем, кто не любит мелодраматичные сюжетные линии');
                aud(6, 'любителям красивых историй отношений', 'зрителям, которым важнее интрига и саспенс');
            }
            if (gSciFiN && rating >= 7.3) {
                aud(8, 'ценителям сложной научной фантастики с проработанной идеей', 'любителям простых прямолинейных сюжетов без философии');
            } else if (gSciFiN) {
                aud(7, 'любителям фантастических миров и необычных допущений', 'тем, кто ищет бытовую реалистичную историю');
                aud(6, 'фанатам киберпанка и антиутопий', 'тем, кого пугает будущее и технологии');
            }
            if (gFantasyN) {
                aud(7, 'поклонникам фэнтези и вымышленных вселенных', 'тем, кто предпочитает реалистичные истории без магии');
                aud(6, 'любителям эпических квестов и сказочных существ', 'зрителям, которые не переносят пафос и сказочность');
            }
            if (gMysteryN) {
                aud(7, 'любителям детективов и историй с разгадкой', 'зрителям, которым не нравится следить за уликами и подсказками');
                aud(6, 'тем, кто любит собирать пазл сюжета в голове', 'фанатам прямолинейного повествования без тайн');
            }
            if (gDocN) {
                aud(8, 'тем, кто интересуется темой и любит документалистику', 'зрителям, которые ищут развлекательный художественный сюжет');
                aud(6, 'любителям реальных историй и фактов', 'тем, кому скучно смотреть на говорящие головы и хронику');
            }
            if (dDarkCom) {
                aud(7, 'любителям чёрного юмора и острой сатиры', 'тем, кого задевают шутки на мрачные темы');
                aud(6, 'ценителям абсурда и нестандартного взгляда на вещи', 'зрителям с традиционным чувством юмора');
            }
            if (kwEmot || (mm.sadness || 0) >= 6 || mDrama >= 7) {
                aud(8, 'тем, кто ищет сильную эмоциональную история и не против поплакать', 'зрителям, которым сейчас не до тяжёлых переживаний');
                aud(6, 'любителям катарсиса и глубоких душевных терзаний', 'тем, кто хочет просто расслабиться и не грузиться');
            }
            if (kwUplift) {
                aud(7, 'тем, кому хочется светлого и жизнеутверждающего кино', 'зрителям в поиске мрачной атмосферы и тяжёлой драмы');
                aud(6, 'людям, которым нужна мотивация и вера в лучшее', 'циничным зрителям, не верящим в хэппи-энды');
            }
            if (rating >= 8 && votes >= 1000) {
                aud(7, 'поклонникам качественного, высоко оценённого кино', 'зрителям, которым важнее развлечение, а не признание критиков');
            } else if (votes > 0 && votes < 1500 && rating >= 7.6) {
                aud(6, 'тем, кто ищет недооценённые находки без лишнего шума', 'зрителям, которые доверяют только масштабным хитам');
            }
            if (cViol >= 50) {
                aud(5, 'тем, кто спокойно относится к жёстким сценам на экране', 'чувствительным зрителям и тем, кто смотрит с детьми рядом');
            }
            if (mPace <= 3.5) {
                aud(6, 'любителям медитативного и неторопливого кино', 'тем, кому нужно постоянное действие и динамика');
            }

            var fallbackTarget = 'зрителям, заинтересованным в вечернем просмотре';
            var fallbackAvoid = 'искушённым критикам с очень высокими требованиями';
    ...
    function addBtn(el, rawMovie) {
        try {
            ...
            var movie = sanitizeMovie(rawMovie);
            var btn = $('<div class="full-start__button selector sw-custom-button-enhanced sw-focusable" data-type="should_watch" tabindex="0"><div class="full-start__icon">' + ICON + '</div><span>Стоит ли</span></div>');
