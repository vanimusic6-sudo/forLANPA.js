/**
 * Capsule Mod v14.5 — «Капсула»
 *
 * Правки этой ревизии (поверх v14.4):
 * 1. Свайп на телефоне был перепутан местами: свайп вправо листал назад, а
 *    свайп влево — вперёд. Исправлено на «вправо — следующий, влево — предыдущий»,
 *    как и клавиши вправо/влево на пульте (они уже были верны).
 * 2. Убрал маскота (аватар-персонажа с цитатой) из окон «Компаньон», «Настройки»
 *    и «Оформление» — сам объект и его SVG в коде остались (не используются),
 *    чтобы не трогать лишнее, но в интерфейсе он больше не появляется.
 * 3. Кнопка «Изменить набор» под карточкой (раньше — «Обновить») теперь открывает
 *    короткое меню из двух пунктов: «Обновить этот набор» (прежнее поведение) и
 *    «Выбрать по настроению» (те же плитки настроений, что были в Компаньоне).
 * 4. Мобильная вёрстка: постер и весь блок теперь центрируются колонкой на всех
 *    узких экранах (было — на части экранов постер вставал в строку и обрезался),
 *    ширина постера считается от vw, а не только от em, кнопки собраны через gap
 *    вместо отрицательных margin (это и приводило к обрезке по правому краю).
 * 5. Усилил визуальные отсылки тем, кроме «Матрицы» (она и так хороша): у
 *    «Лаборатории» — водяной знак колбы, у «Нуара» — полосы дождя по стеклу,
 *    у «Свитка» (панда) — бамбуковая кромка и водяной знак, у «Портала» —
 *    водоворот-водяной знак, у «Найт-Сити» — CRT-скан-линии, у «Галактики» —
 *    силуэт «луны смерти» в углу.
 * 6. Подборка расширена до топ-100: капсула, поиск и настроения выдают до 100
 *    карточек (было 6). Чтобы набрать объём, запросы идут по 2–3 страницы на
 *    источник, история показов выросла до 600 id, а мини-постеры в лотке
 *    грузятся лениво (loading="lazy"), чтобы не тянуть сотню картинок разом.
 */
...
(function () {
    ...
    const LANG = 'ru-RU';
    const CAPSULE_SIZE = 100; // v14.5: топ-100 вместо 6
    ...
    const History = {
        read: (cb) => {
            ...
            const withFavorite = (fav) => {
                ...
                withExtras(favHadKeys);
            };
            const withExtras = (favHadKeys) => {
                // Ключи, которые Lampa.Favorite.full() уже включает как под-массивы, повторно не читаем —
                // иначе один и тот же фильм получает вес дважды и модель вкуса перекашивается.
                const extra = ['history', 'view', 'viewed', 'card_history', 'recomends_last',
                    'wath', 'look', 'like', 'book', 'scheduled', 'continued', 'thrown']
                    .filter(k => !favHadKeys || !favHadKeys[k]);
    ...
    const Capsule = {
        ...
        remember: (ids) => {
            ...
            // v14.5: было 80 — при топ-100 этого мало (список повторялся бы сразу),
            // теперь храним до 600 id (~6 полных подборок).
            if (s.length > 600) s = s.slice(s.length - 600);
            pSet('shown', s);
        },
        forget: () => pSet('shown', []),
        build: (taste, opts, cb) => {
            ...
            // v14.5 (топ-100): по 2 страницы рекомендаций на каждый seed,
            // 3 страницы жанрового и 2 ключевого discover — чтобы набрать объём.
            (taste.seeds || []).slice(0, 3).forEach(seed => {
                [0, 1].forEach(off => tasks.push(done => {
                    Net.get(`/${seed.type}/${seed.id}/recommendations`, { page: page + off }, (d) => {
            ...
            if (topG.length) {
                ...
                [0, 1, 2].forEach(off => tasks.push(done => {
            ...
            // v14.5: объёмный добор — топ рейтинга и тренды всегда (хвост топ-100,
            // а при пустой истории — основной источник).
            [0, 1].forEach(off => tasks.push(done => {
                Net.get('/discover/movie', {
                    sort_by: 'vote_average.desc', 'vote_count.gte': 3000,
                    'vote_average.gte': 7.6, page: page + off, include_adult: false
    ...
    const Search = {
        ...
        run: (query, taste, cb, force) => {
            ...
            Search.resolveTags(ctx.tags, (kwIds) => {
                ...
                const discover = (media, pg) => {
                    const p = {
                        sort_by: 'popularity.desc', include_adult: false, page: pg,
                ...
                // v14.5 (топ-100): по 2 страницы на источник, чтобы хватало до 100 карточек.
                if (ctx.type !== 'tv') [0, 1].forEach(off => tasks.push(done => {
                    Net.get('/discover/movie', discover('movie', page + off), (d) => done(markList(d?.results, 'movie', 'search', { query })), () => done([]), { force });
                }));
                if (ctx.type !== 'movie') [0, 1].forEach(off => tasks.push(done => {
                    Net.get('/discover/tv', discover('tv', page + off), (d) => done(markList(d?.results, 'tv', 'search', { query })), () => done([]), { force });
                }));
                if (ctx.tokens.length) [0, 1].forEach(off => tasks.push(done => {
                    Net.get('/search/multi', { query, page: page + off, include_adult: false }, (d) => done(markList(d?.results, null, 'search', { query })), () => done([]), { force });
                }));
                parallel(tasks, (packs) => {
                    const all = packs.flat().filter(Boolean);
        ...
        rank: (list, ctx, taste, force) => {
            ...
            list.forEach(it => {
                ...
                // Уже показанное недавно — не выкидываем совсем (иначе узкий запрос может
                // остаться пустым), но сильно понижаем, чтобы «Обновить» реально давало новое.
            ...
            const ranked = fresh.length >= CAPSULE_SIZE ? fresh : out;
            return ranked.slice(0, CAPSULE_SIZE * 2);
        }
    };
    ...
    const CSS = `
        ...
        .cm-stage{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:2em 3em;overflow-y:auto}
        ...
        @media (max-width:1000px){
            ...
            .cm-stage{padding:1em;align-items:flex-start;overflow-x:hidden}
        ...
        @media (max-width:760px){
            ...
            .cm-stage{padding:.6em;overflow-x:hidden}
            ...
            .cm-tray{padding-top:.6em;max-width:100%}
            .cm-ov{padding:.6em}
    ...
    // Основной персонаж — космонавт (не робот): круглый гермошлем со светоотражением,
    // скафандр с нагрудным пультом и кислородными шлангами. Тематические маски
    // (лаборатория, матрица и т.д.) остаются героями своих вселенных.
    // v14.5: в интерфейсе маскот больше не показывается — объект и SVG остались
    // только как запасной код, модальные окна открываются без него.
    const Robot = {
        svg: (key) => {
            switch (key) {
...
<path d="M93 56 l9 7 v26 l9 7" fill="none" stroke="#05D9E8" stroke-width="3"/>
...
<rect x="35" y="58" width="50" height="52" rx="18" fill="#EDF1F9"/>
<rect x="35" y="58" width="50" height="52" rx="18" fill="none" stroke="rgba(0,0,0,.2)"/>
<rect x="44" y="70" width="32" height="24" rx="9" fill="#0B1220"/>
<path d="M46 72 a7 7 0 0 1 7 -7 h22 a7 7 0 0 1 7 7 v20 a7 7 0 0 1 -7 7 h-22 a7 7 0 0 1 -7 -7 z" fill="#0B1220"/>
<circle cx="60" cy="82" r="9" fill="#101A2C"/>
<circle cx="60" cy="82" r="9" fill="none" stroke="#3D5A86" stroke-width="1"/>
<circle cx="57" cy="79" r="2.6" fill="rgba(255,255,255,.75)" class="cmr-visorshine"/>
<circle cx="55" cy="80.5" r="2.4" fill="#7FD8FF" class="cmr-eye"/>
<circle cx="65" cy="80.5" r="2.4" fill="#7FD8FF" class="cmr-eye"/>
<rect x="49" y="98" width="22" height="6" rx="2" fill="#8FA6C0"/>
<circle cx="53" cy="101" r="1.4" fill="#FF7A2F" class="cmr-pulse"/>
<circle cx="60" cy="101" r="1.4" fill="#7FD8FF" class="cmr-pulse"/>
<circle cx="67" cy="101" r="1.4" fill="#FFD75E"/>
<path d="M35 68 q-10 4 -9 22 q1 8 9 10" fill="none" stroke="#8FA6C0" stroke-width="3.5" stroke-linecap="round"/>
<path d="M85 68 q10 4 9 22 q-1 8 -9 10" fill="none" stroke="#8FA6C0" stroke-width="3.5" stroke-linecap="round"/>
<circle cx="26" cy="90" r="4.5" fill="#FF7A2F"/>
<circle cx="94" cy="90" r="4.5" fill="#FF7A2F"/>
</g>
</svg>`;
            }
        }
    ...
    document.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(dx) > 70 && Math.abs(dx) > Math.abs(dy) * 1.6) {
            // v14.5: свайп вправо (палец идёт слева направо, dx > 0) — следующий фильм,
            // свайп влево (dx < 0) — предыдущий. Симметрично кнопкам вправо/влево на ТВ.
    ...
    const Modal = {
        ...
        open: (opts) => {
            ...
            const nodes = [];
            let gridLen = 0;
            // v14.5: маскот больше не добавляется ни в одно окно (opts.mascot игнорируется).
        ...
        // Настроения — сетка 2×N: вверх/вниз должны идти по столбцу, а не по плоскому
        // индексу (иначе «вниз» на плитках прыгало по диагонали). Список ниже плиток —
        // обычная одноколоночная навигация.
        move: (dir) => {
            const st = Modal.stack[Modal.stack.length - 1];
            ...
            if (inGrid) {
                ...
                // left/right в одноколоночном списке ничего не делают
            }
    ...
    const VIEW = {
        ...
        render: () => {
            ...
            VIEW.list.forEach((item, i) => {
                ...
                if (item.poster_path) {
                    const mi = el('img');
                    mi.loading = 'lazy'; // v14.5: в лотке до 100 мини — не грузим всё сразу
                    mi.decoding = 'async';
    ...
    const Companion = {
        ...
        // Кнопка «Изменить набор» под карточкой фильма — короткое меню из двух пунктов,
        // без захода в полное меню компаньона.
        changeSet: () => {
            Modal.open({
    ...
    (() => {
        try {
            ...
            addMenu();
            console.log('[Капсула] v14.5 загружена');
        } catch (e) {
