(function () {
    ...
    var INTERESTING_TAGS = [
        ...
        { re: /found footage|найденн[а-я]* плёнк/i, text: '📹 Найденная плёнка' },
        { re: /anthology|альманах|альтернативн истори/i, text: '📚 Альманах историй' },
        { re: /road trip|дорожн приключен/i, text: '🚗 Дорожное приключение' },
        { re: /survival|выживани/i, text: '🏔 Выживание' },
        { re: /courtroom|судебн процесс|trial/i, text: '⚖️ Судебная драма' },
        { re: /holiday|christmas|new year|новогодн|рождеств/i, text: '🎄 Праздничная атмосфера' }
    ];
    ...
    var FEATURES = [
        ...
        { re: /alien|инопланет/i, text: '👽 Пришельцы' },
        { re: /pirate|пират/i, text: '🏴‍☠️ Пираты' },
        { re: /wizard|witch|волшебник|ведьм/i, text: '🧙 Волшебники и ведьмы' },
        { re: /assassin|наёмн[иы]к убийц/i, text: '🗡 Наёмные убийцы' },
        { re: /small town|провинциальн городок/i, text: '🏘 Атмосфера маленького городка' }
    ];

    // Пулы формулировок: несколько вариантов текста на одну и ту же ситуацию,
    // чтобы карточки разных фильмов не звучали как под копирку. Выбор детерминирован
    // через pickSeeded (см. ниже), так что один и тот же фильм всегда даёт один текст.
    function fillTpl(tpl, vals) { return tpl.replace(/\{(\w+)\}/g, function(_, k){ return (vals[k] !== undefined) ? vals[k] : ''; }); }

    var RATING_HIGH_TPL = ['⭐ Высокий рейтинг: {r} ({v} оценок)', '⭐ Зрители ставят {r} — солидный результат ({v} оценок)', '⭐ Крепкая оценка {r} при {v} голосах'];
    var RATING_GOOD_TPL = ['👍 Хороший рейтинг: {r} ({v} оценок)', '👍 Зрителям нравится: {r} из 10 ({v} оценок)', '👍 Стабильно хорошие отзывы: {r} ({v} голосов)'];
    var RATING_DECENT_TPL = ['👌 Неплохой рейтинг: {r} ({v} оценок)', '👌 Средне-хорошая оценка {r} ({v} голосов)', '👌 Вполне достойно: {r} из 10 ({v} оценок)'];
    var RATING_LOW_TPL = ['📉 Низкий рейтинг: {r}', '📉 Зрители не впечатлены: {r} из 10', '📉 Оценки ниже среднего: {r}'];
    var RATING_WEAK_TPL = ['👎 Слабый рейтинг: {r}', '👎 Заметно провальные оценки: {r}', '👎 Зрители в основном разочарованы: {r}'];
    var POPULAR_HIT_TPL = ['🔥 Настоящий хит', '🔥 Сейчас у всех на слуху', '🔥 На пике популярности'];
    var POPULAR_MED_TPL = ['📈 Популярный фильм', '📈 Набирает обороты', '📈 Заметный интерес зрителей'];
    var QUALITY_BAD_TPL = ['📺 Плохое качество (экранка)', '📺 Экранка — картинка так себе', '📺 Качество подкачало (кам-рип)'];
    var QUALITY_4K_TPL = ['🎥 Отличное качество (4K UHD)', '🎥 Максимальная чёткость — 4K UHD', '🎥 Картинка в 4K, придраться не к чему'];
    var REVIEW_POS_TPL = ['💬 {who} очень хвалят фильм', '💬 {who} в восторге от увиденного', '💬 {who} оставляют тёплые отзывы'];
    var REVIEW_NEG_TPL = ['💬 {who} в отзывах ругают фильм', '💬 {who} остались разочарованы', '💬 {who} пишут в основном негативные отзывы'];
    var REVIEW_MIX_TPL = ['💬 Споры в отзывах: на любителя', '💬 Мнения зрителей разделились', '💬 Отзывы противоречивые — как повезёт'];
    var PACE_HIGH_TPL = ['⚡ Быстрый темп без раскачки', '⚡ Всё несётся вперёд без пауз', '⚡ Ритм держит в тонусе с первых минут'];
    var FEAR_HIGH_TPL = ['🔥 Держит в напряжении', '🔥 Саспенс не отпускает', '🔥 Напряжение растёт с каждой сценой'];
    var DRAMA_HIGH_TPL = ['😢 Трогательная история', '😢 Эмоционально пробирает по-настоящему', '😢 История, которая западает в память'];
    var IMMERSION_HIGH_TPL = ['🌌 Полное погружение в атмосферу', '🌌 Мир фильма затягивает целиком', '🌌 Атмосфера продумана до мелочей'];
    var VERDICT_STRONG_YES = ['ОДНОЗНАЧНО СТОИТ', 'ОБЯЗАТЕЛЬНО К ПРОСМОТРУ'];
    var VERDICT_YES = ['СТОИТ СМОТРЕТЬ', 'МОЖНО СМЕЛО СМОТРЕТЬ'];
    var VERDICT_STRONG_NO = ['ТОЧНО НЕ СТОИТ', 'ЛУЧШЕ ПРОПУСТИТЬ'];
    var VERDICT_NO = ['НЕ СТОИТ', 'СКОРЕЕ НЕ ТО'];
    var VERDICT_MEH = ['НА ЛЮБИТЕЛЯ', 'СПОРНО, НО МОЖНО ПОПРОБОВАТЬ'];
    var FALLBACK_TARGET_TPL = ['тем, кто ищет хороший фильм на вечер', 'тем, кто просто хочет посмотреть что-то стоящее', 'зрителям без особых предпочтений по жанру'];
    var FALLBACK_AVOID_TPL = ['зрителям со строгими жанровыми вкусами', 'тем, кто ищет что-то совсем конкретное', 'привередливым к жанру зрителям'];
    ...
    function numSetting(key, def) {
        // BUGFIX: `parseFloat(x) || def` откатывался к def, если результат — 0
        // (например пользователь выбрал "Любой" рейтинг = 0). Теперь проверяем isNaN явно.
        var n = parseFloat(getSetting(key, String(def)));
    ...
    function getSettings() {
        return {
            ...
            min_rating: numSetting('min_rating', 6),
            font_scale: numSetting('font_scale', 20)
        };
    }
    ...
    function injectCSS() {
        // BUGFIX: раньше стили жили под versioned id (v51..v59) и при каждом обновлении
        // плагина нужно было руками дописывать старый id в список на удаление — забыли бы
        // один раз, и в DOM копились бы мёртвые <style> теги. Теперь id один и стабильный,
        // а его содержимое просто перезаписывается при каждой загрузке скрипта.
        var STYLE_ID = 'sw-plugin-styles';
        ...
        s.innerHTML =
            ...
            '.sw-loader-emoji{font-size:2.8em;line-height:1;animation:swFloat 2.2s ease-in-out infinite;will-change:transform;transition:opacity .25s ease}' +
            '@keyframes swFloat{0%,100%{transform:translateY(0) rotate(0deg)}50%{transform:translateY(-8px) rotate(-6deg)}}' +
            ...
            '.sw-verdict-word{position:relative;font-family:var(--sw-mono);font-size:2.2em;font-weight:700;letter-spacing:.04em;line-height:1;margin:0 0 10px;text-transform:uppercase;opacity:0;transform:translateY(10px) scale(.96);transition:opacity .55s cubic-bezier(.16,.8,.24,1),transform .55s cubic-bezier(.34,1.56,.64,1);will-change:transform,opacity}' +
            ...
            '.sw-confidence{display:inline-flex;align-items:center;gap:6px;font-family:var(--sw-mono);font-size:.62em;font-weight:700;letter-spacing:.02em;color:var(--sw-mut);text-transform:none;margin:0 0 14px;opacity:0;transition:opacity .5s ease .15s}' +
            '.sw-confidence.appear{opacity:1}' +
            ...
            '.sw-tab-btn{transition:opacity .18s,background .18s ease,border-color .18s,box-shadow .18s,transform .18s ease}' +
            '.sw-tab-btn.active{opacity:1;color:#fff;background:#262b30;border-color:#e9ecef;box-shadow:0 2px 8px rgba(0,0,0,.25);transform:translateY(-1px)}' +
    ...
    // Небольшой детерминированный хэш: одна и та же связка "фильм + метка поля" всегда
    // даёт один и тот же индекс, поэтому при повторном открытии карточки формулировки
    // не "прыгают" — но разные фильмы получают разные варианты фраз для разнообразия.
    function movieSeed(movie) {
        var s = String(movie.id || movie.tmdb_id || movie.title || movie.name || 'x');
        var h = 0;
        for (var i = 0; i < s.length; i++) { h = ((h << 5) - h + s.charCodeAt(i)) | 0; }
        return h;
    }
    function pickSeeded(arr, seed, tag) {
        if (!arr || !arr.length) return '';
        var s = seed + '|' + (tag || '');
        var h = 0;
        for (var i = 0; i < s.length; i++) { h = ((h << 5) - h + s.charCodeAt(i)) | 0; }
        var idx = Math.abs(h) % arr.length;
        return arr[idx];
    }
    ...
    // BUGFIX: раньше локаль строилась механически как "en-EN", "uk-UK" и т.п. —
    // невалидные BCP-47 регионы. Теперь известные языки маппятся на настоящий регион,
    // а неизвестные всё ещё получают разумный fallback вместо мусорного кода.
    var LANG_REGION_MAP = { ru: 'RU', en: 'US', uk: 'UA', be: 'BY', de: 'DE', fr: 'FR', es: 'ES', it: 'IT', pt: 'PT', tr: 'TR', pl: 'PL', kz: 'KZ' };
    function curLangCode() {
        try {
            var l = (window.Lampa && Lampa.Storage) ? Lampa.Storage.get('language', 'ru') : 'ru';
            l = (l || 'ru').toLowerCase();
            var region = LANG_REGION_MAP[l] || l.toUpperCase();
            return l + '-' + region;
    ...
    var US_RATING_MAP = { 'G':0,'PG':7,'PG-13':13,'R':17,'NC-17':17,'TV-MA':17,'TV-14':14,'TV-PG':7,'TV-G':0,'TV-Y7':7,'TV-Y':0,'MA':17,'18':18,'16':16,'12':12,'12A':12,'15':15 };
    function mapUSRating(s) {
        // BUGFIX: раньше `|| null` превращал легитимный возрастной рейтинг 0 (G/TV-G/TV-Y)
        // в null, потому что 0 — falsy в JS. Теперь проверяем через hasOwnProperty.
        var key = (s || '').toUpperCase().trim();
        return US_RATING_MAP.hasOwnProperty(key) ? US_RATING_MAP[key] : null;
    }
    ...
    function analyze(rawMovie) {
        ...
        return Promise.all([loadCredits(movie), loadMeta(movie), Promise.resolve(lampaLocal(movie))]).then(function(arr){
            ...
            if (meta.popularity > 100) add('card','pro',pickSeeded(POPULAR_HIT_TPL, seed, 'popular'), 15, 'popular');
            else if (meta.popularity > 30) add('card','pro',pickSeeded(POPULAR_MED_TPL, seed, 'popular'), 8, 'popular');

            if (votes >= 3000 && adj >= 8.0) add('card','pro',fillTpl(pickSeeded(RATING_HIGH_TPL, seed, 'ratehi'), { r: rating.toFixed(1), v: fmtN(votes) }), 30, 'ratehi');
            else if (votes >= 500 && adj >= cfg.min_rating) add('card','pro',fillTpl(pickSeeded(RATING_GOOD_TPL, seed, 'ratehi'), { r: rating.toFixed(1), v: fmtN(votes) }), 20, 'ratehi');
            else if (votes >= 100 && adj >= cfg.min_rating) add('card','pro',fillTpl(pickSeeded(RATING_DECENT_TPL, seed, 'ratehi'), { r: rating.toFixed(1), v: fmtN(votes) }), 14, 'ratehi');
            ...
            if (votes >= 100 && adj < cfg.min_rating) add('card','con',fillTpl(pickSeeded(RATING_LOW_TPL, seed, 'ratelow'), { r: rating.toFixed(1) }), 22, 'ratelow');
            else if (votes >= 50 && rating > 0 && rating < 5) add('card','con',fillTpl(pickSeeded(RATING_WEAK_TPL, seed, 'ratelow'), { r: rating.toFixed(1) }), 26, 'ratelow');
            ...
            if (/CAM|TS|HDCAM|HDRIP|TELECINE|SCR|WORKPRINT|TELESYNC/i.test(q)) add('card','con',pickSeeded(QUALITY_BAD_TPL, seed, 'quality'), 26, 'quality');
            else if (/4K|UHD|2160p/i.test(q)) add('card','pro',pickSeeded(QUALITY_4K_TPL, seed, 'quality'), 10, 'quality');
            ...
            if (rt.total >= 2 && rt.tone === 'pos') add('reviews','pro',fillTpl(pickSeeded(REVIEW_POS_TPL, seed, 'rev'), { who: who }), 20, 'rev');
            else if (rt.total >= 2 && rt.tone === 'neg') add('reviews','con',fillTpl(pickSeeded(REVIEW_NEG_TPL, seed, 'rev'), { who: who }), 22, 'rev');
            else if (rt.total >= 2 && rt.tone === 'mix') add('reviews','pro',pickSeeded(REVIEW_MIX_TPL, seed, 'rev'), 10, 'rev');
            ...
            var vClass, vWord;
            if (score > 40) { vClass = 'yes'; vWord = pickSeeded(VERDICT_STRONG_YES, seed, 'verdict'); }
            else if (score > 15) { vClass = 'yes'; vWord = pickSeeded(VERDICT_YES, seed, 'verdict'); }
            else if (score < -40) { vClass = 'no'; vWord = pickSeeded(VERDICT_STRONG_NO, seed, 'verdict'); }
            else if (score < -15) { vClass = 'no'; vWord = pickSeeded(VERDICT_NO, seed, 'verdict'); }
            else { vClass = 'meh'; vWord = pickSeeded(VERDICT_MEH, seed, 'verdict'); }
            ...
            if (mPace >= 8.0) add('moods','pro',pickSeeded(PACE_HIGH_TPL, seed, 'pace'), 14);
            ...
            else if (mFear >= 7.0) add('moods','pro',pickSeeded(FEAR_HIGH_TPL, seed, 'fear'), 14);
            ...
            else if (mDrama >= 7.0) add('moods','pro',pickSeeded(DRAMA_HIGH_TPL, seed, 'drama'), 12);
            ...
            else if (mImmersion >= 7.5) add('moods','pro',pickSeeded(IMMERSION_HIGH_TPL, seed, 'immersion'), 14);
            ...
            var fallbackTarget = pickSeeded(FALLBACK_TARGET_TPL, seed, 'fbtarget');
            var fallbackAvoid = pickSeeded(FALLBACK_AVOID_TPL, seed, 'fbavoid');
            ...
            return {
                ...
                vClass: vClass, vWord: vWord, metaRich: metaRich, lowConf: lowConf,
    ...
    function buildReadyInner(a) {
        ...
        var confidenceHtml = a.lowConf ? '<div class="sw-confidence" id="sw-confidence">⚠️ мало данных — вывод приблизительный</div>' : '';
    ...
    function bindDice(html, a) {
        ...
        // Раньше бросок был чистым 50/50 и совершенно не зависел от разбора фильма —
        // выглядело странно, когда явный "не стоит" выпадал как "стоит смотреть".
        // Теперь вероятность взвешена по реальной оценке (a.norm, 5..95%), но исход
        // всё ещё случаен — это по-прежнему "положиться на случай", а не дублирование вердикта.
        var yesChance = 0.5;
        if (a && typeof a.norm === 'number') {
            yesChance = Math.max(0.12, Math.min(0.88, a.norm / 100));
        }
        ...
        function rollDiceRaw() {
            ...
            var isYes = Math.random() < yesChance;
    ...
    function showModal(rawMovie, triggerBtn) {
        try {
            ...
            var phaseEmoji = ['🔍', '💬', '🎭', '⚖️', '📝'];
            var html = $('<div class="sw-wrap"><div id="sw-body"><div class="sw-loader"><div class="sw-loader-emoji">' + phaseEmoji[0] + '</div><div class="sw-loader-text">' + esc(phases[0]) + '</div><div class="sw-loader-progress"></div></div></div></div>');
            ...
            window._sw_loaderTimer = setInterval(function(){
                ...
                if (em.length) {
                    em.css('opacity', 0);
                    setTimeout(function(){ em.text(phaseEmoji[pi]).css('opacity', 1); }, 220);
                }
            }, 650);
            ...
            analyze(movie).then(function(a){
                ...
                setTimeout(function(){
                    ...
                    html.find('#sw-confidence').addClass('appear');
    ...
    // BUGFIX: раньше, если этот скрипт выполнялся до появления window.Lampa,
    // ветка `$(document).ready(startPlugin)` срабатывала, но startPlugin() тут же
    // выходил по `if (!window.Lampa) return;` и плагин больше никогда не пытался
    // стартовать — тихий отказ без единой ошибки в консоли. Теперь есть опрос с ретраями.
    function waitForLampaThenStart(tries) {
        tries = tries || 0;
        if (window.Lampa && Lampa.Listener) {
            bootWithLampa();
            return;
        }
        if (tries > 150) { console.error('[SW] Lampa не появилась, плагин не запущен'); return; }
        setTimeout(function(){ waitForLampaThenStart(tries + 1); }, 200);
    }
    function bootWithLampa() {
        if (window.appready) startPlugin();
        else Lampa.Listener.follow('app', function(e){ if (e.type === 'ready') startPlugin(); });
    }

    try {
        if (window.appready && window.Lampa) startPlugin();
        else if (window.Lampa && Lampa.Listener) bootWithLampa();
        else waitForLampaThenStart(0);
    } catch(e) {
