(function () {
    ...
    function injectCSS() {
        ...
        s.innerHTML =
            ...
            '.sw-custom-button-enhanced.focus{background:rgba(255,255,255,.16);color:#fff}' +
            '.sw-modal-content{padding:26px 30px 48px;color:#e8eaed;font-family:' + DISPLAY + ';font-size:17px;box-sizing:border-box;max-height:88vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-y}' +
            '.sw-modal-content::-webkit-scrollbar{width:6px}.sw-modal-content::-webkit-scrollbar-thumb{background:rgba(255,255,255,.18);border-radius:3px}' +
            ...
            '.sw-dossier{position:relative;background:#26292d;border-radius:18px;padding:30px 32px;margin-bottom:20px;animation:swRise .55s cubic-bezier(.22,1,.36,1) both}' +
            ...
            '.sw-mode-badge{display:inline-flex;align-items:center;gap:6px;font-size:.66em;padding:5px 12px;border-radius:14px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:rgba(255,255,255,.06);color:#9aa1a6}' +
            '.sw-mode-dot{width:5px;height:5px;border-radius:50%;display:inline-block}' +
            '.sw-mode-dot.active{background:#7ec260}.sw-mode-dot.inactive{background:#7d8388}' +
            '.sw-verdict-word{font-size:2.6em;font-weight:800;letter-spacing:.02em;line-height:1;margin:0 0 18px;text-transform:uppercase;opacity:0;transform:translateY(8px);transition:opacity .5s ease,transform .5s ease}' +
            ...
            '.sw-meter{height:5px;border-radius:3px;background:rgba(255,255,255,.12);overflow:hidden}' +
            '.sw-meter-fill{height:100%;width:0;border-radius:3px;transition:width .9s cubic-bezier(.25,.8,.3,1)}' +
            '.sw-meter-fill.yes{background:#7ec260}.sw-meter-fill.no{background:#e05b56}.sw-meter-fill.maybe{background:#e0a93b}' +
            '.sw-dicebtn{position:relative;display:block;width:100%;height:104px;background:#f2f3f5;border:none;border-radius:16px;margin:0 0 22px;overflow:hidden;cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent}' +
            '.sw-dicebtn.focus{box-shadow:0 0 0 3px rgba(255,255,255,.85)}' +
            ...
            '.sw-label{font-family:' + DISPLAY + ';font-size:2.1em;font-weight:800;color:#878c91;letter-spacing:.02em;text-align:center;white-space:nowrap;opacity:1;transition:opacity .25s ease}' +
            '.sw-label.hidden{opacity:0}' +
            '.sw-label.res-yes{color:#3f8f2f}.sw-label.res-no{color:#c9443f}' +
            '.sw-dice{position:absolute;left:16px;top:50%;width:72px;height:72px;margin-top:-36px;will-change:transform;transform:translateZ(0)}' +
            ...
            '.sw-dice.sw-return{animation:swReturn .6s cubic-bezier(.34,1.4,.44,1) forwards}' +
            '@keyframes swReturn{from{transform:translateX(var(--sw-dist)) rotate(720deg)}to{transform:translateX(0) rotate(720deg)}}' +
            '.sw-columns{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:22px}' +
            '.sw-col{position:relative;background:#26292d;border-radius:16px;padding:24px 26px}' +
            ...
            '.sw-title{font-size:.86em;font-weight:800;margin-bottom:16px;text-transform:uppercase;letter-spacing:.06em}' +
            '.sw-title.pros{color:#7ec260}.sw-title.cons{color:#e05b56}' +
            '.sw-list{margin:0;padding-left:20px;font-size:1.02em;line-height:1.7;color:#cdd2d7}' +
            ...
            '@media(max-width:640px){.sw-modal-content{padding:18px 18px 34px;font-size:16px}.sw-columns{grid-template-columns:1fr}.sw-verdict-word{font-size:2em}.sw-label{font-size:1.5em}.sw-label-wrap{padding:0 88px}.sw-dice{width:58px;height:58px;margin-top:-29px;left:12px}}';
    ...
    function bindDice(html) {
        ...
        dice.addEventListener('animationend', function(e){
            try {
                if (e.animationName === 'swRoll') {
                    dice.classList.remove('sw-rolling');
                    dice.classList.add('sw-return');
                } else if (e.animationName === 'swReturn') {
                    dice.classList.remove('sw-return');
                }
            } catch(err) {}
        });
        $(btn).on('hover:enter click keydown', function(e){
            try {
                ...
                var maxDist = Math.max(40, btn.clientWidth - dice.offsetWidth - 20);
                var dist = Math.min(maxDist, btn.clientWidth - dice.offsetWidth - 20);
                dice.setAttribute('data-dist', dist);
                dice.style.setProperty('--sw-dist', dist + 'px');
                dice.classList.remove('sw-rolling'); dice.classList.remove('sw-return');
                void dice.offsetWidth;
                dice.classList.add('sw-rolling');
                var t0 = Date.now(), DUR = 1100, frame = 0;
    ...
    function startPlugin() {
        ...
        console.log('[ShouldWatch] v40.0 (readable UI, dice return fix, more args)');
    }
