/**
 * Capsule Mod v17.1 — «Капсула» (Optimized for 100 items)
 *
 ...
 * 5. ИСПРАВЛЕНО: Увеличен размер капсулы до 100, оптимизирована логика выборки,
 *    исправлены баги пагинации, кэширования и форматирования счетчика.
 */
(function () {
    'use strict';
    ...
    // === ГЛАВНОЕ ИЗМЕНЕНИЕ: Размер капсулы увеличен до 100 ===
    ...
    const Taste = {
        ...
        enrich: (items, limit, cb) => {
            ...
            // УВЕЛИЧЕНО: с 14 до 50 для более точного построения модели вкуса под 100 рекомендаций
    ...
    const Capsule = {
        ...
        // ИСПРАВЛЕНО: Увеличен лимит истории показов с 80 до 300, чтобы избежать повторов при 100 элементах
        remember: (ids) => { let s = Capsule.shown().concat(ids); if (s.length > 300) s = s.slice(s.length - 300); pSet('shown', s); },
        forget: () => pSet('shown', []),
        build: (taste, opts, cb) => {
            ...
            // ИСПРАВЛЕНО: Запрашиваем 1 и 2 страницы для жанров и ключевых слов, чтобы гарантированно получить >100 сырых данных для фильтрации
            if (topG.length) {
                const gParams = { with_genres: topG.slice(0, 2).map(g => g.id).join(','), sort_by: 'popularity.desc', 'vote_count.gte': 200, 'vote_average.gte': clamp(taste.avgVote ? taste.avgVote - 0.4 : 6.4, 6.0, 7.4), include_adult: false };
                tasks.push(done => Net.get('/discover/movie', { ...gParams, page: 1 }, (d) => done(markList(d?.results, 'movie', 'genre')), () => done([]), { force }));
                tasks.push(done => Net.get('/discover/movie', { ...gParams, page: 2 }, (d) => done(markList(d?.results, 'movie', 'genre')), () => done([]), { force }));
            }
            if (topK.length) {
                const kParams = { with_keywords: topK.slice(0, 3).map(k => k.id).join('|'), sort_by: 'popularity.desc', 'vote_count.gte': 120, 'vote_average.gte': 6.2, include_adult: false };
                tasks.push(done => Net.get('/discover/movie', { ...kParams, page: 1 }, (d) => done(markList(d?.results, 'movie', 'keyword', { kw: topK[0]?.name })), () => done([]), { force }));
                tasks.push(done => Net.get('/discover/movie', { ...kParams, page: 2 }, (d) => done(markList(d?.results, 'movie', 'keyword', { kw: topK[0]?.name })), () => done([]), { force }));
            }
            
            if (!taste.seeds?.length && !topG.length) {
                tasks.push(done => Net.get('/discover/movie', { sort_by: 'vote_average.desc', 'vote_count.gte': 3000, 'vote_average.gte': 7.6, page: 1, include_adult: false }, (d) => done(markList(d?.results, 'movie', 'top')), () => done([]), { force }));
            ...
            parallel(tasks, (packs) => {
                ...
                // ИСПРАВЛЕНО: Триггер дозаполнения изменен с < 3 на < 50 для большого размера капсулы
                if (picked.length < 50 && topG.length) {
                    Net.get('/discover/movie', { with_genres: topG[0].id, sort_by: 'vote_average.desc', 'vote_count.gte': 800, 'vote_average.gte': 7.0, page: 1, include_adult: false }, (d) => cb(Capsule.pick(all.concat(markList(d?.results, 'movie', 'relax')), taste, force)), () => cb(picked));
                } else {
                    cb(picked);
                }
            });
        ...
        pick: (all, taste, force) => {
            ...
            // ИСПРАВЛЕНО: Динамический лимит на источник для поддержки 100 элементов (вместо жесткого ограничения в 3)
            const bySrc = {}, final = [];
            const maxPerSrc = Math.ceil(CAPSULE_SIZE / 3); 
            
            for (let i = 0; i < out.length && final.length < CAPSULE_SIZE; i++) {
            ...
            // Добиваем остаток, если строгая фильтрация по источникам не позволила набрать 100
    ...
    const View = {
        ...
        render: () => {
            ...
            // ИСПРАВЛЕНО: Использование padStart для корректного отображения "01 / 100", "45 / 100" и т.д.
            const chipSrc = el('div', 'cm-mchip src cm-mono', esc(View.sourceLabel || 'КАПСУЛА') + ' · ' + String(View.idx + 1).padStart(2, '0') + ' / ' + String(View.list.length).padStart(2, '0'));
    ...
    (() => {
        try {
            ...
            addMenu();
            console.log('[Капсула] v17.1 загружена (100 items optimized)');
