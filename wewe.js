...
/* Веса вкусов по категориям избранного Lampa (реальные категории ядра) */
var CAT_WEIGHT = { like: 5, book: 2, look: 1, scheduled: 1, viewed: 3, continued: 2, wath: 3, thrown: -6, history: 0.6 };
/* Категории, которые считаются "уже видел / запускал" и исключаются из выдачи */
...
/* ============================== СТИЛИ: приглушённая палитра, левая рейка как в референсе ============================== */
var CSS_TEXT = [
...
/* ============================== ИКОНКИ ============================== */
/* Робот — силуэт как на референсе пользователя: круглая голова с двумя глазами,
   наклонённое тело, раскинутые руки, согнутые ноги ("падающая" поза). */
...
/* ============================== НАСТРОЙКИ (только во вкладке настроек Lampa) ============================== */
...
/* ============================== ИСТОРИЯ: реальные API ядра Lampa ============================== */
/* v6: читаем избранное/историю через Lampa.Favorite.get({type}) с реальными категориями
   ядра (like,wath,book,history,look,viewed,scheduled,continued,thrown) + собственный
   список "запускал". Никаких выдуманных ключей storage. */
...
function seenSet(m) {
    try {
        ...
        var ks = Object.keys(s); if (ks.length > 600) { ks.sort(function (a, b) { return s[a] - s[b]; }); for (var i = 0; i < 100; i++) delete s[ks[i]]; }
...
function readHistoryRaw() {
    var byId = {}, order = [];
    function pushItem(card, cat) {
        if (!card || !card.id) return;
        var t = cardType(card), key = t + ':' + card.id, w = CAT_WEIGHT[cat] || 0;
    ...
    for (var cat in CAT_WEIGHT) { var list = favGet(cat); for (var i = 0; i < list.length; i++) pushItem(list[i], cat); }
    var seen = seenGet();
    for (var k in seen) { if (Object.prototype.hasOwnProperty.call(seen, k)) { var parts = k.split(':'); if (!byId[k]) { byId[k] = { id: +parts[1] || parts[1], type: parts[0], weight: 0.4, cats: { seen: 1 }, card: { id: +parts[1] || parts[1] } }; order.push(k); } else byId[k].cats.seen = 1; } }
    var out = []; for (var j = 0; j < order.length; j++) out.push(byId[order[j]]);
    return out;
}
...
/* ============================== ТАЙМЛАЙН (прогресс просмотра ядра) ============================== */
/* v6: кандидат, у которого в родном Timeline есть прогресс >= 10%, считается
   "смотрел/запускал" и не рекомендуется. Хеш файла считаем тем же алгоритмом,
   что и ядро Lampa (Utils.hash). */
var _hashFn = null;
function getHashFn() {
...
var Engine = {
    ...
    classify: function (hist) {
        var ev = [], cut = cfgDepth() === 'month' ? (nowMs() - 30 * dayMs()) : 0, manual = manualGet(), i;
    ...
    extWatched: function (profile) {
        ...
        for (i = 0; i < hist.length; i++) {
            var h = hist[i];
            var excluded = h.cats.seen || false;
            for (var c in CAT_EXCLUDE) if (h.cats[c]) excluded = true;
    ...
    discover: function (profile, extra, cb) {
        ...
        if (!hasProfile) params['vote_count.lte'] = 3000; /* без профиля — не тянем мега-попсу */
    ...
    pick: function (list, profile, watched, seen, mode, n) {
        ...
        for (i = 0; i < list.length && out.length < n * 4; i++) {
            ...
            /* СТРОГИЙ фильтр "уже видел/запускал": id, коллизии id, названия, ручной дизлайк, родной Timeline */
            ...
            /* анти-попса: сверхпопулярность штрафует, "жемчужины" (скромная популярность + высокий рейтинг) поднимаются */
...
function skeletonRows() {
    ...
    for (var r = 0; r < 2; r++) {
        ...
        for (var i = 0; i < 6; i++) strip.appendChild(el('div', 'cm-card cm-skel'); if (0) ; strip.appendChild(document.createTextNode('')));
...
function renderDetails(movie) {
    ...
    seenSet(movie); /* "запускал" — больше не будем рекомендовать */
    var root = state.root;
    var existingRobotWrap = root.querySelector('.cm-robot-wrap');
    if (existingRobotWrap) existingRobotWrap.parentNode.removeChild(existingRobotWrap);
    var content = root.querySelector('.cm-content'); content.innerHTML = '';
...
/* ============================== "СМОТРЕТЬ": родной сервис просмотра ============================== */
/* v6: кнопка Плей выполняет функцию родной кнопки "Смотреть": открывает полную
   карточку и АВТОМАТИЧЕСКИ триггерит её .button--play — именно так в лампе
   открывается выбор источников (онлайн-балансеры, торренты, трейлеры), включая
   кнопки, добавленные любыми плагинами. Своего балансера не изобретаем. */
...
function openWatch(m) {
    ...
    var iv = setInterval(function () {
        ...
        if (tries > 40 || done) { try { clearInterval(iv); } catch (e) {} }
    }, 150);
}

/* ============================== МОДАЛКИ (стек, всегда закрываются) ============================== */
/* v6: модалки больше не зависят от Lampa.Modal/Controller. Выход гарантирован:
   back (глобальный capture-обработчик), кнопка ✕, клик по фону. */
...
/* ============================== ЭКРАННАЯ КЛАВИАТУРА ============================== */
/* v6: запрос роботу вводится через собственную экранную клавиатуру (+ перехват
   физической). Никакого Lampa.Input.edit / нативного input — именно они ломали
   интерфейс и "ничего не отправлялось". */
var KB_ROWS = [
    ['1','2','3','4','5','6','7','8','9','0'],
...
function openKeyboard(title, placeholder, onSubmit) {
    ...
    for (var r = 0; r < KB_ROWS.length; r++) {
        ...
        for (var c = 0; c < KB_ROWS[r].length; c++) {
            (function (ch) {
                ...
                rowDiv.appendChild(k); rowArr.push(k);
            })(KB_ROWS[r][c]);
        }
        body.appendChild(rowDiv); rowsNodes.push(rowArr);
    }
    ...
    body.appendChild(ctrlRow); rowsNodes.push([space, bsp, ok]);
...
/* v6: единый capture-обработчик. Пока активен наш экран — мы сами рулим
   стрелками/enter/back и гасим событие, чтобы Lampa не двоила и не "ломала"
   интерфейс. Back ВСЕГДА закрывает верхнюю модалку. */
...
function CapsuleComponent() {
    ...
    this.start = function () { try { if (window.Lampa && Lampa.Controller && Lampa.Controller.toggle) Lampa.Controller.toggle('content'); } catch (e) {} };
...
function startPlugin() {
    ...
    var boot = function () {
        ...
        setTimeout(function () {
            ...
            registerSettings();
            addMenuItem();
            console.log('[CapsuleMod] v6.0 готов');
        }, 300);
    };
