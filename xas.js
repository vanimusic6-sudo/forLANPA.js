    ...
    */
    ...
    var View = {
        ...
        render: function () {
            ...
            // космонавт — декоративный, кликабелен мышью/тачем, но не участвует в D-pad фокусе
            // (в D-pad за него отвечает кнопка «Компаньон» в acts)
            var aw = el('div', 'cm-astro-wrap');
            var astro = el('div', 'cm-astro', SVG_ASTRO);
            astro.onclick = function () { Companion.open(); };
    ...
    var Companion = {
        open: function () {
            ...
            var items = [
                ...
                { section: 'КАПСУЛА' },
                { label: 'Обновить капсулу', onSelect: function () { View.loading('ОБНОВЛЯЮ'); View.reload(); } },
            ...
            Modal.open({ title: 'Компаньон на связи', text: 'Выбери настроение — соберу капсулу под него. Или скажи словами, что хочешь.', chips: chips, items: items });
        },
        ...
        ask: function () { var self = this; askText('Что ищем?', '', function (v) { self.find(v, v); }); },

        find: function (query, label) {
            if (!query) return;
            View.loading('ИЩУ: ' + query.toUpperCase());
            Search.run(query, View.taste, function (list, ctx) { View.showFound(label || query, list, ctx); });
        },
        ...
        similar: function () {
            ...
            Net.get('/' + seed.type + '/' + seed.id + '/similar', { page: 1 }, function (d) {
                ...
                var out = [];
                for (var i = 0; i < list.length && out.length < CAPSULE_SIZE; i++) { if (t.watched[list[i].id]) continue; out.push(list[i]); }
        ...
        tryRegister: function () {
            try {
                ...
                Lampa.SettingsApi.addParam({
                    ...
                    param: { name: 'cm_theme_select', type: 'select', values: (function () {
                        var o = {}; for (var i = 0; i < THEME_ORDER.length; i++) o[THEME_ORDER[i]] = THEMES[THEME_ORDER[i]].name; return o;
                    })(), default: 'astro' },
                    field: { name: 'Тема оформления' },
                    onChange: function (v) { Themes.set(v.cm_theme_select); }
                });
                Lampa.SettingsApi.addParam({
                    component: 'capsule_mod',
